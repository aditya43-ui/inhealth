...
?>
    ...
    <fieldset>
		<div class="block-tabel">
			<h6><label>Tabel <b>Diagnosa Pasien</b></label></h6>
...
function inputDiagnosa(obj,idDiagnosa)
{
    ...
    var tglDiagnosa = $('#PIPasienMorbiditasT_0_tglmorbiditas').val();
    if(!cekInputDiagnosa(idDiagnosa)) {
        $(obj).parent().parent().css("background-color", "yellow");
        jQuery.ajax({'url':'<?php echo Yii::app()->createUrl('perawatanIntensif/diagnosaTPI/LoadFormDiagnosis')?>',
        ...
        window.parent.myConfirm("Apakah Anda Akan Membatalkan Diagnosa No. "+idDiagnosa+"?","Perhatian!",function(r) {
            if(r){
...
{
    window.parent.myConfirm("Apakah Anda yakin akan menghapus diagnosa dari database?","Perhatian!",function(r) {
        if(r){
...
daftarDiagnosa($('#berdasarKasusPenyakit'));
</script>
