<?php
    ...
    echo CHtml::link(Yii::t('mds', '{icon} Ulang', array('{icon}' => '<i class="entypo-arrows-ccw"></i>')), '', array('class' => 'btn btn-default  btn-ulang',
        'onclick' => 'window.parent.myConfirm("Apakah Anda yakin ingin mengulang ini?","Perhatian!",function(r){if(r) window.location = "'.$this->createUrl('index',['pendaftaran_id'=>$_GET['pendaftaran_id'], 'pasienadmisi_id'=>isset($_GET['pasienadmisi_id'])?$_GET['pasienadmisi_id']:null]).'";}); return false;'));
    echo CHtml::link(Yii::t('mds', '{icon} Cetak', array('{icon}' => '<i class="entypo-print"></i>')), '', array(
        'class' => 'btn btn-success  btn-cetak',
        'rel' =>'tooltip',
        'disabled' => !empty($model->ekstubasipasien_id)?false:true,
        'title' => 'Klik untuk cetak cpis pasien',
        'onclick' => 'cetak("'.$model->ekstubasipasien_id.'")'));
    ?>    
    <?php
    $tips = array(
        '0' => 'simpan',
