...
<div class="form-actions">
    ...
    <?php echo CHtml::link(
        ...
        array('class' => 'btn btn-default', 'onclick' => 'window.parent.myConfirm("Apakah Anda ingin mengulang ini?","Perhatian!",function(r){if(r) window.location = window.location.href;}); return false;')
    ); ?>
    <?php echo CHtml::link(
        Yii::t('mds', '{icon} Pengaturan Tindakan Ruangan', array('{icon}' => '<i class="icon-folder-open icon-white"></i>')),
        $this->createUrl('/perawatanIntensif/ruanganM/admin', array('modul_id' => Yii::app()->session['modul_id'])),
...
function submitdatatindakanruangan()
{
    ...
    if(daftartindakan_id==''){
        window.parent.myAlert('Silakan pilih tindakan ruangan terlebih dahulu!');
    }else{
        $.post("${urlGetTindakanruangan}", {instalasi_id:instalasi_id, ruanganid: ruanganid, daftartindakan_id:daftartindakan_id,},
        function(data){
        if (data.tr == null){
        window.parent.myAlert('Daftar Tindakan Sudah Ada');}
