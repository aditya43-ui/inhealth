...
?>
<?php 
if(count((array)$masukpenunjangs) > 0){
	foreach($masukpenunjangs AS $i => $masukpenunjang){
		
	$modHasilRad = HasilpemeriksaanradT::model()->findByAttributes(array('pasienmasukpenunjang_id'=>$masukpenunjang->pasienmasukpenunjang_id));
	if(isset($modHasilRad)){
			
		
		$pemeriksa = PegawaiM::model()->findByPk($masukpenunjang->pegawai_id);
		$detailHasil = HasilpemeriksaanradT::model()->findAllByAttributes(array('pasienmasukpenunjang_id' => $masukpenunjang->pasienmasukpenunjang_id));
...
<?php
		...
		echo '* Tidak ada pemeriksaan radiologi';
	}
?>
