<legend class="rim">Tabel Jadwal Dokter Rawat Intensif</legend>
                    ...
                    foreach (CustomFunction::getNamaHari() as $key => $value) {
                        ...
                        if ($jumlah > $jumlahHari){
                            ...
                            if (strtolower(trim($value)) == strtolower(trim($tanggal[0]))){
                                $jadwal = JadwaldokterM::model()->findAll('(jadwaldokter_tgl between ? and ?) and instalasi_id =?',array($tahun.'-'.$bulan.'-'.$jumlah, $tahun.'-'.$bulan.'-'.$jumlah, Params::INSTALASI_ID_PI));
                                echo '<td>'.$tgl;
                                $ruangan = array();
