...
{
  public $path_view = "perawatanIntensif.views.riwayatPasien."; //agar bisa di extend
  public function actionGetRiwayatPasien($id)
  {
    ...
    $pages = new CPagination(PIPendaftaranT::model()->count($criteria));
    $pages->pageSize = Params::JUMLAH_PERHALAMAN; //Yii::app()->params['postsPerPage'];
    $pages->applyLimit($criteria);

    $modKunjungan = PIPendaftaranT::model()->with('hasilpemeriksaanlab', 'anamnesa', 'pemeriksaanfisik', 'pasienmasukpenunjang', 'diagnosa')->findAll($criteria);
  ...
  {
    $this->layout = '//layouts/iframe';
    $modPendaftaran = PIPendaftaranT::model()->with('carabayar', 'penjamin')->findByPk($id);
    $modBahan = PIObatalkespasienT::model()->with('obatalkes')->findAllByAttributes(array('pendaftaran_id' => $id));
    $format = new MyFormatter;
    $modPemakaianBahan = new PIObatalkespasienT;
  ...
  public function actionDetailTerapi($id)
  {
    ...
    $format = new MyFormatter;
    $modDetailTerapi = new PIResepturDetailT();
    $modPasien = PIPasienM::model()->findByPK($modPendaftaran->pasien_id);
    $this->render(
  ...
  public function actionDetailTindakan($id)
  {
    $this->layout = '//layouts/iframe';
    $modPendaftaran = PIPendaftaranT::model()->with('carabayar', 'penjamin')->findByPk($id);
    $modTindakan = PITindakanPelayananT::model()->with('daftartindakan')->findAllByAttributes(array('pendaftaran_id' => $id));
    $format = new MyFormatter;
    $modTindakanSearch = new PITindakanPelayananT('search');
    $modPasien = PIPasienM::model()->findByPK($modPendaftaran->pasien_id);
    $this->render(
