...
//class DiagnosaTPIController extends DiagnosaController
//{
//        
//}
class DiagnosaTPIController extends MyAuthController
{
  ...
  public function actionIndex($pendaftaran_id, $pasienadmisi_id)
  {
    ...
    $modDiagnosa = new PIDiagnosaM('searchDiagnosis');
    $modDiagnosa->unsetAttributes();  // clear any default values

    if (isset($_GET['PIDiagnosaM']))
      $modDiagnosa->attributes = $_GET['PIDiagnosaM'];

    $modMorbiditas[0] = new PIPasienMorbiditasT;
    ...
    $modMorbiditas[0]->ruangan_id = $modAdmisi->ruangan_id;
    ...
    $modMorbiditas[0]->pegawai_id = isset($modAdmisi->pegawai_id) ? $modAdmisi->pegawai_id : $modPendaftaran->pegawai_id;

    $modKasuspenyakitDiagnosa = new PIKasusPenyakitDiagnosaM('search');
    ...
    if (isset($_GET['PIKasusPenyakitDiagnosaM'])) {
      $modKasuspenyakitDiagnosa->attributes = $_GET['PIKasusPenyakitDiagnosaM'];
      $modKasuspenyakitDiagnosa->jeniskasuspenyakit_id = $modPendaftaran->jeniskasuspenyakit_id;
      $modKasuspenyakitDiagnosa->diagnosa_kode = $_GET['PIKasusPenyakitDiagnosaM']['diagnosa_kode'];
    }

    $modDiagnosaicdixM = new PIDiagnosaicdixM('search');
    $modDiagnosaicdixM->unsetAttributes();  // clear any default values
    if (isset($_GET['PIDiagnosaicdixM']))
      $modDiagnosaicdixM->attributes = $_GET['PIDiagnosaicdixM'];
    $modSebabDiagnosa = PISebabDiagnosaM::model()->findAll();

    $newInput = false;
    if (isset($_POST['Morbiditas'])) {
      $newInput = true;
      $modMorbiditas = $this->saveDiagnosa($_POST['Morbiditas'], $modPasien, $modPendaftaran, $modAdmisi);
      $successSave = $this->successSave;
    }

    $listMorbiditas = PIPasienMorbiditasT::model()->findAllByAttributes(array('pendaftaran_id' => $pendaftaran_id));
  ...
  protected function saveDiagnosa($diagnosas, $modPasien, $modPendaftaran, $modAdmisi)
  {
    $valid = true;
    $golUmur = $this->cekGolonganUmur($modPendaftaran->golonganumur_id);
    foreach ($diagnosas as $i => $diagnosa) {
      $morbiditas[$i] = new PIPasienMorbiditasT;
      ...
      $morbiditas[$i]->ruangan_id = $modAdmisi->ruangan_id;
      ...
      $morbiditas[$i]->tglmorbiditas = $_POST['PIPasienMorbiditasT'][0]['tglmorbiditas'];
      //$morbiditas[$i]->kasusdiagnosa = $_POST['PIPasienMorbiditasT'][0]['kasusdiagnosa'];
      $morbiditas[$i]->kasusdiagnosa = $this->getKasusDiagnosa($modPendaftaran->pasien_id, $diagnosa['diagnosa']);
      $morbiditas[$i]->pegawai_id = $_POST['PIPasienMorbiditasT'][0]['pegawai_id'];
    ...
    if ($valid) {
      foreach ($morbiditas as $j => $morbiditasPasien) {
        ...
        // untuk save otomatis ke kasuspenyakitdiagnosa_m
        $criteria = new CDbCriteria();
        $criteria->addCondition('diagnosa_id =' . $morbiditasPasien->diagnosa_id);
        $criteria->addCondition('jeniskasuspenyakit_id =' . $morbiditasPasien->jeniskasuspenyakit_id);
        $cek = KasuspenyakitdiagnosaM::model()->find($criteria);
        if (!($cek)) {
          $modKasusPenyakitDiagnosa = new KasuspenyakitdiagnosaM;
          $modKasusPenyakitDiagnosa->diagnosa_id = $morbiditasPasien->diagnosa_id;
          $modKasusPenyakitDiagnosa->jeniskasuspenyakit_id = $morbiditasPasien->jeniskasuspenyakit_id;
          $modKasusPenyakitDiagnosa->save();
        }
      }
      //echo 'VALID';
  ...
  public function actionAjaxDeleteDiagnosa()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      try {
        $pasienMorbiditas = PIPasienMorbiditasT::model()->findAllByAttributes(
          array(
        ...
        if (count((array)$pasienMorbiditas) > 0) {
          $deleteDiagnosa = PIPasienMorbiditasT::model()->deleteAllByAttributes(
            array(
              'diagnosa_id' => $idDiagnosa
            )
          );
  ...
  public function actionPrintDiagnosa($pendaftaran_id)
  {
    ...
    $modPasien = PIPasienM::model()->findByPk($modPendaftaran->pasien_id);
    $listMorbiditas = PIPasienMorbiditasT::model()->findAllByAttributes(array('pendaftaran_id' => $pendaftaran_id));
    $modMorbiditas = PIPasienMorbiditasT::model()->findByAttributes(array('pendaftaran_id' => $pendaftaran_id));
  ...
  public function actionLoadFormDiagnosis()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      if (!empty($idKelDiagnosa)) {
        $modDiagnosaicdixM = DiagnosaicdixM::model()->findAll('diagnosaicdix_aktif IS TRUE');
        $modSebabDiagnosa = SebabdiagnosaM::model()->findAll('sebabdiagnosa_aktif IS TRUE');
        $modDiagnosa = DiagnosaM::model()->findByPk($idDiagnosa);

        echo CJSON::encode(array(
          'status' => 'create_form',
          'form' => $this->renderPartial('/diagnosaTPI/_formLoadDiagnosis', array(
            'modDiagnosa' => $modDiagnosa,
