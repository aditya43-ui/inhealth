...
class RuanganMController extends MyAuthController
{
  ...
  public function actionUpdate($id)
  {
    ...
    $model = PIRuanganM::model()->findByPk($id);
  ...
  {
    //if(!Yii::app()->user->checkAccess(Params::DEFAULT_UPDATE)){throw new CHttpException(401,Yii::t('mds','You are prohibited to access this page. Contact Super Administrator'));}                                             
    PIRuanganM::model()->updateByPk($id, array('ruangan_aktif' => false));
  ...
  public function actionCreate()
  {
    ...
    if (isset($_POST['PIRuanganM'])) {
      ...
      try {
        ...
        $valid = true;
        foreach ($_POST['PIRuanganM'] as $i => $item) :
          if (is_integer($i)) {
            $model = new PIRuanganM;
            $random = rand(0000000, 9999999);
            $model->attributes = $_POST['PIRuanganM'][$i];
            $model->instalasi_id = $_POST['PIRuanganM']['instalasi_id'];
  ...
  //                        $transaction = Yii::app()->db->beginTransaction();
  //                            try {
  //                                    $model->attributes=$_POST['PIRuanganM'];
  //                                    $model->save();
  //                                    $jumlahKelasPelayanan=count((array)$_POST['kelaspelayanan_id']);
  //                                    $jumlahJenisKasusPenyakit=count((array)$_POST['jeniskasuspenyakit_id']);
  ...
  {
    $dataProvider = new CActiveDataProvider('PIRuanganM');
  ...
  public function actionPrint()
  {
    ...
    $model = new PIRuanganM();
    $model->ruangan_id = Yii::app()->user->getState('ruangan_id');
    $model->attributes = $_GET['PIRuanganM'];
