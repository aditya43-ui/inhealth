...
class InformasiJadwalDokterPIController extends MyAuthController
{
  /**
  ...
  public function actionIndex()
  {
    ...
    $model = new PIJadwaldokterM;
    $model->instalasi_id = Params::INSTALASI_ID_PI;
    $listHari = array(
      'Senin' => 'Senin',
      'Selasa' => 'Selasa',
      'Rabu' => 'Rabu',
    ...
    if (Yii::app()->request->isAjaxRequest) {
      if (isset($_GET['PIJadwaldokterM'])) {
        $mulai = (!empty($_GET['PIJadwaldokterM']['jadwaldokter_mulai'])) ? date('Y-m-d', strtotime('01 ' . $_GET['PIJadwaldokterM']['jadwaldokter_mulai'])) : date('Y-m-d');
        $tgl = explode('-', $mulai);
        $day = cal_days_in_month(CAL_GREGORIAN, $tgl[1], $tgl[0]);
        $grid = $this->createGrid($day, $tgl[1], $tgl[0], $_GET['PIJadwaldokterM']);
    ...
    if (isset($_REQUEST['PIJadwaldokterM'])) {
      $model->attributes = $_REQUEST['PIJadwaldokterM'];
    }
    $this->render(
      'index',
