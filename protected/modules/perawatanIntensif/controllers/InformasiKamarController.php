...
class InformasiKamarController extends MyAuthController
{
  public function actionIndex()
  {
    ...
    $model = PIInformasikamarinapV::model()->findAll('instalasi_id = ' . Yii::app()->user->getState('instalasi_id') . ' AND  kamarruangan_aktif = true order by ruangan_nama ASC, kelaspelayanan_nama ASC, kamarruangan_nokamar ASC, kamarruangan_nobed ASC');
    $row = $this->renderKamarRuangan($model);
    if ((isset($_POST['ajax'])) && (isset($_POST['ruangan']))) {
      $ruangan = $_POST['ruangan'];
      $model = PIInformasikamarinapV::model()->findAll(((!empty($ruangan)) ? "ruangan_id =" . $ruangan . " and " : "") . 'kamarruangan_aktif = true order by ruangan_id, kelaspelayanan_id, kamarruangan_nokamar, kamarruangan_nobed');
      $row = $this->renderKamarRuangan($model);
...
//==============================Akhir Mencari Tarif Kamar   
  ...
  protected function renderKamarRuangan($model)
  {
    ...
    foreach ($list1 as $i => $v) {
      ...
      $tarifumum = PIInformasikamarinapV::model()->getTarif($v['kelaspelayanan_id'], $v['ruangan_id'], 1);
      $tarifbpjs = PIInformasikamarinapV::model()->getTarif($v['kelaspelayanan_id'], $v['ruangan_id'], 3);
      $tarifjamkespa = PIInformasikamarinapV::model()->getTarif($v['kelaspelayanan_id'], $v['ruangan_id'], 4);
