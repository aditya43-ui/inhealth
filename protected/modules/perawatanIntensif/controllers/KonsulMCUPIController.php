...
Yii::import('rawatInap.controllers.KonsulMCURIController');
Yii::import('rawatInap.models.*');
class KonsulMCUPIController extends KonsulMCURIController
{
}
    /*
	...
	public $path_view = 'perawatanIntensif.views.konsulMCUPI.';
...
//            $modKonsul->pegawai_id = $modPendaftaran->pegawai_id;
            $modKonsul->pegawai_id = isset($modAdmisi->pegawai_id) ? $modAdmisi->pegawai_id : $modPendaftaran->pegawai_id;
...
//                        $modTarif = PITariftindakanM::model()->findByAttributes(array('daftartindakan_id'=>Params::DAFTARTINDAKAN_ID_KONSUL, 'komponentarif_id'=>Params::KOMPONENTARIF_ID_TOTAL, 'kelaspelayanan_id'=>$modPendaftaran->kelaspelayanan_id,'jenistarif_id'=>$jenistarif));
						...
						$listTarifTindakan = $_POST['PITindakanPelayananT'];
						foreach($listTarifTindakan as $i => $item){
							if(isset($item['cbTindakan'])){
								...
								$modTindakanPelayanan->daftartindakan_id = $_POST['PITindakanPelayananT'][$i]['daftartindakan_id'];
								$modTindakanPelayanan->tgl_tindakan = date( 'Y-m-d H:i:s');

								$modTindakanPelayanan->tarif_satuan = (isset($_POST['PITindakanPelayananT'][$i]['harga_tariftindakan']) ? $_POST['PITindakanPelayananT'][$i]['harga_tariftindakan'] : 0);
    ...
    /*
            ...
            $modRiwayatKonsul = PIKonsulPoliT::model()->findAllByAttributes(array('pendaftaran_id'=>$pendaftaran_id, 'asalpoliklinikkonsul_id'=>$ruangan_id));
