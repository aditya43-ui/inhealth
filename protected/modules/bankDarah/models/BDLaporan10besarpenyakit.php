 ...
 * @author sujana
 */
class BDLaporan10besarpenyakit extends Laporan10besarpenyakitV {
    ...
    public function searchTable() {
		...
		if (!empty($this->ruangan_id)){
			$criteria->select = 'count(diagnosa_id) as jumlah, diagnosa_id, diagnosa_nama as data,  diagnosa_nama as diagnosa_nama, diagnosa_kode, ruangan_id, ruangan_nama';
			$criteria->group = 'diagnosa_nama, diagnosa_id, diagnosa_kode, ruangan_id, ruangan_nama';
		}
		else if (!empty($this->instalasi_id)){
			$criteria->select = 'count(diagnosa_id) as jumlah, diagnosa_id, diagnosa_nama as data, diagnosa_kode, diagnosa_nama as diagnosa_nama, instalasi_id, instalasi_nama';
			$criteria->group = 'diagnosa_nama, diagnosa_id, diagnosa_kode, instalasi_id, instalasi_nama';
		}else{
			$criteria->select = 'count(diagnosa_id) as jumlah, diagnosa_id, diagnosa_nama as data, diagnosa_nama as diagnosa_nama, diagnosa_kode';
...
//            $criteria->limit = $this->jumlahTampil;
    ...
    public function searchPrint() {
        ...
        $this->tgl_akhir = $format->formatDateTimeForDb($this->tgl_akhir);
        if (!empty($this->ruangan_id)){
			$criteria->select = 'count(diagnosa_id) as jumlah, diagnosa_id, diagnosa_nama as data, diagnosa_kode, diagnosa_nama as diagnosa_nama, ruangan_id, ruangan_nama';
			$criteria->group = 'diagnosa_nama, diagnosa_id, diagnosa_kode, ruangan_id, ruangan_nama';
		}
		else if (!empty($this->instalasi_id)){
			$criteria->select = 'count(diagnosa_id) as jumlah, diagnosa_id, diagnosa_nama as data, diagnosa_kode, diagnosa_nama as diagnosa_nama, instalasi_id, instalasi_nama';
