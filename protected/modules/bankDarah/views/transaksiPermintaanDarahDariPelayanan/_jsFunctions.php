...
function cekKunjungan() {
    ...
    if(pendaftaran_id == '') {
        myAlert('Silahkan Pilih Kunjugan Terlebih Dahulu');
        return false;
    }
    $("#dialogTipePaket").dialog("open");
}

function cekDataPencatatanStok() {
    var pendaftaran_id = $('#pendaftaran_id').val();
    var jeniskomponendarah_id = [];

    var jumlahtr = $('#content-pencatatan-stok table tbody > tr').length;
    
    
    if(jumlahtr < 1) {
        myAlert('Belum Ada Daftar Pencatatan Stok');
        return false;
    }
    $('#content-pencatatan-stok table tbody > tr').each(function(){
        var tipepaket = $(this).find("input[name$='[jeniskomponendarah_id]']").val();
        jeniskomponendarah_id.push(tipepaket);
    });
    
    console.log(jumlahtr, jeniskomponendarah_id);
    $.post('<?= $this->createUrl('cekDataPencatatanStok') ?>', {
        pendaftaran_id:pendaftaran_id,
        jeniskomponendarah_id:jeniskomponendarah_id
    }, function(data){
        if(data.sukses == 1) {
            $("#dialogDaftarTindakanPaket").dialog('open');
        } else {
            if(data.belumSimpan == 1) {
                $('#peringatan').html(data.pesan);
                $('#dialogPeringatan').dialog('open');
            } else {
                myAlert(data.pesan);
            }
        }
...
function setCheckedPemeriksaanDariStok(){
    var jumlahtr = $("#form-tindakanpemeriksaan table tr").length;
    var pendaftaran_id = $('#pendaftaran_id').val();
    $.post('<?= $this->createUrl('SalinPemeriksaanDariStok') ?>', {
        pendaftaran_id:pendaftaran_id,
        jumlahtr:jumlahtr
    }, function(data) {
        if(data.sukses == 1) {
...
function simpanPencatatanStok(obj, jeniskomponendarah_id) { 
    ...
    var permintaankepenunjang_id = $(obj).parents('tr').find('.permintaankepenunjang_id').val();
    ...
    $.post('<?= $this->createUrl('simpanPencatatanStok') ?>', {
        ...
        if(data.sukses == 1) {
            ...
            $(obj).parents('tr').find('.jeniskomponendarah_id').val(jeniskomponendarah_id);
            $(obj).parents('tr').find('.permintaankepenunjang_id').val(data.datapermintaanpenunjang.permintaankepenunjang_id);
            $('#pasienkirimkeunitlain_id').val(data.datakirimkeunitlain.pasienkirimkeunitlain_id);
            disabledInput(obj);
        } else {
...
function setPencatatanStok(jeniskomponendarah_id, jeniskomponenedarah_nama) {
    var jumlahtr = $('#content-pencatatan-stok tbody > tr').length;
    $('#content-pencatatan-stok').addClass('animation-loading');
    $.post('<?= $this->createUrl('setPencatatanStok') ?>', {
        jeniskomponendarah_id:jeniskomponendarah_id,
        jeniskomponenedarah_nama:jeniskomponenedarah_nama,
        jumlahtr:jumlahtr,
    }, function(data){
        console.log(data);
        $('#content-pencatatan-stok tbody').append(data.html);
        // $('.jumlahkantong').css('cursor', 'pointer');
        // $('.diambil').css('cursor', 'pointer');
        // $('.dititip').css('cursor', 'pointer');
        $('#content-pencatatan-stok').removeClass('animation-loading');
    }, 'json');
    console.log(jeniskomponendarah_id, jeniskomponenedarah_nama, jumlahtr);
}
...
function disabledInput(obj) {
    $(obj).parents('tr').find('.diambil, .dititip, .jumlahkantong').attr('readonly', true);
    $(obj).parents('tr').find('.jenisvolumejumlahkantong, .jenisvolumedititip, .jenisvolumediambil').attr('disabled', true);
    $(obj).parents('tr').find('.ubahpencatatan').removeClass('hide');
    $(obj).parents('tr').find('.simpanPencatatan').addClass('hide');
}
...
function setKunjungan(pendaftaran_id, datajson){
    $("#form-datakunjungan > div").addClass("animation-loading");
    $('#content-pencatatan-stok').addClass('animation-loading');
    $('#content-pencatatan-stok table tbody').html('');
    $.ajax({
        type:'POST',
        ...
        success:function(data){
            if(data.sukses == 1) {
                ...
                $('#content-pencatatan-stok table tbody').append(data.html);
                ...
                $("#BDPasienKirimKeUnitLainV_nama_pegawai").val(data.nama_dokter);
                $("#BDPasienKirimKeUnitLainV_golongandarah").val(data.golongandarah);
                $("#BDPasienKirimKeUnitLainV_diagnosa_nama").val(data.diagnosa_utama);
