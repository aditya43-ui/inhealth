...
<script>
    function setPetugas(nama, id) {
        $("#<?php echo CHtml::activeId($model, 'peg_pemeriksa_id') ?>").val(id);
        $("#<?php echo CHtml::activeId($model, 'peg_pemeriksa_nama') ?>").val(nama);
        $("#dialogPetugas").dialog('close');

        $("#<?php echo CHtml::activeId($model, 'peg_pemeriksa_nama') ?>").blur();
    }

    function hasilKesimpulan() {
        var anti_a = $(".anti-a").find("input:radio:checked").attr('value');
        var anti_b = $(".anti-b").find("input:radio:checked").attr('value');
        var anti_d = $(".anti-d").find("input:radio:checked").attr('value');

        if (anti_a != '' && anti_b != '' && anti_d != '') {
            $.ajax({
                type: 'POST',
                url: '<?php echo $this->createUrl('hasilKesimpulan'); ?>',
                data: {anti_a: anti_a, anti_b: anti_b, anti_d: anti_d},
                dataType: "json",
                success: function (data) {
                    if (data.sukses == 1) {
                        $("#<?php echo CHtml::activeId($model, 'kesimpulan_uji') ?>").val(data.kesimpulan);

                        // setTimeout($("#<?php echo CHtml::activeId($model, 'kesimpulan_uji') ?>").blur(), 500);
                    } else {
                        myAlert(data.pesan);
                    }
...
//                           "BDInfostokkantongdarahV[gol_darah]":golongandarah,			
    ...
    function setKantong(data) {
        ...
        console.log(row_no);
        $("#table-lembarkerja tbody tr").each(function () {
            ...
            if (stokkantong != caristokkantong) {
                if ($(this).data('row') == row_no) {
                    if(data.no_kantongpabrik != null) {
                        $(this).find("#no_kantongdarah").val(data.no_kantongpabrik);
                        $(this).find('.no_kantongpabrik').val(data.no_kantongpabrik);
                    } else {
                        $(this).find("#no_kantongdarah").val(data.nomorbarcode);
                        $(this).find('.no_kantongpabrik').val(data.nomorbarcode);
                    }
...
?>
    var row = <?php echo CJSON::encode(array('html' => $this->renderPartial($this->path_view . 'form/_rowLembarKerja', array('modPemeriksaanGolDar' => $modPemeriksaanGolDar), true))); ?>;

    function tambahRowBarang(obj) {
        var last = "";
        if (obj != null) {
            $(obj).parents("tr").after(row.html);
            renameInputRowBarang($("#table-lembarkerja"));
            last = $("#table-lembarkerja tbody tr").eq($(obj).parents("tr").index() + 1);

            setAutoKontrol($("#autoKontrol"));
            //console.log($(obj).parents("tr").index());
        } else {
            $("#table-lembarkerja tbody").append(row.html);
            renameInputRowBarang($("#table-lembarkerja"));
            last = $("#table-lembarkerja tbody tr:last-child");
        }
    ...
    function pengujianKompatibilitas(obj) {
        ...
        if (jenis == true) {
            $.ajax({
                type: 'POST',
                url: '<?php echo $this->createUrl('hasilKesimpulanKantong'); ?>',
    ...
    $(document).ready(function () {
        ...
        var ru  = jQuery('.dropdownpencarian');
        ...
        setValidasiCekDisabled($("#pengujiankantongdarah-form"), function () {
            var jml = $("#form-pilihpemeriksaan").find('input:radio:checked').length;
            if (jml < 3) {
                return false;
            }
            return true;
        ...
        $("#form-pilihpemeriksaan").find('input:radio').click(function () {
            hasilKesimpulan();
        });
