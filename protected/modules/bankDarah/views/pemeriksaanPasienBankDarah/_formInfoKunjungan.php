        ...
        <div class="controls">
            ...
            <?php 
                $this->widget('MyJuiAutoComplete', array(
                                ...
                                'htmlOptions'=>array('placeholder'=>'No. Masuk Penunjang','class'=>'all-caps','rel'=>'tooltip','title'=>'No. masuk penunjang / klik icon untuk mencari data kunjungan',
                                    'onkeyup'=>"return $(this).focusNextInputField(event)",                                    
                                    ),
                            )); 
            ?>
...
$this->beginWidget('zii.widgets.jui.CJuiDialog', array( // the dialog
    'id'=>'dialogKunjungan',
    'options'=>array(
        'title'=>'Pencarian Data Kunjungan Pasien Bank Darah',
        'autoOpen'=>false,
        'modal'=>true,
        'width'=>980,
        'height'=>480,
    ...
    $modDialogKunjungan = new BDPasienMasukPenunjangV('searchDialogKunjungan');
    $modDialogKunjungan->unsetAttributes();
    $modDialogKunjungan->ruangan_id = Yii::app()->user->getState('ruangan_id');
    $modDialogKunjungan->tglmasukpenunjang = date('m/d/Y') . ' - ' . date('m/d/Y');
    if(isset($_GET['BDPasienMasukPenunjangV'])) {
        $modDialogKunjungan->attributes = $_GET['BDPasienMasukPenunjangV'];
    }
    ...
    $this->widget('ext.bootstrap.widgets.BootGridView',array(
            ...
            'columns'=>array(
                    ...
                    'no_masukpenunjang',
                    array(
						'header'=>'Tanggal Pendaftaran / Masuk Penunjang',
						'type'=>'raw',
						'value'=>'MyFormatter::formatDateTimeForUser($data->tgl_pendaftaran)
								.(isset($data->tglmasukpenunjang) ? "<br>".MyFormatter::formatDateTimeForUser($data->tglmasukpenunjang) : "")',
						'filter'=>  
                                                        CHtml::activeTextField($modDialogKunjungan, 'tglmasukpenunjang', array('class'=>'span3','readonly'=>true)),
...
//				jQuery("#'.CHtml::activeId($modDialogKunjungan, 'tglmasukpenunjang').'").datepicker(jQuery.extend({showMonthAfterYear:false}, jQuery.datepicker.regional["id"], {"dateFormat":"dd M yy","maxDate":"d","timeText":"Waktu","hourText":"Jam","minuteText":"Menit","secondText":"Detik","showSecond":true,"timeOnlyTitle":"Pilih Waktu","timeFormat":"hh:mm:ss","changeYear":true,"changeMonth":true,"showAnim":"fold","yearRange":"-80y:+20y"}));
//				jQuery("#'.CHtml::activeId($modDialogKunjungan, 'tglmasukpenunjang').'_date").on("click", function(){jQuery("#'.CHtml::activeId($modDialogKunjungan, 'tglmasukpenunjang').'").datepicker("show");});
                                jQuery("#'.CHtml::activeId($modDialogKunjungan, 'tglmasukpenunjang').'").daterangepicker({
                                    "maxDate": "' . date('m/d/Y') . '",
                                    "showDropdowns": true,
                                });
			}',
...
<script>
    $(document).ready(function(){
        $('input[name="BDPasienMasukPenunjangV[tglmasukpenunjang]"]').daterangepicker({
