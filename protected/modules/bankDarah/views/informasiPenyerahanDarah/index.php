...
Yii::app()->clientScript->registerScript('search', "
    ...
    $('#penyerahandarah-r-search').submit(function(){
            $.fn.yiiGridView.update('penyerahandarah-r-grid', {
                    data: $(this).serialize()
            });
            return false;
    });
...
<div class="panel panel-gradient">
    <div class="panel-heading">
        <div class="panel-title">
            <i class="glyphicon glyphicon-file"></i> Informasi <b>Penerimaan Darah Pasien</b>
        </div>
    </div>
    <div class="panel-body">
        ...
        <div class="panel panel-success">
            <div class="panel-heading">
                <div class="panel-title">
                    <i class="entypo-credit-card"></i> Tabel <b>Penerimaan Darah Pasien</b>
                </div>
            </div>
            <div class="panel-body table-responsive">
                <?php
                $this->widget('ext.bootstrap.widgets.BootGridView', array(
                    'id' => 'penyerahandarah-r-grid',
                    'replaceUrl' => true,
                    'dataProvider' => $model->searchInformasiPenerimaanDarah(),
                    'template' => "{summary}\n{items}\n{pager}",
                                ...
                                if (empty($data['is_pasiensama']) && $data['is_pasiensama'] === false) {
                                    return CHtml::link(
                                        ...
                                        array(
                                            ...
                                            // "title"=>"Klik untuk melakukan penerimaan darah"
                                        )
                                    );
                                } else {
                                    if (empty($data['penyerahandarah'])) {
                                        if (!empty($data['penyiapandarah'])) {
                                            echo CHtml::link("BELUM DITERIMA", Yii::app()->createUrl('bankDarah/penyerahanDarah/index&id=' . $data['permintaandarah_id']), array("rel" => "tooltip", "title" => "Klik untuk melakukan Penerimaan Darah"));
                                        } else {
                                            echo CHtml::link("BELUM DITERIMA", "javascript:;", array('onclick' => '', 'data-html' => true));
                                        }
                                    } else {
                                        ...
                                        if (!empty($data['penyiapandarahid'])) {
                                            if (count((array)$data['penyerahandarahid']) != count((array)$data['penyiapandarahid'])) {
                                                ...
                                                if (count((array)$data['penyiapandarahid']) != count((array)$data['ujikompatibilitasrelease'])) {
                                                    echo CHtml::link("BELUM DITERIMA", "javascript:;", array('onclick' => '', 'data-html' => true));
                                                    // echo CHtml::link("<i class='icon-pasienrujuk'></i>","javascript:;",array("rel"=>"tooltip","title"=>"Klik untuk melakukan pengiriman Darah", 'onclick' => 'myAlert("pengujian pengiriman darah belum dilakukan ")','data-html'=>true));
                                                } else {
...
$this->beginWidget('zii.widgets.jui.CJuiDialog', array(
    ...
    'options' => array(
        'title' => 'Batal Permintaan Darah Pasien',
        'autoOpen' => false,
        'modal' => true,
        'width' => 700,
        'height' => 370,
...
$this->beginWidget('zii.widgets.jui.CJuiDialog', array(
    ...
    // additional javascript options for the dialog plugin
    'options' => array(
        'title' => 'Detail Penerimaan Darah Pasien',
        'autoOpen' => false,
        'width' => 1000,
        'height' => 500,
        'resizable' => false,
...
$this->beginWidget('zii.widgets.jui.CJuiDialog', array(
    ...
    // additional javascript options for the dialog plugin
    'options' => array(
        'title' => 'Verifikasi Permintaan Darah Pasien',
        'autoOpen' => false,
        'width' => 1000,
        'height' => 500,
        'resizable' => false,
