...
<table class="table table-bordered table-condensed">
    ...
    <tbody id="tab_penyiapan">
        <?php
        $a = 0;
        foreach ($pengujianKompat as $row => $val) :

            foreach ($val['det'] as $item) {

                echo $this->renderPartial($this->path_view . "form/_rowPenyiapan", array(
                    'model' => $model,
                    'item' => $item,
                    'a' => $a,
                ), true);
...
<script>
    ...
    function setPetugasReferal(data) {
        ...
        $("#tab_penyiapan tr").eq(idx).find(".peg_referal_id").val(data.pegawai_id);
        $("#tab_penyiapan tr").eq(idx).find(".peg_referal_nama").val(data.nama_pegawai);
        $("#PenyiapandarahT_lamapenyiapan_detik").blur();
        $("#dialogPetugas").dialog("close");
        $("#tab_penyiapan tr").eq(idx).find(".tanggal").show();
    ...
    function setPetugasLabeling(data) {
        ...
        $("#tab_penyiapan tr").eq(idx).find(".peg_pelabelan").val(data.pegawai_id);
        $("#tab_penyiapan tr").eq(idx).find(".peg_pelabelan_nama").val(data.nama_pegawai);
        $("#PenyiapandarahT_lamapenyiapan_detik").blur();
