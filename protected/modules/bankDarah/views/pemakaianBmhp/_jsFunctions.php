...
function hitungSubTotal(obj)
{
    ...
    var qty_stok = parseFloat($(obj).parents('tr').find('input[name$="[qty_stok]"]').val());
    var hargasatuan_oa = parseFloat($(obj).parents('tr').find('input[name$="[hargasatuan_oa]"]').val());
    subtotal = (qty * hargasatuan_oa).toFixed(2);
	subtotal = Math.ceil(subtotal);
    $(obj).parents('tr').find('input[name$="[iurbiaya]"]').val(subtotal);
    if(qty > qty_stok){
        $(obj).val(qty_stok);
...
function totalKonversi(){
	...
	var jml = (qty_input / jmlkemasan).toFixed(2);
	if(jml <= 0){
		jml = 0;
	}
	$('#jmlkonversi').val(jml);
...
function totalJumlah(){
	...
	var jml = (jmlkonversi * jmlkemasan).toFixed(2);
	if(jml <= 0){
		jml = 0;
	}
	$('#qty_input').val(jml);
