...
$this->beginWidget('zii.widgets.jui.CJuiDialog', array( // the dialog
    'id'=>'dialogKunjungan',
    'options'=>array(
        'title'=>'Pencarian Data Rujukan Ke Bank Darah',
        'autoOpen'=>false,
        'modal'=>true,
        'width'=>980,
        'height'=>480,
    ...
    $modDialogKunjungan = new BDPasienKirimKeUnitLainV('searchDialogKunjungan');
    $modDialogKunjungan->unsetAttributes();
    $modDialogKunjungan->ruangan_id = Yii::app()->user->getState('ruangan_id');
    $modDialogKunjungan->tgl_kirimpasien = date('m/d/Y') . ' - ' . date('m/d/Y');
    if(isset($_GET['BDPasienKirimKeUnitLainV'])) {
        $modDialogKunjungan->attributes = $_GET['BDPasienKirimKeUnitLainV'];
    }
    ...
    $this->widget('ext.bootstrap.widgets.BootGridView',array(
            ...
            'columns'=>array(
                    ...
                    'no_pendaftaran',
                    array(
						'header'=>'Tanggal Pendaftaran / Rujuk',
						'type'=>'raw',
						'value'=>'MyFormatter::formatDateTimeForUser($data->tgl_pendaftaran)
								.(isset($data->tgl_kirimpasien) ? "<br>".MyFormatter::formatDateTimeForUser($data->tgl_kirimpasien) : "")',
						'filter'=>
                                                        CHtml::activeTextField($modDialogKunjungan, 'tgl_kirimpasien', array('class'=>'span3','readonly'=>true)),
...
//				jQuery("#'.CHtml::activeId($modDialogKunjungan, 'tgl_kirimpasien').'_date").on("click", function(){jQuery("#'.CHtml::activeId($modDialogKunjungan, 'tgl_kirimpasien').'").datepicker("show");});
                                jQuery("#'.CHtml::activeId($modDialogKunjungan, 'tgl_kirimpasien').'").daterangepicker({
                                    "maxDate": "' . date('m/d/Y') . '",
                                    "showDropdowns": true,
                                });
			}',
...
<script>
    $(document).ready(function(){
        $('input[name="BDPasienKirimKeUnitLainV[tgl_kirimpasien]"]').daterangepicker({
