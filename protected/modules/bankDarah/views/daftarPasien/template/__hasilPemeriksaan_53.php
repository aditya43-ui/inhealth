<table border="0" width="100%" class="grid" cellpadding="0" cellspacing="0">
    ...
    <tbody>
        <?php
            ...
            {
                $rows = '';
                if(count((array)$val['pemeriksaan']) > 1)
                {
                    $namapemeriksaan = '';
                    $hasil = '';
                    $satuan = '';
                    ...
                    foreach($val['pemeriksaan'] as $temp)
                    {
                        ...
                        if($val_pem)
                        {
                            $rows .= '<td valign="top" rowspan="'. count((array)$val['pemeriksaan']) .'">'. $idx .'</td>';
                            $rows .= '<td valign="top" width="10%" rowspan="'. count((array)$val['pemeriksaan']) .'"><b>'. $val['kelompok'] . '</b></td>';
                            $val_pem = false;
                        }
                        $rows .= '<td valign="top" width="15%">'. $temp['namapemeriksaan'] . '</td>';
                    ...
                    foreach($val['pemeriksaan'] as $temp)
                    {
                        ...
                        $row .= '<tr>';
                        $row .= '<td>'. $idx .'</td>';
                        $row .= '<td colspan="2"><div>'. $temp['kelompok'] .'</div></td>';
