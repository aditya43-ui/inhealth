...
<table style="width:100%;font-family: arial;font-size: 10pt;">
    <tr ><?php $format=new MyFormatter(); $tgl = explode(" ",$modHasilPeriksa->update_time); $tgl_periksa = $format->formatDateTimeForDb($tgl[0]." ".$tgl[1]." ".$tgl[2]);?>
        <td width="50%" style="border:none;"><p style="margin: 0; text-align: center;"><?php echo Yii::app()->user->getState("kabupaten_nama").", ".$format->formatDateTimeId(date($tgl_periksa)); ?>
                ...
                <div style="font-family:arial;font-size:16pt;">
                    <b>
                        <p style="margin: 0; text-align: center;">
                            <h3>HASIL PEMERIKSAAN BANK DARAH</h3>
                        </p>
                    </b>
                </div>
            </td>
...
$urlPrint=  Yii::app()->createAbsoluteUrl('/bankDarah/pencatatanHasilPemeriksaan/print', array('pasienmasukpenunjang_id'=>$masukpenunjang->pasienmasukpenunjang_id));
...
//        echo CHtml::htmlButton(Yii::t('mds','{icon} PDF',array('{icon}'=>'<i class="entypo-print"></i>')),array('class' => 'btn btn-danger', 'type'=>'button','onclick'=>'print(\'PDF\')'));     
        echo CHtml::htmlButton(Yii::t('mds','{icon} Print',array('{icon}'=>'<i class="entypo-print"></i>')),array('class'=>'btn btn-info', 'type'=>'button','onclick'=>'print(\'PRINT\')'));     
        if ($goldarah > 0){
            echo CHtml::htmlButton(Yii::t('mds','{icon} Print Kartu Golongan Darah',array('{icon}'=>'<i class="entypo-print"></i>')),array('class'=>'btn btn-info', 'type'=>'button','onclick'=>'printKartuGolonganDarah(\'PDF\')'));
        }
        if(!isset($_GET['popup']))
            echo CHtml::link(Yii::t('mds', '{icon} Kembali', array('{icon}'=>'<i class="entypo-arrows-ccw"></i>')), $this->createUrl('index',array('modul_id'=>Yii::app()->session['modul_id'])), array('class'=>'btn btn-danger'));
}
?> 
