 ...
 * 
 */
class PenyiapanDarahController extends MyAuthController
{
  public $path_view = "bankDarah.views.penyiapanDarah.";
  public function actionIndex($id = null, $ujidarahtube_id = null, $frame = 0, $penyiapandarah_ke = null, $pendaftaran_id = null, $pasienkirimkeunitlain_id = null)
  {
    ...
    if (!empty($pasienkirimkeunitlain_id)) {
      ...
      $pengujianSlide = UjidarahpasienT::model()->findByAttributes(array(
        'permintaandarah_id' => $pasienkirimkeunitlain_id,
       ...
       */
      $pengujianTube = UjidarahpasienT::model()->findByAttributes(['pendaftaran_id' => $pendaftaran_id, 'penyiapandarah_id' => null]);
      // echo '<pre>';var_dump($pengujianTube);die;
      $model->ujidarahtube_id = $ujidarahtube_id;
      $pengujianKompat = array();
      if (!empty($pengujianTube)) {
        ...
        $criUji->join = "   LEFT JOIN penyiapandarah_t siap ON siap.ujikompatibilitas_id = t.ujikompatibilitas_id "
        ...
        if (isset($_GET['sukses'])) {
          ...
          $criUji->addCondition(" ujidarahpasien_id = '" . $pengujianTube->ujidarahpasien_id . "' AND siap.penyiapandarah_id is null");
        }
        // $criUji->addInCondition("rilis", Params::getRilis());

        // echo '<pre>';var_dump($criUji);die;
    ...
    if (isset($_POST['PenyiapandarahT'])) {
      ...
      $ok = true;
      try {
        $ujidarahtube_id = $_POST['PenyiapandarahT']['ujidarahtube_id'];
        $gen = MyGenerator::penyiapanDarahKe($pasienkirimkeunitlain_id);
        foreach ($_POST['PenyiapandarahT']['detail'] as $ujikompatibilitas_id => $detail) {
          ...
          $submodel->ujikompatibilitas_id = $ujikompatibilitas_id;
          ...
          // echo '<pre>';var_dump($submodel->validate(), $submodel);die;
          if ($submodel->validate()) {
            $ok = $ok && $submodel->save();
            $modUjikompatibilitas = UjikompatibilitasT::model()->findByPk($ujikompatibilitas_id);
            $modStokKantong = $modUjikompatibilitas->stokkantong;

            if(!empty($modStokKantong)) {
              $modStokKantong->ujikompatibilitas_id = $ujikompatibilitas_id;
              $modStokKantong->update();
            }
          } else {
            $ok = false;
          }
        ...
        if ($ok) {
          if(!empty($pengujianTube->ujidarahpasien_id)) {
              $pengujianTube->penyiapandarah_id = $submodel->penyiapandarah_id;
              $pengujianTube->update();
          }
          $trans->commit();
          $this->notifPenyiapanDarah($modPasienKirimKeunitlain, $pendaftaran);
          Yii::app()->user->setFlash('success', "Data Berhasil Disimpan");
          $this->redirect(array('index', 'id' => $id, 'ujidarahtube_id' => $ujidarahtube_id, 'penyiapandarah_ke' => $submodel->penyiapandarah_ke, 'frame' => $frame, 'sukses' => 1, 'pendaftaran_id' => $pendaftaran_id, 'pasienkirimkeunitlain_id' => $pasienkirimkeunitlain_id));
        } else {
          $trans->rollback();
