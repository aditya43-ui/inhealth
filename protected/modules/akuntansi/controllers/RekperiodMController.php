...
class RekperiodMController extends MyAuthController
{
  ...
  public function actionCreate()
  {
    ...
    if (isset($_POST['AKRekperiodM'])) {
      ...
      if ($model->save()) {
        Yii::app()->user->setFlash('success', 'Data ' . $model->deskripsi . ' berhasil disimpan.');
  ...
  {
    $this->pageTitle = Yii::app()->name . " - Periode Akuntansi";
    $format = new MyFormatter();
  ...
  private function cekSessionPeriode()
  {
    ...
    // Yii::app()->user->getState('periode_ids') = $periode;
  }

  private function cekValidasiPeriode($model)
  {
    $cr = new CDbCriteria();
    $cr->addCondition("('" . $model->perideawal . "'::date between perideawal and sampaidgn " .
      " or '" . $model->sampaidgn . "'::date between perideawal and sampaidgn) "
      . "and isclosing = false");

    if (!empty($model->rekperiod_id)) {
      $cr->addCondition('rekperiod_id <> ' . $model->rekperiod_id);
    }

    $rek = RekperiodM::model()->find($cr);
    // var_dump($cr, $model->attributes, !empty($rek)); die;
    return empty($rek);
  }
}
