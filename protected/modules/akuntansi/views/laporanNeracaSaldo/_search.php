...
<div class="row-fluid">
    ...
    <div class="col-sm-6">
    <div class='control-group'>
        <?php echo CHtml::label('Rekening','kodeRekening', array('class'=>'control-label')) ?>
            <div class="controls">
            <?php echo $form->hiddenField($model,'rekening5_id', array('class' => '')) ?>
                <?php
        ...
        array(
            ...
            'value' => 'CHtml::Link("<i class=\"icon-form-check\"></i>","javascript::void(0)",array("class"=>"btn-small",
				...
				"onClick" =>" 
                    ...
                    $(\"#'.CHtml::activeId($model,'namaRekening').'\").val(\"".$data->namarekeninglast."\");
...
<script>
    ...
    function cekSaldoAwalPeriode(periodeposting_id) {
        $.post('<?php echo $this->createUrl('cekSaldoAwalPeriode')?>', {periode: periodeposting_id}, function(data) {
            if (data.ok == 1) $("#searchLaporan").submit();
            else {
                myConfirm("Saldo awal untuk periode ini tidak ditemukan. Apakah Anda akan melanjutkan?", "Perhatian", function(r) {
                    if (r) {
                        $("#searchLaporan").submit();
                    }
                });
