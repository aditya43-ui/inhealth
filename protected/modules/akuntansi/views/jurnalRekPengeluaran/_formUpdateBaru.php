...
<div class="row">
    ...
    <div class="col-sm-6">
        <div class="control-group">
            <?php echo $form->textFieldRow($model, 'jenispengeluaran_nama', array('empty' => '-- Pilih --', 'onkeypress' => "return $(this).focusNextInputField(event)", 'readonly' => true));
            ?>
        </div>
    </div>
    <div class="col-sm-6">
...
<?php
$rd = AKJnsPengeluaranRekM::model()->findAllByAttributes(array(
    'debitkredit' => 'D',
    'jenispengeluaran_id' => $model->jenispengeluaran_id,
), array('order' => 'rekening5_id ASC'));
$rk = AKJnsPengeluaranRekM::model()->findAllByAttributes(array(
    'debitkredit' => 'K',
    'jenispengeluaran_id' => $model->jenispengeluaran_id,
), array('order' => 'rekening5_id ASC'));
?>
        ...
        <table class="table table-bordered datatable" id="tab_rekening_debit">
            ...
            <tbody>
                ...
                foreach ($rd as $item) {
                    ...
                    echo $this->renderPartial('_rowRekeningJnsPengeluaran', array('item' => $item, 'r' => $r, 'dk' => 'D'), true);
                }
                ?>
            </tbody>
        </table>
    ...
    <div class="panel-body table-responsive">
        <table class="table table-bordered datatable" id="tab_rekening_kredit">
            ...
            <tbody>
                ...
                foreach ($rk as $item) {
                    $r = Rekening5M::model()->findByPk($item->rekening5_id);
                    echo $this->renderPartial('_rowRekeningJnsPengeluaran', array('item' => $item, 'r' => $r, 'dk' => 'K'), true);
                }
                ?>
            </tbody>
        </table>
...
<div class="form-actions">
    ...
    ?>
    <?php
    echo CHtml::link(Yii::t('mds', '{icon} Reset', array('{icon}' => '<i class="entypo-arrows-ccw"></i>')), Yii::app()->createUrl($this->module->id . '/jurnalRekPenjamin/admin'), array(
    ...
    'columns' => array(
        array(
            ...
            'value' => 'CHtml::Link("<i class=\"icon-form-check\"></i>","#",array("class"=>"btn-small",
				"id" => "selectRekDebit",
				"onClick" =>"
					$(\"#AKJenispengeluaranM_rekening_debit\").val(\"$data->rekeninglast_id\");
    ...
    'columns' => array(
        array(
            ...
            'value' => 'CHtml::Link("<i class=\"icon-form-check\"></i>","#",array("class"=>"btn-small",
				"id" => "selectRekDebit",
				"onClick" =>"
                $(\"#AKJenispengeluaranM_rekening_kredit\").val(\"$data->rekeninglast_id\");
...
<script type="text/javascript">
    ...
    function batalRekening(obj) {
        myConfirm("Apakah Anda yakin ingin menghapus rekening ini?", "Perhatian!",
            function(r) {
                if (r) {
                    $("#hapusrekening-supplier > tbody").append("<tr><td><input type='hidden' value='" + $(obj).parents("tr").find(".jnspengeluaranrek_id").val() + "' name='delete[jnspengeluaranrek_id][]'></td></tr>");
    ...
    function tambahRekening(st) {
        var idK = $("#<?php echo CHtml::activeId($model, 'rekening_kredit') ?>").val();
        var idD = $("#<?php echo CHtml::activeId($model, 'rekening_debit') ?>").val();
        var id = '';
        var tipe = '';
        ...
        $.post('<?php echo $this->createUrl('formRekening'); ?>', {
            ...
            if (st == 'K') {
                $("#<?php echo CHtml::activeId($model, 'rekKredit') ?>").val("");
                $("#<?php echo CHtml::activeId($model, 'rekening_kredit') ?>").val("");

                $("#tab_rekening_kredit tbody").append(data.dat);
            } else if (st == 'D') {
                $("#<?php echo CHtml::activeId($model, 'rekDebit') ?>").val("");
                $("#<?php echo CHtml::activeId($model, 'rekening_debit') ?>").val("");
