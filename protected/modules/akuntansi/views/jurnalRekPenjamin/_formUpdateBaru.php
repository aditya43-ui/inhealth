...
<div class="row">
    <!--<p class="help-block"><?php // echo Yii::t('mds', 'Fields with <span class="required">*</span> are required.') 
                                ?></p>-->
    <?php echo $form->errorSummary($modPenjamin); ?>
    <div class="col-sm-6">
    ...
    <div class="col-sm-6">
        <div class='control-group'>
            ...
            <div class="controls">
                <?php
                //$modSupplier->rekening_debit = $modPenjamind->rekening5_id;
                echo $form->hiddenField($modPenjamin, 'rekening_debit', array('class' => 'span3', 'maxlength' => 50)); ?>
                <?php
                //var_dump($modPenjamin["D"]->rekeningdebit->nmrekening5); die;
                //    if (!empty($modPenjamind->rekening5_id)) $modPenjamin->rekDebit = $modPenjamind->rekening5->nmrekening5;
        ...
        <div class='control-group'>
            ...
            <div class="controls">
                <?php
                //$modSupplier->rekening_kredit = $modPenjamink->rekening5_id;
                //if (!empty($modPenjamink->rekening5_id)) $modSupplier->rekKredit = $modPenjamink->rekening5->nmrekening5;
                echo $form->hiddenField($modPenjamin, 'rekening_kredit', array('class' => 'span3', 'maxlength' => 50)); ?>
                <?php
                $this->widget('MyJuiAutoComplete', array(
                    'model' => $modPenjamin,
                    'attribute' => 'rekKredit',
...
<?php
$rd = AKPenjaminRekM::model()->findAllByAttributes(array(
    'debitkredit' => 'D',
    'penjamin_id' => $modPenjamin->penjamin_id,
), array('order' => 'rekening5_id ASC'));
$rk = AKPenjaminRekM::model()->findAllByAttributes(array(
    'debitkredit' => 'K',
    'penjamin_id' => $modPenjamin->penjamin_id,
), array('order' => 'rekening5_id ASC'));
?>
        ...
        <table class="table table-bordered datatable" id="tab_rekening_debit">
            ...
            <tbody>
                ...
                foreach ($rd as $item) {
                    ...
                    echo $this->renderPartial('_rowRekeningPenjamin', array('item' => $item, 'r' => $r, 'dk' => 'D'), true);
                }
                ?>
            </tbody>
        </table>
    ...
    <div class="panel-body table-responsive">
        <table class="table table-bordered datatable" id="tab_rekening_kredit">
            ...
            <tbody>
                ...
                foreach ($rk as $item) {
                    $r = Rekening5M::model()->findByPk($item->rekening5_id);
                    echo $this->renderPartial('_rowRekeningPenjamin', array('item' => $item, 'r' => $r, 'dk' => 'K'), true);
                }
                ?>
            </tbody>
        </table>
        ...
        array(
            ...
            'value' => 'CHtml::Link("<i class=\"icon-form-check\"></i>","#",array("class"=>"btn-small",
				...
				"onClick" =>"
                    ...
                    $(\"#AKPenjaminpasienM_rekDebit\").val(\"$data->namarekeninglast\");                                                
        ...
        array(
            ...
            'value' => 'CHtml::Link("<i class=\"icon-form-check\"></i>","#",array("class"=>"btn-small",
				...
				"onClick" =>"
                    ...
                    $(\"#AKPenjaminpasienM_rekKredit\").val(\"$data->namarekeninglast\");                                                
...
<script type="text/javascript">
    ...
    function batalRekening(obj) {
        myConfirm("Apakah Anda yakin ingin menghapus rekening ini?", "Perhatian!",
            function(r) {
                if (r) {
                    $("#hapusrekening-supplier > tbody").append("<tr><td><input type='hidden' value='" + $(obj).parents("tr").find(".penjaminrek_id").val() + "' name='delete[penjaminrek_id][]'></td></tr>");
    ...
    function tambahRekening(st) {
        var idK = $("#<?php echo CHtml::activeId($modPenjamin, 'rekening_kredit') ?>").val();
        var idD = $("#<?php echo CHtml::activeId($modPenjamin, 'rekening_debit') ?>").val();
        var id = '';
        var tipe = '';
        ...
        $.post('<?php echo $this->createUrl('formRekening'); ?>', {
            ...
            if (st == 'K') {
                $("#<?php echo CHtml::activeId($modPenjamin, 'rekKredit') ?>").val("");
                $("#<?php echo CHtml::activeId($modPenjamin, 'rekening_kredit') ?>").val("");

                $("#tab_rekening_kredit tbody").append(data.dat);
            } else if (st == 'D') {
                $("#<?php echo CHtml::activeId($modPenjamin, 'rekDebit') ?>").val("");
                $("#<?php echo CHtml::activeId($modPenjamin, 'rekening_debit') ?>").val("");
