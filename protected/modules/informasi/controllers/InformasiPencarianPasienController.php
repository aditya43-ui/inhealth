...
class InformasiPencarianPasienController extends MyAuthController
{
  ...
  public $path_view = 'informasi.views.informasiPencarianPasien.';
  ...
  public function actionDetailKonsul($id)
  {
    ...
    $modPendaftaran = INPendaftaranT::model()->with('carabayar', 'penjamin')->findByPk($id);
    $this->render(
  ...
  public function actionCariPasien()
  {
    ...
    if (isset($_GET['INPasienM'])) {
      $model->attributes = $_GET['INPasienM'];
      $model->ceklis = $_REQUEST['INPasienM']['ceklis'];
      $model->tgl_rm_awal  = isset($_REQUEST['INPasienM']['tgl_rm_awal']) ? $format->formatDateTimeForDb($_REQUEST['INPasienM']['tgl_rm_awal']) : null;
      $model->tgl_rm_akhir = isset($_REQUEST['INPasienM']['tgl_rm_akhir']) ? $format->formatDateTimeForDb($_REQUEST['INPasienM']['tgl_rm_akhir']) : null;
      $model->tgl_awall = $format->formatDateTimeForDb($_REQUEST['INPasienM']['tgl_awall']);
      $model->tgl_akhirl = $format->formatDateTimeForDb($_REQUEST['INPasienM']['tgl_akhirl']);
      $model->diagnosa_kode = isset($_REQUEST['INPasienM']['diagnosa_kode']) ? $_REQUEST['INPasienM']['diagnosa_kode'] : null;
      $model->diagnosa_nama = isset($_REQUEST['INPasienM']['diagnosa_nama']) ? $_REQUEST['INPasienM']['diagnosa_nama'] : null;
      //                    $model->ceklis = isset($_REQUEST['INPasienM']['ceklis'])?$_REQUEST['INPasienM']['ceklis']:0;
      // print_r($model->ceklis);
      // exit();
    }
    // if(isset($_REQUEST['INPasienM'])){
    //     $model->attributes = $_REQUEST['INPasienM'];
    //     $model->tgl_rm_awal  = $format->formatDateTimeForDb($_REQUEST['INPasienM']['tgl_rm_awal']);
    //     $model->tgl_rm_akhir = $format->formatDateTimeForDb($_REQUEST['INPasienM']['tgl_rm_akhir']);
    //     $model->ceklis = $_REQUEST['INPasienM']['ceklis'];
  ...
  public function actionUbahPasienOld($id)
  {
    ...
    if (isset($_POST['INPasienM'])) {
      $random = rand(0000000, 9999999);
      $format = new MyFormatter();
      $model->attributes = $_POST['INPasienM'];
      ...
      if (!empty($model->photopasien)) { //if user input the photo of patient
        ...
        if ($model->save()) {
          ...
          //$model->tgl_rekam_medik  = $format->formatDateTimeForDb($_POST['INPasienM']['tgl_rekam_medik']);
  ...
  public function actionUbahPasien($id)
  {
    ...
    $format = new MyFormatter();
    $temLogo = $model->photopasien;
    $modPegawai = new INPegawaiM;

    if (!empty($model->pegawai_id)) {
      $modPegawai = INPegawaiM::model()->findByPk($model->pegawai_id);
    }

    if (isset($_POST['INPasienM']) && isset($_POST['PendaftaranT'])) {
      $random = rand(0000000, 9999999);
      $model->attributes = $_POST['INPasienM'];
      ...
      if (isset($_POST['INPegawaiM'])) {
        $model->pegawai_id = $_POST['INPegawaiM']['pegawai_id'];
      }
  ...
  {
    $model =  INPasienM::model()->findByPk($id);
    if ($model === null)
  ...
  {
    $this->layout = '//layouts/iframe';
    $modPendaftaran = new INPendaftaranT();
    $modPendaftaran->pasien_id = $pasien_id;
    $modPasien = INPasienM::model()->findByPk($modPendaftaran->pasien_id);
    $this->render($this->path_view . '_gridListKunjungan', array('modPendaftaran' => $modPendaftaran, 'modPasien' => $modPasien));
  }
  ...
  public function actionAutocompleteNobadge()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      $criteria->limit = 50;
      $models = INPasienM::model()->findAll($criteria);
  ...
  public function actionSetNobadge()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      $models = INPegawaiM::model()->findByPk($pegawai_id);
      if (!empty($models)) {
  ...
  public function actionDetailTindakan($id)
  {
    ...
    $modPasien = PasienM::model()->findByPK($modPendaftaran->pasien_id);
    $this->render(
      $this->path_view_pencarian . '_tindakan',
      array(
        'modPendaftaran' => $modPendaftaran,
