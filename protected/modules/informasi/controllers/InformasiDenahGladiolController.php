...
/**
 * pembuatan fungsi denah gladiol
 * RSST-4431
 ...
 * 
 */
class InformasiDenahGladiolController extends MyAuthController
{
  /**
  ...
  public function actionIndex($instalasi_id = '', $ruangan_id = '')
  {
    ...
    if ((isset($_POST['ajax'])) && (isset($_POST['ruangan']))) {
      ...
      $criteria->addCondition(((!empty($ruangan)) ? "t.ruangan_id =" . $ruangan . " and " : "") . 't.kamarruangan_aktif = true order by t.ruangan_id, t.kelaspelayanan_id, t.kamarruangan_nokamar, t.kamarruangan_nobed');
      $model = INInformasikamarinapV::model()->findAll($criteria);

      $row = $this->renderKamarRuangan($model, !empty($_POST['ruangan']) ? RuanganM::model()->findByPK($_POST['ruangan'])->ruangan_nama : "");
  ...
  protected function renderKamarRuangan($model, $ruangan_nama)
  {
    ...
    foreach ($pasien_kamar as $rowkamar) {
      ...
      $result .= isset($rowkamar['no_kamar']) ? $rowkamar['no_kamar'] : "";
      $result .= '</div>';
    ...
    $result .= '<script>
                       $( document ).ready(function() {
                        ...
                        draw();
                          ...
                          icontoilet="' . Yii::app()->request->baseUrl . '/images/denah/marker/icon-toilet.svg"; 
                          var imgsrcs = [icontoilet,iconkosong, iconlakilaki,iconperempuan,iconfarmasi,iconrenovasi,iconmusola,icongudang];
                          var farmasi = [];
                          var i=0;
                          var j=0;
                          //digunakan untuk validasi ruangan berdasarkan no kamar dan jenis kelamin
                            ...
                            for(j=0;j<pasien_kamar.length;j++){
                             ...
                             var hotspots=[ 
                                {id:"1",kamarruangan_nokamar:"farmasi",kelaspelayanan_nama:"",ruangan_id:"614",uri:"",x:60,y:550,radius:30,tip:"Bed:"},
                                {id:"1",kamarruangan_nokamar:"toilet",kelaspelayanan_nama:"",ruangan_id:"614",uri:"",x:960,y:300,radius:30,tip:"Bed:"},  
                            ];
                             hotspots[0].uri=iconfarmasi;
                             hotspots[0].tip="Apotek";
                             hotspots[1].uri=icontoilet;
                             hotspots[1].tip="Toilet";
