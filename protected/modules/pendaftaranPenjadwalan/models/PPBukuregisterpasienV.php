    ...
    public function searchTableLaporan()
	{
		...
		if (!empty($this->pegawai_id)){
			if (is_array($this->pegawai_id)){
				$criteria->addInCondition('p.pegawai_id', $this->pegawai_id);
			}else{
				$criteria->addCondition('p.pegawai_id ='. $this->pegawai_id);
			}
		}
		$criteria->order = 'instalasi_nama';
		$criteria->addCondition('DATE(t.tgl_pendaftaran) BETWEEN \''.$this->tgl_awal.'\'::date AND \''.$this->tgl_akhir.'\'::date');
