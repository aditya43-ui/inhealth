...
class PPPendaftaranT extends PendaftaranT
{
	...
	public $slotantrian;
	public $no_luarjadwal, $is_bpjs_manual, $no_buatjanji, $is_bpjs_rj = 0;
	public $is_kunjungan = 1;
	public $penerimapasien;
	...
	public function getRuanganItemsVK($instalasi_id = null)
	{
		...
		if (!empty($instalasi_id)) {
			//RSPMC-1046
			$criteria->addCondition(" instalasi_id= " . $instalasi_id);
			$criteria->addCondition("ruangan_id IN('" . Params::RUANGAN_ID_PERAWATAN_DARURAT . "', 538)");
		}
		$criteria->addCondition('ruangan_aktif = true');
		$criteria->order = "ruangan_nama";
		return RuanganM::model()->findAll($criteria);
	 ...
	 */
	public function getJenisKasusPenyakitItems($instalasi_id = null)
	{
		$criteria = new CdbCriteria();

		if(!empty($instalasi_id)){
			if($instalasi_id == Params::INSTALASI_ID_RD){
				$criteria->join = "JOIN kasuspenyakitruangan_m ON t.jeniskasuspenyakit_id = kasuspenyakitruangan_m.jeniskasuspenyakit_id JOIN ruangan_m ON ruangan_m.ruangan_id = kasuspenyakitruangan_m.ruangan_id";
				$criteria->addCondition('ruangan_m.instalasi_id = ' . $instalasi_id);
				$criteria->addCondition('ruangan_m.ruangan_aktif = true ');
			...
			$criteria->join = "JOIN kasuspenyakitruangan_m ON t.jeniskasuspenyakit_id = kasuspenyakitruangan_m.jeniskasuspenyakit_id JOIN ruangan_m ON ruangan_m.ruangan_id = kasuspenyakitruangan_m.ruangan_id";
			$criteria->addCondition("instalasi_id IN ('" . Params::INSTALASI_ID_RJ . "','" . Params::INSTALASI_ID_GIGI . "') ");
			// $criteria->addCondition('ruangan_m.instalasi_id = ' . Params::INSTALASI_ID_RJ);
			$criteria->addCondition('ruangan_m.ruangan_aktif = true ');
		}
	...
	public function getJenisKasusPenyakitItemsRI($ruangan_id = null)
	{
		$criteria = new CdbCriteria();
		$criteria->join = "JOIN kasuspenyakitruangan_m ON t.jeniskasuspenyakit_id = kasuspenyakitruangan_m.jeniskasuspenyakit_id JOIN ruangan_m ON ruangan_m.ruangan_id = kasuspenyakitruangan_m.ruangan_id";
	...
	public function getSMF($kode = null)
	{
		$criteria = new CdbCriteria();

		if (!empty($kode)) {
			$criteria->join = "JOIN kasuspenyakitruangan_m ON t.jeniskasuspenyakit_id = kasuspenyakitruangan_m.jeniskasuspenyakit_id JOIN ruangan_m ON ruangan_m.ruangan_id = kasuspenyakitruangan_m.ruangan_id";
			$criteria->compare('ruangan_m.kode_bpjs' , $kode);
		} else {
			$criteria->join = "JOIN kasuspenyakitruangan_m ON t.jeniskasuspenyakit_id = kasuspenyakitruangan_m.jeniskasuspenyakit_id JOIN ruangan_m ON ruangan_m.ruangan_id = kasuspenyakitruangan_m.ruangan_id";
			// $criteria->addCondition('ruangan_m.instalasi_id = ' . Params::INSTALASI_ID_RJ);
		}
	...
	public function getRuanganPenyakitItems($jeniskasuspenyakit_id = null, $instalasi_id = null)
	{
		$criteria = new CdbCriteria();
		
		if(!empty($jeniskasuspenyakit_id)){
			$criteria->join = "JOIN kasuspenyakitruangan_m ON t.ruangan_id = kasuspenyakitruangan_m.ruangan_id";
			$criteria->addCondition('kasuspenyakitruangan_m.jeniskasuspenyakit_id = '. $jeniskasuspenyakit_id);
			if(!empty($instalasi_id)){
				$criteria->addCondition('instalasi_id = '. $instalasi_id );
			}
			$criteria->addCondition('ruangan_aktif = true ');
	...
	public function getCaraBayarItems()
	{
		if(Yii::app()->user->getState('modul_id') == Params::MODUL_ID_PENDAFTARAN) {
			$condition = array('order'=>'carabayar_nama', 'condition' => 'carabayar_id !=' . Params::CARABAYAR_ID_MANDIRI_PIUTANG);
		} else {
			$condition = array('order'=>'carabayar_nama');
		}
		return CarabayarM::model()->findAllByAttributes(array('carabayar_aktif'=>true), $condition);
	}
	/**
