...
class PPBuatJanjiPoliT extends BuatjanjipoliT {
    ...
    public $is_checkin, $waktucheckin;
    public $slotantrian;
    ...
    public function search() {
        ...
        // $criteria->join = 'left join ruangan_m createruangan on createruangan.ruangan_id = t.create_ruangan '
        //         . 'left join pegawai_m pegawai on pegawai.pegawai_id = t.pegawai_id '
        //         . 'left join ruangan_m ruangan on ruangan.ruangan_id = t.ruangan_id '
        //         . 'left join pasien_m pasien on pasien.pasien_id = t.pasien_id';
        $criteria->join = 'left join pegawai_m pegawai on pegawai.pegawai_id = t.pegawai_id '
                . 'left join ruangan_m ruangan on ruangan.ruangan_id = t.ruangan_id '
                . 'left join pasien_m pasien on pasien.pasien_id = t.pasien_id';

        // if (!empty($this->tgl_awal) && !empty($this->tgl_akhir)) {
        //     $criteria->addBetweenCondition('DATE(t.tglbuatjanji)', $this->tgl_awal, $this->tgl_akhir);
        // }
        // if (!empty($this->tgl_awal_janji) && !empty($this->tgl_akhir_janji) ) {
        //     $criteria->addBetweenCondition('DATE(t.tgljadwal)', $this->tgl_awal_janji, $this->tgl_akhir_janji);   
        // }

        if (empty($this->nama_pasien) && !empty($this->tgl_awal_janji) && !empty($this->tgl_akhir_janji)) {
            $criteria->addBetweenCondition('DATE(t.tgljadwal)', $this->tgl_awal_janji, $this->tgl_akhir_janji);   
        }

        if (!empty($this->nama_pasien)){
            $criteria->addBetweenCondition('LOWER(pasien.nama_pasien)',strtolower($this->nama_pasien), true);
         
        }

        //  if ($this->tgl_awal_janji && $this->tgl_akhir_janji) {
        //      $criteria->addBetweenCondition('DATE(t.tgljadwal)', $this->tgl_awal_janji, $this->tgl_akhir_janji);
        //  }
       ...
       if (!empty($this->carabayar_id)) {
        ...
        // var_dump(Yii::app()->user->getState('instalasi_id'));die;
        // $criteria->addCondition('createruangan.instalasi_id ='. Yii::app()->user->getState('instalasi_id'));
        // $criteria->with = array('pegawai', 'ruangan', 'pasien','createruangan');
        
        return new CActiveDataProvider($this, array(
            'criteria' => $criteria,
            'sort'=>array(
                'defaultOrder'=>'tgljadwal desc',
            ),
        ));
    }
