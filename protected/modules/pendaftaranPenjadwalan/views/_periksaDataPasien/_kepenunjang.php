...
foreach($modMasukPenunjang as $row){
        ...
        $modSurat = SuratketeranganR::model()->findByAttributes(array(
            'pendaftaran_id'=>$row->pendaftaran_id,
            'jenissurat_id'=>Params::SURAT_KETERANGAN_KEMATIAN,
        ));
        ...
        } else if ($modSurat && $row->ruangan_id == Params::RUANGAN_ID_FORENSIK) {
            $subresult .= "".CHtml::link("<i class='icon-list-alt'></i> ".$row->ruangan_nama,Yii::app()->controller->createUrl("detailSuratKeteranganKematian",array("pendaftaran_id"=>$row->pendaftaran_id, "suratketerangan_id"=>$modSurat->suratketerangan_id)),array("id"=>"$row->no_pendaftaran","target"=>"pesan","rel"=>"tooltip","title"=>"Klik untuk Detail Surat keterangan Kematian '".$row->ruangan_nama."'", "onclick"=>"window.parent.$('#dialogDetailHasilLab').dialog('open');"))."<br>";
