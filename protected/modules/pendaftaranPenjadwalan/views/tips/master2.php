<span class="required"><i>Bagian dengan tanda * harus diisi.</i></span>
<p>
<table style="width:100%; padding: none; border: none;">
  <tr>
    <td style="vertical-align: middle;">1. </td>
    <td>Icon <i class="icon-form-lihat"></i> untuk melihat data lebih rinci.</td>
  </tr>
  <tr>
    <td style="vertical-align: middle;">2. </td>
    <td>Icon <i class="icon-form-ubah"></i> untuk mengubah data.</td>
  </tr>
  <tr>
    <td style="vertical-align: middle;">3. </td>
    <td>Icon <i class="icon-form-sampah"></i> untuk menghapus data.</td>
  </tr>
   <tr>
    <td style="vertical-align: middle;">4. </td>
    <td>Tombol <button class="btn btn-info" name="yt1" onclick="" type="button" disabled>
<i class="icon-book icon-white"></i>
PDF
</button> untuk membuka data dengan PDF.</td>
  </tr>
   <tr>
    <td style="vertical-align: middle;">5. </td>
    <td>Tombol <button class="btn btn-info" name="yt2" onclick="" type="button" disabled>
<i class="icon-pdf icon-white"></i>
Excel
</button> untuk membuka data dengan program Excel.</td>
  </tr>
  <tr>
    <td style="vertical-align: middle;">6. </td>
    <td>Tombol <button class="btn btn-info" name="yt3" onclick="" type="button" disabled>
<i class="entypo-print"></i>
Print
</button> untuk mencetak data.</td>
  </tr>
   <tr>
    <td style="vertical-align: middle;">7. </td>
    <td>Tombol <button class="search-button btn" href="#" disabled>
<i class="icon-accordion icon-white"></i>
Pencarian Lanjut
</button> untuk pencarian data lebih lanjut.</td>
  </tr>
</table>
</p>
