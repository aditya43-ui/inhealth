                    ...
                    foreach (CustomFunction::getNamaHari() as $key => $value) {
                        ...
                        if ($jumlah > $jumlahHari){
                            ...
                            if (strtolower(trim($value)) == strtolower(trim($tanggal[0]))){
                                $jadwal = JadwaldokterM::model()->findAll('(jadwaldokter_tgl between ? and ?) and ruangan_id = ? ORDER BY jadwaldokter_buka ASC',array($tahun.'-'.$bulan.'-'.$jumlah, $tahun.'-'.$bulan.'-'.$jumlah, $variable['id']));
                                echo '<td><b>'.$tgl.'</b>';
                                $ruangan = array();
