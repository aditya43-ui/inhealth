                        ...
                        if ($jumlah > $jumlahHari){
                            ...
                            if (strtolower(trim($value)) == strtolower(trim($tanggal[0]))){
                               // $jadwal = BuatjanjipoliT::model()->findAll('(tglbuatjanji between ? and ?) and ruangan_id =?',array($tahun.'-'.$bulan.'-'.$jumlah, $tahun.'-'.$bulan.'-'.$jumlah, $variable['id']));
								...
								$cri->addCondition(" ruangan_id = ".(isset($variable['id'])?$variable['id']:0)." ");
                                ...
                                foreach ($jadwal as $counter => $row) {
                                    ...
                                    $ruangan[$row->dokter_id][$counter] = $row->attributes;                                    
                                    $ruangan[$row->dokter_id][$counter]['nama_pegawai'] = $row->namaLengkap;									
									$ruangan[$row->dokter_id][$counter]['jumlahpasien'] = $row->getjumlahPasien($row->ruangan_id,$row->tgl_pendaftaran,$row->dokter_id);
                                    $ruangan[$row->dokter_id]['active'] = ($row->ruangan_id == $ru) ? 'active' : '';
                                }
