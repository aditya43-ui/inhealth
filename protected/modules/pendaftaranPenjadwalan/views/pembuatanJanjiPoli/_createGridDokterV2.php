                        ...
                        if ($jumlah > $jumlahHari){
                            ...
                            if (strtolower(trim($value)) == strtolower(trim($tanggal[0]))){
                                if (!empty($variable['id'])) {
                                    $jadwal = JadwaldokterM::model()->findAll('(jadwaldokter_tgl between ? and ?) and pegawai_id = ? ORDER BY jadwaldokter_buka ASC',array($tahun.'-'.$bulan.'-'.$jumlah, $tahun.'-'.$bulan.'-'.$jumlah, $variable['id']));
                                } else {
                                    $jadwal = array();
                                }
                                echo '<td><b>'.$tgl.'</b>';
