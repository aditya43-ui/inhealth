...
if ($model->jnspelayanan == 1) {
    ...
    $model->klsRawatNaik = $modPasienAdmisi->kelaspelayanan->kelaspelayanan_id;
}
...
<div class="form-actions">
    ...
    ?>
    <?php echo CHtml::htmlButton(Yii::t('mds', '{icon} Save', array('{icon}' => '<i class="entypo-check"></i>')), array('class' => 'btn btn-primary', 'disabled' => $disabledSave, 'onclick' => 'cekInput(this,14);return false;')); ?>
...
<script>
    ...
    function pilihJenisKunjunganBPJS() {
        ...
        var assesmen = $("#ARSepT_asesmen_pelayanan").val();
        if (jenis == "0") {
        ...
        $("#<?php echo CHtml::activeId($model, 'asesmen_pelayanan') ?> select").val(assesmen);
        if (jenis == "2" || jenis == "0") {
            // $("#ARSepT_asesmen_pelayanan").val(4);
            $("#<?php echo CHtml::activeId($model, 'asesmen_pelayanan') ?> select").val(4);
        }
    }

    $(document).ready(function() {
        var jenispelayanan = "<?php echo $model->jnspelayanan ?>";
        var penangunggjawab = "<?php echo $model->penanggungjwb_naikkls_id ?>";
        ...
        if (jenispelayanan == 1) {
            ...
            if (penangunggjawab != "") {
                $("#ARSepT_isNaikKelas").prop('checked', true)
                $('.is-naikkelas').show()
                $('.is-naikkelas').find(".not-required").addClass("required").removeClass("not-required");
                $("#ARSepT_penanggungjwb_naikkls_id").prop("disabled", false).parents(".control-group").show();
            } else {
                $('.is-naikkelas').hide()
                $('.is-naikkelas').find(".required").addClass("not-required").removeClass("required");
                $("#ARSepT_klsRawatNaik").val("");
