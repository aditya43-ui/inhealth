...
<div class="panel panel-success">
    <div class="panel-heading">
        <div class="panel-title">Data Pencarian Pasien</div>
    </div>
    <div class="panel-body">
        <?php
        echo $this->renderPartial('_formRekamMedik_new', array('form' => $form, 'model' => $model, 'modRujukanBpjs' => $modRujukanBpjs, 'modAsuransiPasien' => $modAsuransiPasien, 'modAsuransiPasienBpjs' => $modAsuransiPasienBpjs, 'modInfoKunjungan' => $modInfoKunjungan));
        ?>
    </div>
</div>
<div class="panel panel-success">
    ...
    <div class="panel-body" id="content-bpjs">
        <?php echo $this->renderPartial('_formSEP_new', array('form' => $form, 'model' => $model, 'modRujukanBpjs' => $modRujukanBpjs, 'modAsuransiPasien' => $modAsuransiPasien, 'modAsuransiPasienBpjs' => $modAsuransiPasienBpjs, 'modPengajuanApproval' => $modPengajuanApproval)); ?>
    </div>
</div>
...
<div class="row-fluid">
    <div class="form-actions">
        ...
        ?>
        <!-- di comment terlebih dahulu untuk mengecek inputan yang ada pada form nya -->
        ...
        <?php echo CHtml::htmlButton(Yii::t('mds', '{icon} Save', array('{icon}' => '<i class="entypo-check"></i>')), array('class' => (isset($_GET['sukses'])) ? 'btn btn-primary' : 'btn btn-primary', 'disabled' => $disabledSave, 'onclick' => 'cekInputNew(this);return false;')) ?>
...
<?php echo $this->renderPartial("pendaftaranPenjadwalan.views.pendaftaranRawatJalan.form.srk._indexSRK", array('modRujukanBpjs' => $modRujukanBpjs, 'modSep' => $model, 'form' => $form), true); ?>
<?php $this->endWidget(); ?>
<?php
...
<script>
    ...
    function cekInputNew(obj) {
        ...
        if (!cekNoPonselSEP()) {
            $(".form-sep-new").removeClass('animation-loading');
            return false;
        }
        if (status == 1) {
            $(".form-sep-new").addClass('animation-loading');
            $("#assep-t-form").submit();
        }
    }
    ...
    function cekNoPonselSEP() {
        ...
        var noPonselSep = $("#<?php echo CHtml::activeId($model, 'no_telpon_peserta') ?>").val();
        var isValid = /^\d+$/.test(noPonselSep);

        if (!isValid) {
            myAlert('Nomor Telepon Harus Berupa Angka');
            $("#<?php echo CHtml::activeId($model, 'no_telpon_peserta') ?>").focus();
            return false;
        }

        if (noPonselSep.length < 10) {
            myAlert("No. Handphone yang diinputkan kurang dari 10 karakter");
            $("#<?php echo CHtml::activeId($model, 'no_telpon_peserta') ?>").focus();
            return false;

        } else if (noPonselSep.length != 10 && noPonselSep.length > 14) {
            $("#<?php echo CHtml::activeId($model, 'no_telpon_peserta') ?>").focus();
