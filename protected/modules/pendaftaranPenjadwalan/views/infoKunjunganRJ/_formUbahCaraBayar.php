...
<div id="divNonBridging" hidden>
    <div class="control-group">
        ...
        <div class="controls">
            <?php
            echo CHtml::checkBox('is_bridging', false, ['rel' => 'tooltip', 'title' => 'Klik jika non bridging', 'onkeyup' => "return $(this).focusNextInputField(event);", 'onclick' => 'set_nonbridging(this)']);
            ?>
        </div>
    </div>
</div>
<div id="divInputNonBridging" hidden>
    ...
    <?php echo $form->dropDownListRow($modAsuransiPasien, 'kelastanggunganasuransi_id', CHtml::listData($modPendaftaran->getKelasPelayananItems(), 'kelaspelayanan_id', 'kelaspelayanan_nama'), array('disabled' => true, 'empty' => '-- Pilih --', 'class' => 'kelastanggunganasuransi_id', 'onkeypress' => "return $(this).focusNextInputField(event)", 'style' => 'width:120px;')); ?>
    <div class="control-group">
...
<div id="divAsuransiBpjs" hidden>
    ...
    <?php echo $form->textFieldRow($modAsuransiPasienBpjs, 'nokartuasuransi', array('placeholder' => 'Nomor Kartu Asuransi', 'class' => 'span3 nokartuasuransi', 'onkeyup' => "return $(this).focusNextInputField(event);", 'maxlength' => 13)); ?>
    <?php echo $form->textFieldRow($modAsuransiPasienBpjs, 'namapemilikasuransi', array('placeholder' => 'Nama Lengkap Pemilik Asuransi', 'class' => 'span3 namapemilikasuransi', 'onkeyup' => "return $(this).focusNextInputField(event);", 'maxlength' => 50)); ?>
    <div class="control-group">
        ...
        <div class="controls">
            <?php echo $form->dropDownList(
                ...
                array(
                    'class' => 'span3 jenispeserta_id', 'empty' => '-- Pilih --', 'onkeyup' => "return $(this).focusNextInputField(event)",
                )
            ); ?>
    ...
    ?>
    <?php echo $form->dropDownListRow($modAsuransiPasienBpjs, 'kelastanggunganasuransi_id', CHtml::listData(PPPendaftaranT::model()->getKelasTanggunganItems(), 'kelasbpjs_id', 'kelaspelayanan_nama'), array('disabled' => true, 'empty' => '-- Pilih --', 'class' => 'span3 kelastanggunganasuransi_id', 'onkeyup' => "return $(this).focusNextInputField(event)")); ?>
...
<div id="divAsuransiBpjsRD">
    <div class="row-fluid bpjs-igd">
        ...
        <div class="span6">
            <div class="control-group">
                ...
                <div class="controls">
                    ...
                    $this->widget('MyJuiAutoComplete', array(
                        ...
                        'source' => 'js: function(request, response) {
                        $.ajax({
                            url: "' . $this->createUrl('pendaftaranRawatDarurat/AutocompleteItemSEP') . '",
                            dataType: "json",
                            data: {
                                term: request.term,
                                item: "poli",
            ...
            $this->Widget('ext.bootstrap.widgets.BootAccordion', array(
                ...
                'content' => array(
                    'content-suplesi' => array(
                        ...
                        'isi' => $this->renderPartial('pendaftaranPenjadwalan.views.pendaftaranRawatDarurat._formSuplesi', array(
                            'form' => $form,
                            'model' => $modSep,
                        ), true),
                        'active' => false,
            ...
            <?php echo $form->textAreaRow($modSep, 'catatansep', array('placeholder' => '', 'class' => 'span3 catatansep', 'onkeyup' => "return $(this).focusNextInputField(event)")); ?>
...
<div class="form-actions">
    ...
    if (!isset($_GET['sukses'])) {
        ...
        echo CHtml::htmlButton(
            $model->isNewRecord ? Yii::t('mds', '{icon} Create', array('{icon}' => '<i class="entypo-check"></i>')) : Yii::t('mds', '{icon} Save', array('{icon}' => '<i class="entypo-check"></i>')),
            array('class' => 'btn btn-danger', 'type' => 'button', 'onKeypress' => 'return formSubmit(this,event)', 'onclick' => 'cekNoBPJS(this);', 'id' => 'btn_submit')
        );
    } else {
        echo CHtml::htmlButton(
            $model->isNewRecord ? Yii::t('mds', '{icon} Create', array('{icon}' => '<i class="entypo-check"></i>')) : Yii::t('mds', '{icon} Save', array('{icon}' => '<i class="entypo-check"></i>')),
...
<script type="text/javascript">
    ...
    function onClickAsuransi() {
        ...
        if (cara_bayar == cara_bayar_bpjs) {
            ...
            if (Yii::app()->user->getState('isbridging')) {
            ?>
                if ("<?php echo $modPendaftaran->instalasi_id; ?>" == <?php echo Params::INSTALASI_ID_IGD ?>) {
                    ...
                    $('#divAsuransiBpjsRD input').removeAttr('disabled');
                    $('#divAsuransiBpjsRD select').removeAttr('disabled');
                    $('#divAsuransiBpjsRD textarea').removeAttr('disabled');
                    $('#divAsuransiBpjsRD').show();
                    $('#divNonBridging').show();

                } else {

                    $('#divAsuransiBpjsRD').hide();
                    $('#divAsuransiBpjsRD input').attr('disabled', 'true');
                    $('#divAsuransiBpjsRD select').attr('disabled', 'true');
                    $('#divAsuransiBpjsRD textarea').attr('disabled', 'true');
                    $('#divAsuransiBpjsRD input').attr('value', '');
                    $('#divAsuransiBpjsRD select').attr('value', '');
                    $('#divAsuransiBpjsRD textarea').attr('value', '');
    ...
    function listPenjamin(idCaraBayar) {
        $.post("<?php echo $this->createUrl('getListPenjamin') ?>", {
            ...
            success: function(data) {
                ...
                if (obj.metaData.code == '201') {
                    ...
                    if (obj.response != null) {
                        ...
                        $('#<?php echo CHtml::activeId($modSep, 'ppkpelayanan'); ?>').val(faskes[0].kode);
...
echo $this->renderPartial('pendaftaranPenjadwalan.views.pendaftaranRawatDarurat._pencarianDpjpMelayani');
$this->endWidget();
?>