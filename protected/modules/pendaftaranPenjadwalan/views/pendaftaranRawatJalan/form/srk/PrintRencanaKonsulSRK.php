...
<table border = "0"  style = "text-align:left; color:black" width="100%">
    <tr>
        ...
        <td><?php 
            ...
            }else if(!empty($model->kodedokterkontrol)){
                $peg = PegawaiM::model()->findByAttributes(array('kodedokter_bpjs' => $model->kodedokterkontrol));
                echo $peg->namaLengkap;
            } 
            else {
                $peg = PegawaiM::model()->findByAttributes(array('kodedokter_bpjs'=> $res['response']['kodeDokterPembuat']));
                echo $peg->namaLengkap;
            }
    ...
    <tr>
        ...
        <td width="10">:</td>
        <td><?php echo empty($asuransi->nokartuasuransi) ? $res['response']['sep']['peserta']['noKartu'] : $asuransi->nokartuasuransi; ?></td>
    </tr>
    <tr>
        <td width="150">Nama Peserta</td>
        <td width="10">:</td>
        <td><?php echo empty($modPasien->nama_pasien) ? $res['response']['sep']['peserta']['nama']: $modPasien->nama_pasien; ?></td>
    </tr>
    <tr>
        <td width="150">Tgl. Lahir</td>
        <td width="10">:</td>
        <td><?php echo empty($modPasien->tanggal_lahir)? $res['response']['sep']['peserta']['tglLahir']: MyFormatter::formatDateTimeForUser($modPasien->tanggal_lahir) ?></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td width="10">:</td>
        <td><?php echo !empty($modPendaftaran->tglrenkontrol) ? MyFormatter::formatDateTimeForUser($modPendaftaran->tglrenkontrol) : (!empty($model->tglkontrol) ? MyFormatter::formatDateTimeForUser($model->tglkontrol) : MyFormatter::formatDateTimeForUser($res['response']['tglRencanaKontrol'])) ?></td>
    </tr>
    <tr>
