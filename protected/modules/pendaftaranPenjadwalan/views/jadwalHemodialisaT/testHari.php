...
?>
    ...
    <table>
		...
		<tr>
			...
			<td width="100px">
				<?php
                    echo CHtml::dropDownList('jadwalHD[IdHari]', '', CHtml::listData(PPJadwalhariM::model()->findAllByAttributes(array('jadwalhari_aktif'=>true)), 'jadwalhari_id', 'jadwalhari_nama'), 
                                            array('empty'=>'-- Pilih --',
                                                  'id'=>'IdHari',
...
//		$('#tanggalHemodialisa_0_0').datetimepicker(jQuery.extend({showMonthAfterYear:false}, jQuery.datepicker.regional['id'], {'showAnim':'fold','dateFormat':'yy-mm-dd','changeFirstDay':false,'changeMonth':true,'timeText':'Waktu','hourText':'Jam','minuteText':'Menit','secondText':'Detik','showSecond':true,'timeOnlyTitle':'Pilih Waktu','timeFormat':'hh:mm:ss','changeYear':true,'yearRange':'-80y:+20y'}));
        ...
        var test = jQuery('input[name*="[jadwalhemodialisa_tgl]"]').val();
		console.log(test);
//				.datetimepicker(
//            jQuery.extend(
//                {
...
function insertInputJadwal(idTabel, idShift, idRuangan, obj)
{
    ...
    var input = '';
	input += '<li><div class="input-append"><input type="text" class="spans3" name="jadwalHemo[jadwal]['+idTabel+']['+idShift+']['+idRuangan+'][namapasien]['+jmlBaris+']" id="namaPasien_'+idTabel+'_'+idShift+'_'+idRuangan+'_'+jmlBaris+'" style="float:left;" onkeypress="return $(this).focusNextInputField(event)" class="ui-autocomplete-input" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true"><span class="add-on"><a href="javascript:void(0);" id="" onclick="setDialog(this);"><i class="icon-list"></i><i class="entypo-search"></i></a></span></div></li>';
    input += '<input type="hidden" class="span1" name="jadwalHemo[jadwal]['+idTabel+']['+idShift+']['+idRuangan+'][pasien_id]['+jmlBaris+']">';
	input += '<li><input type="text" class="spans2 beds" name="jadwalHemo[jadwal]['+idTabel+']['+idShift+']['+idRuangan+'][jeniskelamin]" disabled></li>';
    input += '<li><select style="display:inline-block;" name="jadwalHemo[jadwal]['+idTabel+']['+idShift+']['+idRuangan+'][bed]['+jmlBaris+']" id="jadwalHemo_'+idTabel+'_'+idShift+'_'+idRuangan+'_'+jmlBaris+'" type="text" class="inputDokter spanBed beds"></select></li>';
...
function setTindakanAuto(pasien_id){
	...
	var datanya = 0;
    $.get('<?php echo Yii::app()->createUrl('pendaftaranPenjadwalan/jadwalhemodialisaT/infoPasien'); ?>',{pasien_id: pasien_id},function(data){
