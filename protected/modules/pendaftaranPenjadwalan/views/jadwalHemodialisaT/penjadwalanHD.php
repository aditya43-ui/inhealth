...
$this->breadcrumbs = array(
    'Buat Jadwal Hemodialisa',
);
?>
<div class="panel panel-gradient">
    <div class="panel-heading">
        <div class="panel-title">
            <i class="glyphicon glyphicon-briefcase"></i> <b>Buat Jadwal Hemodialisa</b>
        </div>
    </div>
    <div class="panel-body">
        ...
        <div class="row">
            <div class="col-sm-6">
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <?php
                        echo CHtml::dropDownList('jadwalHD[IdHari]', '', CHtml::listData(PPJadwalhariM::model()->findAllByAttributes(array('jadwalhari_aktif' => true)), 'jadwalhari_id', 'jadwalhari_nama'), array(
                            'empty' => '-- Pilih --',
                            'id' => 'IdHari',
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        ...
                        echo CHtml::checkBoxList('jadwalHD[ruangan]', $selected_Array2 = array(), $type_list2);
                        ?>
                    </div>
                </div>
                <div class="control-group" style='margin-left:120px;'>
                    <div class="controls">
                        ...
                        &nbsp;
                        <?php
                        echo CHtml::link(Yii::t('mds', '{icon} Reset', array('{icon}' => '<i class="entypo-arrows-ccw"></i>')), $this->createUrl('penjadwalanHD'), array('class' => 'btn btn-default'));
                        ?>
                    </div>
                </div>
            </div>
            <div class="col-sm-6">
                <div class="control-group">
                    ...
                    <div class="controls">
                        ...
                        echo CHtml::checkBoxList('jadwalHD[shift]', $selected_Array = array(), $type_list);
                        ?>
                    </div>
                </div>
            </div>
        ...
        <table style="width: 100%; border: none;">
            ...
            if (isset($_GET['totalData'])) {
                ...
                $jadwalhemo_id = $_GET['jadwalhemodialisa_id'];
                ...
                echo '<br>';
                echo CHtml::htmlButton(Yii::t('mds', '{icon} Simpan', array('{icon}' => '<i class="entypo-check"></i>')), array('class' => (isset($_GET['sukses'])) ? 'btn btn-primary' : 'btn btn-primary submit', 'type' => 'button', 'disabled' => true));
                echo CHtml::htmlButton(Yii::t('mds', '{icon} Print', array('{icon}' => '<i class="entypo-print"></i>')), array('class' => 'btn btn-info', 'type' => 'button', 'onclick' => 'print(\'PRINT\')'));
                echo CHtml::htmlButton(
                    Yii::t('mds', '{icon} Print Jadwal', array('{icon}' => '<i class="entypo-print"></i>')),
                    array('class' => 'btn btn-info', 'type' => 'button', 'onclick' => 'printJadwal()')
                );
                echo CHtml::link(Yii::t('mds', '{icon} Reset', array('{icon}' => '<i class="entypo-arrows-ccw"></i>')), Yii::app()->createUrl($this->module->id . '/jadwalhemodialisaT/penjadwalanHD'), array('class' => 'btn btn-danger'));
                $controller = Yii::app()->controller->id; //mengambil Controller yang sedang dipakai
                $module = Yii::app()->controller->module->id; //mengambil Module yang sedang dipakai
                $urlPrint = Yii::app()->createAbsoluteUrl($module . '/' . $controller . '/print');
                $urlPrintJadwal = Yii::app()->createAbsoluteUrl($module . '/' . $controller . '/printJadwal');
                $js = <<< JSCRIPT
...
{
    var caraPrint = "PRINT";
	window.open("${urlPrintJadwal}"+"&jadwalhemodialisa_id="+"${jadwalhemo_id}"+"&caraPrint="+caraPrint,"",'location=_new, width=900px');
}
JSCRIPT;
                        ...
                        <div class="untukdisable">
                            <div class="lfloat">
                                <?php echo CHtml::htmlButton(
                                    ...
                                    array('title' => 'Simpan jadwal', 'class' => (isset($_GET['sukses'])) ? 'btn btn-danger' : 'btn btn-danger submit', 'type' => 'button', 'disabled' => true)
                                ); ?>
                            </div>
                            <div class="lfloat">
                                <?php echo CHtml::link(
                                    Yii::t('mds', '{icon} Reset', array('{icon}' => '<i class="entypo-arrows-ccw"></i>')),
                                    Yii::app()->createUrl($this->module->id . '/jadwalhemodialisaT/penjadwalanHD'),
                            ...
                            <div class="lfloat">
                                ...
                                <?php echo CHtml::htmlButton(
                                    Yii::t('mds', '{icon} Print Jadwal', array('{icon}' => '<i class="entypo-print"></i>')),
                                    array('class' => 'btn btn-info', 'type' => 'button', 'disabled' => true, 'onclick' => 'printJadwal()')
                                ); ?>
                            </div>
                        </div>
                        <?php
...
<script type="text/javascript">
    function generateInput() {
        ...
        var shiftId = $('#jadwalHD_shift').find("input[name*='shift']:checked").val();
        var ruanganId = $('#jadwalHD_ruangan').find("input[name*='ruangan']:checked").val();
        //	alert(shiftId);
        if (idhari != '') {
    ...
    function insertInputJadwal(idTabel, idShift, idRuangan, Ruangan, obj) {
        ...
        $.post("<?php echo $this->createUrl('ajaxListBed') ?>", {
            ...
            //		input += '<li><div class="input-append"><input type="text" class="spans3" name="jadwalHemo[jadwal]['+idTabel+'][shift]['+idShift+'][ruangan_id]['+idRuangan+'][namapasien]['+jmlBaris+']" id="namaPasien_'+idTabel+'_'+idShift+'_'+idRuangan+'_'+jmlBaris+'" style="float:left;" onkeypress="return $(this).focusNextInputField(event)" class="ui-autocomplete-input" autocomplete="on" role="textbox" aria-autocomplete="list" aria-haspopup="true"><span class="add-on"><a href="javascript:void(0);" id="" onclick="setDialog(this);"><i class="icon-list"></i><i class="entypo-search"></i></a></span></div></li>'; // komen . jangan disini, render lewat controller RSSP-269
            input += '<li>' + data.input + '</li>';
            input += '<input type="hidden" class="span1" name="jadwalHemo[jadwal][' + idTabel + '][shift][' + idShift + '][ruangan_id][' + idRuangan + '][no_urut][' + jmlBaris + ']" value=' + jmlBaris + ' > '; // RSSP-269
            input += '<input type="hidden" class="span1" name="jadwalHemo[jadwal][' + idTabel + '][shift][' + idShift + '][ruangan_id][' + idRuangan + '][pasien_id][' + jmlBaris + ']">';
            input += '<li><input type="text" class="spans2 beds" name="jadwalHemo[jadwal][' + idTabel + '][shift][' + idShift + '][ruangan_id][' + idRuangan + '][jeniskelamin]" disabled></li>';
            input += '<li><select style="display:inline-block;" name="jadwalHemo[jadwal][' + idTabel + '][shift][' + idShift + '][ruangan_id][' + idRuangan + '][bed][' + jmlBaris + ']" id="jadwalHemo_' + idTabel + '_' + idShift + '_' + idRuangan + '_' + jmlBaris + '" type="text" class="inputDokter spanBed beds"></select></li>';
            ...
            id_autocomplete = 'jadwalHemo_jadwal_' + idTabel + '_shift_' + idShift + '_ruangan_id_' + idRuangan + '_namapasien_' + jmlBaris + ''; // RSSP-269
            jQuery('#' + id_autocomplete + '').autocomplete( // RSSP-269
                {
                    ...
                    'source': function(request, response) {
                        $.ajax({
                            url: "<?php echo Yii::app()->createUrl('pendaftaranPenjadwalan/jadwalhemodialisaT/AutoCompletePasien'); ?>",
                            dataType: "json",
                            data: {
                                term: request.term,
                            },
            ...
            $('#jadwalHemo_' + idTabel + '_' + idShift + '_' + idRuangan + '_' + jmlBaris).html(data.options);
            cekDisabled('form');
        }, 'json');
    }
    ...
    function setTindakanAuto(pasien_id) {
        ...
        obj = $("#" + parent);
        var tanggal1 = $(obj).parents('table').prev().find("input[name*='jadwalhemodialisa_tgl']").val();
        var tanggal2 = $(obj).parents('table').find("input[name*='jadwalhemodialisa_tgl']").val();
        ...
        var datanya = 0;
        $.get('<?php echo Yii::app()->createUrl('pendaftaranPenjadwalan/jadwalhemodialisaT/infoPasien'); ?>', {
            pasien_id: pasien_id
        }, function(data) {
