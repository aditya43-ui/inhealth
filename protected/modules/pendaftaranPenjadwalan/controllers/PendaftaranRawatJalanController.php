...
";
    ...
    public function simpanPasien($modPasien, $post)
    {
        ...
        $is_rm_manual = $post['is_rm_manual'] ?? 0;
        $no_rekam_medik = $post['no_rekam_medik'] ?? null;

        $dataPasien = PPPasienM::model()->findByAttributes(array('no_rekam_medik' => $no_rekam_medik));
        ...
        if (!empty($rm_old)) {
            $modPasien->no_rekam_medik = $rm_old;
        }
        ...
        // var_dump($modPasien->attributes, $post); die;
            ...
            if (!empty($_POST['ruangan_id'])) {
                ...
                if (!empty($modRuangan)) {
                    if (($modRuangan->instalasi_id != Params::INSTALASI_ID_RJ && $modRuangan->instalasi_id != Params::INSTALASI_ID_RD)) {
                        ...
                        $data = $model->getJenisKasusPenyakitItems($modRuangan->instalasi_id);
                    }
                } else {
                    $data = $model->getJenisKasusPenyakitItems($modRuangan->instalasi_id);
                }
                ...
                case '19': // khususu untuk mencari data spri
                    ...
                    if (!empty($str)) {
                        ...
                        if (!empty($json['response']) && $json['response'] != "") {
                            ...
                            if (!empty($modSuratRI)) {
                                $modPasienMoribiditas = PasienmorbiditasT::model()->findByAttributes(array('pendaftaran_id' => $modSuratRI->pendaftaran_id, 'kelompokdiagnosa_id' => Params::KELOMPOKDIAGNOSA_UTAMA), ['order' => 'pasienmorbiditas_id DESC']);
