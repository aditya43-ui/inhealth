...
class PengkajianAskepController extends MyAuthController {
	...
	public function actionIndex() {
		...
		$modPengkajian->pegawai_id = Yii::app()->user->getState('pegawai_id');
		$modPengkajian->nama_pegawai = Yii::app()->user->getState('nama_pegawai');
		...
		$this->render('index', array(
			'modInfoPengkajian' => $modInfoPengkajian,
...
//			}
	...
	public function actionLoadPasien($pendaftaran_id, $instalasi_id) {
		if (Yii::app()->request->isAjaxRequest) {
			$data = null;
                if (isset($pendaftaran_id)) {
                ...
                if ($instalasi_id== Params::INSTALASI_ID_RD) {
                    $data = InfokunjunganrdV::model()->findByAttributes(array('pendaftaran_id' => $pendaftaran_id));
                } else if ($instalasi_id == Params::INSTALASI_ID_RJ) {
                    $data = InfokunjunganrjV::model()->findByAttributes(array('pendaftaran_id' => $pendaftaran_id));
                } else if ($instalasi_id == Params::INSTALASI_ID_HD) {
                    $data = InfokunjunganhdV::model()->findByAttributes(array('pendaftaran_id' => $pendaftaran_id));
                } else {
    ...
    public function actionLoadTambahPenunjang() {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            $criteria->select = 't.tglmasukpenunjang,t.ruangan_id,ruangan.ruangan_nama, t.jeniskasuspenyakit_id, jeniskasuspenyakit.jeniskasuspenyakit_nama, pendaftaran_id, pasienmasukpenunjang_id, pasien_id';
