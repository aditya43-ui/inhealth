<tr class="rencanaaskepdet">
    ...
    <td class="tujuan" width="100px">
        <?php
        echo '<label>per</label> ' . CHtml::activeTextField($modDetail, '[0]rencanaaskepdet_hari', array('class' => 'span1', 'disabled' => true)) . CHtml::activeDropDownList($modDetail, '[0]rencanaaskepdet_estimasiwaktu', LookupM::getItemsUrutan('estimasiwaktu'), array('class' => 'span2', 'style' => 'width:100px', 'disabled' => true)) . $modDetail->tujuan_nama;
        echo CHtml::activeHiddenField($modDetail, '[0]tujuan_id', array('value' => $modDetail->tujuan_id, 'disabled' => true));
        ?>
    </td>
    <td class="kriteriahasil">
        ...
        if (!empty($modDetail->diagnosakep_id)) {
            ...
            echo CHtml::activeHiddenField($modDetail, '[0]tautansdki_slki_id', array('value' => $luaranutama_id, 'disabled' => true));
            echo CHtml::activeTextField($modDetail, '[0]tautansdki_slki_nama', array('value' => $luaranutama, 'class' => 'span3', 'disabled' => true));
            echo '</div></div>';
            echo '<div class="control-group keperawatan">
                    <div class="controls">
                    <label>Ekspektasi &nbsp;</label>';
            echo CHtml::activeHiddenField($modDetail, '[0]tujuan_id', array('value' => $tujuan_id));
            echo CHtml::activeTextField($modDetail, '[0]tujuan_nama', array('value' => $tujuan_nama, 'class' => 'span3', 'disabled' => true));
            echo '</div></div>';
            if($luaranutama_id != ''){
                    ...
                    foreach ($head as $val){
                        ...
                        foreach ($tail as $i => $row) {
                            echo '<tr class="criteria">
                                    ...
                                    ' . CHtml::activeCheckBox($modDetail, '[0]kriteriahasildet_id', array('onkeyup' => "return $(this).focusNextInputField(event);", 'value' => $row['kriteriahasildet_id'], 'disabled' => true))
                                    . '</span>
                                    </td>
                                    <td style="background-color:#f8f8f8; border: 1px solid #ededed !important;">
                                    ...
                                    <td style="background-color:#f8f8f8; border: 1px solid #ededed !important;">
                                    ' . CHtml::activeTextField($modDetail, '[0]rencanaaskep_ir', array('class' => 'span2', 'disabled' => true)) . '
                                    </td>
                                    </tr>';
                        }
    ...
    <td class="intervensi">
        ...
        if (!empty($modDetail->diagnosakep_id)) {
            ...
            echo CHtml::activeHiddenField($modDetail, '[0]intervensi_nama', array('value' => $modDetail->intervensi_nama, 'class' => 'span2', 'disabled' => true));

            $cekIntervensiUtama = IntervensiM::model()->findByAttributes(array('diagnosakep_id' => $modDetail->diagnosakep_id, 'intervensi_nama' => 'Intervensi Utama'));
            if (!empty($cekIntervensiUtama)) {
                ...
                echo CHtml::activeCheckBoxList($modDetail, '[0]intervensidet_id', CHtml::listData(IntervensidetM::model()->findAllByAttributes(
                                        array('intervensidet_aktif' => true, 'intervensi_id' => $cekIntervensiUtama->intervensi_id)), 'intervensidet_id', 'intervensidet_indikator'), (array('style' => 'float: left', 'disabled' => true, 'onkeyup' => "return $(this).focusNextInputField(event);", 'class' => 'intervensinya', 'onclick' => 'setTindakan(this)')));
            }
            $cekIntervensiPendukung = IntervensiM::model()->findByAttributes(array('diagnosakep_id' => $modDetail->diagnosakep_id, 'intervensi_nama' => 'Intervensi Pendukung'));
