...
//            $stokQty = Yii::app()->db->createCommand()->select('sum(qtystok_current) as qtystok_current, obatalkes_id')->from('stokobatalkes_t')->group('obatalkes_id')->where("obatalkes_id = $id AND ruangan_id = 59")->queryAll();
    ...
    public function searchObatFarmasiRuangan() {
        ...
        // $criteria->join = 'join obatalkesdetail_m on obatalkesdetail_m.obatalkes_id = t.obatalkes_id ';
        // $criteria->select = '*, obatalkesdetail_m.komposisi';


        $criteria->with = array('sumberdana', 'satuankecil', 'generik');
        ...
        if (!empty($this->obatalkesdetail_id)) {
            $criteria->addCondition("obatalkesdetail_id = " . $this->obatalkesdetail_id);
        }
        ...
        if (!empty($this->komposisi)) {
            $criteria->join .= (' JOIN obatalkesdetail_m ON obatalkesdetail_m.obatalkes_id = t.obatalkes_id');
            $criteria->compare('LOWER(obatalkesdetail_m.komposisi)', strtolower($this->komposisi), true);
        }
