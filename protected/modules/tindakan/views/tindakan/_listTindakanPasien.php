...
<?php foreach ($modTindakans as $i => $modTindakan) { ?>
    <tr>
        ...
        <td>
            <?php
            $pendaftaran = PendaftaranT::model()->findByPk($modTindakan->pendaftaran_id);
            echo !empty($modTindakan->nopelayanan) ? ($pendaftaran->no_pendaftaran . "" . $modTindakan->nopelayanan) : "" ?>
        </td>
        <td>
            <?php 
