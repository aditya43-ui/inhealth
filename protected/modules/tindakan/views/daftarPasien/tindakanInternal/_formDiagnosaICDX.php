    ...
    <td>
        <?php
            echo $form->dropDownList($modUraian,"[$i]pegawai_id", CHtml::listData(DokterV::model()->findAllByAttributes(array(
                ...
                array('onkeypress'=>"return $(this).focusNextInputField(event)", 'class'=>'span2 required'
            ));
        ?>
    </td>
    <td>
        <?php
            echo $form->dropDownList($modUraian,"[$i]ppds_id", CHtml::listData(PpdsM::model()->findAllByAttributes(array(
                'ruangan_id'=>Yii::app()->user->getState('ruangan_id'),
            )), "ppds_id", "ppds_nama"),
