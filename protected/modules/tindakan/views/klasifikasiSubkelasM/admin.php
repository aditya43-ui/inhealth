...
<div class="col-md-12">
	...
	<div class="panel-heading">
		<div class="panel-title">Pengaturan Klasifikasi Sub kelas</div>
	</div>
...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('klasifikasisubkelas-m-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
<!--<div class="block-tabel">-->
	...
	'columns'=>array(
                ...
                'klasifikasisubkelas_noterperinci',
                'klasifikasisubkelas_nama',
                array(
                    'header'=>'Status',
                    'value'=>'($data->klasifikasisubkelas_aktif == 1 ) ? "Aktif" : "Tidak Aktif"',
		...
		// ),
                 array(
                    'header'=>'Hapus',
                    'type'=>'raw',
                    'value'=>'($data->klasifikasisubkelas_aktif)?CHtml::link("<i class=\'entypo-cancel\'></i> ","javascript:removeTemporary($data->klasifikasisubkelas_id)",array("id"=>"$data->klasifikasisubkelas_id","rel"=>"tooltip","title"=>"Menonaktifkan Subkelas"))." ".CHtml::link("<i class=\'entypo-trash\'></i> ", "javascript:deleteRecord($data->klasifikasisubkelas_id)",array("id"=>"$data->klasifikasisubkelas_id","rel"=>"tooltip","title"=>"Hapus Subkelas")):CHtml::link("<i class=\'icon-form-check\'></i> ","javascript:addTemporary($data->klasifikasisubkelas_id, 1)",array("id"=>"$data->klasifikasisubkelas_id","rel"=>"tooltip","title"=>"Mengaktifkan Subkelas"))." ".CHtml::link("<i class=\'entypo-trash\'></i> ", "javascript:deleteRecord($data->klasifikasisubkelas_id)",array("id"=>"$data->klasifikasisubkelas_id","rel"=>"tooltip","title"=>"Hapus Subkelas"));',
...
<!--</div>-->
<?php 
echo CHtml::link(Yii::t('mds', '{icon} Tambah Klasifikasi Sub Kelas', array('{icon}'=>'<i class="entypo-plus"></i>')), $this->createUrl('create',array('modul_id'=> Yii::app()->session['modul_id'])), array('class'=>'btn btn-success'))."&nbsp&nbsp";
...
{
    $("#klasifikasisubkelas-m-search :input[name='"+ obj.name +"']").val(obj.value);
}
function print(caraPrint)
{
    window.open("${urlPrint}/"+$('#klasifikasisubkelas-m-search').serialize()+"&caraPrint="+caraPrint,"",'location=_new, width=900px');
}
JSCRIPT;
