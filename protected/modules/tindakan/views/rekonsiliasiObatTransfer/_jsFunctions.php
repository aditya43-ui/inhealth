...
function generateRowObat(obj){
  ...
  for(var i=0; i<$(obj).find('.nama_obat').length; i++){
      ...
      trRow.attr('id','RekonobattransferdetT_'+i+'_nama_obat');
      trRow.attr('name','RekonobattransferdetT['+i+'][nama_obat]');
  }
  for(var i=0; i<$(obj).find('.dosis').length; i++){
      var trRow = $(obj).find('.dosis').eq(i);
      trRow.attr('id','RekonobattransferdetT_'+i+'_dosis');
      trRow.attr('name','RekonobattransferdetT['+i+'][dosis]');
  }
  for(var i=0; i<$(obj).find('.frekuensi').length; i++){
      var trRow = $(obj).find('.frekuensi').eq(i);
      trRow.attr('id','RekonobattransferdetT_'+i+'_frekuensi');
      trRow.attr('name','RekonobattransferdetT['+i+'][frekuensi]');
  }
  for(var i=0; i<$(obj).find('.cara_pemberian').length; i++){
      var trRow = $(obj).find('.cara_pemberian').eq(i);
      trRow.attr('id','RekonobattransferdetT_'+i+'_cara_pemberian');
      trRow.attr('name','RekonobattransferdetT['+i+'][cara_pemberian]');
  }
  for(var i=0; i<$(obj).find('.waktu_pemberian').length; i++){
      var trRow = $(obj).find('.waktu_pemberian').eq(i);
      trRow.attr('id','RekonobattransferdetT_'+i+'_waktu_pemberian');
      trRow.attr('name','RekonobattransferdetT['+i+'][waktu_pemberian]');
  }
  for(var i=0; i<$(obj).find('.jumlah_obat').length; i++){
      var trRow = $(obj).find('.jumlah_obat').eq(i);
      trRow.attr('id','RekonobattransferdetT_'+i+'_jumlah_obat');
      trRow.attr('name','RekonobattransferdetT['+i+'][jumlah_obat]');
  }
  for(var i=0; i<$(obj).find('.tindaklanjut').length; i++){
      var trRow = $(obj).find('.tindaklanjut').eq(i);
      trRow.attr('id','RekonobattransferdetT_'+i+'_tindaklanjut');
      trRow.attr('name','RekonobattransferdetT['+i+'][tindaklanjut]');
  }
  for(var i=0; i<$(obj).find('.keterangan').length; i++){
      var trRow = $(obj).find('.keterangan').eq(i);
      trRow.attr('id','RekonobattransferdetT_'+i+'_keterangan');
      trRow.attr('name','RekonobattransferdetT['+i+'][keterangan]');
  }
