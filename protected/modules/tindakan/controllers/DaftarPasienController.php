...
class DaftarPasienController extends MyAuthController
{
  ...
  public function actionIndex()
  {
    ...
    $model = new TNInfokunjungantindakanV('searchDaftarPasien');
    ...
    if (Yii::app()->user->getState('unitkerja_id') == 46) {
      $model->pegawai_id = Yii::app()->user->getState('pegawai_id');
    }
    

    if (isset($_GET['TNInfokunjungantindakanV'])) {
      $model->attributes = $_GET['TNInfokunjungantindakanV'];
      $model->ceklis = $_REQUEST['TNInfokunjungantindakanV']['ceklis'];
      $format = new MyFormatter();
      $model->tgl_awal  = $format->formatDateTimeForDb($_REQUEST['TNInfokunjungantindakanV']['tgl_awal']);
      $model->tgl_akhir = $format->formatDateTimeForDb($_REQUEST['TNInfokunjungantindakanV']['tgl_akhir']);
      $model->tgl_awall  = $format->formatDateTimeForDb($_REQUEST['TNInfokunjungantindakanV']['tgl_awall']);
      $model->tgl_akhirl = $format->formatDateTimeForDb($_REQUEST['TNInfokunjungantindakanV']['tgl_akhirl']);
      $model->no_rekam_medik = $_REQUEST['TNInfokunjungantindakanV']['no_rekam_medik'];
      $model->ruangan_id = Yii::app()->user->getState('ruangan_id');
    }
  ...
  public function actionSimpanGambar()
  {
    ...
    $data['sukses'] = 0;
    $data['pesan'] = 'Pesan';
    if(isset($_POST['HasilpemeriksaantindakanT']) && isset($_FILES['HasilpemeriksaantindakanT'])) {
        $post = $_POST['HasilpemeriksaantindakanT'][0];
        $file = $_FILES['HasilpemeriksaantindakanT'];

        $modPemeriksaantindakan = HasilpemeriksaantindakanT::model()->findByPk($post['hasilpemeriksaantindakan_id']);

        if(!empty($modPemeriksaantindakan)) {
          $modPemeriksaantindakan->dokfiletindakan_nama = $post['dokfiletindakan_nama'];
          $modPemeriksaantindakan->dokfiletindakan_filepath = $file['name'][0]['dokfiletindakan_filepath'];
          if($modPemeriksaantindakan->save()) {
            $temp_file = $file['tmp_name'][0]['dokfiletindakan_filepath'];
            $target_file = Params::pathFileHasilPemeriksaanTindakanDirectory() . $modPemeriksaantindakan->dokfiletindakan_filepath;
            if(move_uploaded_file($temp_file, $target_file)){
                $data['sukses'] = 1;
                $data['pesan'] = 'Data berhasil di Upload';
            } else {
                $data['pesan'] = 'Data gagal di Upload';
            }
          } else {
            $data['pesan'] = 'Gagal Simpan ke database';
          }
        }
    } else {
  ...
  protected function loadHasilPemeriksaan($id) {
      $model = HasilpemeriksaantindakanT::model()->findByPk($id);
      if ($model === null)
  ...
  function actionDeleteRecord() {
    $hasilpemeriksaantindakan_id = $_POST['hasilpemeriksaantindakan_id'];
    if(HasilpemeriksaantindakanT::model()->deleteByPk($hasilpemeriksaantindakan_id)) {
      $data['sukses'] = 1;
    } else {
      $data['sukses'] = 0;
    }
  ...
  public function actionHasilPemeriksaan($pendaftaran_id, $pasien_id, $pasienmasukpenunjang_id, $caraPrint = '', $hasilpemeriksaantindakan_id = null)
  {

    $modPasienMasukPenunjang = TNMasukPenunjangV::model()->findByAttributes(array('pasienmasukpenunjang_id' => $pasienmasukpenunjang_id));

    $modRiwayat = new HasilpemeriksaantindakanT();
    ...
    $modHasilPemeriksaan = new HasilpemeriksaantindakanT();
  
    if(!empty($hasilpemeriksaantindakan_id)) {
      $modHasilPemeriksaan = HasilpemeriksaantindakanT::model()->findByPk($hasilpemeriksaantindakan_id);
      if(empty($modHasilPemeriksaan)) {
        $modHasilPemeriksaan = new HasilpemeriksaantindakanT();
      }
    }

    if (isset($_POST['HasilpemeriksaantindakanT'])) {
      // echo '<pre>';
      // var_dump($_POST,$_FILES);die;
      $transaction = Yii::app()->db->beginTransaction();
      try {
        $id_pendaftaran = $_POST['TNMasukPenunjangV']['pendaftaran_id'];
        $id_pasien = $_POST['TNMasukPenunjangV']['pasien_id'];
        $id_pasienmasukpenunjang = $_POST['TNMasukPenunjangV']['pasienmasukpenunjang_id'];
        $file = $_FILES['HasilpemeriksaantindakanT'];
        // for ($i = 0; $i < count((array)$_POST['HasilpemeriksaantindakanT']); $i++) {
          $post = $_POST['HasilpemeriksaantindakanT'];
          
          
          $modHasilPemeriksaan->attributes = $_POST['TNMasukPenunjangV'];
          $modHasilPemeriksaan->tglpemeriksaantindakan = MyFormatter::formatDateTimeForDb($post['tglpemeriksaantindakan']);
          $modHasilPemeriksaan->hasilpemeriksaantindakan = $post['hasilpemeriksaantindakan'];
          $modHasilPemeriksaan->kesimpulantindakan = $post['kesimpulantindakan'];
          $modHasilPemeriksaan->ruangan_id = Yii::app()->user->getState('ruangan_id');
          $modHasilPemeriksaan->pegawai_id = Yii::app()->user->getState('pegawai_id');

          // echo '<pre>';
          // var_dump($modHasilPemeriksaan);die;
          // keperluan upload
          $temp_file = $file['tmp_name']['dokfiletindakan_filepath'];
          if(!empty($temp_file)) {
            $modHasilPemeriksaan->dokfiletindakan_filepath = $file['name']['dokfiletindakan_filepath'];
            $modHasilPemeriksaan->dokfiletindakan_nama = $post['dokfiletindakan_nama'];
          }
          $target_file = Params::pathFileHasilPemeriksaanTindakanDirectory() . $modHasilPemeriksaan->dokfiletindakan_filepath;

          // var_dump(!empty($temp_file));die;
        ...
        if ($this->successSaveHasil) {
          ...
          if ($penunjang->ruanganasal_id == $penunjang->ruangan_id) {
            ...
            $judul = 'Pasien sudah periksa Tindakan';
          ...
          // echo '<pre>';
          // var_dump($modHasilPemeriksaan->getErrors());die;
        ...
        // var_dump($exc);die;
        $transaction->rollback();
        Yii::app()->user->setFlash('error', "Data gagal disimpan [2]" . MyExceptionMessage::getMessage($exc, true));
      }
    }
  ...
  public function actionInfoPasien($id)
  {
    ...
    $modPendaftaran = RJPendaftaranT::model()->findByPk($id);
    $modRincian = TNInfokunjungantindakanV::model()->findByAttributes(array('pendaftaran_id' => $id));
...
";
  ...
  public function actionGetAntrianTerakhir()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      $model = TNInfokunjungantindakanV::model()->find($criteria);
      if (!empty($model)) {
  ...
  public function actionKonsultasiInternal($konsulpoli_id)
  {
    ...
    if (isset($_POST['RJKonsulPoliT'])) {
      ...
      try {
        ...
        $model->uraian_konsuljjawaban = isset($_POST['RJKonsulPoliT']['uraian_konsuljjawaban']) ? $_POST['RJKonsulPoliT']['uraian_konsuljjawaban'] : '';
  ...
  public function actionPrintRencanaKontrol($id = null)
  {
    ...
    if ($caraPrint == 'PRINT') {
      $this->layout = '//layouts/printWindows';
      $this->render(
        'rawatJalan.views.daftarPasien.PrintRencanaKonsulSRK',
        array(
          'modPendaftaran' => $modPendaftaran,
          'judul' => $judul,
          'judul2' => $judul2,
