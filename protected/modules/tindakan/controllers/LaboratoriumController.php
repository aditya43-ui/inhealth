...
class LaboratoriumController extends MyAuthController
{
  ...
  protected function savePasienKirimKeUnitLain($modPendaftaran, $ruangan_lab)
  {
    ...
    $modKirimKeUnitLain->is_cyto = isset($_POST['RJPasienKirimKeUnitLainT']['is_cyto']) ? $_POST['RJPasienKirimKeUnitLainT']['is_cyto'] : false;
...
// echo '<pre>'; var_dump($_POST); die();
      ...
      $postPemeriksaan['kelaspelayanan_id'] = Params::KELASPELAYANAN_ID_TANPA_KELAS;
        ...
        $critPeriksa = new CDbCriteria();
        $critPeriksa->compare('LOWER(pemeriksaanlab_nama)', strtolower($postPemeriksaan['pemeriksaanlab_nama']), true);
        $critPeriksa->addCondition('pemeriksaanlab_aktif = true');
        $critPeriksa->order = "pemeriksaanlab_urutan";
        $modPeriksaLab = RJPemeriksaanLabM::model()->findAll($critPeriksa);
...
// echo '<pre>'; var_dump($arr_jns); die();
  ...
  public function actionCekTindakan()
  {
    if (Yii::app()->getRequest()->getIsAjaxRequest()) {
      ...
      $crit->select = "daftartindakan_id, pemeriksaanlab_id, pendaftaran_id, t.pasienkirimkeunitlain_id";
      $crit->join = "join pasienkirimkeunitlain_t p on p.pasienkirimkeunitlain_id = t.pasienkirimkeunitlain_id ";
      $crit->addCondition('date(tglpermintaankepenunjang) = current_date');
