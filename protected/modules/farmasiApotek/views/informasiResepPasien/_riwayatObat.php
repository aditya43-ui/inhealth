...
<table class="items table table-bordered table-striped table-condensed">
    ...
    <tbody>
        ...
        if ($tipe == 1) {
            ...
            foreach ($dat as $item) {
                $resep = PenjualanresepT::model()->findByAttributes(['reseptur_id'=>$item->reseptur_id]);
                array_push($oa, array(
					'r'=>$item->rke,
                    ...
                    'tglpenjualan'=>$resep->tglpenjualan ?? '',
            ...
            foreach ($dat as $item) {
                $resep = PenjualanresepT::model()->findByPk($item->penjualanresep_id);
                array_push($oa, array(
					'r'=>$item->rke,
                    'jenis'=>empty($item->obatalkes->jenisobatalkes_id)?"-":$item->obatalkes->jenisobatalkes->jenisobatalkes_nama,
                    'nama'=>$item->obatalkes->obatalkes_nama,
        ...
        <tr>
            ...
            <td><?php echo MyFormatter::formatDateTimeForUser($item['tglpenjualan']);?></td>
