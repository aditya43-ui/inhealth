...
<?php $riwayatPasien = Yii::app()->createUrl('rawatJalan/daftarPasien/getRiwayatPasienLama&id=' . $modReseptur->pasien_id); 
 ...
 $(obj).closest('.waktu-ri').find('.wakturanap').val(wkt.join(', '));
    ...
    function verifikasi(obj, status) {
        ...
        if(status == 1) {
            ...
            $(obj).parents('tr').find('.namaobat').removeAttr('style');
            $(obj).parents('tr').find('.kodeobat').removeAttr('style');
        } else if(status == 0) {
            $(obj).parents('tr').find('.statusVerifikasi').val('Tidak Di Setujui');
            $(obj).parents('tr').find('.namaobat').attr('style','text-decoration: line-through;');
            $(obj).parents('tr').find('.kodeobat').attr('style','text-decoration: line-through;');
        } else {
    ...
    function hitungSubTotal(obj) {
        ...
        var qty = $(obj).parents('tr').find('input[name$="[qty_dilayani]"]').val();
    ...
    function hitungTotal() {
        ...
        $('#table-obatalkespasien > tbody > tr').each(function() {
            ...
            var qty = $(this).find('input[name*="[qty_dilayani]"]').val();
    ...
    function pilihObatalkes(obatalkes_id, obatalkes_nama, stok, hargajual, harganetto, obatalkes_kode, sumberdana_id, sumberdana_nama, satuankecil_id, satuankecil_nama, baru) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var stokDitabel = obatalkesyangsama.parents('tr').find("input[name$='[jmlstok]']").val();
                if (obatalkesyangsama.val() && stokDitabel > 0) { //jika ada obat sudah ada di table
    ...
    function tambahObatalkes(obj) {
        ...
        $(table).find('tr:last-child input[name$="[obatalkes_nama_api]"]').autocomplete({
            ...
            'select': function(event, ui) {
                ...
                $("#tindakan_untuk").val($(this).prop('id'));
                console.log(ui.item)
                setObatDariApiUntukAutoComplete(ui.item.kode, ui.item.sumberdana, ui.item.stok, ui.item.hargajual, ui.item.StFornas, ui.item.satuan, ui.item.nama, ui.item.HPP);
                return false;
            },
            'source': function(request, response) {
                $.ajax({
                    url: "<?php echo $this->createUrl('AutocompleteObatApi'); ?>",
                    dataType: "json",
                    data: {
                        term: request.term,
                        penjamin_id: <?php echo $modPendaftaran->penjamin_id ?>,
    ...
    function cekValiditas() {
        if (requiredCheck($("form"))) {
            ...
            var verif_allnotapprove = [];
            var notifcekverifikasiobat = false;
            ...
            $("#table-obatalkespasien tbody tr").each(function() {
                ...
                var verif = $(this).find('input[name$="[is_verifkasiapoteker]"]').val();
                if(verif == '') {
                    is_verif = true;
                }

                if(verif == '1') {
                    notifcekverifikasiobat = true;
                }
            ...
            if(!notifcekverifikasiobat) {
                myAlert('Tidak ada obat yang disetujui, silahkan cek kembali atau lakukan ubah obat dan/atau hapus resep jika tidak ada obat yang disetujui');
                return false;
            }
            ...
            if(is_verif) {
                myAlert('Silahkan verifikasi obat terlebih dahulu');
                return false;
            }

            if(is_signa) {
                myAlert('Silahkan isi frekuensi');
                return false;
            }
            if(is_etiket) {
                myAlert('Silahkan isi etiket penggunaan');
                return false;
            }
            ...
            // diaktifkan pada konfig di sistemadministrator
            <?php if($konfig->is_notiftglkadaluarsa) : ?>
                if (!kadaluarsa) {
                    myAlert("Ada Obat yang sudah melewati tanggal kadaluarsa, yaitu :" + pesan);
                    return false;
                }
            <?php endif; ?>
            ...
            $("#table-obatalkespasien tbody tr").each(function() {
                ...
                var verif = $(this).find('input[name$="[is_verifkasiapoteker]"]').val();
                console.log(qty == 0 && verif != '1', verif);
...
// }
    ...
    $(document).ready(function() {
        ...
        $("td.waktu-ri").each(function() {

            console.log('td waktu ri');

            var waktu = $(this).find('.wakturanap').val();
            var waktu_arr = waktu.split(", ");

            var ini = $(this);

            if(waktu_arr.length > 0) {
                waktu_arr.forEach(function(val, idx) {

                    console.log(ini.find('.cb-waktu[data-val="' + val + '"]').data('val'));

                    setTimeout(() => {

                        console.log('tercek seharusnya');
                        ini.find('.cb-waktu[data-val="' + val + '"]').prop('checked', true);
