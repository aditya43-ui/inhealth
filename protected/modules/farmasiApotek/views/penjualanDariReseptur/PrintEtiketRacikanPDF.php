...
array_push($obat,   $oa->obatalkes_nama . " - " . $modObat->permintaan_dosis . " " . (!empty($modRDet[$i]->satuankekuatan) ? $modRDet[$i]->satuankekuatan : ""));
...
<?php } 
        echo $this->renderPartial('PrintEtiketRacikanPDF_page2', array('pasien' => $pasien, 'penjualan' => $penjualan, 'pendaftaran' => $pendaftaran, 'rke' => $modObat->rke, 'obat' => $obat, 'modResepturDet' => $modResepturDet, 'modDet' => $modDet, 'modRDet' => $modRDet));
        /* if (empty($caraPrint)) {
            echo CHtml::htmlButton('<i class="icon-print icon-white"></i> Print', array(
                'class'=>'btn btn-info', 'onclick'=>'printOA('.$modObat->obatalkespasien_id.');'
            ));
