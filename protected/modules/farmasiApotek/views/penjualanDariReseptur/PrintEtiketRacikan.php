...
$modResepturDet = ResepturdetailT::model()->findAll("reseptur_id = " . $modReseptur->reseptur_id . " and racikan_id = " . $_GET['racikan']);
 ...
 $crke->select = 'rke';
 $crke->group = 'rke';
 $crke->addCondition("reseptur_id = " .$modReseptur->reseptur_id . "and racikan_id = 1");
 $crke->order = 'rke';
 $resepturdet_rke = ResepturdetailT::model()->findAll($crke);
...
if(isset($modRDet[$i]->satuankekuatan)) {
    $satuan = $modRDet[$i]->satuankekuatan;
} else {
    $satuan = '';
}
array_push($obat,   $oa->obatalkes_nama . " - " . $modObat->permintaan_oa . " " . $satuan);
