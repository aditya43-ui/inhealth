    ...
    <div class="controls">
        <?php
            if (($jenisform=='tambah')){
                ...
                echo $form->dropDownListRow($model, 'notriage_pasien_id', FANotriagePasienT::getDropTrigaePasien($model->pendaftaran_id), array('empty'=>'-- Pilih --','class' => 'span3 bedtriage required', 'style'=>'width:200px;','onkeypress' => "return $(this).focusNextInputField(event);", 'onchange' => 'tambahNobed(this,"ubah")', 'maxlength' => 50));
            }
        ?>
    </div>
</div>
...
<script type="text/javascript">
    ...
    function tambahNobed(obj,jenis)
    {
        ...
        $.post("<?php echo $this->createUrl('loadTriage'); ?>", {id: id,jenis:jenis},
            function(data){
                $('#FANotriagePasienT_no_bed_triage').val(data.no_bed_triage);
                $('#FANotriagePasienT_keterangan').val(data.keterangan);
                $('#FANotriagePasienT_notriage_pasien_id').val(data.notriage_pasien_id);
                $('#FANotriagePasienT_bed_triage_id').val(data.bed_triage_id);
            },
        "json");
    }
