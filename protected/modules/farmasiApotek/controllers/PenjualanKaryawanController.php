        ...
        if (isset($_POST['FAPenjualanResepT'])) {
            ...
            if ($this->penjualantersimpan) {
                ...
                if (count((array)$_POST['FAObatalkesPasienT']) > 0) {
                    ...
                    $detailGroups = array();
                    // echo "<pre>";
                    // echo print_r($_POST['FAObatalkesPasienT']).exit();
...
//        $pasien = PasienM::model()->findByPk($modPenjualan->pasien_id);
        ...
        $isi .= "Pegawai Peminta : ".(empty($pegawaiPasien) ? "-" : $pegawaiPasien->namaLengkap)."<br/>";
...
//        die;
    ...
    protected function savePenjualanResep($modPasien, $penjualanResep) {
        ...
        $modPenjualan->jenispenjualan = Params::JENISPENJUALAN_KARYAWAN;
        $modPenjualan->attributes = $penjualanResep;
        ...
        $modPenjualan->discount = $penjualanResep['discount'];
        $modPenjualan->totalppn = $penjualanResep['totalppn'];
    ...
    protected function simpanObatAlkesPasien2($modPasien, $modPenjualan, $postObatAlkesPasien) {
        ...
        $modObatAlkes->qty_oa = MyFormatter::formatNumberForDb($modObatAlkes->qty_oa);
...
//                $modObatAlkes->discount = $postDetail['discount'];
                ...
                $modObatAlkes->discount = $postObatAlkesPasien['discount'];
...
//                $modObatAlkes->discount = $postDetail['discount'];
                ...
                // $modObatAlkes->discountkaryawan = $postDetail['discountkaryawan'];
                // $modObatAlkes->totaldiscountkaryawan = $postDetail['totaldiscountkaryawan'];
                // $modObatAlkes->ppnpersen = $postDetail['ppnpersen'];
                // $modObatAlkes->jumlahppn = $postDetail['jumlahppn'];
            }
            $modObatAlkes->iurbiaya = $modObatAlkes->hargajual_oa;
        }
    ...
    public function actionPrint($penjualanresep_id, $caraPrint = null) {
        ...
        $judul_print = "RINCIAN PENJUALAN OBAT PEGAWAI";
    ...
    public function actionSetFormObatAlkesPasien()
    {
        if(Yii::app()->request->isAjaxRequest) {
            ...
            if(!empty($konfigFarmasi->marginpenjualankaryawan)){
                $marginjual = (($oa->hpp * $konfigFarmasi->marginpenjualankaryawan)/100);
...
//            $hargajual = $this->cekHargaJualBerdasarkanInstalasi($oa, $instalasi);
            ...
            //    foreach($modStokOAs AS $i => $stok){
                    ...
                    $modObatAlkesPasien->hargajual_oa = ($jumlah * $modObatAlkesPasien->hargasatuan_oa);
                    ...
                    $modObatAlkesPasien->persen_discount = ((isset($konfigFarmasi->discountkaryawan) && !empty($konfigFarmasi->discountkaryawan)) ? $konfigFarmasi->discountkaryawan : 0);
