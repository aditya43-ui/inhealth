<?php
$gets = "";
if(isset($_GET)){
    foreach($_GET AS $name => $get){
        if($name != "r")
            $gets .= "&".$name."=".$get;
    }
}
?>
<?php $baseUrl = Yii::app()->createUrl("/");?>
<script type='text/javascript'>
function setTab_checklist(obj){
    $(obj).parents("ul").find("li").each(function(){
        $(this).removeClass("active");
//        $(this).attr("onclick","setTab(this);");
    });
    $(obj).addClass("active");
//    $(obj).removeAttr("onclick","setTab(this);");
    var tab = $(obj).attr("tab");
    var frameObj = document.getElementById("frame");
    resetIframe_checklist(frameObj);
    $(frameObj).attr("src","<?php echo $baseUrl;?>?r="+tab+"<?php echo $gets;?>");
    $(frameObj).parent().addClass("animation-loading");
    $(frameObj).load(function(){
        $(frameObj).parent().removeClass("animation-loading");
        resizeIframe_checklist(frameObj);
    });
    return false;
}

function resetIframe_checklist(obj) {
    obj.style.height = 128 + 'px';
}

function resizeIframe_checklist(obj) {
    obj.style.height = (obj.contentWindow.document.body.scrollHeight) + 'px';
}

function resizeIframeJs_checklist(obj) {
    var h1 = obj.height();
    var h2 = 100;
    var h3 = h2+h1;

    obj.attr("style",'height:'+h3+'px');
}


</script>
