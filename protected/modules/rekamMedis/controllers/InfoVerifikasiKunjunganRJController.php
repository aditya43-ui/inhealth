...
class InfoVerifikasiKunjunganRJController extends MyAuthController
{
  public $path_view = 'rekamMedis.views.infoVerifikasiKunjunganRJ.';
  ...
  public function actionIndex()
  {
    ...
    $modInfoVerifikasiKunjuganRJ->statusperiksa = Params::STATUSPERIKSA_SUDAH_PULANG;
    ...
    $this->render($this->path_view . 'index', array('format' => $format, 'modInfoVerifikasiKunjuganRJ' => $modInfoVerifikasiKunjuganRJ, 'modRujukan' => $modRujukan));
  }
  /**
      ...
      try {
        ...
        if ($model->save()) {
          ...
          if (isset($_POST['PPAsuransipasienbpjsM'])) {
            ...
            $modPendaftaran->save();
            var_dump($modPendaftaran->getErrors());
            die;
          } else {
            $this->asuransipasientersimpan = true;
          }
  ...
  public function actionValidasiDiagnosa()
  {
    ...
    echo CJSON::encode($validate);

    // echo "<pre>";
    // var_dump($validate);die;
  }

  function actionCekValidasiVerifikasi() {
    $pendaftaran_id = $_POST['pendaftaran_id'];
    $menu = $_POST['menu'];
    ...
    if($menu == 'RJ' || $menu == 'RD') {
      ...
      if(!empty($modAdmisi->pasienpulang_id)) {
        // rawat inap
        $data['sukses'] = 1;
      } else {
        $data['sukses'] = 0;
  ...
  function actionSetDropDownKondisiKeluar() {
    $carakeluar_id = $_POST['carakeluar_id'];

    $modKondisi = KondisiKeluarM::model()->findAllByAttributes(['carakeluar_id' => $carakeluar_id]);

    $data['option'] = '<option value>-- Pilih --</option>';

    if(!empty($modKondisi)){
      foreach($modKondisi as $i => $value) {
        $data['option'] .= '<option value="' . $value->kondisikeluar_id . '">'; 
        $data['option'] .= $value->kondisikeluar_nama;
        $data['option'] .= '</option>'; 
      }
    }
