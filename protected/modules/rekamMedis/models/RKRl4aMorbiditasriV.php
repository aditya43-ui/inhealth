<?php
class RKRl4aMorbiditasriV extends Rl4aMorbiditasriV
{
        public $jmlpsn;
	public static function model($className=__CLASS__)
	{
        ...
        public function getSumDiagnosa($diagnosa_id,$golonganumur_id,$jeniskelamin){
            ...
            $modMorbiditas = Rl4aMorbiditasriV::model()->findAll($criteria);
            $totPasien = 0;
        ...
        public function getSumKeluar($diagnosa_id,$jeniskelamin){
            ...
            $criteria->addCondition('diagnosa_id = '.$diagnosa_id);
            $criteria->compare('LOWER(jeniskelamin)',strtolower($jeniskelamin),true);
            $criteria->select = $criteria->group.', count(jmlpsnhidupmeninggal) AS jmlpsn';
            $modMorbiditas = Rl4aMorbiditasriV::model()->findAll($criteria);
            $totPasien = 0;
                foreach($modMorbiditas as $key=>$jml){
        ...
        public function getTotalKeluar($diagnosa_id,$status){
            ...
            $format = new MyFormatter();
            $criteria=new CDbCriteria();
            $criteria->group = 'diagnosa_id, jmlpsnhidupmeninggal';

            $criteria->addCondition('diagnosa_id = '.$diagnosa_id);
            $criteria->compare('LOWER(jmlpsnhidupmeninggal)',strtolower($status),true);
            $criteria->select = $criteria->group.', count(jmlpsnhidupmeninggal) AS jmlpsn';
            $modMorbiditas = Rl4aMorbiditasriV::model()->findAll($criteria);
            $totPasien = 0;
                foreach($modMorbiditas as $key=>$jml){
        ...
        public function getSumTotalKeluar($status){
            ...
            $format = new MyFormatter();
            $criteria=new CDbCriteria();
            $criteria->group = 'jmlpsnhidupmeninggal';
