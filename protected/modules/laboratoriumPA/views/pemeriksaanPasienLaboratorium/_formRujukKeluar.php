...
<div class="col-sm-6">
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <?php echo CHtml::activeDropDownList($modRujukKeluar,'['.$i.']supir_id', PegawaiV::model()->getPegawaiJabatanItems(Params::JABATAN_ID_DRIVER) ,array('empty'=>'-- Pilih --','onkeyup'=>"return $(this).focusNextInputField(event)", 'class'=>'span3 txt_sopir')); ?>
            <?php //echo $form->dropDownListRow($modRujukKeluar,'['.$i.']dokterpengirim_id', CHtml::listData(ROPendaftaranT::model()->getDokterItems($modPasienMasukPenunjang->ruangan_id,'userlogin'), 'pegawai_id', 'nama_pegawai') ,array('empty'=>'-- Pilih --','onkeyup'=>"return $(this).focusNextInputField(event)", 'class'=>'span3')); ?>
        </div>
    </div>
...
echo CHtml::htmlButton(Yii::t('mds', '{icon} Save', array('{icon}' => '<i class="'.MyIcon::getIcons('simpan').'"></i>')), array('class' => 'btn btn-primary', 'type' => 'button', 'onkeypress' => 'formSubmit(this,event);', 'onclick'=>'simpanRujukanKeluar();'));
