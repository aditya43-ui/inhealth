...
function setFormHasilPemeriksaan(){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            setKeterangan(); 
            cekDisabled($('#pemeriksaanlaboratorium-form'));
        },
        error: function (jqXHR, textStatus, errorThrown) { console.log(errorThrown);}
    });
}
...
function generatePicker(){
    $("#form-hasilpemeriksaanpa table > tbody > tr").find(".tglperiksapa").datetimepicker(
        jQuery.extend(
                    {
    ...
    function verifikasi(pasienmasukpenunjang_id, jenis_verifikasi) {
        myConfirm('Apakah anda menyetujui pemeriksaan pasien ini?', 'Perhatian!', function (r) {
            if (r) {
                $('.panel-gradient').addClass("animation-loading");
                $.ajax({
                    type: 'POST',
                    url: '<?php echo $this->createUrl('DaftarPasien/cekVerifikasi'); ?>',
                    data: {pasienmasukpenunjang_id: pasienmasukpenunjang_id, jenis_verifikasi: jenis_verifikasi},
                    dataType: "json",
                    success: function (data) {
                        if (data.status == true) {
                            $('.panel-gradient').removeClass("animation-loading");
                            location.reload();
                        } else {
                            myAlert(data.pesan);
                        }
    ...
    function batalVerifikasi(pasienmasukpenunjang_id, jenis_verifikasi) {
        myConfirm('Apakah anda yakin untuk membatalkan verifikasi pasien ini?', 'Perhatian!', function (r) {
            if (r) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?php echo $this->createUrl('DaftarPasien/BatalVerifikasi'); ?>',
                    data: {pasienmasukpenunjang_id: pasienmasukpenunjang_id, jenis_verifikasi: jenis_verifikasi},
                    dataType: "json",
                    success: function (data) {
                        if (data.status == true) {
...
<?php if(!empty($modKunjungan->pasienmasukpenunjang_id)){ ?>
    ...
    setRiwayatDiagnosa();
<?php } ?>
    setTimeout(function(){generatePicker();},500);
