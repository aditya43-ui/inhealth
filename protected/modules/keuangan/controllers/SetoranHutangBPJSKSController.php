...
class SetoranHutangBPJSKSController extends MyAuthController
{
  public $path_view = 'keuangan.views.setoranHutangBPJSKS.';
  ...
  {
    $this->pageTitle = Yii::app()->name . " - Setoran Pajak Bpjs Kesehatan";
    ...
    $modBuktiKeluar->untukpembayaran = "Setoran BPJSKS - " . $modBuktiKeluar->no_setorpajakpembelian;
              ...
              if (isset($_POST['PenggajianpegT']) && count((array)$_POST['PenggajianpegT']) > 0) {
                ...
                foreach ($_POST['PenggajianpegT'] as $detailData) {
                  if ($detailData['checklist'] == 1) {
                    ...
                    $modelDet->jenissetoran = Params::JENISSETORAN_BPJSKS;
                    ...
                    if ($modelDet->save()) {
                      ...
                      $criteriaPeg->select = "t.penggajianpeg_id, t.periodegaji, sum(case when penggajiankomp_t.komponengaji_id = 117 then penggajiankomp_t.jumlah else 0 end) as totalpajak";
                      ...
                      $criteriaPeg->addCondition('penggajiankomp_t.komponengaji_id = 117');
  ...
  public function actionSetFromHutangPajak()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      $criteria->select = "t.periodegaji, sum(case when penggajiankomp_t.komponengaji_id = 117 then penggajiankomp_t.jumlah else 0 end) as totalpajak";
      $criteria->group = "t.periodegaji";
      $criteria->join = "JOIN penggajiankomp_t ON penggajiankomp_t.penggajianpeg_id = t.penggajianpeg_id";
      $criteria->addCondition('penggajiankomp_t.komponengaji_id = 117');
            ...
            foreach ($modPenggajianPegAll as $dataSisaHutang) {
              ...
              if (count((array)$modSisaHutang) > 0) {
                foreach ($modSisaHutang as $dataHutang) {
                  $modSetoran = SetoranpajakT::model()->findByAttributes(array('setoranpajak_id' => $dataHutang->setoranpajak_id, 'jenissetoran' => Params::JENISSETORAN_BPJSKS, 'batalpegawai_id' => NULL));
                  if (isset($modSetoran)) {
  ...
  public function actionAmbilDataRekColumn()
  {
    if (Yii::app()->getRequest()->getIsAjaxRequest()) {
      ...
      if (isset($jmlBayar) && $jmlBayar > 0) {
        ...
        $criteria->compare('column_name', Params::REKENINGCOLUMN_COLUMN_SETORANPAJAKBPJSKS, false);
