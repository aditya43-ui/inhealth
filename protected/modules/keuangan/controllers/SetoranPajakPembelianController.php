...
class SetoranPajakPembelianController extends MyAuthController
{
  public $path_view = 'keuangan.views.setoranPajakPembelian.';
  ...
  public function actionIndex()
  {
    ...
    $model->jenissetoran = Params::JENISSETORAN_PPHPEMBELIAN;
        ...
        if (isset($_POST['KUTandabuktikeluarT'])) {
          ...
          if ($modBuktiKeluar->validate()) {
            if ($modBuktiKeluar->save()) {
              ...
              if (isset($_POST['FakturpembelianV'])) {
                if (isset($_POST['FakturpembelianV'])) {
                  foreach ($_POST['FakturpembelianV'] as $detailData) {
                    if ($detailData['checklist'] == 1) {
                      ...
                      $modelDet->jenissetoran = Params::JENISSETORAN_PPHPEMBELIAN;
                      $modelDet->keterangansetoran = $detailData['keterangan'];

                      if ($modelDet->save()) {
                        $saveDetail = true;
                        if (!empty($modelDet->terimapersediaan_id)) {
                          $modTerima = TerimapersediaanT::model()->findByPk($modelDet->terimapersediaan_id);
                          $pajak = (isset($modTerima->pajak) ? $modTerima->pajak->pajak_nama : "");
                        } else if (!empty($modelDet->fakturpembelian_id)) {
                          $modTerima = FakturpembelianT::model()->findByPk($modelDet->fakturpembelian_id);
                          $pajak = (isset($modTerima->pajak) ? $modTerima->pajak->pajak_nama : "");
                        } else if (!empty($modelDet->terimabahanmakan_id)) {
                          $modTerima = TerimabahanmakanT::model()->findByPk($modelDet->terimabahanmakan_id);
                          $pajak = (isset($modTerima->pajak) ? $modTerima->pajak->pajak_nama : "");
                        }
                      }
                    }
              ...
              if (Yii::app()->user->getState('isjurnalotomatis') == true) {
                ...
                if (isset($_POST['RekeningakuntansiV'])) {
                  if (count((array)$_POST['RekeningakuntansiV']) > 0) {
                    $modJurnalRekening = $this->saveJurnalRekening($_POST['KUSetoranpajakT'], $modBuktiKeluar, $pajak);
                    ...
                    foreach ($_POST['RekeningakuntansiV'] as $dataRekening) {
                      ...
                      //                                                $urut = 0;
                      //                                                
                      //                                                if(isset($dataRekening['saldodebit']) && $dataRekening['saldodebit'] > 0){
                      //                                                    $jumlah = $dataRekening['saldodebit'];
                      //                                                    $debitkredit = "D";
                      //                                                    $urut = 0;
                      //                                                }
                      //                                                if(isset($dataRekening['saldokredit']) && $dataRekening['saldokredit'] > 0){
                      //                                                    $jumlah = $dataRekening['saldokredit'];
                      //                                                    $debitkredit = "K";
                      //                                                    $urut = 1;
                      //                                                }
  ...
  public function actionSetFormFakturPembelian()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      $tgl_awal = isset($_POST['tgl_awal']) ? MyFormatter::formatDateTimeForDb($_POST['tgl_awal']) : date('Y-m-d');
      $tgl_akhir = isset($_POST['tgl_akhir']) ? MyFormatter::formatDateTimeForDb($_POST['tgl_akhir']) : date('Y-m-d');
      ...
      $dataDetail = FakturpembelianV::model()->findAll($criteria);
      ...
      if (count((array)$dataDetail) > 0) {
        ...
        foreach ($dataDetail as $i => $data) {
          ...
          if ($data->typefaktur == 'barang') {
            $modSetoran = SetoranpajakT::model()->findByAttributes(array('terimapersediaan_id' => $data->faktur_id));
            $pajakNilai = (isset($modSetoran) ? $modSetoran->jmlpembayaran : 0);
          } else if ($data->typefaktur == 'obatalkes') {
            $modSetoran = SetoranpajakT::model()->findByAttributes(array('fakturpembelian_id' => $data->faktur_id));
            $pajakNilai = (isset($modSetoran) ? $modSetoran->jmlpembayaran : 0);
          } else if ($data->typefaktur == 'gizi') {
            $modSetoran = SetoranpajakT::model()->findByAttributes(array('terimabahanmakan_id' => $data->faktur_id));
            $pajakNilai = (isset($modSetoran) ? $modSetoran->jmlpembayaran : 0);
          }

          $data->pajakpph = ($data->pajakpph - $pajakNilai);
          if ($data->pajakpph > 0) {
        ...
        $pesan = 'Data Setoran Hutang Pajak tidak ditemukan';
      }
  ...
  protected function saveJurnalRekening($model, $modBuktiKeluar, $pajak)
  {
    ...
    $modJurnalRekening->tglbuktijurnal = $format->formatDateTimeForDB($model['tglsetoranpajak']);
    ...
    $modJurnalRekening->tglreferensi = $format->formatDateTimeForDB($model['tglsetoranpajak']);
  ...
  //    public function actionAmbilDataRekeningPajak() {
  //        if (Yii::app()->getRequest()->getIsAjaxRequest()) {
  //            $pajak_id = isset($_POST['pajak_id']) ? $_POST['pajak_id'] : null;
  //            $criteria = new CDbCriteria;
  //            $criteria->join = "JOIN rekening5_m ON rekening5_m.rekening5_id = t.rekening5_id "
  ...
  //            if ($model) {
  //                echo CJSON::encode(
  //                        $this->renderPartial($this->path_view . '__formKodeRekening', array('model' => $model, 'debitkredit' => 'D'), true)
  //                );
  //            }
  //            Yii::app()->end();
  //        }
