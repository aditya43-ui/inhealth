...
class PembayaranIurBeaController extends MyAuthController
{
  public $succesSave = true;
  public $pesan = "succes";
  public $is_action = "insert";
  public $path_view = 'keuangan.views.pembayaranIurBea.';
  ...
  public function actionIndex()
  {
    ...
    $modPenUmum->jenispenerimaan_id = 19;
    ...
    if (isset($_POST['KUPenerimaanUmumT'])) {
      ...
      try {
        ...
        // var_dump($modTandaBukti->attributes, $modPenUmum->attributes); die;
    ...
    if (Yii::app()->request->isAjaxRequest) {
      ...
      if (isset($data_parsing['KUPenerimaanUmumT'])) {
        ...
        try {
          ...
          // var_dump($modPenUmum->attributes); 
  ...
  public function saveTandaBukti($postTandaBukti)
  {
    ...
    //$modTandaBukti->keterangan_pembayaran = "-";
    ...
    for ($i = 0; $i < count((array)$arrPostUraian); $i++) {
      if (strlen($arrPostUraian[$i]['uraiantransaksi']) > 0) {
        ...
        if ($modUraian[$i]->validate()) {
          $modUraian[$i]->save();
          $valid = true;
          if (!empty($arrPostUraian[$i]['iurbea_id'])) {
            $ok = KUIurbeaT::model()->updateByPk($arrPostUraian[$i]['iurbea_id'], array(
              'uraianpenumum_id'=>$modUraian[$i]->uraianpenumum_id,
              'penerimaanumum_id'=>$modUraian[$i]->penerimaanumum_id,
            ));
          }
        } else {
  ...
  public function actionLoadIurBea() {
    if (!Yii::app()->request->isAjaxRequest) {
      Yii::app()->end();
    }
    ...
    $models = KUIurbeaT::model()->findAllByAttributes(array(
    ...
    if (count($models) == 0) {
      echo CJSON::encode(array(
        ...
        'html'=>'',
        'total_bea'=>0,
        'msg'=>'Data Iur Bea tidak ditemukan',
      ));
      Yii::app()->end();
    }
    ...
    foreach ($models as $idx => $model) {
      $det = new KUUraianpenumumT;
      $det->volume = 1;
      $det->hargasatuan = MyFormatter::formatNumberForPrint($model->totalinacbg_naikkelasperawatan);
      $det->totalharga = MyFormatter::formatNumberForPrint($model->totalinacbg_naikkelasperawatan);
      $det->uraiantransaksi = "Pembayaran Iur Bea - ".$model->notransaksiiurbea;
      $det->satuanvol = 'KALI';
      $det->iurbea_id = $model->iurbea_id;

      $total_bea += $model->totalinacbg_naikkelasperawatan;

      $modUraians[] = $det;

    }
    $html .= $this->renderPartial($this->path_view . '_rowUraian', array('modUraian' => $modUraians), true);
