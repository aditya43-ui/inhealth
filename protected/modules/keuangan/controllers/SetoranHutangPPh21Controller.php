...
class SetoranHutangPPh21Controller extends MyAuthController
{
  public $path_view = 'keuangan.views.setoranHutangPPh21.';
  ...
  public function actionIndex()
  {
    ...
    $modBuktiKeluar->untukpembayaran = "Setoran PPh 21 Pegawai - " . $modBuktiKeluar->no_setorpajakpembelian;
              ...
              if (isset($_POST['PenggajianpegT']) && count((array)$_POST['PenggajianpegT']) > 0) {
                ...
                foreach ($_POST['PenggajianpegT'] as $detailData) {
                  if ($detailData['checklist'] == 1) {
                    ...
                    $modelDet->jenissetoran = Params::JENISSETORAN_PPHPEGAWAI;
                    $modelDet->keterangansetoran = $detailData['keterangan'];

                    if ($modelDet->save()) {
                      $saveDetail = true;
                      $periodeGaji = MyFormatter::formatDateTimeForDb($detailData['periodegaji']);
                      $modPegGaji = PenggajianpegT::model()->findAllByAttributes(array('periodegaji' => $periodeGaji, 'pajak_id' => $detailData['pajak_id']));
  ...
  public function actionSetFromHutangPajak()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      $criteria->select = "t.periodegaji, pajak_m.pajak_id, pajak_m.pajak_nama, sum(totalpajak) as totalpajak";
      $criteria->group = "t.periodegaji, pajak_m.pajak_id, pajak_m.pajak_nama";
