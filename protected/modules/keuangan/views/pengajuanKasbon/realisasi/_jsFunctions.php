<script type="text/javascript">
	...
	var trUraian = new String(<?php echo CJSON::encode($this->renderPartial($this->path_view. 'realisasi/_rowUraian', array('form' => $form, 'modUraian' => $modUraian, 'removeButton' => true), true)); ?>);
//var trUraian=new String(<?php //echo CJSON::encode($this->renderPartial('realisasi/_rowUraian',array('form'=>$form,'modUraian'=>array(0=>$modUraian[0]),'removeButton'=>true),true)); ?>);
...
function getDataRekening(params)
	{
		...
		$.post('<?php echo Yii::app()->createUrl(Yii::app()->controller->module->id . '/' . Yii::app()->controller->id . '/GetDataRekeningByJnsPenerimaan'); ?>', {jenispenerimaan_id: params, is_kasbon: 1},
		function (data) {
			if (data != null) {
				$("#tblInputRekening > tbody").append(data.replace());
				renameRowRekening();
...
//        }
		...
		if(!requiredCheck($("#akpenerimaan-umum-t-form"))){
			...
			function (data) {
				if (data.status == 'ok')
				{
					...
					window.location.replace("<?php echo $this->createUrl('realisasi'); ?>");
    ...
    {
        unformatNumberSemua();
		var biayaAdministrasi = 0; //parseInt($('#KUTandabuktibayarT_biayaadministrasi').val());
		var biayaMaterai = 0; //parseInt($('#KUTandabuktibayarT_biayamaterai').val());
		var vol = parseInt($('#KUPenerimaanUmumT_volume').val());
		var harga = parseFloat($('#KUPenerimaanUmumT_hargasatuan').val());
                
        var pph22 = 0; //parseFloat($('#KUPenerimaanUmumT_persenpph_22').val());
        var pph23 = 0; //parseFloat($('#KUPenerimaanUmumT_persenpph_23').val());
        var pph21 = 0; //parseFloat($('#KUPenerimaanUmumT_persenpph_21').val());
        var ppn = 0; //parseFloat($('#KUPenerimaanUmumT_persenppn').val());

        var jmlNetto = (vol * harga);
        var jmlpph21 = 0; //((pph21 * jmlNetto)/100);
        var jmlpph22 = 0; //((pph22 * jmlNetto)/100);
        var jmlpph23 = 0; //((pph23 * jmlNetto)/100);
        var jmlppn = 0; //((ppn * jmlNetto)/100);
        var totalharga = ((jmlNetto + jmlpph21 + jmlpph22 + jmlpph23)-(jmlppn + biayaAdministrasi + biayaMaterai));
        console.log("TOTAL", totalharga, vol, harga);
...
//	});
    ...
    $(document).ready(function() {
		...
		getDataRekening($("#KUPenerimaanUmumT_jenispenerimaan_id").val());
    	getSebagaiBayar($("#KUPenerimaanUmumT_jenisKodeNama").val());
