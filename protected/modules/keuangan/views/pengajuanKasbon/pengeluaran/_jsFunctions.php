...
var trUraian=new String(<?php echo CJSON::encode($this->renderPartial($this->path_view. 'pengeluaran/_rowUraian',array('form'=>$form,'modUraian'=>array(0=>$modUraian[0]),'removeButton'=>true),true));?>);
...
function getDataRekening(params)
{
    ...
    $.post('<?php echo Yii::app()->createUrl(Yii::app()->controller->module->id . '/' . Yii::app()->controller->id . '/GetDataRekeningByJnsPengeluaran');?>', {jenispengeluaran_id:params, is_kasbon:1},
        function(data){
			if(data != null){
            ...
            $.post('<?php echo Yii::app()->createUrl(Yii::app()->controller->module->id . '/' . Yii::app()->controller->id . '/SimpanPengeluaran');?>', {jenis_simpan:jenis_simpan, data:$('#akpengeluaran-umum-t-form').serialize()},
                function(data){
                    if(data.status == 'ok')
                    {
                        ...
                        window.location.replace("<?php echo $this->createUrl('pengeluaranKas'); ?>");
...
{
    var biayaAdministrasi = 0; //parseFloat(unformatNumber($('#KUTandabuktikeluarT_biayaadministrasi').val()));
    ...
    var ppn_total = 0; // (ppn/100) * totalharga;
    var pph21_total = 0; //(pph21/100) * totalharga;
    var pph22_total = 0; //(pph22/100) * totalharga;
    var pph23_total = 0; //(pph23/100) * totalharga;
...
$(document).ready(function(){
    ...
    getDataRekening($("#KUPengeluaranumumT_jenispengeluaran_id").val());
    getSebagaiBayar($("#KUPengeluaranumumT_jenisKodeNama").val());
