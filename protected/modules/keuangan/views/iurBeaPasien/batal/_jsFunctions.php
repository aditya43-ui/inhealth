...
var is_approvebatal = <?php echo empty($is_approvebatal) ? 0 : $is_approvebatal ?>;
...
function loadListIurBiaya() {

    $(".tab_list").addClass("animation-loading");

    $.post('<?php 
    
    if (!empty($is_approvebatal) && $is_approvebatal == 1) {
        echo $this->createUrl('listIurBeaBatal');
    } else {
        echo $this->createUrl('listIurBea');
    }

    ?>', {
        pendaftaran_id: pendaftaran_id
    }, function(data) {
        $(".tab_list_bea").html(data.html);
        $(".tab_list").removeClass("animation-loading");
    }, 'json');
}
...
function approveBatalBeaDialog(iurbea_id) {
    myConfirm("Anda yakin untuk approve membatalan ini ?", "Peringatan", function(r) {
        if (r) {
            $.post('<?php echo $this->createUrl('approveBatalBea'); ?>', {
                iurbea_id: iurbea_id
            }, function(data) {
                if (data.ok == 1) {
                    myAlert(data.msg);
