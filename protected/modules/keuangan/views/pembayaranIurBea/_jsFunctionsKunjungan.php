...
function setKunjungan(pendaftaran_id, no_pendaftaran, no_rekam_medik, pasienadmisi_id, instalasi_id ){
    ...
    $.ajax({
        type:'POST',
        url:'<?php echo $this->createUrl('/keuangan/iurBeaPasien/GetDataKunjungan'); ?>',
...
function loadTagihanPasien(data) {
    ...
    $("#KUTandabuktibayarT_darinama_bkm").val(data.no_pendaftaran + ' - ' + data.no_rekam_medik + ' - ' + data.namadepan + data.nama_pasien);
    $("#KUTandabuktibayarT_alamat_bkm").val(data.alamat_pasien);
    $("#KUTandabuktibayarT_sebagaipembayaran_bkm").val("Pembayaran Pelayanan Iur Bea");

    // load uraian
    loadIurBea(data.pendaftaran_id);
...
function loadIurBea(pendaftaran_id) {
    $("#tblInputUraian").addClass("animation-loading");
    $.post('<?php echo $this->createUrl('loadIurBea'); ?>', {pendaftaran_id: pendaftaran_id}, function(data) {
        if (data.ok) {
            $("#tblInputUraian tbody").html(data.html);
            $("#KUPenerimaanUmumT_hargasatuan").val(formatNumber(data.total_bea));
            $("#KUPenerimaanUmumT_totalharga").val(formatNumber(data.total_bea));
            totalTagihan();
            hitungTotalHarga();
        } else {
            myAlert(data.msg);
        }
        $("#tblInputUraian").removeClass("animation-loading");
    }, 'json');
}
