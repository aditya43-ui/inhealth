...
function loadDataFaktur(){
    ...
    var jenispengeluaran_id = $('#<?php echo CHtml::activeId($model, 'jenispengeluaran_id') ?>').val();
    
    if(tgl_awal !== "" && tgl_akhir !== "" && pajak_id !== ""){
        $.ajax({
            type:'POST',
            url:'<?php echo $this->createUrl('SetFromHutangPajak'); ?>',
            data: {pajak_id:pajak_id, tgl_awal: tgl_awal, tgl_akhir: tgl_akhir, jenispengeluaran_id: jenispengeluaran_id},
            dataType: "json",
        ...
        if(tgl_awal == "" && tgl_akhir == ""){
            myAlert("Tanggal Pengeluaran harus diisi!");
        }else{
            if(pajak_id == ''){
...
function validasiSetoranHutang(){
    if(requiredCheck($('#setoranhutangpengluarankas-t-form'))){
        ...
        if(jml < 1){
            ...
            $('#setoranhutangpengluarankas-t-form').submit();
        }
    }
    return false;
} 
...
$(document).ready(function(){
    ...
    var jenispengeluaran_id = jQuery('#<?php echo CHtml::activeId($model, 'jenispengeluaran_id') ?>');	
     jQuery(jenispengeluaran_id).multiselect({
