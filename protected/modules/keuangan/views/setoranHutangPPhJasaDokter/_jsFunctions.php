...
function loadDataFaktur(){
    ...
    if(periodetahun != "" && pajak_id != ""){
        $.ajax({
            type:'POST',
            url:'<?php echo $this->createUrl('SetFromHutangPajak'); ?>',
            data: {pajak_id:pajak_id, periodetahun: periodetahun},
            dataType: "json",
        ...
        if(periodetahun == ''){
            myAlert("Tahun Periode Jasa harus diisi!");
        }else{
            if(pajak_id == ''){
...
{       
    ...
    var pajakpph = parseFloat($(obj).parents('tr').find('input[name$="[total_pajak]"]').val());
...
function hitungTotal() {
    ...
    $("#table-setoran").find("tbody > tr").each(function () {
        if ($(this).find(".checklist").is(":checked")){
            totalTagihan += parseFloat($(this).find('input[name*="[total_pajak]"]').val());
...
function hitungPlorate(obj){
    ...
    if(checkdata===false){
        ...
        $("#table-setoran").find("tbody > tr").each(function () {
            if ($(this).find(".checklist").is(":checked")){
                var pajak = parseFloat($(this).find('input[name*="[total_pajak]"]').val());
...
function validasiFaktur(){
    if(requiredCheck($('#setoranhutangpphjasadokter-t-form'))){
        ...
        if(jml < 1){
            ...
            $('#setoranhutangpphjasadokter-t-form').submit();
        }
    }
    return false;
} 
