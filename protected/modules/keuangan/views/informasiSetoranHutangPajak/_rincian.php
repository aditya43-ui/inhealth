            ...
            <td>
                <div class="content">
                    ...
                    <table width="85%" style='margin-left:auto; margin-right:auto;' class="border">
                        <thead class="border">
                            <th>No.</th>
                            <th>Periode <?php echo (($jenispajak == Params::JENISSETORAN_PPHJASADOKTER) ? "Jasa" : "Gaji"); ?></th>
                            <th>Total Utang</th>
                            <th>Total Setoran</th>
                        ...
                        foreach ($model as $i => $modData) {
                            $modSisahutang = SisahutangpajakT::model()->findByAttributes(array('setoranpajak_id' => $modData->setoranpajak_id));
                            $periode = "-";
                            if (isset($modSisahutang)) {
                                if ($jenispajak == Params::JENISSETORAN_PPHJASADOKTER) {
                                    $modPeg = PembayaranjasaT::model()->findByPk($modSisahutang->pembayaranjasa_id);
                                    $periode = (isset($modPeg) ? MyFormatter::getMonthId(date('m', strtotime($modPeg->periodejasa))) . " " . date('Y', strtotime($modPeg->periodejasa)) : "");
                                } else {
                                    $modPeg = PenggajianpegT::model()->findByPk($modSisahutang->penggajianpeg_id);
                                    $periode = (isset($modPeg) ? MyFormatter::getMonthId(date('m', strtotime($modPeg->periodegaji))) . " " . date('Y', strtotime($modPeg->periodegaji)) : "");
                                }
                            }
                        ?>
                            <tr class="border">
                                ...
                                <td style="text-align:right;"><?php echo MyFormatter::formatNumberForPrint($modData->totalhutang, 2); ?></td>
                                <td style="text-align:right;"><?php echo MyFormatter::formatNumberForPrint($modData->jmlpembayaran, 2); ?></td>
                                <td style="text-align:right;"><?php echo MyFormatter::formatNumberForPrint($modData->totalsisahutang, 2); ?></td>
                                <td>
