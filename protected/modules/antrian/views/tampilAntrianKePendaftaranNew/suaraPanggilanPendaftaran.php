...
if(count($noantrians) > 0){
    ...
    foreach($noantrians AS $i => $noantrian){        
        $split_kode = str_split(strtolower(trim($modLokets[$i]->modelantrian->modelantrian_singkatan)));        
        ?>
        <div id="jplayer_<?php echo $i;?>">
            ...
            if(count($split_kode) > 0){
                foreach($split_kode as $a => $kode){
            ?>
                <div id="jquery_jplayer_<?php echo $i;?>_kodeantri_<?php echo $a ?>" class="jp-jplayer kode"></div>
            <?php 
                }
            }
            ?>
        ...
        <script type="text/javascript">
            ...
            <?php
                if (count($split_kode) > 0){
                    foreach($split_kode as $a => $kode){
            ?>
                        setPlaylist("<?php echo $i;?>","kodeantri_<?php echo $a ?>","<?php echo $kode; ?>");
            <?php
                    }
                }
            ?>
    ...
    var soundDat = [
        <?php foreach ($noantrians as $i => $noantrian) { ?>
            ...
            if(is_numeric($modLokets[$i]->loket_singkatan)){
                $loket_split = explode(" ", strtolower(MyFormatter::formatNumberTerbilang((int)$modLokets[$i]->loket_singkatan)));
                foreach($loket_split as $ii => $nomor){ ?>
                {name: "<?php echo $nomor; ?>"},
                <?php }
            }else { ?>
                {name: "<?php echo strtolower(trim($modLokets[$i]->loket_singkatan)); ?>"}, <?php
            }
        } ?>
    ];
