...
<?php 
	...
	'columns'=>array(
        ...
        array(
            'header'=>'Loket Antrian',
            'name'=>'modelantrian_id',
            'type'=>'raw',
            'value'=>function($data) {
                $model = ModelantrianM::model()->findByPk($data->modelantrian_id);
                if (empty($model)) return null;
                
                return $model->modelantrian_nama." (".$model->modelantrian_kode.")";
            },
            'filter'=>CHtml::activeDropDownList($modKarcisTerakhir, 'modelantrian_id', CHtml::listData(ModelantrianM::model()->findAllByAttributes(array(
                'modelantrian_kode'=>array('A', 'B', 'U')
            )), 'modelantrian_id', 'modelantrian_nama'), array('empty'=>'')),
        ),
		array(
                    'name'=>'racikan_id',
                    'type'=>'raw',
		...
		'noantrian',
                ...
                array(
                    ...
                    'value'=>'CHtml::Link("<i class=\"entypo-print\"></i>","javascript:void(0);",
                        array(
                              "onclick"=>"printKarcisFarmasiResep($data->reseptur_id,\"PRINT\")",
                              "rel"=>"tooltip",
                        ))',
                        'htmlOptions'=>array(
                            'style'=>'text-align: center;'
