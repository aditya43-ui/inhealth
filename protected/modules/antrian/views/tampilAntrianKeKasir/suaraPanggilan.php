...
<?php 
$jenissuara = KonfigsystemK::model()->findByPk(Params::getDefaultProfilRS())->jenissuaraantrian; 
$jenissuara = isset($jenissuara)?$jenissuara:'PEREMPUAN_2';
?>
...
$(document).ready(function() {
    ...
    var soundDat = [
        <?php foreach ($noantrians as $i => $noantrian) { 
            ...
            $modela = ModelantrianM::model()->findByPk($modLokets[$i]->modelantrian_id);
            
            ?>
        {name: "bell panggil pendaftaran"},
        {name: "nomer"},
        {name: "noantrian"},
        {name: "<?php echo strtolower(trim($modela->modelantrian_singkatan)); ?>"},
        <?php   
        $noantrian_split = explode(" ", strtolower(MyFormatter::formatNumberTerbilang((int)$noantrian)));
        foreach($noantrian_split as $ii => $nomor){ ?>
        {name: "<?php echo $nomor; ?>"},
