...
$jenissuara = isset($jenissuara)?$jenissuara->jenissuaraantrian:'PEREMPUAN';
?>
    ...
    $("#jquery_jplayer_" +i+"_"+id).jPlayer( {
        ...
        repeat: function(event) { // Override the default jPlayer repeat event handler
                if(event.jPlayer.options.loop) {
                        ...
                        $(this).unbind(".jPlayerRepeat").unbind(".jPlayerNext");
                        $(this).bind($.jPlayer.event.ended + ".jPlayer.jPlayerNext", function() {
                            if(id === "antrian"){ //setelah antrian (bukanpertama)
...
$(document).ready(function() {
    ...
    var soundDat = [
        ...
        {name: "noantrian"},
        <?php if (!empty($loket)): ?>
        {name: "<?php echo strtolower($loket); ?>"},
        <?php endif; ?>

        // suara modelantrian_singkatan
        <?php if(!empty($modelantrian)) : 
            $modelantrian_split = str_split(strtolower($modelantrian));
            foreach($modelantrian_split as $i => $mod) {
        ?>
                {name: "<?php echo strtolower($mod); ?>"},
        <?php } endif; ?>

        // suara kode antrian
        <?php if(!empty($kodeantrian)) : 
            $kodeantrian_split = str_split(strtolower($kodeantrian));
            foreach($kodeantrian_split as $ii => $kode) {
        ?>
                {name: "<?php echo strtolower($kode); ?>"},
        <?php } endif; ?>

        // suara untuk noantrian
        <?php   
        $noantrian_split = explode(" ", strtolower(MyFormatter::formatNumberTerbilang((int)$noantrian)));
        foreach($noantrian_split as $ii => $nomor){ ?>
        {name: "<?php echo $nomor; ?>"},
