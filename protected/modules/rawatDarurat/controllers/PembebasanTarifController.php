...
class PembebasanTarifController extends MyAuthController
{
  ...
  public $pathView = "rawatDarurat.views.pembebasanTarif.";
  ...
  public function actionIndex()
  {
    ...
    $model = new RDPembebasantarifT;
    $model->tglpembebasan = date('d M Y H:i:s');

    if (isset($_POST[get_class($model)])) {
      $model->attributes = $_POST[get_class($model)];
      $modPendaftaran = RDPendaftaranT::model()->findByPk($_POST[get_class($modPendaftaran)]['pendaftaran_id']);
      $modPasien = RDPasienM::model()->findByPk($_POST[get_class($modPendaftaran)]['pasien_id']);
  ...
  protected function savePembebasan($model, $dataPembebasan)
  {
    ...
    $modPembebasan = new RDPembebasantarifT;
  ...
  protected function saveTindakanPelayanan($dataPembebasan)
  {
    ...
    foreach ($rows as $j => $row) {
      ...
      $modTindakan = RDTindakanPelayananT::model()->findByPk($tindakanpelayanan_id);
      ...
      if (count((array)$attributes) > 0) {
        ...
        $tindakan = RDTindakanPelayananT::model()->updateByPk($tindakanpelayanan_id, $attributes);
  ...
  public function actionDataPasien()
  {
    ...
    $pendaftaran_id = $_POST['pendaftaran_id'];
    $modPasien = RDPasienM::model()->findByPk($pasien_id);
  ...
  public function actionLoadDataPasien()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      $data = RDInfokunjunganrdV::model()->findByAttributes(array('pendaftaran_id' => $_POST['pendaftaran_id']));
      $post = null;
