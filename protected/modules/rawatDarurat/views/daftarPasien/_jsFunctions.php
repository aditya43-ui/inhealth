...
function cekVerifikasiTindakLanjut(obj, id) {
    $.post('<?php echo $this->createUrl('verifikasiTindakLanjut') ?>', {id: id}, function(data) {
        if (data.ok == 1) {
            $("#frameTindakLanjut").prop('src', '<?php echo Yii::app()->controller->createUrl("/".Yii::app()->controller->module->id."/".Yii::app()->controller->id."/tindakLanjutRI"); ?>&pendaftaran_id=' + id
                    + "&instalasi_id=<?php echo Params::INSTALASI_ID_RJ; ?>");
...
// {
//     // $('#temp_idPendaftaranDP').val(pendaftaran_id);
//     $('#RDNotriagePasienT_pendaftaran_id').val(pendaftaran_id);
