...
<?php echo $form->hiddenField($model, 'notriage_pasien_id'); ?>
    ...
    <div class="controls">
        <?php
               if (($jenisform=='tambah')){
                ...
                echo $form->dropDownListRow($model, 'notriage_pasien_id', RDNotriagePasienT::getDropTrigaePasien2
                ($model->pendaftaran_id), array('empty'=>'-- Pilih --','class' => 'span3 bedtriage required', 'style'=>'width:200px;','onkeypress' => "return $(this).focusNextInputField(event);", 'onchange' => 'tambahNobed(this,"ubah")', 'maxlength' => 50));
            }?>
    </div>
</div>
<div class="control-group">
...
<div class="form-actions">
    ...
    //     );
    // } else {
        echo CHtml::htmlButton(Yii::t('mds','{icon} Save',array('{icon}'=>'<i class="entypo-check"></i>')),array('class' => 'btn btn-danger', 'type'=>'submit','onKeypress'=>'return formSubmit(this,event)','disabled' => false));
        echo CHtml::htmlButton(
            Yii::t('mds','{icon} Cancel', array('{icon}'=>'<i class="entypo-arrows-ccw"></i>')), 
            array('class' => 'btn btn-default', 'type'=>'button','onClick'=>'closeDialog();')
        );
...
<script type="text/javascript">
    ...
    function tambahNobed(obj)
    {
        var id = $(obj).find(":selected").val();
        $.post("<?php echo $this->createUrl('loadTriage'); ?>", {id: id},
            function(data){
