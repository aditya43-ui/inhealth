...
<div class="panel panel-success">
    ...
    <div class="panel-body">
        ...
        <div class="col-sm-6">
            ...
            <div class="control-group box-inap" hidden>
                <?php echo CHtml::label('Catatan Rencana Ruang Inap', 'penerimapasien', array('class' => 'control-label')) ?>
                <?php //echo $form->labelEx($modelPulang,'kondisikeluar_id', array('class'=>'control-label')) 
                ?>
                <div class="controls">
        ...
        <div class="col-sm-6">
            ...
            <div class="panel panel-success box-meninggal" hidden>
                ...
                <div class="panel-body">
                    <div class="control-group">
                        <?php 
                            if(empty($modelPulang->tgl_meninggal)) {
                ...
                <div class="col-sm-12">
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <?php echo $form->textArea($modKematian, 'penyebabkematian', array('placeholder' => 'Penyebab Kematian', 'class' => 'span3')); ?>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="panel panel-success" hidden>
    <div class="panel-heading">
        <div class="panel-title">Surat Kematian Pasien</div>
    </div>
    <div class="panel-body panel-suratkematian">
        <?php 
            $this->renderPartial('_formSuratKematian', ['form' => $form, 'modKematian' => $modKematian])
        ?>
    </div>
</div>
...
<div class="form-actions">
    <?php echo CHtml::htmlButton(
        ...
        array('class' => 'btn btn-danger', 'type' => 'button', 'onclick' => 'cekSPRI(' .$modPendaftaran->pendaftaran_id .')', 'disabled' => ($tersimpan) ? true: false)
    ); ?>
    <?php echo CHtml::htmlButton(
    ...
    <span class="box-meninggal">
        <?php 
            ...
            array("class"=>"btn btn-success", 
                    ...
                    'disabled' => ($tersimpan) ? false : true
            ));
        ?>
       
        <?php 
            $cekResume = ResumemedisR::model()->findByAttributes(['pendaftaran_id' => $modPendaftaran->pendaftaran_id, 'ismeninggal' => true], ['order' => 'resumemedis_id desc']);
            if(!empty($cekResume)) {
                echo CHtml::htmlButton(
...
<script>
    ...
    function updateSorotX() {
        ...
        $("#table-diagnosameninggal tbody tr").each(function() {
            var diagnosa_id = $(this).find('input[name$="[diagnosa_id]"]').val();
            $("#PPdiagnosa-m-grid table tbody #pilih_dialog_" + diagnosa_id).parents("tr").find("td").addClass("sorot");
        });
    }
    ...
    function addRowDiagnosa(diagnosa_id, diagnosa_kode, diagnosa_nama, diagnosa_namalainnya) {
        ...
        $.post('<?= $this->createUrl('addRowDiagnosa') ?>', {
            ...
            var is_diganosa = false
            // cek diagnosa yang sama, jika sama tidak maka row ter input
            $('#table-diagnosameninggal tbody tr').each(function(){
                diagnosaTR = $(this).find('input[name$="[diagnosa_id]"]').val();
                if(diagnosaTR == diagnosa_id) {
                    is_diganosa = true;
                }
            });
    ...
    function cekSPRI(pendaftaran_id) {
        var diagnosa_meninggal = $('#table-diagnosameninggal tbody tr').length;
    
        var carakeluar_id = $('.carakeluar_id').val();
        if(carakeluar_id == 4 && diagnosa_meninggal < 1) {
            myAlert('Diagnosa kematian wajib ditambahkan!');
            return false;
        }

        // cek required untuk dom kematian
        var harusisi = false;
        if(carakeluar_id == 4) {
            $('.panel-suratkematian').find('input,textarea').not('.multiselect-search, .multiselect-container input').each(function(){
        ...
        // if(harusisi) {
        //     window.parent.myAlert('Silahkan isi yang bertanda bintang!');
        //     return false;
        // }

        // jika cara keluar selain rawat inap
        if(carakeluar_id != 5) {
            $.post('<?= $this->createUrl('cekSPRI') ?>', {
                pendaftaran_id:pendaftaran_id
            }, function(data){
                if(data.adaSPRI == 1) {
                    myAlert('Pasien tidak dapat ditindak lanjut, karena masih terdapat SPRI yang outstanding. Harap hubungi TPP atau Rekam Medis untuk menghapus SPRI terlebih dahulu!');
                    return false;
                } else {
...
<?php if ($tersimpan == true && !$modelPulang->isDead) { ?>
    <script>
