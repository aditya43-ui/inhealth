...
<div class="panel panel-gradient">
    <div class="panel-heading">
        <div class="panel-title">
            <i class="fas fa-envelope"></i> Kirim <b>Pesan Pelamar</b> <?php echo $this->is_blast ? "- SMS Blast" : ""; ?>
        </div>
    </div>
                ...
                <tr>
                    <td width="100%">
                        ...
                        $this->widget('ext.bootstrap.widgets.BootGridView', array(
                            'id' => 'pasien-m-grid',
                            'dataProvider' => $modPelamar->searchDialogSms(),
                            'filter' => $modPelamar,
                            'template' => "{summary}\n{items}{pager}",
                            ...
                            'columns' => array(
                                ...
                                array(
                                    ...
                                    'filter' => CHtml::listData(LookupM::model()->findAllByAttributes(array('lookup_type' => 'statusperkawinan')), 'lookup_value', 'lookup_name'),
                                ),
                                //                        'statusperkawinan',
                                'alamat_pelamar',
                                //                        'agama',
                                ...
                                array(
                                    'name' => 'nomobile_pelamar',
                                    'type' => 'raw',
                                    'value' => 'CHtml::Link("<i class=\"icon-pencil\"></i>$data->nomobile_pelamar",Yii::app()->createUrl("' . Yii::app()->controller->module->id . '/KirimPesan/UbahNomorPelamar",array("pelamar_id"=>$data->pelamar_id,"frame"=>true)),
												array("class"=>"", 
													  "target"=>"iframeNomor",
													  "onclick"=>"$(\"#editNomor\").dialog(\"open\");",
										))',
                                    'filter' =>
                                        CHtml::activeTextField($modPelamar, 'nomobile_pelamar', array('style' => 'width: calc(100% - 30px) !important; float: left;',))
                                        . ' ' .
                                        CHtml::activeCheckBox($modPelamar, 'nomor_valid', array('style' => 'width: initial !important; margin: 8px !important;', 'uncheckValue' => 0, 'rel' => 'tooltip', 'title' => 'Tampilkan nomor valid')),
                                    //                            'filter' => CHtml::activeCheckBox($modPelamar, 'nomor_valid', array('uncheckValue'=>0,'rel'=>'tooltip','title'=>'Tampilkan nomor valid')).CHtml::activeTextField($modPelamar, 'nomobile_pelamar'),
                                ),
                                array(
                                    ...
                                    'type' => 'raw',
                                    'htmlOptions' => array('style' => 'text-align: center;'),
                                    'value' => 'CHtml::hiddenField("pelamar_id", $data->pelamar_id).CHtml::checkBox("Pelamar[".$data->pelamar_id."]", $data->pilih, array("class"=>"pilih","onclick"=>"pilihSatu(this,\'$data->pelamar_id\')"))',
                                ),
                            ),
                            'afterAjaxUpdate' => 'function(id, data){
						jQuery(\'' . Params::TOOLTIP_SELECTOR . '\').tooltip({"placement":"' . Params::TOOLTIP_PLACEMENT . '"});
						$(\'input[name="PelamarT[tglmelamar]"]\').daterangepicker({
							"maxDate": "' . date('m/d/Y') . '",
							"showDropdowns": true,
						}, afterPilihTanggal);
						cekStatusSelect();
...
<script type="text/javascript">
    ...
    function selectAll() {
        var tglmelamar = $("#pasien-m-grid tr.filters").find('input[name*="[tglmelamar]"]').val();
        var nama_pelamar = $("#pasien-m-grid tr.filters").find('input[name*="[nama_pelamar]"]').val();
        var tempatlahir_pelamar = $("#pasien-m-grid tr.filters").find('input[name*="[tempatlahir_pelamar]"]').val();
        var alamat_pelamar = $("#pasien-m-grid tr.filters").find('input[name*="[alamat_pelamar]"]').val();
        var nomobile_pelamar = $("#pasien-m-grid tr.filters").find('input[name*="[nomobile_pelamar]"]').val();

        var jeniskelamin = $("#pasien-m-grid tr.filters").find('select[name*="[jeniskelamin]"]').val();
        var statusperkawinan = $("#pasien-m-grid tr.filters").find('select[name*="[statusperkawinan]"]').val();
        ...
        jQuery.ajax({
            'url': '<?php echo $this->createUrl('SelectAllPelamar') ?>',
            'data': {
                tglmelamar: tglmelamar,
                nama_pelamar: nama_pelamar,
                tempatlahir_pelamar: tempatlahir_pelamar,
                alamat_pelamar: alamat_pelamar,
                nomobile_pelamar: nomobile_pelamar,
    ...
    function pilihSatu(obj, pelamar_id) {
        var d = document.getElementById('nomortujuan');

        if ($(obj).is(':checked')) {
            d.innerHTML += "<input id='Nomor_" + pelamar_id + "' class='span3 Nomor' type='hidden' name='Nomor[]' value='" + pelamar_id + "' readonly='readonly'>";
        } else {
            $('#nomortujuan #Nomor_' + pelamar_id).detach();
        }
    }
    ...
    function cekInsert() {
        ...
        if (jumlahPilih > 0) {
            ...
            myAlert("Silakan pilih pelamar yang akan dikirimi pesan!");
        }
    }
