...
<?php 
    echo $this->renderPartial('_formDataPasienPulang',array('form'=>$form,'modHD'=>$modHD)) 
?>
...
<!--                    <?//php if(Yii::app()->user->getState('instalasi_id') == Params::INSTALASI_ID_RI) { ?>
        ...
        <?php //echo $this->renderPartial('_formRujukanKeluarHD',array('form'=>$form,'modelPulang'=>$modelPulang,'modRujukanKeluar'=>$modRujukanKeluar)) ?>
...
<script>
    ...
    function pasienmeninggal(value)
    {
        if(value == "<?php echo Params::KONDISIKELUAR_ID_MENINGGAL_1 ?>" || value == "<?php echo Params::KONDISIKELUAR_ID_MENINGGAL_2 ?>")
        {
             ...
             $('#dialogPasienPulang #HDPasienPulangT_tgl_meninggal').removeAttr('disabled');
        }
        else
        {
            $('#dialogPasienPulang #isDead').removeAttr('checked');
            $('#dialogPasienPulang #HDPasienPulangT_tgl_meninggal').attr('disabled','true');
        }
    }
    $('#dialogPasienPulang #isDead').change(function(){
        if ($(this).is(':checked')){
                $('#dialogPasienPulang #HDPasienPulangT_tgl_meninggal').removeAttr('disabled');
                $('#dialogPasienPulang #HDPasienPulangT_kondisipulang').val('<?php echo Params::KONDISIKELUAR_ID_MENINGGAL_1 ?>');
                $('#dialogPasienPulang #HDPasienPulangT_tgl_meninggal').val('<?php echo date('Y-m-d H:i:s') ?>');
        }else{
                $('#dialogPasienPulang #HDPasienPulangT_tgl_meninggal').attr('disabled','true');
                $('#dialogPasienPulang #HDPasienPulangT_kondisipulang').val('');
                $('#dialogPasienPulang #HDPasienPulangT_tgl_meninggal').val('');
