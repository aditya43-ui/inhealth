...
class HDMonitoringPostHdT extends MonitoringPostHdT{
    ...
    public function getShift($id){
        ...
        if (!empty($id)){
            $cri = new CDbCriteria();
            $cri->select = "t.shift_id, shift_m.shift_nama";
            $cri->join = "JOIN shift_m on t.shift_id=shift_m.shift_id";        
            $cri->addCondition("jadwalhemodialisa_id = ".$id);
            $shift = JadwalhemodialisaT::model()->findAll($cri);
        }
        return $shift;
    }
}
