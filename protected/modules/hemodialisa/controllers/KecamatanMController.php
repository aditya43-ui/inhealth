...
class KecamatanMController extends MyAuthController
{
	...
	public function actionCreate()
	{
		...
		$model=new HDKecamatanM;

		// Uncomment the following line if AJAX validation is needed
		

		if(isset($_POST['HDKecamatanM']))
		{
                                    $valid=true;
                                    foreach($_POST['HDKecamatanM'] as $i=>$item)
                                    {
                                        if(is_integer($i)) 
                                        {
                                            $model=new HDKecamatanM;
                                            if(isset($_POST['HDKecamatanM'][$i]))
                                                $model->attributes=$_POST['HDKecamatanM'][$i];
                                                $model->kecamatan_aktif = true;
                                                $model->kabupaten_id = $_POST['HDKecamatanM']['kabupaten_id'];
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['HDKecamatanM']))
		{
			$model->attributes=$_POST['HDKecamatanM'];
			if($model->save()){
                                Yii::app()->user->setFlash('success', '<strong>Berhasil!</strong> Data berhasil disimpan.');
				$this->redirect(array('admin'));
                        }
	...
	{
		$dataProvider=new CActiveDataProvider('HDKecamatanM');
	...
	public function actionAdmin()
	{
		...
		$model=new HDKecamatanM('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['HDKecamatanM']))
			$model->attributes=$_GET['HDKecamatanM'];
	...
	{
		$model=HDKecamatanM::model()->findByPk($id);
		if($model===null)
	...
	{              
		...
		{
                $id = $_POST['id'];
                $kelurahan = HDKelurahanM::model()->findByAttributes(array('kecamatan_id'=>$id));
                if ($kelurahan){
...
//                    $this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('admin'));
        ...
        if(isset($_POST['id']))
        {
           $update = HDKecamatanM::model()->updateByPk($id,array('kecamatan_aktif'=>false));
           if($update)
            {
                if (Yii::app()->request->isAjaxRequest)
                {
                ...
                public function actionPrint()
                {
                    ...
                    $model= new HDKecamatanM;
                    $model->attributes=$_REQUEST['HDKecamatanM'];
