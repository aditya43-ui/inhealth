...
class HardnessController extends MyAuthController {
    ...
    public $path_view = 'hemodialisa.views.hardness.';
    ...
    public function actionCreate() {
        ...
        $model = new HDHdHardnessT();
        $model->tgl_monitoring = date('Y-m-d');

        if (isset($_POST['HDHdHardnessT'])) {
            $ok = true;
            $trans = Yii::app()->db->beginTransaction();
            try {
                $model->attributes = $_POST['HDHdHardnessT'];
                ...
                if ($ok) {
                    ...
                    $trans->commit();
                    $this->redirect(array('admin', 'hd_hardness_id' => $model->hd_hardness_id, 'sukses' => 1));
                } else {
                    $trans->rollback();
    ...
    public function actionAdmin() {
        ...
        $model = new HDHdHardnessT('search');
        $model->unsetAttributes();  // clear any default values
        $model->tgl_awal = date('Y-m-01');
        $model->tgl_akhir = date('Y-m-d');
        if (isset($_GET['HDHdHardnessT'])) {
            $model->attributes = $_GET['HDHdHardnessT'];
            $model->tgl_awal = isset($_GET['HDHdHardnessT']['tgl_awal'])?MyFormatter::formatDateTimeForDb($_GET['HDHdHardnessT']['tgl_awal']):null;
            $model->tgl_akhir = isset($_GET['HDHdHardnessT']['tgl_akhir'])?MyFormatter::formatDateTimeForDb($_GET['HDHdHardnessT']['tgl_akhir']):null;
        }
    ...
    public function loadModel($id) {
        $model = HDHdHardnessT::model()->findByPk($id);
        if ($model === null)
    ...
    public function actionRemoveTemporary() {       
        $id = $_POST['id'];
        if (isset($_POST['id'])) {
            $update = HDHdHardnessT::model()->updateByPk($id, array('jaringansumberbiomaterial_aktif' => false));
            if ($update) {
                if (Yii::app()->request->isAjaxRequest) {
                    echo CJSON::encode(array(
                        'status' => 'proses_form',
    ...
    public function actionAktif() {       
        $id = $_POST['id'];
        if (isset($_POST['id'])) {
            $update = HDHdHardnessT::model()->updateByPk($id, array('jaringansumberbiomaterial_aktif' => true));
            if ($update) {
                if (Yii::app()->request->isAjaxRequest) {
                    echo CJSON::encode(array(
                        'status' => 'proses_form',
    ...
    public function actionPrint() {
        ...
        $judulLaporan = 'Water Treatment Quality Plant Control<br/>Hardness Testing Routine Sampling<br/>For Preetreatment Series-1 & 2';
        $caraPrint = $_REQUEST['caraPrint'];
        
        $data = [
            'judul_laporan' => $judulLaporan,
            'no_dok' => 'Form. WTP-IHD 02',
            'alias' => '',
            'nama_lengkap' => '',
            'no_rm' => '',
            'tanggal_lahir' => '',
