...
$js = <<< JS
    ...
    function refreshGrafikGaris(statuspasien){
        $('#garis').addClass("animation-loading");
        $('#speedo').addClass("animation-loading");
        $.ajax({
            type: "POST",
            url: "${urlGrafik}",
            data: params+"&statuspasien="+statuspasien,
            dataType: "json",
    ...
    $('#batang').bind('jqplotClick', function (ev, seriesIndex, pointIndex, data,jqplot) {
        ...
        var statuspasien = "";
        if(data != null){
            statuspasien = jqplot.data[0][[data.data[0]][0]-1][0];
        }
        refreshGrafikGaris(statuspasien);
    });
    $('#pie').bind('jqplotClick', function (ev, seriesIndex, pointIndex, data,jqplot) {
        $(".jqplot-target").attr("style","position:relative;width:100%;");
        var statuspasien = "";
        if(data != null){
            statuspasien = data.data[0];
        }
        refreshGrafikGaris(statuspasien);
    });
    refreshGrafikGaris("");
