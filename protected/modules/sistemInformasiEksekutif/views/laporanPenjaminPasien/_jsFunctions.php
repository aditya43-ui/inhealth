...
$js = <<< JS
    ...
    function refreshGrafikGaris(penjamin_nama){
        $('#garis').addClass("animation-loading");
        $('#speedo').addClass("animation-loading");
        $.ajax({
            type: "POST",
            url: "${urlGrafik}",
            data: params+"&penjamin_nama="+penjamin_nama,
            dataType: "json",
    ...
    $('#batang').bind('jqplotClick', function (ev, seriesIndex, pointIndex, data,jqplot) {
        ...
        var penjamin_nama = "";
        if(data != null){
            penjamin_nama = jqplot.data[0][[data.data[0]][0]-1][0];
        }
        refreshGrafikGaris(penjamin_nama);
    });
    $('#pie').bind('jqplotClick', function (ev, seriesIndex, pointIndex, data,jqplot) {
        $(".jqplot-target").attr("style","position:relative;width:100%;");
        var penjamin_nama = "";
        if(data != null){
            penjamin_nama = data.data[0];
        }
        refreshGrafikGaris(penjamin_nama);
    });
    refreshGrafikGaris("");
