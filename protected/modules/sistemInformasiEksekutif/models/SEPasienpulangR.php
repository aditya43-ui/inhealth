...
class SEPasienpulangR extends PasienpulangR {
    ...
    /**
     * digunakan untuk combobox
     * @return list
     */
    public function getInstalasiPelayanan()
    {
        $data = Yii::app()->db->createCommand()
                ->select('instalasi_id'
                        . ', instalasi_nama')
                ->from('pasienpulang_r')
                ->where('instalasi_id is not null')
                ->group('instalasi_id, instalasi_nama')
                ->order('instalasi_nama')
                ->queryAll();
        
        $list = CHtml::listData($data, 'instalasi_id', 'instalasi_nama');
        return $list;
