    ...
    if($caraPrint=='EXCEL')
        {
             header('Content-Type: application/vnd.ms-excel');
              header('Content-Disposition: attachment;filename="'.$judulLaporanDiag.'-'.date("Y/m/d").'.xls"');
              header('Cache-Control: max-age=0');     
        }
    echo $this->renderPartial('application.views.headerReport.headerDefault1LogoRSKiri',array('judulLaporan'=>$judulLaporanDiag, 'colspan'=>3)); 
}else{
    echo $this->renderPartial('application.views.headerReport.headerDefault1LogoRSKiri',array('judulLaporan'=>$judulLaporanDiag, 'colspan'=>3)); 
}
?>
<table >
    ...
    <tr>
        ...
        <td>Tgl. Diagnosa</td>
        <td>:</td>
        <td><?php echo (isset($detailHasildiagnosa[0]->tglmorbiditas) ? MyFormatter::formatDateTimeForUser($detailHasildiagnosa[0]->tglmorbiditas) : "-"); ?> </td>
    </tr>
...
<table class="table table-bordered table-condensed">
    ...
    <tbody>
        <?php 
            foreach($detailHasildiagnosa as $i=>$hasil){
        ?>
        <tr>
            <td><?php echo ($i+1); ?></td>
