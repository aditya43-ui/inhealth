...
<div class="col-sm-6">
    <div class="control-group">
        ...
        <div class="controls">
            ...
            //                                'htmlOptions'=>array('placeholder'=>'No. Badge','rel'=>'tooltip','title'=>'No. Badge untuk mencari pasien',
    ...
    <div class="control-group">
        <?php echo CHtml::label("Cari " . $modPasien->getAttributeLabel('norm_lama'), 'norm_lama', array('class' => 'control-label')) ?>
        <div class="controls">
            <?php
            $this->widget('MyJuiAutoComplete', array(
                'name' => 'cari_no_rekam_medik_lama',
                'value' => $modPasien->norm_lama,
                'source' => 'js: function(request, response) {
                                               $.ajax({
                                                   url: "' . Yii::app()->createUrl('mcu/PendaftaranPasien/AutocompletePasienLama') . '",
                                                   dataType: "json",
                                                   data: {
                                                       norm_lama: request.term,
                                                   },
                                                   success: function (data) {
                                                           response(data);
                                                   }
                ...
                'options' => array(
                    ...
                    'select' => 'js:function( event, ui ) {
                                            $(this).val( ui.item.value);
                                            setPasienLamaNorm(ui.item.pasien_id,ui.item.norm_lama);
                                            return false;
                                        }',
                ),
                'tombolDialog' => array('idDialog' => 'dialogPasien'),
                'htmlOptions' => array(
                    'placeholder' => 'No. Rekam Medik Lama', 'rel' => 'tooltip', 'title' => 'No. RM Lama untuk mencari pasien',
                    'onkeyup' => "return $(this).focusNextInputField(event)",
                    //                                    'onblur'=>"if($(this).val()=='') setPasienBaru(); else setPasienLama('',this.value)",
                    'class' => 'numbers-only span2'
                ),
            ...
            ?>
            <?php echo $form->error($modPasien, 'norm_lama'); ?>
        </div>
    </div>
    <div class="control-group">
        <?php echo CHtml::label('No. Fingerprint', 'nofingerprint', array('class' => 'control-label')) ?>
        <div class="controls">
            <?php echo $form->textField($modPasien, 'nofingerprint', array('readonly' => true, 'class' => 'span2', 'onkeyup' => "return $(this).focusNextInputField(event);")); ?>
            <?php echo CHtml::htmlButton(Yii::t('mds', '{icon}', array('{icon}' => '<i class="entypo-search"></i>')), array('id' => 'bth-carifingerprint', 'title' => 'Klik untuk mencari pasien menggunakan fingerprint', 'rel' => 'tooltip', 'class' => 'btn  btn-mini btn-primary', 'onclick' => 'cariFingerprint();')); ?>
        </div>
    </div>
    ...
    <div class="control-group">
        ...
        <div class="controls">
            ...
            $this->widget('MyJuiAutoComplete', array(
                ...
                'htmlOptions' => array('placeholder' => 'No. Identitas Pasien', 'rel' => 'tooltip', 'title' => 'No. Identitas untuk masukan data / mencari pasien', 'onkeyup' => "return $(this).focusNextInputField(event)", 'class' => 'span2 angkahuruf-only all-caps', 'onblur'=>'setInputBerdasarkanNoKTP();'),
            ));
            ?>
    ...
    <div class="control-group">
        <?php echo CHtml::label('No. Asuransi', 'Asuransi', array('class' => 'control-label')) ?>
        <div class="controls">
            <?php
            $this->widget('MyJuiAutoComplete', array(
                'model' => $modPasien,
                'attribute' => 'no_asuransi_pasien',
                'source' => 'js: function(request, response) {
                                               $.ajax({
                                                   url: "' . Yii::app()->createUrl('mcu/PendaftaranPasien/AutocompletePasienLama') . '",
                                                   dataType: "json",
                                                   data: {
                                                       no_asuransi_pasien: request.term,
                                                   },
                                                   success: function (data) {
                                                           response(data);
                                                   }
                ...
                'htmlOptions' => array('placeholder' => 'No. Asuransi Pasien', 'rel' => 'tooltip', 'title' => 'No. Asuransi untuk masukan data / mencari pasien', 'onkeyup' => "return $(this).focusNextInputField(event)", 'class' => 'span2'),
            ));
            ?>
...
<div class="col-sm-6">
    ...
    <?php echo $form->textFieldRow($modPasien, 'nama_perusahaan_bekerja', array('placeholder' => 'Tempat Bekerja', 'class' => 'span3 all-caps', 'onkeyup' => "return $(this).focusNextInputField(event);")); ?>
    ...
    <div style="text-align: center;">
        <?php echo CHtml::htmlButton(
            ...
            array(
                'class' => 'btn btn-danger', 'onclick' => "$('#dialog-addphoto').dialog('open');",
...
$this->widget('ext.bootstrap.widgets.BootGridView', array(
    ...
    'columns' => array(
        ...
        array(
            ...
            'filter' => Chtml::dropDownList('MCPasienM[jeniskelamin]', $modDataPasien->jeniskelamin, LookupM::model()->getItems('jeniskelamin'), array('empty' => '-- Pilih --')),
        ...
        array(
            ...
            'filter' => $this->widget(
                ...
                array(
                    ...
                    'htmlOptions' => array('readonly' => false, 'class' => 'span3 dtPicker3', 'id' => 'tanggal_lahir_pasien', 'placeholder' => '23 Jan 1993'),
                ),
                true
            ),
        ...
        array(
            ...
            'filter' => CHtml::dropDownList('MCPasienM[statusrekammedis]', $modDataPasien->statusrekammedis, LookupM::model()->getItems('statusrekammedis'), array('empty' => '-- Pilih --')),
            'value' => '$data->statusrekammedis',
        ),
    ),
    'afterAjaxUpdate' => 'function(id, data){
				jQuery(\'' . Params::TOOLTIP_SELECTOR . '\').tooltip({"placement":"' . Params::TOOLTIP_PLACEMENT . '"});
				jQuery("#tanggal_lahir_pasien").datepicker(jQuery.extend({showMonthAfterYear:false}, jQuery.datepicker.regional["id"], {"dateFormat":"dd M yy","maxDate":"d","timeText":"Waktu","hourText":"Jam","minuteText":"Menit","secondText":"Detik","showSecond":true,"timeOnlyTitle":"Pilih Waktu","timeFormat":"hh:mm:ss","changeYear":true,"changeMonth":true,"showAnim":"fold","yearRange":"-80y:+20y"}));
				jQuery("#tanggal_lahir_pasien_date").on("click", function(){jQuery("#tanggal_lahir_pasien").datepicker("show");});
			}',
));
$this->endWidget();
?>
...
$this->widget('ext.bootstrap.widgets.BootGridView', array(
    ...
    'dataProvider' => $modDataPasien->searchDialogPegawai(),
    'filter' => $modDataPasien,
    'template' => "{summary}\n{items}\n{pager}",
    'itemsCssClass' => 'table table-striped table-bordered table-condensed',
    'columns' => array(
        array(
            ...
            'value' => 'CHtml::Link("<i class=\"icon-form-check\"></i>","javascript:void(0);",array("class"=>"btn-small", 
									...
									"onClick" => "
										setPasienLama(\"$data->pasien_id\");
										$(\"#dialogPasienPegawai\").dialog(\"close\");
									"))',
        ),
        array(
            'header' => 'No. Badge',
    ...
    'afterAjaxUpdate' => 'function(id, data){
				jQuery(\'' . Params::TOOLTIP_SELECTOR . '\').tooltip({"placement":"' . Params::TOOLTIP_PLACEMENT . '"});
				jQuery("#' . CHtml::activeId($modDataPasien, 'tanggal_lahir') . '").datepicker(jQuery.extend({showMonthAfterYear:false}, jQuery.datepicker.regional["id"], {"dateFormat":"dd M yy","maxDate":"d","timeText":"Waktu","hourText":"Jam","minuteText":"Menit","secondText":"Detik","showSecond":true,"timeOnlyTitle":"Pilih Waktu","timeFormat":"hh:mm:ss","changeYear":true,"changeMonth":true,"showAnim":"fold","yearRange":"-80y:+20y"}));
				jQuery("#' . CHtml::activeId($modDataPasien, 'tanggal_lahir') . '_date").on("click", function(){jQuery("#' . CHtml::activeId($modDataPasien, 'tanggal_lahir') . '").datepicker("show");});
			}',
));
$this->endWidget();
?>
