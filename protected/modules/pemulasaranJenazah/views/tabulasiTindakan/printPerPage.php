...
if(!empty($notatindakan_last)) {
  ...
  $condition = ' pendaftaran_id = ' . $modPendaftaran->pendaftaran_id . ' and nopelayanan = \'' . $notatindakan_last->nopelayanan . '\'  and verifbataltindakan_id is null ';

  if(isset($_GET['is_all'])) {
    $condition = ' pendaftaran_id = ' . $modPendaftaran->pendaftaran_id . ' and verifbataltindakan_id is null ';
  }

  if(isset($_GET['nopelayanan'])) {
    $nopelayanan = $_GET['nopelayanan'];
    $condition = ' pendaftaran_id = ' . $modPendaftaran->pendaftaran_id . ' and nopelayanan = \'' . $_GET['nopelayanan'] . '\'  and verifbataltindakan_id is null ';
  }
    ...
    foreach($notatindakan as $i => $nota) {
        ...
        while(!$is_last) {
          ...
          $tindakan = TindakanPelayananT::model()
                      ...
                      )->findAll(array('condition' => "pendaftaran_id = $modPendaftaran->pendaftaran_id and nopelayanan = '$nota->nopelayanan'  and verifbataltindakan_id is null", "order"=>"tgl_tindakan, tindakanpelayanan_id", 'limit' => $limit, 'offset' => $offset));

          $tindakan_next = TindakanPelayananT::model()
                      ->with('daftartindakan', 'dokter1', 'dokter2',
                        'dokterPendamping', 'dokterAnastesi', 'dokterDelegasi',
                        'bidan', 'suster', 'perawat', 'tipePaket'
                        )->findAll(array('condition' => "pendaftaran_id = $modPendaftaran->pendaftaran_id and nopelayanan = '$nota->nopelayanan'  and verifbataltindakan_id is null ", "order"=>"tgl_tindakan, tindakanpelayanan_id", 'limit' => $limit, 'offset' => $offset + $limit));
