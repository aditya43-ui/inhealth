...
foreach ($modTindakan as $tindakan) {
    $nopelayanan = $tindakan->noNota;
    $total_tindakan += $tindakan->tarif_tindakan;
    if (empty($colspans[$nopelayanan])) {
        $colspans[$nopelayanan] = array(
            'isuse'=>false,
            'cnt'=>0
        );
    }
    $colspans[$nopelayanan]['cnt']++;
}
...
?>
    <tr>
        ...
        <!-- simpanCeklisVerifikasi(this); hitungTotalSemuaTind(); -->
            <?php echo CHtml::checkBox("pembayaran[$i][checked]", (!empty($tindakan->verifikasitagihan_id) && $is_batal != 1), array(
                'onclick'=>'setCeklisNota(this)',
                'uncheckValue'=>null, 
                'class'=>'pilihan pilih-cek',
                'data-nota'=>$tindakan->noNota,
            )) ?>
        </td>
        <td>
        ...
        <?php if (!$colspans[$nopelayanan]['isuse']): ?>
        <td rowspan="<?php echo $colspans[$nopelayanan]['cnt'] ?>">
            <?php 
        ...
        <td>
            ...
            <?php echo CHtml::hiddenField("pembayaran[$i][tarif_tindakan]", MyFormatter::formatNumberForPrint($tindakan->tarif_tindakan, 2), array('readonly'=>true,'onblur'=>'hitungTarifTindakan(this,'.$tindakan->tindakanpelayanan_id.'); hitungTotalSemuaTind();','class'=>'inputFormTabel integer-decimal span2 tarif_tindakan', 'style'=>'text-align: right; width:100px;')); ?>

        </td>
        <?php if (!$colspans[$nopelayanan]['isuse']): 
            $colspans[$nopelayanan]['isuse'] = true;
            ?>
        <td rowspan="<?php echo $colspans[$nopelayanan]['cnt'] ?>">
            <?php echo CHtml::textField("total_tarif", MyFormatter::formatNumberForPrint($total_tindakan, 2), array('readonly'=>true,'class'=>'inputFormTabel integer-decimal span2', 'style'=>'text-align: right; width:100px;')); ?>
        </td>
        <?php endif; ?>
    </tr>
<?php } ?>
