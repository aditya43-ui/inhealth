...
//          if (!$tindakanAktif){
          ...
          $penjualan = $obatAlkes->penjualanresep;

          if (!empty($penjualan)) {
            // var_dump($penjualan->penjualanresep_id);
            ...
            if (!empty($tindakan)) {
                ...
                if (!empty($tindakan->verifbataltindakan_id)) {
                    continue;
                }
            }
          }
...
//              $oaIurbiaya = round(($oaHargasatuan + $oaCyto)); // iur biaya tanpa $oaCyto
              ...
              $subTotalOa = ($oaIurbiaya * $obatAlkes->qty_oa); 
              $subTotalOa = ($subTotalOa > 0) ? $subTotalOa : 0;
              $totalbayarOa = $totalbayarOa + $totIurOa; 
          } else {
...
//              $oaIurbiaya = round(($oaHargasatuan + $oaCyto) - ($oaSubsidiasuransi + $oaSubsidipemerintah + $oaSubsidirs)); // iur biaya tanpa $oaCyto
    ...
    <tr>
        ...
        <td><?php echo CHtml::checkBox("pembayaranAlkes[$i][checked]", (!empty($obatAlkes->verifikasitagihan_id) && $is_batal != 1), array('onchange'=>'hitungTotalSemuaOa();', 'class'=>'ceklis_oa')) ?></td>
        <td><?php echo MyFormatter::formatDateTimeForUser($obatAlkes->penjualanresep->tglpenjualan) ?></td>
        <td>
            ...
            ?>
            <?php echo CHtml::hiddenField("pembayaranAlkes[$i][obatalkespasien_id]" ,$obatAlkes->obatalkespasien_id, array('readonly'=>true,'class'=>'obatalkespasien_id inputFormTabel span2')); ?>
        ...
        <td>
            <?php
                $oaIurbiaya = $obatAlkes->hargasatuan_oa * $obatAlkes->qty_oa;
                $totaloa += $oaIurbiaya;
            ?>
            <?php echo CHtml::textField("pembayaranAlkes[$i][iurbiaya]", MyFormatter::formatNumberForPrint($oaIurbiaya), array('readonly'=>true,'onblur'=>'hitungTotalSemuaOa()','class'=>'inputFormTabel number span1 integer2 qty_oa input_iurbiaya hide')); ?>

            <?php echo CHtml::textField("pembayaranAlkes[$i][iurbiaya_1]", MyFormatter::formatNumberForPrint($oaIurbiaya), array('readonly'=>true,'onblur'=>'hitungTotalSemuaOa();','class'=>'inputFormTabel integer2 span2 input_iurbiaya_1', 'style'=>'width: 100px;')); ?>
        </td>
        <td>
            <?php //echo $obatAlkes->daftartindakan_id ?>
            <?php echo CHtml::textField("pembayaranAlkes[$i][sub_total]", 0, array('readonly'=>true,'class'=>'inputFormTabel integer2 span2 subtotal_oa', 'style'=>'width: 100px;')); ?>
        </td>
    </tr>
<?php 
    ...
    <tr class="trfooter">
        ...
        <td colspan="3">Input Total <?php //echo $subsidiOa['max']; ?></td>
        ...
        <td>
            <?php echo CHtml::textField("totaliurbiaya_oa", $totIurOa, array('readonly'=>true,'class'=>'inputFormTabel integer2 span2 hide', 'style'=>'width: 100px;')); ?>
        </td>
        <td>
            <?php echo CHtml::textField("totalbayar_oa", $totaloa, array('readonly'=>true,'class'=>'inputFormTabel integer2 span2', 'style'=>'width: 100px;')); ?>
        </td>
    </tr>
