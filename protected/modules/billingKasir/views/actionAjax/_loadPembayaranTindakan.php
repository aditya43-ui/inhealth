...
<?php foreach($modTindakan as $i=>$tindakan) { ?>
    <tr>
        ...
        <td>
            ...
            <?php 
                  ...
                  if(!empty($subsidi['max'])){
                      ...
                      $subsidiAsuransi = (isset($tindakan->verifikasitagihan_id)) ? $tindakan->subsidiasuransi_tindakan : $subsidiAsuransi; 

                      $subsidiPemerintah = 0; 
                      $subsidiRumahSakit = (isset($tindakan->verifikasitagihan_id)) ? $tindakan->subsisidirumahsakit_tindakan : 0;
                      ...
                      $iurBiaya = (isset($tindakan->verifikasitagihan_id)) ? $tindakan->iurbiaya_tindakan : round($tarifSatuan + $tarifCyto);
                      ...
                      $subsidiAsuransi = (isset($tindakan->verifikasitagihan_id)) ? $tindakan->subsidiasuransi_tindakan : $subsidi['asuransi'];  
                      $subsidiPemerintah = $subsidi['pemerintah']; 
                      $subsidiRumahSakit = (isset($tindakan->verifikasitagihan_id)) ? $tindakan->subsisidirumahsakit_tindakan : $subsidi['rumahsakit'];

                      $totSubAsuransi = $totSubAsuransi + $subsidiAsuransi;
                      $totSubPemerintah = $totSubPemerintah + $subsidiPemerintah; 
                      $totSubRs = $totSubRs + $subsidiRumahSakit; 
                      // $iurBiaya = round(($tarifSatuan + $tarifCyto) - ($subsidiAsuransi + $subsidiPemerintah + $subsidiRumahSakit)); 
                      $iurBiaya = (isset($tindakan->verifikasitagihan_id)) ? $tindakan->iurbiaya_tindakan : round($tarifSatuan + $tarifCyto) - ($subsidiAsuransi + $subsidiPemerintah + $subsidiRumahSakit);
            ...
            <?php //echo CHtml::hiddenField("pembayaran[$i][discount_tindakan]",$discountTindakan, array('readonly'=>true)); ?>
        ...
        <td>
            <?php echo CHtml::textField("pembayaran[$i][discount_tindakan]",$discountTindakan, array('readonly'=>true, 'class'=>'inputFormTabel currency lebar3','onkeypress'=>"return $(this).focusNextInputField(event)")); ?>
        </td>
        <td>
        ...
        <td>
            <?php echo CHtml::textField("pembayaran[$i][subsidipemerintah_tindakan]", $subsidiPemerintah, array('readonly'=>true,'class'=>'inputFormTabel currency lebar3','onkeypress'=>"return $(this).focusNextInputField(event)")); ?>
        </td>
        <td>
            <?php echo CHtml::textField("pembayaran[$i][subsisidirumahsakit_tindakan]", $subsidiRumahSakit, array('readonly'=>true,'onblur'=>'hitungTotalSemuaTind();','class'=>'inputFormTabel currency lebar3','onkeypress'=>"return $(this).focusNextInputField(event)")); ?>
        </td>
        <td>
