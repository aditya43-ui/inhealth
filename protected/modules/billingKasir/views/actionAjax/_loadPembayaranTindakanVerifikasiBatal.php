...
foreach($modTindakan as $i=>$tindakan) { ?>
    <?php 
      ...
      $raw_qty = $tindakan->qty_tindakan;
      $tindakan->qty_tindakan = number_format($tindakan->qty_tindakan, 2, ",", "");
?>
    <tr <?php echo $nopelayanan == $tindakan->nopelayanan ? 'hidden' : '' ?>>
        <td>
            <?php if($nopelayanan != $tindakan->nopelayanan):?>
                <?php echo CHtml::textField('no_urut',0,array('class'=>'un-integer nourut','style'=>'width:30px','readonly'=>true)); ?>
            <?php endif;?>
        </td>
        <td>
        <!-- simpanCeklisVerifikasi(this); hitungTotalSemuaTind(); -->
            <?php echo CHtml::checkBox("pembayaran[$i][checked]", (!empty($tindakan->verifikasitagihan_id) && $is_batal != 1), array('onchange'=>'cekTotalSemuaTind(this);','uncheckValue'=>null, 'class'=>'pilihan pilih-cek pilih-tin')) ?>
        </td>
        <td>
        <?php
                ...
                if(!empty($tindakan->verifrenctindakan_id)) {
                    $verif = VerifrenctindakanT::model()->findByPk($tindakan->verifrenctindakan_id);
                    $tgl_tin = MyFormatter::formatDateTimeForUser($verif->create_time);
                }
            ...
            <?php echo CHtml::hiddenField("pembayaran[$i][nopelayanan]",$tindakan->nopelayanan, array('readonly'=>true, 'class' => 'nopel')); ?>
        </td>
        <td hidden>
        ...
        <td hidden>
            <?php echo CHtml::textField("pembayaran[$i][qty_tindakan]", $tindakan->qty_tindakan, array('readonly'=>true,'onblur'=>'hitungTarifTindakan(this,'.$tindakan->tindakanpelayanan_id.'); hitungTotalSemuaTind();','class'=>'inputFormTabel integer-decimal span1 qty_tindakan hide', 'style'=>'text-align: right;')); ?>
        </td>
              ...
              if (count((array)$modTindakanKomp) == 0) {
                  ...
                  if (!empty($jenis)) {
                      ...
                      foreach ($tarif as $key => $item) {
                          ...
                          $modkom->tarif_tindakankomp = $item->harga_tariftindakan * $raw_qty;
        ...
        <td>
            <?php
                echo CHtml::link("<icon class='icon-form-verifikasi'></icon>",
                    Yii::app()->controller->createUrl('batalVerifikasiTagihan/detail', array('pendaftaran_id' => $tindakan->pendaftaran_id, 'nopelayanan' => $tindakan->nopelayanan)),
                    array(
                        'target' => 'iframeDetail',
                        'data-toggle' => 'tooltip',
...
<?php 
    $nopelayanan = $tindakan->nopelayanan;
