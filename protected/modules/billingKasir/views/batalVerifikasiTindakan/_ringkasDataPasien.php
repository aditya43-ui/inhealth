...
<div class="row">
    <div class="col-sm-6">
        <div class="control-group">
            <div class=" control-label"><?php echo CHtml::Label("Instalasi <span style='color:red'>*</span>", 'instalasi_id', array('class' => '')); ?></div>
            <div class="controls">
                <?php
                if(!empty($modPendaftaran->pendaftaran_id)){
                    ...
                    echo CHtml::textField('instalasi_nama',$modPendaftaran->instalasi_nama,array('readonly'=>true,'class'=>'span3', 'onkeyup'=>"return $(this).focusNextInputField(event);")); 
                }else{

                    $listInstalasi = CHtml::listData(BKInstalasiM::model()->getInstalasiPelayananRawat(),'instalasi_id','instalasi_nama');
                    $instalasi_id = Yii::app()->user->getState('instalasi_id');
                    
                    if (empty($listInstalasi[$instalasi_id])) {
                        $instalasi_id = Params::INSTALASI_ID_RD;
                    }

                    echo CHtml::dropDownList('instalasi_id',$instalasi_id,$listInstalasi,array('onchange'=>'resetDataPasien(); refreshDialogPendaftaran();','class'=>'span3','onkeyup'=>"return $(this).focusNextInputField(event)",)); 
                }
                ?>
            </div>
        </div>
        ...
        if (Yii::app()->user->getState('instalasi_id') != Params::INSTALASI_ID_KASIR2) {
            $ruanganHidden = "hidden";
        }
        ?>
        <div class="control-group" <?php echo $ruanganHidden; ?>>
            <?php echo CHtml::Label('Ruangan', 'ruangan_nama', array('class' => 'control-label', 'label' => 'No. Pendaftaran')); ?>
            <div class="controls">
                <?php 
                    echo CHtml::textField('BKPendaftaranT[ruangan_nama]', Yii::app()->user->getState('ruangan_nama'), [
                        'id' => 'ruangan_nama', 'readonly' => 'true'
                    ]);
                ?>
            </div>
        </div>
...
$modDialogPasien = new BKInformasikasirrawatjalanV('');
$modDialogPasien->unsetAttributes();
$modDialogPasien->instalasi_id = Yii::app()->user->getState('instalasi_id');

$modDialogPasien->instalasi_ids = " = " . Yii::app()->user->getState('instalasi_id');

if($modDialogPasien->instalasi_id == 4) {
    
    $modDialogPasien->instalasi_ids = " in (4, 14, 38, 79)";
}
...
$this->widget('ext.bootstrap.widgets.BootGridView', array(
    ...
    'dataProvider' => $modDialogPasien->searchDialogKunjunganBatalTindakan(),
    ...
    'columns' => array(
        array(
            ...
            'value' => function ($data) {
                ...
                return CHtml::Link('<i class="icon-form-check"></i>', 'javascript:void(0);', array(
                    ...
                    'onClick' => 'isiDataPasien(' . $res . '); loadPembayaran(' . $data->pendaftaran_id .  ',' . $data->ruangan_id . '); $("#dialogPasien").dialog("close"); return false;'
                ));
            },
        ),
        array(
        ...
        array(
            ...
            'filter' => CHtml::activeDropDownList($modDialogPasien, 'ruangan_id', CHtml::listData(
                ...
                ), array('condition' => 'ruangan_aktif is true', 'order' => 'ruangan_nama')),
...
<script type="text/javascript">
    ...
    function loadPembayaran(pendaftaran_id, ruangan_id, penjualanresep, nonota) {

        <?php $tindakanParam = 'tindakan : 1'; ?>
        $.post('<?php echo Yii::app()->createUrl('billingKasir/ActionAjax/loadPembayaranVerifikasiTindakan'); ?>', {
            pendaftaran_id: pendaftaran_id,
            ruangan_id: ruangan_id,
            is_batal: 1,
            penjualanResep: penjualanresep,
            <?php echo $tindakanParam; ?>,
            nonota:nonota
        }, function(data) {
    ...
    function refreshDialogPendaftaran() {
        ...
        // var instalasiNama = $("#instalasi_nama").val();
    ...
    $(function() { 
        setTimeout(refreshDialogPendaftaran, 1000);
        // refreshDialogPendaftaran();
    });
</script>