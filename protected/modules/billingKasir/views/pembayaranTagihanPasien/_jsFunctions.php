...
var inacbg_kelaspelayanan = 0;
var inacbg_kelastanggungan = 0;
var inacbg_iurbiaya = 0;
 ...
 * @returns {undefined}
 */
function setKunjungan(pendaftaran_id, no_pendaftaran, no_rekam_medik, pasienadmisi_id, instalasi_id, penjamin_id, var_alokasidana_id){
    $("#form-datakunjungan > div").addClass("animation-loading");

    // console.log("ALOK 1", var_alokasidana_id);

    const instalasi_ri = [INSTALASI_ID_RI, INSTALASI_ID_ICU, 79, 38, 14, 85, 100];

    // console.log("INSTALASI", instalasi_id);
    ...
    $.ajax({
        ...
        data: {
            ...
            alokasidana_id:var_alokasidana_id
        },
        dataType: "json",
        success:function(data){
			if (data.notif.ok == 0) {
                ...
                myConfirm(data.notif.msg, "Peringatan!", function(r) {
                    if (r) {
                        alokasidana_id = var_alokasidana_id;
                        $(".alokasidana_id").val(var_alokasidana_id);
            ...
            if (data.uangmuka_iurbea_ready != 1) {
                myAlert(data.uangmuka_iurbea_pesan);
                $("#form-datakunjungan > div").removeClass("animation-loading");
                setKunjunganReset();
                return false;
            }
...
function hitungTotalTindakan() {
        ...
        $("#form-rinciantindakan").find("input[name$='[is_pilihtindakan]'][type='checkbox']").each(function() {
            ...
            var is_umum = row_tindakan.find("input[name$='[subsidiasuransi_tindakan]']").data("is_umum");
            ...
            if (is_umum == 1) {
                subtotal = subsidiasuransi_tindakan;
            } else {
                subtotal = 0;
            }
            ...
            sisatagihan = 0; //((qty_tindakan * (tarif_satuan + tarifcyto_tindakan)) - discount_tindakan - subsidiasuransi_tindakan - subsisidirumahsakit_tindakan - pembebasan_tindakan - subsidipemerintah_tindakan);
            ...
            if ($(this).is(":checked")) {
                ...
                row_tindakan.find("input[name$='[subtotal]']").val(subtotal);
                row_tindakan.find("input[name$='[iurbiaya_tindakan]']").val(subtotal);
                row_tindakan.find("input[name$='[jmlbayar_iurtindakan]']").val(subtotal);
                ...
                if (carabayar_id == '<?= Params::CARABAYAR_ID_BPJS?>' && (instalasi_id == '<?= Params::INSTALASI_ID_RI ?>' || instalasi_id == '<?= Params::INSTALASI_ID_PERAWATAN_INTENSIF ?>')) {
                    ...
                    row_tindakan.find("input[name$='[jmlselisihbpjs]']").val(selisih);
                } */
                ...
                row_tindakan.find("input[name$='[subtotal]']").val(0);
                row_tindakan.find("input[name$='[iurbiaya_tindakan]']").val(0);
                row_tindakan.find("input[name$='[jmlbayar_iurtindakan]']").val(0);
            }
        });
...
function hitungTotalOa(){
    ...
    $("#form-rincianobatalkes").find("input[name$='[is_pilihoa]'][type='checkbox']").each(function(){
        ...
        var subsidirs = 0; //parseFloat($(this).parents('tr').find("input[name$='[subsidirs]']").val());
        var jasapelayanan_farmasi = parseFloat($(this).parents('tr').find("input[name$='[jasapelayanan_farmasi]']").val());
        var carabayar_id = $(this).parents("tr").data("carabayar_id");
        ...
        if (carabayar_id == CARABAYAR_ID_MEMBAYAR) {
            subtotaloa = subsidiasuransi;
        } else {
            subtotaloa = 0;
        }
        ...
        console.log("TOTALE", subsidiasuransi, subsidirs);
...
function hitungJmlpembulatan(){
    ...
    */
    ...
    if(konfig_pembulatan > 0 && totaldibayarpasien > 0) {
        ...
        var nilai_mod = Math.ceil((nilai_total)/konfig_pembulatan) * konfig_pembulatan;
...
function hitungJmlpembayaran(){
    ...
    var asuransi = parseFloat(unformatNumber($("#<?php echo CHtml::activeId($model,'total_inacbg');?>").val())) + parseFloat(unformatNumber($("#<?php echo CHtml::activeId($model,'totalsubsidiasuransi');?>").val()));
    ...
    if(carabayar_id == <?php echo Params::CARABAYAR_ID_BPJS?> && inacbg_kelastanggungan != "" && inacbg_kelastanggungan != 0){
        jmlbayardikurangi = inacbg_iurbiaya;
    }
    ...
    } else if(carabayar_id == <?php echo Params::CARABAYAR_ID_BPJS?>){
        var jmliurbiaya = (jmlpembayaran - asuransi - rs - pem - uangmuka);
        if (inacbg_kelastanggungan != "" && inacbg_kelastanggungan != 0) {
            jmliurbiaya = inacbg_iurbiaya - uangmuka;
        }
    } else{
        var jmliurbiaya = (jmlpembayaran - asuransi - rs - pem - uangmuka);
    }
    ...
    */
    ...
    if (carabayar_id == <?php echo Params::CARABAYAR_ID_BPJS; ?> || carabayar_id == <?php echo Params::CARABAYAR_ID_BPJS_TENAGAKERJA; ?>) {
        ...
        if(konfig_pembulatan_bpjs > 0) {
            var nilai_mod = Math.ceil((jmliurbiaya)/konfig_pembulatan_bpjs) * konfig_pembulatan_bpjs;
    ...
    if(konfig_pembulatan > 0) {
        ...
        var nilai_mod = Math.ceil((selisih)/konfig_pembulatan) * konfig_pembulatan;
...
function cekBayarBank(obj) {
    ...
    if(konfig_pembulatan > 0) {
        var nilai_mod = Math.ceil((iurbiayaBulat)/konfig_pembulatan) * konfig_pembulatan;
...
function getCarabayar(){
    ...
    $.ajax({
        ...
        data: {'carapembayaran':carapembayaran, 'penjamin_id':penjamin_id},
        dataType: "json",
        success:function(data){
        ...
        $.ajax({
            ...
            success: function (data) {
                $("#form-rinciantindakan").find("tbody > tr").each(function () {
                    ...
                    $(this).find("select[name$='[penjamin_id]']").addClass('penjamin_' + penjamintemp_id);
                });
...
function setAsuransiKelas() {
    ...
    function setAlokasiDana() {
        console.log("ALOKASI", alokasidana_id);
        if (alokasidana_id == null) {
            return false;
        }
        $.post('<?php echo $this->createUrl('loadAlokasiDana'); ?>', {
            alokasidana_id: alokasidana_id
        }, function(data) {
            $("#BKPembayaranpelayananT_totalbiayatindakan").val(formatThousandDecimal(parseFloat(data.totalbiayatindakan)));
            $("#BKPembayaranpelayananT_totalbiayaoa").val(formatThousandDecimal(Math.round(parseFloat(data.totalbiayaoa))));
            $("#BKPembayaranpelayananT_totalbiayapelayanan").val(formatThousandDecimal(Math.round(parseFloat(data.totalbiayapelayanan))));
            $("#BKTandabuktibayarT_jmlpembayaran").val(formatThousandDecimal(parseFloat(data.totalbiayapelayanan)));
            $("#BKPembayaranpelayananT_total_inacbg").val(formatThousandDecimal(parseFloat(data.total_inacbg)));
            $("#BKPembayaranpelayananT_totalsubsidiasuransi").val(formatThousandDecimal(parseFloat(data.totalsubsidiasuransi)));
            $("#BKPembayaranpelayananT_totaliurbiaya").val(formatThousandDecimal(parseFloat(data.totaliurbiaya)));
            $("#BKPembayaranpelayananT_selisihuntungrugibpjs").val("0,00");
            $("#inacbg_kelaspelayanan").val(formatThousandDecimal(parseFloat(data.totalinacbg_naikkelasperawatan)));
            $("#inacbg_kelastanggungan").val(formatThousandDecimal(parseFloat(data.total_inacbg)));

            $("#tot_tarif_semua").val(formatThousandDecimal(parseFloat(data.totalbiayapelayanan)));
            $("#tot_subsidiasuransi_semua").val(formatThousandDecimal(parseFloat(data.totalsubsidiasuransi)));
            $("#total_semua").val(formatThousandDecimal(parseFloat(data.totaliurbiaya)));

            inacbg_kelaspelayanan = data.totalinacbg_naikkelasperawatan;
            inacbg_kelastanggungan = data.total_inacbg;
            inacbg_iurbiaya = data.totaliurbiaya;
