            ...
            <td>
                <div class="content">
                    ...
                    foreach ($modRincians2 as $item) {
                        ...
                        // var_dump($item_subtotal);
                        $item->tarif_satuan = (($item->tarif_satuan * 100) / 100);
                        $subtotalkotor += $item_subtotal; // (($item->qty_tindakan * $item->tarif_satuan * 100) / 100) - $item->discount_tindakan;
                        $subtotal += $item_subtotal; // (round($item->qty_tindakan * $item->tarif_satuan * 100) / 100) - ($item->subsidiasuransi_tindakan + $item->subsidipemerintah_tindakan + $item->subsisidirumahsakit_tindakan);

                        $tanggal = date('d/m/Y', strtotime($item->tgl_tindakan));
                        $daftartindakan_id = $item->tindakanpelayanan_id . "_" . ($item->is_alkes ? "0" : "1");
                        ...
                        if (empty($grp[$item->ruangan_id]['content'][$idx_line])) {
                            $grp[$item->ruangan_id]['content'][$idx_line] = array(
                                ...
                                'subp' => ($item->subsidipemerintah_tindakan),
                                'subr' => ($item->subsisidirumahsakit_tindakan),
                                'subtotal' => $item_subtotal, // (($item->qty_tindakan * $item->tarif_satuan) - ($item->discount_tindakan + $item->subsidiasuransi_tindakan + $item->subsidipemerintah_tindakan + $item->subsisidirumahsakit_tindakan)),
                                'subtotalkotor' => $item_subtotal, //(($item->qty_tindakan * $item->tarif_satuan) - $item->discount_tindakan),
                            ...
                            $grp[$item->ruangan_id]['content'][$idx_line]['subtotal'] += $item_subtotal; //(($item->qty_tindakan * $item->tarif_satuan) - ($item->discount_tindakan + $item->subsidiasuransi_tindakan + $item->subsidipemerintah_tindakan + $item->subsisidirumahsakit_tindakan));
                            $grp[$item->ruangan_id]['content'][$idx_line]['subtotalkotor'] += $item_subtotal; //(($item->qty_tindakan * $item->tarif_satuan) - $item->discount_tindakan);
...
<table width="100%" class="tab_detail">
    <thead class="font">
        <?php /*<th style='border: 1px solid;text-align: center;' hidden>No.</th> */ ?>
    ...
    <tbody>
        <?php foreach ($grp as $item) : ?>
            ...
            foreach ($item['content'] as $item2) :
                ...
                if ($item2['subtotalkotor'] == 0) {
                    continue;
                }

            ?>
                <tr>
                    <?php /* <td hidden><?php echo "*."; ?></td> */ ?>
        ...
        <?php /* if (!empty($bayaruangmuka)) { ?>
        <tr>
            <td></td>
            <td></td>
            <td colspan="2" style="text-align: right;">Total Uang Muka</td>
            <td style="text-align: left;">:</td>
            <td style="text-align: right;"><?php echo MyFormatter::formatNumberForPrint($bayaruangmuka->totaluangmuka, 2, true); ?></td>
        </tr>
        <tr>
            <td></td>
            <td></td>
            <td colspan="2" style="text-align: right;">Pemakaian Uang Muka</td>
            <td style="text-align: left;">:</td>
            <td style="text-align: right;"><?php echo MyFormatter::formatNumberForPrint($bayaruangmuka->pemakaianuangmuka, 2, true); ?></td>
        </tr>
        <tr>
            <td></td>
            <td></td>
            <td colspan="2" style="text-align: right;">Sisa Uang Muka</td>
            <td style="text-align: left;">:</td>
            <td style="text-align: right;"><?php echo MyFormatter::formatNumberForPrint($bayaruangmuka->sisauangmuka, 2, true); ?></td>
        </tr>
        <?php } */ ?>
        ...
        <tr>
            ...
            <td style="text-align: left;">:</td>
            <td style="text-align: right;"><?php echo MyFormatter::formatNumberForPrint($tandabukti->jmlpembulatan, 2, true); ?></td>
        </tr>
        <tr>
            <td></td>
            <td></td>
