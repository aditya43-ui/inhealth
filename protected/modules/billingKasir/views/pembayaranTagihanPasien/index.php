...
<?php echo $form->hiddenField($model, 'alokasidana_id', array('class'=>'alokasidana_id')); ?>
<div class="row">
    <div class="col-md-12">
        <div class="panel panel-gradient">
            <div class="panel-heading">
                <div class="panel-title">Pembayaran <strong>Tagihan Pasien</strong></div>
            </div>
                    ...
                    <div class="panel-body">
                        <div class="row-fluid">
                            <div style="overflow-x: auto; max-width: 100%;" id="form-rinciantindakan">
                                <?php $this->renderPartial($this->path_view.'_formRincianTindakanBayar', array('dataTindakans'=>$dataTindakans)); ?>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="panel-body">
                        <div class="row-fluid">
                            <div style="overflow-x: auto; max-width: 100%;" id="form-rincianobatalkes">
                                    <?php $this->renderPartial($this->path_view.'_formRincianObatalkesBayar', array('dataOas'=>$dataOas)); ?>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-actions">
                    ...
                    <?php
                        ...
                        // if($model->isNewRecord){
                            ...
                            echo "&nbsp;";
                            echo ($model->carabayar_id != Params::CARABAYAR_ID_MEMBAYAR) ? "" : CHtml::link(Yii::t('mds', '{icon} Print Kuitansi', array('{icon}'=>'<i class="icon-print icon-white"></i>')), 'javascript:void(0);', array('class'=>'btn btn-info','onclick'=>"printKuitansi();return false",'disabled'=>$model->carabayar_id != Params::CARABAYAR_ID_MEMBAYAR ));
...
<?php if(isset($_GET['sukses']) && $_GET['sukses'] == 1) { ?>
    <?php
    if ($model->penjamin_id == 904) {
        $this->beginWidget('zii.widgets.jui.CJuiDialog', array(
            'id'=>'dialog_bayar_piutang',
            'options'=>array(
                    'title'=>'Formulir Pernyataan Pasien Piutang',
                    'autoOpen'=>false,
                    'width'=>450,
                    'height'=>500,
            ),
    ...
    ?>
    <iframe id="frame_bayar_piutang" src="" style="width: 100%; height: 95%"></iframe>
    <?php 
        $this->endWidget();
    }
    ?>
    ...
    <script>
        $(function(){
            <?php if ($model->penjamin_id == Params::PENJAMIN_ID_UMUM) { ?>
            printKuitansi();
            <?php } ?>
        });

        <?php if ($model->penjamin_id == 904) { ?>
            $(document).ready(function() {
                setTimeout(function() {
                    $("#frame_bayar_piutang").prop("src", "<?php echo $this->createUrl('bayarPiutang', array('id'=>$model->pembayaranpelayanan_id)); ?>");
                    $("#dialog_bayar_piutang").dialog("open");
                }, 3000);
            });
        <?php } ?>
    </script>
...
<script>
    ...
    $('input, select, textarea').not('#BKTandabuktibayarT_uangditerima, #BKTandabuktibayarT_darinama_bkm, #BKInformasikasirrawatjalanV_items, #InformasipembayaranrawatjalanV_items, #no_pendaftaran').attr('readonly', true);
    $('.filters').find('input, select, textarea').attr('readonly', false);
</script>