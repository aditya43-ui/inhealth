	...
	<div class="control-group">
        ...
        <div class="controls">
            ...
            if(!empty($modKunjungan->pendaftaran_id)){
                ...
                echo CHtml::dropDownList('instalasi_id',$modKunjungan->instalasi_id,CHtml::listData(BKInstalasiM::model()->getInstalasiPelayananRawat(),'instalasi_id','instalasi_nama'),array('empty'=>'-- Pilih --','onchange'=>'resetPencarianRuangan(); setKunjunganReset();refreshDialogKunjungan();','class'=>'span3','onkeyup'=>"return $(this).focusNextInputField(event)",)); 
            }
            ?>
        </div>
    </div>
                                ...
                                'source'=>'js: function(request, response) {
                                 ...
                                 'options'=>array(
                                       ...
                                       'select'=>'js:function( event, ui ) {
                                            ...
                                            setKunjungan(ui.item.pendaftaran_id, ui.item.no_pendaftaran, ui.item.no_rekam_medik, ui.item.pasienadmisi_id, ui.item.instalasi_id);
                                            return false;
                                        }',
                                ),
...
$this->beginWidget('zii.widgets.jui.CJuiDialog', array( // the dialog
    ...
    $is_umum = false;
    if (strtolower($this->id) == "pembayarantagihanpasien") {
        //$modDialogKunjungan->carabayar_id = Params::CARABAYAR_ID_MEMBAYAR;
        $modDialogKunjungan->is_carabayarpenjaminan = false;
        $is_umum = true;
    } else if (strtolower($this->id) == "pembayarantagihanpasienpenjamin") {
        $modDialogKunjungan->is_carabayarpenjaminan = true;
        $is_umum = false;
    }
    ...
    $arr_kunjungan = array(
        array(
            ...
            'value'=>function($data) use ($controller_id) {
                ...
                if ($controller_id == "alokasidana") {
                    return CHtml::Link('<i class="icon-form-check"></i>',"javascript:void(0);",array(
                        "class"=>"btn-small", 
                        "id" => "selectPendaftaran",
                        "onclick" => "
                            setKunjungan($data->pendaftaran_id, '', '', '', $data->instalasi_id);
                ...
                return CHtml::Link('<i class="icon-form-check"></i>',"javascript:void(0);",array(
                    ...
                    "id" => "selectPendaftaran",
                    "onclick" => "
                        setKunjungan($data->pendaftaran_id, '', '', '', $data->instalasi_id, $data->penjamin_id, '".$data->alokasidana_id."');
                        $('#dialogKunjungan').dialog('close');
                    "));
            },
        ),
    ...
    if (strtolower($this->id) == "alokasidana") {
        $prov = $modDialogKunjungan->searchDialogKunjunganAlokasiDana();
    } else {
        $arr_kunjungan = array_merge($arr_kunjungan, array(
            array(
                'header'=>'Tanggal Alokasi Biaya',
                'name'=>'tglalokasi',
                'type'=>'raw',
                'value'=>'MyFormatter::formatDateTimeForUser($data->tglalokasi)',
                'filter'=> false,
            ),
        ));
...
//                    'jeniskelamin',
        ...
        array(
            ...
            'filter'=>$is_umum ? false : CHtml::activeDropDownList($modDialogKunjungan, 'carabayar_id', CHtml::listData(
                    $filter_jenis_penjamin, 'carabayar_id', 'carabayar_nama'), array(
                        'empty'=>'-- Pilih --',
                    )),
    ...
    if($controller == 'pembayaranTagihanPasienPenjamin') {
        ...
        $crList->compare('carabayar_id', $modDialogKunjungan->carabayar_id);
        $crList->compare('penjamin_aktif', true);
        $crList->addCondition('carabayar_id != 1');
        $crList->order = 'penjamin_nama';
        $listPenjamin = PenjaminpasienM::model()->findAll($crList);
        ...
        $arr_kunjungan = array_merge($arr_kunjungan, array(
            array(
                ...
                'filter'=>CHtml::activeDropDownList($modDialogKunjungan, 'penjamin_id', CHtml::listData($listPenjamin, 'penjamin_id', 'penjamin_nama'), array(
                        'empty'=>'-- Pilih --',
                    )),
            )
        ));
