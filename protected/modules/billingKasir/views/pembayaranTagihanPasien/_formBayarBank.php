...
<script>
    ...
    function tambahBayar() {
        ...
        last_main.find(".main_nominal").maskMoney({ "symbol": "", "defaultZero": true, "allowZero": true, "decimal": ",", "thousands": ".", "precision": 0 });



        renameInput();
        jQuery(last_detail).find(".tgltransaksi").attr("id", "tgltransaksi_" + tgl_idx);
        jQuery(last_detail).find(".tgljatuhtempo").attr("id", "tgljatuhtempo_" + tgl_idx);
        jQuery(last_detail).find(".tgltransaksi2").attr("id", "tgltransaksi2_" + tgl_idx);
        jQuery(last_detail).find(".tgljatuhtempo2").attr("id", "tgljatuhtempo2_" + tgl_idx);
        ...
        $("#tgltransaksi_" + tgl_idx).datetimepicker(
          jQuery.extend(
            {
        ...
        $("#tgljatuhtempo_" + tgl_idx).datetimepicker(
            jQuery.extend(
            {
                showMonthAfterYear: false
            },
        ...
        $("#tgltransaksi2_" + tgl_idx).datetimepicker(
          jQuery.extend(
            {
                showMonthAfterYear: false
            },
        ...
        $("#tgljatuhtempo2_" + tgl_idx).datetimepicker(
            jQuery.extend(
            {
                showMonthAfterYear: false
            },
    ...
    function pilihJenis(obj) {
        ...
        if (ispiutangbank == 1) {
            ...
            $(detail).find(".panel_jenispembayaran_debit .tgljatuhtempo2").val(tgljatuhtempo);
        ...
        if (ispembayarandigital == 1) {
            ...
            $(detail).find(".panel_jenispembayaran_digital .tgljatuhtempo2").val(tgljatuhtempo);
        }
        ...
        switch (jenis) {
            ...
            case "DEBIT CARD":
                $(detail).find(".panel_jenispembayaran_debit").show();
                $(detail).find(".panel_jenispembayaran_debit :input").prop("disabled", false);
                // $(".no_mobile_pengirim").attr("placeholder", "No yang terhubung Akun Debit");
                // $(".email_pengirim").attr("placeholder", "Email yang terhubung Akun Debit");
                // remove attribute
                $("label[for=JenispembayaranT_no_mobile_pengirim]").find($("span[class=required]")).remove();
                $("label[for=JenispembayaranT_no_mobile_pengirim]").removeClass('error required').addClass('notrequired');
                $("label[for=JenispembayaranT_email_pengirim]").find($("span[class=required]")).remove();
                $("label[for=JenispembayaranT_email_pengirim]").removeClass('error required').addClass('notrequired');
                $(".no_mobile_pengirim").removeClass("required");
                $(".no_mobile_pengirim").removeClass('error').addClass('inputnotrequired');
                $(".email_pengirim").removeClass("required");
                $(".email_pengirim").removeClass('error').addClass('inputnotrequired');
                // hide
                $("label[for=JenispembayaranT_no_mobile_pengirim]").hide()
                $("label[for=JenispembayaranT_email_pengirim]").hide()
                $(".email_pengirim").hide();
                $(".no_mobile_pengirim").hide();
            ...
            case "CREDIT CARD":
                $(detail).find(".panel_jenispembayaran_debit").show();
                $(detail).find(".panel_jenispembayaran_debit :input").prop("disabled", false);
                $(".no_mobile_pengirim").attr("placeholder", "No yang terhubung Akun Credit");
                $(".email_pengirim").attr("placeholder", "Email yang terhubung Akun Credit");

                $(".email_pengirim").addClass("required");
                $("label[for=JenispembayaranT_email_pengirim]").append("<span class=required> *</span>");
                $(".email_pengirim").removeClass('error').addClass('inputnotrequired');

                $(".no_mobile_pengirim").addClass("required");
                $("label[for=JenispembayaranT_no_mobile_pengirim]").append("<span class=required> *</span>");
                $(".no_mobile_pengirim").removeClass('error').addClass('inputnotrequired');
                //show
                $("label[for=JenispembayaranT_no_mobile_pengirim]").show()
                $("label[for=JenispembayaranT_email_pengirim]").show()
                $(".email_pengirim").show();
                $(".no_mobile_pengirim").show();
                break;
        }
        */
    }
