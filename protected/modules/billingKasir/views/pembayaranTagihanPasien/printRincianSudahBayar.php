...
<p style="text-align: center;"><?php echo $modPembayaran->nopembayaran; ?></p>
<br/>
...
<table class="identitas" width="100%">
    ...
    <tr>
        <td>Penjamin</td>
        <td></td>
        <td>:<?php echo $modPendaftaran->penjamin_nama == NULL ? '-': $modPendaftaran->penjamin_nama; ?></td>
...
//        $res = (($val_pulang - $val_adms)/ (3600 * 24)) + 1;
    ...
    <tr hidden>
        <td hidden>Dokter</td><td>:</td><td nowrap><?php //echo empty($modPendaftaran->pasienadmisi_id) ? $modPendaftaran->pegawai->namaLengkap : $admisi->pegawai->namaLengkap; ?></td>
        <tdhidden>Lama Rawat</td><td>:</td><td nowrap><?php //echo $res." Hari (".$str.")"; ?></td>
    </tr>
    <?php else : ?>
    <tr hidden>
...
<table width="100%" class="tab_detail">
    ...
    <tbody>
        ...
        <tr>
            <td colspan="3"style="text-align: left;"> Terbilang: #<?php echo MyFormatter::kataTerbilang($subtotalkotor); ?> # </td>
            <td colspan="3" style="text-align: right;">Total </td>
            <td colspan="2" style="text-align: right;"><?php echo MyFormatter::formatNumberForPrint($subtotalkotor,0); ?></td>
        </tr>
        <!-- <tr>
            <td colspan="6" style="text-align: right;">Total Tangguhan Asuransi </td>
            <td colspan="2" style="text-align: right;"><?php //echo MyFormatter::formatNumberForPrint((!empty($modPembayaran->total_inacbg)? $modPembayaran->total_inacbg : $modPembayaran->totalsubsidiasuransi),0); ?></td>
        </tr> -->
        <tr>
            <td colspan="6" style="text-align: right;">Keringanan Akhir </td>
            <td colspan="2" style="text-align: right;"><?php echo MyFormatter::formatNumberForPrint($modPembayaran->totaldiscount,2); ?></td>
        </tr>
        <tr>
            <td colspan="5"></td>
            <td colspan="3" style="border-top:1px solid #333;"></td>
        ...
        <tr>
            ...
            <td colspan="2" style="text-align: right;"><?php echo MyFormatter::formatNumberForPrint($grand_totals); ?></td>
        </tr>


        <?php if ($tandabukti->biayaadministrasi != 0): ?>
            <tr class="closing footee">
                <td colspan="6">Administrasi</td><td colspan = '2',style="text-align: right;"><?php echo MyFormatter::formatNumberForPrint($tandabukti->biayaadministrasi,2); ?></td>
            </tr>
        <?php endif; ?>
        ...
        ?>
            ...
            <?php if ($modPembayaran->totalpembebasan != 0): ?>
                <tr class="closing footee">
                    <td colspan="9">Total Pembebasan</td><td style="text-align: right;"><?php echo MyFormatter::formatNumberForPrint($modPembayaran->totalpembebasan,2); ?></td>
                </tr>
            <?php endif; ?>
            ...
            <tr class="closing footee">
                <td colspan="9">Total Tanggungan Rumah Sakit</td><td style="text-align: right;"><?php echo MyFormatter::formatNumberForPrint($modPembayaran->totalsubsidirs,2); ?></td>
            </tr>
            <?php endif; ?>
...
?>
    <table width = 40%>
        <tr>
            <td>Dibayar Tunai: <?php echo MyFormatter::formatNumberForPrint($grand_totals - ($modPembayaran->total_inacbg + $modPembayaran->totalsubsidiasuransi + $subp + $subr) + $tandabukti->jmlpembulatan); ?></td>
            <td>Jaminan: <?php echo MyFormatter::formatNumberForPrint($subsidiasuransi_tindakan); ?> </td>
        </tr>
        <tr style="text-align: left; border-style: solid; height:50px;">
            <td>Jenis Penjamin  : <?php echo $modPendaftaran->carabayar->carabayar_nama; ?></td>
            <td><?php echo MyFormatter::formatNumberForPrint($grand_totals - ($modPembayaran->total_inacbg + $modPembayaran->totalsubsidiasuransi + $subp + $subr) + $tandabukti->jmlpembulatan); ?></td>
        </tr>
    </table>
    <br><br>
    <table width='100%'>
        ...
        <tr>
            <td> <?php echo $modPendaftaran->tgl_pendaftaran; ?></td>
            <td align='center' style ='text-align:right'>Kasir<?php echo Yii::app()->user->getState('nama_pegawai'); ?></td>
        </tr>
        <tr>
            <td><p>- INOVICE INI BERLAKU SEBAGAI KWITANSI</p></td>
        </tr>
    </table>
     </div>
</td>
