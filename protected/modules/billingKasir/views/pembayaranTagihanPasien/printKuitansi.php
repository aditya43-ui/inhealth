...
<table width="100%">
    ...
    $nobuktibayar = 'null';
    if(!empty($modTandaBukti->nobuktibayar)) {
        $nobuktibayar = substr($modTandaBukti->nobuktibayar, 0, 6);
    }
    ...
    if (isset($caraPrint)){ ?>
                            ...
                            <td align="center" valig="middle" colspan="3">
                                <table align="center" cellspacing=0 width="100%">
                                    <tbody>
                                        ...
                                        if(!empty($modAlokasi) && $modAlokasi->carabayar_id == Params::CARABAYAR_ID_MEMBAYAR) {
                                            $alokasiUmum = AlokasidanaT::model()->findByAttributes(array(
                                                'carabayar_id'=>Params::CARABAYAR_ID_BPJS,
                                                'pendaftaran_id'=>$modPendaftaran->pendaftaran_id,
                                            ), array(
                                                'condition'=>'pasienadmisi_id is not null and orderbatalpembayaranpelayanan_id is null and iurbea_id is not null'
                                            ));

                                            if (!empty($alokasiUmum)) {
                                                $bayar_td = 'IUR BEA KENAIKAN KELAS PERAWATAN';
                                            }
                                        } 

                                        if($bayar_td == '-' && !empty($modAlokasiDet)) {
                        ...
                        <td align="left"><?php echo $modTandaBukti->nobuktibayar;?></td>
                                        ...
                                        <tr>
                                            ...
                                            <td class="terbilang">
                                                <?php
                            $totalBayar = ($uangmuka->pemakaianuangmuka ?? 0) + $modTandaBukti->bank_nominal + $modTandaBukti->uangditerima - $modTandaBukti->uangkembalian;
                                if($totalBayar == 0)
                                {
                                    echo '-';
                                }else{
