...
<table width="100%">
    ...
    <tr>
		...
		<td class="col-isi"><?php echo (isset($modInfo->no_rekam_medik) ? $modInfo->no_rekam_medik : ""); ?></td>
		
		<td class="col-judul"><b>No. Pendaftaran</b></td>
		<td class="col-titik">:</td>
		<td class="col-isi"><?php echo (isset($modInfo->no_pendaftaran) ? $modInfo->no_pendaftaran : ""); ?></td>
    </tr>
	<tr>
        <td class="col-judul"><b>Nama Pasien</b></td>
		<td class="col-titik">:</td>
		<td class="col-isi"><?php echo (isset($modInfo->namadepan) ? $modInfo->namadepan : "")." ".(isset($modInfo->nama_pasien) ? $modInfo->nama_pasien : "");?></td>
		
		<td class="col-judul"><b>Tanggal Pendaftaran</b></td>
		<td class="col-titik">:</td>
		<td class="col-isi"><?php echo (isset($modInfo->tgl_pendaftaran) ? $format->formatDateTimeForUser($modInfo->tgl_pendaftaran) : ""); ?></td>
    </tr>
	<tr>
        <td class="col-judul"><b>Tanggal Lahir</b></td>
		<td class="col-titik">:</td>
		<td class="col-isi"><?php echo (isset($modInfo->tanggal_lahir) ? $format->formatDateTimeForUser($modInfo->tanggal_lahir) : ""); ?></td>
		
		<td class="col-judul"><b>Ruangan Terakhir</b></td>
		<td class="col-titik">:</td>
		<td class="col-isi"><?php echo (isset($modInfo->ruangan_nama) ? $modInfo->ruangan_nama : ""); ?></td>
    </tr>
	<tr>
        <td class="col-judul"><b>Jenis Kelamin</b></td>
		<td class="col-titik">:</td>
		<td class="col-isi"><?php echo (isset($modInfo->jeniskelamin) ? $modInfo->jeniskelamin : ""); ?></td>
		
		<td class="col-judul"><b>Penjamin</b></td>
		<td class="col-titik">:</td>
		<td class="col-isi"><?php echo (isset($modInfo->penjamin_nama) ? $modInfo->penjamin_nama : ""); ?></td>
    </tr>
</table>
<br>
<table width="100%">
...
//				$totalObatAlkes += $biaya_lain + $obatalkes->hargajual_oa;
				$totalObatAlkes += $biaya_lain + $hargajual_oa;
		?>
		<tr class="border-tr-td">
...
<?php
if (isset($_GET['frame'])){
    echo CHtml::link(Yii::t('mds', '{icon} Print Tagihan', array('{icon}'=>'<i class="icon-print icon-white"></i>')), 'javascript:void(0);', array('class'=>'btn btn-info','onclick'=>"print();"));
?>
    <script type='text/javascript'> 
    function print(){
//        window.open("<?php // echo Yii::app()->createUrl("billingKasir/PembayaranTagihanPasien/RincianTagihanPasien", array("pendaftaran_id"=>$_GET['pendaftaran_id'], "pasienadmisi_id"=>(isset($_GET['pasienadmisi_id']) ? $_GET['pasienadmisi_id'] : null))) ?>","",'location=_new, width=1024px');
		window.open("<?php echo Yii::app()->createUrl(Yii::app()->controller->module->id."/".Yii::app()->controller->id."/RincianTagihanPasien", array("pendaftaran_id"=>$_GET['pendaftaran_id'], "pasienadmisi_id"=>(isset($_GET['pasienadmisi_id']) ? $_GET['pasienadmisi_id'] : null))) ?>","",'location=_new, width=1024px');
    }
    </script>
<?php
}
