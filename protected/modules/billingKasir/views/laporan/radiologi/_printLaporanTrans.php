...
<?php 
    ...
    $sort = true;
    if (isset($caraPrint)){
        $data = $model->searchPrintRadiologi();
        $template = "{items}";
        $sort = false;
        if ($caraPrint == "EXCEL")
            $table = 'ext.bootstrap.widgets.BootExcelGridView';
    } else{
        $data = $model->searchTableRadiologi();
         $template = "{summary}\n{items}\n{pager}";
    }
?>
<?php
        ...
        array(
            ...
            'columns'=>array(
                ...
                array(
                    ...
                    'footer'=>number_format($model->getSumTarifTindakanRadiologi(),0,",","."),
                ),
                array(
                    ...
                    'value'=>'$data->no_pendaftaran',
                     'footerHtmlOptions'=>array('style'=>'text-align:right;'),
                    'footer'=>number_format($model->getSumTotalSubsidiRadiologi(),0,",","."),
                ),
                array(
                    'header'=>'Nama Pasien',
                    'type'=>'raw',
