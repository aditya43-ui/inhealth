            ...
            <td>
                <div class="content">
                    ...
                    <table class="identitas" width="100%">
                        ...
                        <tr>
                            <td>Penanggung</td>
                            <td>:<?php echo $modPenanggungjawab == NULL ? '-': $modPenanggungjawab->nama_pj;//echo $modPenanggungjawab == null ? '-':$modPenanggungjawab->nama_pj ; ?></td>
                            <td>No Polis</td>
                    ...
                    foreach ($modRincian as $item) {
                        ...
                        if (!$item->is_alkes && !empty($dt) && $dt->daftartindakan_akomodasi) {
                            array_unshift($modRincian2, $item);
                        } else {
                            $modRincian2[] = $item;
                        }
                    }
                    unset($modRincian);
                    foreach ($modRincian2 as $item) {
...
if (isset($_GET['frame'])){
    ...
    if(empty($loginpeg) || $this->id != 'pembayaranTagihanKarcis'){
        echo CHtml::link(Yii::t('mds', '{icon} INVOICE', array('{icon}'=>'<i class="icon-print icon-white"></i>')), 'javascript:void(0);', array('class'=>'btn btn-info','onclick'=>"print();"));
    }
?>
