...
// if (!isset($_GET['frame'])){
//     echo $this->renderPartial('billingKasir.views.pembayaranTagihanPasien._headerPrint');
// }
...
$nama_ruangan = $modPendaftaran->instalasi->instalasi_nama;

if (!empty($admisi)) {
    $instalasi = InstalasiM::model()->findByPk(Params::INSTALASI_ID_RI);
    $nama_ruangan = $instalasi->instalasi_nama;
}
...
<div style="text-align: center; font-weight: bold; font-size: 16pt">
  RINCIAN PENJUALAN OBAT <?php echo strtoupper($nama_ruangan); ?>
</div>
<br><br>
...
foreach ($modPenjualan as $item):
    $cr = new CDbCriteria();
    $cr->join = 'join obatalkes_m o on o.obatalkes_id = t.obatalkes_id';
    ...
    if (!empty($oa->oasudahbayar_id)) continue;
    ...
    <td width="50%">
      <table style="width: 100%; border: none;">
        <tr>
          ...
          <td width="5px">:</td>
          <td>
            <?php echo (isset($modPasienAdmisi) ? (isset($modPasienAdmisi->ruangan)?$modPasienAdmisi->ruangan->ruangan_nama:"") :(isset($modPendaftaran->ruangan)? $modPendaftaran->ruangan->ruangan_nama : "")); ?>
          </td>
        </tr>
        <tr>
          ...
          <td>:</td>
          <td>
            <?php echo (isset($modPasienAdmisi) ? (isset($modPasienAdmisi->kelaspelayanan)?$modPasienAdmisi->kelaspelayanan->kelaspelayanan_nama:"") :(isset($modPendaftaran->kelaspelayanan)? $modPendaftaran->kelaspelayanan->kelaspelayanan_nama : "")); ?>
          </td>
        </tr>
        <tr>
          ...
          <td>:</td>
          <td>
            Rp <?php echo MyFormatter::formatNumberForPrint($gtotal,2); ?>
          </td>
        </tr>
      </table>
    </td>
...
<table class="tab_detail">
    ...
    <tfoot>
        <tr>
            ...
            <td class="num"><?php echo MyFormatter::formatNumberForPrint($total, 2); ?></td>
        </tr>
        <?php if(!empty($item->jasapelayanan_farmasi)){ ?>
        <tr>
            <td colspan="6" style="text-align: right">Jasa Pelayanan Farmasi</td>
            <td class="num"><?php echo MyFormatter::formatNumberForPrint($item->jasapelayanan_farmasi, 2); ?></td>
        </tr>
        <?php } ?>
        <tr>
            <td colspan="6" style="text-align: right">Total Keseluruhan</td>
            <td class="num"><?php echo MyFormatter::formatNumberForPrint(($total + $item->jasapelayanan_farmasi), 2); ?></td>
        </tr>
    </tfoot>
</table>
<br>
...
<?php foreach ($modRetur as $item):
    ...
    // $cr->addCondition('oa.oasudahbayar_id is null and oa.qty_oa');
    // $cr->compare('sb.pembayaranpelayanan_id', $modPembayaran->pembayaranpelayanan_id);
    $cr->order = 'o.obatalkes_nama';
...
?>
     ...
     */
    function print(){
        window.open("<?php echo Yii::app()->createUrl("billingKasir/RincianTagihanFarmasi/RincianBiayaFarmasi", array("id"=>$_GET['id'], "caraPrint"=>"PRINT")) ?>","",'location=_new, width=1024px');
    }
    </script>
<?php
}else{
