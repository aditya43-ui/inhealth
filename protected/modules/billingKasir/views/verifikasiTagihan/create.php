...
<div class="panel panel-gradient">
    <div class="panel-heading">
        <div class="panel-title">
            <i class="glyphicon glyphicon-briefcase"></i> Transaksi <b>Verifikasi Tagihan</b>
    ...
    <div class="panel-body">
        ...
        <div class="panel panel-success">
            ...
            <div class="panel-body">
                <div class="search-form">
                    ...
                    <?php
                    $this->renderPartial(
                        'billingKasir.views.verifikasiTagihan._ringkasDataPasien',
                        array(
                        ...
                        <table id="tblBayarTind" class="table table-bordered table-striped table-condensed">
                            <thead>
                                <tr>
                                    ...
                                    <th width="100" hidden>Komponen Tarif</th>
                                    <th width="100" hidden>Verifikasi Tarif</th>
                                    <th width="100" hidden>Tarif Cyto</th>
                                    <th width="100" hidden>Keringanan (Rp)</th>
                                    <!-- <th width="100">Tanggungan Asuransi</th> -->
                                    <!-- <th width="100">Tanggungan Rumah Sakit</th> -->
                                    <th width="100" hidden>Tanggungan Pasien</th>
        ...
        <div class="panel panel-success">
            ...
            <div class="panel-body">
                    ...
                    <table id="tblBayarOA" class="table table-bordered table-striped table-condensed">
                        <thead>
                            <tr>
                                ...
                                <th width="100">Jumlah</th>
                                <th width="100">Nominal Tarif (Rp)</th>
                                <th width="100" hidden>Adm/Serv/Kons</th>
                                <th width="100" hidden>Tanggungan Asuransi</th>
                                <!--<th width="100">Tanggungan Pemerintah</th>-->
                                <!-- <th width="100">Tanggungan Rumah Sakit</th> -->
                                <!-- <th width="100">Tanggungan Pasien</th> -->
                                <th width="100">Sub Total</th>
                                <!--<th width="20" style="text-align:center;">&nbsp;-->
                                <!--
                ...
                <?php echo CHtml::htmlButton(
                    Yii::t('mds', 'Simpan', array('{icon}' => '<i class="entypo-search"></i>')),
                    array('title' => 'Simpan', 'class' => 'btn btn-primary', 'type' => 'button', 'onclick' => 'simpanCeklisVerifikasiOA()')
                ); ?>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function hitungTotalSemuaTind() {
        ...
        console.log($('#tblBayarTind').find('input[name$="[qty_tindakan]"]'));
        $('#tblBayarTind').find('input[name$="[qty_tindakan]"]').each(function() {
            console.log('tes hitung semua tind');
            ...
            if ($(this).parents('tr').find('.pilih-cek').is(':checked')) {

                console.log('verif tercek');
...
//                myAlert("Uang diterima tidak boleh kurang dari jumlah pembayaran");
    ...
    */
    ...
    function simpanSemuaCeklisVerifikasi() {
       ...
       if(cek > 0 && ok) {
           myAlert('Data Telah Diverifikasi!');
       }
    ...
    function simpanCeklisVerifikasi(obj) {
        ...
        $.post('<?php echo $this->createUrl('simpanCeklisVerifikasiTindakan')?>', $(obj).parents("tr").find(":input").serialize(), function(data) {
            if (data.ok == 1) {
    ...
    function simpanCeklisVerifikasiOA() {
        $("#daftar_alkes").addClass('animation-loading');

        $.post('<?php echo $this->createUrl('simpanCeklisVerifikasiOA')?>', $("#daftar_alkes :input").serialize(), function(data) {
            if (data.ok == 1) {
                toastr.success(data.msg);
                $("#daftar_alkes table tbody tr").each(function() {
                    if ($(this).find(".ceklis_oa").is(":checked")) {
                        $(this).detach();
                    }
                });
            } else {
            ...
            $("#daftar_alkes").removeClass('animation-loading');

            renameInputRow('#tblBayarOA');

            var tot = 0;

            $('.subtotal_oa').each(function () {

                tot += unformatNumber($(this).val());
            });

            $('#totalbayar_oa').val(formatNumber(tot));
