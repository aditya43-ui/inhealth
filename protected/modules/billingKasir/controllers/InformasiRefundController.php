...
class InformasiRefundController extends MyAuthController
{
  public function actionIndex()
  {
    $this->pageTitle = Yii::app()->name . " - Refund Tindakan";
    $format = new MyFormatter();
    $model = new BKInformasirefundV('searchInformasiRetur');
    $model->unsetAttributes();
    $model->tgl_awal = date('d M Y');
    $model->tgl_akhir = date('d M Y');
    // $model->belumbayar = "false";
    if (isset($_GET['BKInformasirefundV'])) {
      $format = new MyFormatter();
      $model->attributes = $_GET['BKInformasirefundV'];
      $model->tgl_awal = $format->formatDateTimeForDb($_GET['BKInformasirefundV']['tgl_awal']);
      $model->tgl_akhir = $format->formatDateTimeForDb($_GET['BKInformasirefundV']['tgl_akhir']);
    }
  ...
  {
    $model = PembebasantarifT::model()->findByPk($id);
    // var_dump($model);die;
    if ($model === null)
