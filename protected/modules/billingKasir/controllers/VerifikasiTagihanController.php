...
class VerifikasiTagihanController extends MyAuthController
{
  ...
  public $path_view = 'billingKasir.views.verifikasiTagihan.';
...
//                            echo ($persen_diskonOA);exit;
    ...
    if (isset($_POST['pembayaran'])) {
      ...
      try {
        foreach ($_POST['pembayaran'] as $detail) {
          ...
          $crVerifikasi->addCondition('tglverifikasi::date = current_date');
          ...
          // var_dump($verifikasi->attributes);

          if ((isset($detail['checked']) && $detail['checked'] == 1)) {

            if (empty($verifikasi_rencana)) {
              $verifikasi_rencana = new VerifrenctindakanT;
  
              $verifikasi_rencana->tglverifikasirenc = date('Y-m-d H:i:s');
              $verifikasi_rencana->noverifikasi_renc = MyGenerator::noVerifikasiTindakan();
              $verifikasi_rencana->petugas_verif_id = Yii::app()->user->getState('pegawai_id');
              $verifikasi_rencana->mengetahui_id = Yii::app()->user->getState('pegawai_id');
  
              $verifikasi_rencana->create_time = date('Y-m-d H:i:s');
              $verifikasi_rencana->create_loginpemakai_id = Yii::app()->user->id;
              $verifikasi_rencana->create_ruangan = Yii::app()->user->getState('ruangan_id');

              $ok = $ok && $verifikasi_rencana->save();
            }

            if ($tindakan->verifikasitagihan_id != $verifikasi->verifikasitagihan_id) {
              $tindakan->verifikasitagihan_id = $verifikasi->verifikasitagihan_id;
              $tindakan->verifrenctindakan_id = $verifikasi_rencana->verifrenctindakan_id;
              $tindakan->isverifbataltindakan = false;
              $ok = $ok && $tindakan->save(false, array('verifikasitagihan_id', 'verifrenctindakan_id'));
            }
          }
        ...
        if ($ok) {
          ...
          echo CJSON::encode(array(
            'ok'=>1,
            'msg'=>'Tindakan berhasil diverifikasi.',
          ));
          ...
          echo CJSON::encode(array(
            'ok'=>0,
            'msg'=>'Tindakan gagal diverifikasi.',
          ));
        }
      ...
      try {
        ...
        foreach ($_POST['pembayaranAlkes'] as $detail) {
          $oa = ObatalkespasienT::model()->findByAttributes(array(
            'obatalkespasien_id'=>$detail['obatalkespasien_id'],
          ), array(
            'condition'=>'oasudahbayar_id is null'
          ));
          ...
          // foreach ($oas as $oa_det) {
            if ((isset($detail['checked']) && $detail['checked'] == 1)) {
              if ($oa_det->verifikasitagihan_id != $verifikasi->verifikasitagihan_id) {
                $oa_det->verifikasitagihan_id = $verifikasi->verifikasitagihan_id;
                $ok = $ok && $oa_det->save(false, array('verifikasitagihan_id'));
              }

              if (!empty($oa_det->penjualanresep_id)) {
                $tindakan = TindakanpelayananT::model()->findByAttributes(array(
                  'penjualanresep_id'=>$oa_det->penjualanresep_id
                ));
            ...
            // set verifikasi pada penjualan resep
            //$penjualan = PenjualanresepT::model()->findByPk($oa_det->penjualanresep_id);
  ...
  public function actionInformasi($linkHalaman =null)
  {
    ...
    $model = new InfoverifikasitagihanV;
    $model->tgl_awal = date('d M Y', strtotime('-7 days'));
    $model->tgl_akhir = date('d M Y');
    if (isset($_GET['InfoverifikasitagihanV'])) {
      $model->attributes = $_GET['InfoverifikasitagihanV'];

      $model->tgl_awal = MyFormatter::formatDateTimeForDB($_GET['InfoverifikasitagihanV']['tgl_awal']);
      $model->tgl_akhir = MyFormatter::formatDateTimeForDB($_GET['InfoverifikasitagihanV']['tgl_akhir']);
    }
  ...
  public function getJsonKunjungan($data)
  {
    ...
    if (!empty($pendaftaran->penanggungjawab_id)) {
  ...
  function actionCekValidasiPJADanAkomodasi() {
    $pendaftaran_id = $_POST['pendaftaran_id'];
    $data['statusPJA'] = 1;
    $data['statusAkomodasi'] = 1;

    $pendaftaran = PendaftaranT::model()->findByPk($pendaftaran_id);

    // var_dump($pendaftaran_id, $pendaftaran->instalasi_id, $pendaftaran->pasienadmisi_id);

    $is_ada = !empty($pendaftaran_id);
    if (empty($pendaftaran->pasienadmisi_id)) {
      if ($pendaftaran->instalasi_id != Params::INSTALASI_ID_RD) {
        $is_ada = false;
      }
    ...
    if($is_ada) {
      $tindakan = TindakanpelayananT::model()->findByAttributes(array(
        ...
        'condition'=>"(isapprovaltindaklanjut = false or isapprovaltindaklanjut is null) and masukkamar_id is null"
      ));
      ...
      // cek akomodasi
      $criteria = new CDbCriteria();
      $criteria->join = 'JOIN daftartindakan_m dm ON dm.daftartindakan_id = t.daftartindakan_id';
      $criteria->addCondition('dm.daftartindakan_akomodasi is true and nopelayanan is not null');
      $criteria->addCondition('pendaftaran_id=' . $pendaftaran_id);
      $tindakanAkomodasi = TindakanpelayananT::model()->find($criteria);
      if(empty($tindakanAkomodasi)) {
        // verifikasi akomodasi belum dilakukan
        $data['statusAkomodasi'] = 0;
      }
    }
