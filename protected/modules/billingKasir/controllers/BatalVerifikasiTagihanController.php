...
class BatalVerifikasiTagihanController extends MyAuthController
{
  public $layout = '//layouts/column1';
  public $path_view = 'billingKasir.views.batalVerifikasiTagihan.';
...
//                            echo ($persen_diskonOA);exit;
  ...
  public function actionSimpanCeklisVerifikasiTindakan() {
    ...
    if (isset($_POST['pembayaran'])) {
      ...
      try {
        foreach ($_POST['pembayaran'] as $detail) {
          ...
          $verdetail = VerifikasitagihantindakanR::model()->findByAttributes(array(
            'tindakanpelayanan_id'=>$detail['tindakanpelayanan_id']
          ));
          $pendaftaran = PendaftaranT::model()->findByPk($tindakan->pendaftaran_id);

          if (empty($verdetail)) {
            $verdetail = new VerifikasitagihantindakanR;
            $verdetail->attributes = $detail;
          }

          if (!$verdetail->isNewRecord) {
            $verifikasi = VerifikasitagihanT::model()->findByPk($verdetail->verifikasitagihan_id);
          }

          if (empty($verifikasi)) {
            $verifikasi = VerifikasitagihanT::model()->findByPk($tindakan->verifikasitagihan_id);
          }

          if (empty($verifikasi)) {
            $crVerifikasi = new CDbCriteria;
            $crVerifikasi->compare('pendaftaran_id', $tindakan->pendaftaran_id);
            
            if ($detail['instalasi_id'] == Params::INSTALASI_ID_RI) {
              $crVerifikasi->addCondition('pasienadmisi_id is not null');
            } else {
              $crVerifikasi->addCondition('pasienadmisi_id is null');
            }
  
            $verifikasi = VerifikasitagihanT::model()->find($crVerifikasi);
          }
          ...
          if (!empty($tindakan->verifrenctindakan_id)) {
            $verifikasi_rencana = VerifrenctindakanT::model()->findByPk($tindakan->verifrenctindakan_id);
          }
          ...
          if ($verifikasi->isNewRecord) {
            ...
            /*
            if ($detail['instalasi_id'] == Params::INSTALASI_ID_RI) {
              $verifikasi->pasienadmisi_id = $pendaftaran->pasienadmisi_id;
            } else {
              $verifikasi->pasienadmisi_id = null;
            }
            */
          }
          ...
          if ((isset($detail['checked']) && $detail['checked'] == 1)) {
            ...
            if (!empty($tindakan->verifikasitagihan_id)) {
              // $tindakan->verifikasitagihan_id = null;
              // $tindakan->save(false, array('verifikasitagihan_id', 'verifrenctindakan_id'));
            }

            // var_dump($tindakan->verifrenctindakan_id); die;
            
            if (!empty($tindakan->verifrenctindakan_id)) {

              if (empty($verifikasi_rencana)) {
                $verifikasi_rencana = VerifrenctindakanT::model()->findByPk($tindakan->verifrenctindakan_id);
              
              }
              $verifikasi_rencana->petugasbatal_id = Yii::app()->user->getState('pegawai_id');
              $verifikasi_rencana->bataltindakanpelayanan_id = $tindakan->tindakanpelayanan_id;
              $verifikasi_rencana->tglbataltindakanpelayanan = date('Y-m-d H:i:s');
              $verifikasi_rencana->save(false, array('petugasbatal_id', 'bataltindakanpelayanan_id', 'tglbataltindakanpelayanan'));


              $tindakan->update_time = date('Y-m-d H:i:s');
              $tindakan->verifrenctindakan_id = null;
              $tindakan->isverifbataltindakan = true;


              // $ok = $ok && VerifrenctindakanT::model()->deleteByPk($verifikasi_rencana->verifrenctindakan_id);
            }


            $tindakan->isverifbataltindakan = true;
            $tindakan->update_time =date('Y-m-d H:i:s');
            $tindakan->update_loginpemakai_id = Yii::app()->user->getState('pegawai_id');

            $tindakan->save(false, array('verifrenctindakan_id', 'isverifbataltindakan', 'update_time', 'update_loginpemakai_id'));

            if (!empty($tindakan->penjualanresep_id)) {
              $oa = ObatalkespasienT::model()->findAllByAttributes(array('penjualanresep_id'=>$tindakan->penjualanresep_id));

              foreach ($oa as $oa_det) {
                if (!empty($oa_det->verifikasitagihan_id)) {
                  // $oa_det->verifikasitagihan_id = null;
                  // $ok = $ok && $oa_det->save(false, array('verifikasitagihan_id'));
                  
                  // var_dump($oa_det->attributes);
                }
              }
            }
          ...
          // var_dump($verifikasi->save(), $verifikasi->errors, $verifikasi->attributes);

          $verdetail->verifikasitagihan_id = $verifikasi->verifikasitagihan_id;
          $verdetail->qty_tindakan_sebelum = $verdetail->qty_tindakan_sesudah = MyFormatter::formatRupiahForDB($detail['qty_tindakan']);
          $verdetail->tarif_satuan_sebelum = $verdetail->tarif_satuan_sesudah = MyFormatter::formatRupiahForDB($detail['tarif_satuan']);
          $verdetail->tarif_tindakan_sebelum = $verdetail->tarif_tindakan_sesudah = MyFormatter::formatRupiahForDB($detail['tarif_tindakan']);
          $verdetail->discount_tindakan_sebelum = $verdetail->discount_tindakan_sesudah = MyFormatter::formatRupiahForDB($detail['discount_tindakan']);  
          $verdetail->subsidiasuransi_tindakan_sebelum = $verdetail->subsidiasuransi_tindakan_sesudah = MyFormatter::formatRupiahForDB($detail['subsidiasuransi_tindakan']);  
          $verdetail->subsisidirumahsakit_tindakan_sebelum = $verdetail->subsisidirumahsakit_tindakan_sesudah = MyFormatter::formatRupiahForDB($detail['subsisidirumahsakit_tindakan']);  
          $verdetail->is_verifikasi = !(isset($detail['checked']) && $detail['checked'] == 1);
          $ok = $ok && $verdetail->save();


          // var_dump($verdetail->attributes, $verdetail->errors);

          // var_dump($ok, $verdetail->attributes, $detail); die;
        ...
        if ($ok) {
          ...
          echo CJSON::encode(array(
            'ok'=>1,
            'msg'=>'Verifiasi Tindakan berhasil dibatalkan.',
          ));
          ...
          echo CJSON::encode(array(
            'ok'=>0,
            'msg'=>'Verifiasi Tindakan gagal dibatalkan.',
          ));
        }
      ...
      try {
        ...
        foreach ($_POST['pembayaranAlkes'] as $detail) {
          $oas = ObatalkespasienT::model()->findAllByAttributes(array(
            ...
            'condition'=>'oasudahbayar_id is null and verifikasitagihan_id is not null'
          ));
          ...
          //$crVerifikasi = new CDbCriteria;
          //$crVerifikasi->compare('pendaftaran_id', $oa->pendaftaran_id);
          //$crVerifikasi->compare('verifikasitagihan_id')
          ...
          */
          ...
          $verifikasi = VerifikasitagihanT::model()->findByPk($oa->verifikasitagihan_id);
  ...
  public function actionDetail($pendaftaran_id, $nopelayanan = null)
  {
    $this->layout = '//layouts/iframe';

    $view = InfobatalverifrenctindakanV::model()->findAll("pendaftaran_id = $pendaftaran_id");
    $pendaftaran = PendaftaranT::model()->findByPk($pendaftaran_id);

    // echo '<pre>'; var_dump($pendaftaran->attributes); die;
    $pasien = $pendaftaran->pasien;
    $tindakan_ids = [];

    foreach($view as $v) {
      array_push($tindakan_ids, $v->tindakanpelayanan_id);
    }

    $ct = new CDbCriteria;
    $ct->addInCondition("tindakanpelayanan_id", $tindakan_ids);
   ...
   */
  public function actionUbahVerifikasi()
  {
    if (Yii::app()->request->isAjaxRequest) {
      
      $transaction = Yii::app()->db->beginTransaction();
      try {
        $tindakans = TindakanpelayananT::model()->findAll("isverifbataltindakan = true and pendaftaran_id = " . $_POST['pendaftaran_id']);

        $ok = true;

        if(!empty($tindakans)) {
          foreach($tindakans as $tin) {
            $tin->isverifbataltindakan = false;
            $tin->verifrenctindakan_id = null;
            $tin->verifikasitagihan_id = null;
            $tin->update_time = date('Y-m-d H:i:s');
            $tin->update_loginpemakai_id = Yii::app()->user->id;
            $ok &= $tin->save();

            if (!empty($tin->penjualanresep_id)) {
              $oa = ObatalkespasienT::model()->findAllByAttributes(array(
                'penjualanresep_id'=>$tin->penjualanresep_id
              ), array(
                'condition'=>'verifikasitagihan_id is not null',
              ));

              foreach ($oa as $item) {
                // $oa->isverifbataltindakan = false;
                // $oa->verifrenctindakan_id = null;
                $item->verifikasitagihan_id = null;

                $ok &= $item->save(false, array('verifikasitagihan_id'));
        ...
        if ($ok) {
          $transaction->commit();
          $data['pesan'] = "Data verifikasi berhasil diubah!";
          $data['sukses'] = 1;
        } else {
          $transaction->rollback();
          $data['pesan'] = "Data verifikasi gagal diubah1!";
          $data['sukses'] = 0;
        }
      } catch (Exception $exc) {
        $transaction->rollback();
        echo '<pre>'; var_dump($exc); die;
        $data['pesan'] = "Data verifikasi gagal diubah2! :" . MyExceptionMessage::getMessage($exc, true);
      }
