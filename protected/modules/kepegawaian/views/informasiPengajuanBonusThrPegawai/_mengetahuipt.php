...
<div class="row">
	...
	<div class="col-sm-4" style="text-align:center;">
			...
			if(isset($_GET['sukses'])){
				...
				if($model->mengetahui_pt == Yii::app()->user->getState('pegawai_id')){
                                    echo CHtml::link(Yii::t('mds',' Mengetahui (PT)'),
                                    $this->createUrl($this->id.'/index'),
                                    array('class' => 'btn btn-danger',
                                            'onclick'=>'myConfirm("Apakah Anda yakin?","Perhatian!",
                                            function(r) {if(r) window.location = "'.$this->createUrl('ApproveMengetahuiPT',array('pengbonusthr_id'=>$model->pengbonusthr_id,'approve'=>true)).'";} ); return false;'));
                                }
                                else{
                                    echo CHtml::link(Yii::t('mds',' Mengetahui (PT)'),
                                    $this->createUrl($this->id.'/index'),
                                    array('class' => 'btn btn-danger',
                                            'onclick'=>'myAlert("Maaf, Anda tidak berhak Mengapprove Pegawai Mengetahui PT ini."); return false;'));
                                }
			}
			?>
		</div>
...
<?php
    ...
    $urlPrint= $this->createUrl('printApproveMengetahuiPT',array('pengbonusthr_id'=>$model->pengbonusthr_id));
