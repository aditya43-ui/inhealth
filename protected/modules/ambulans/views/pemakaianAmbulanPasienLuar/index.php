...
/**
 * view utama untuk menampilkan form pemakaian ambulans pasien luar
 * 
...
$this->breadcrumbs = array(
    'Pemesanan Ambulans' => Yii::app()->request->getUrlReferrer(),
    'Pemakaian Ambulans Pasien Luar',
);
...
<div class="panel panel-gradient">
    <div class="panel-heading">
        <div class="panel-title">
            <i class="fas fa-ambulance"></i> Pemakaian Ambulans <b>Pasien Luar</b>
        </div>
    </div>
    <div class="panel-body">
        ...
        <?php echo $form->hiddenField($modPemakaian, 'is_pasien', array('readonly' => true, 'class' => 'span3', 'onkeyup' => "return $(this).focusNextInputField(event)")); ?>
        <?php
        $this->Widget('ext.bootstrap.widgets.BootAccordion', array(
            ...
            'content' => array(
                'content-pasien' => array(
                    ...
                    'isi' => $this->renderPartial('_formDataPasien', array(
                        'form' => $form,
                        'modPemakaian' => $modPemakaian,
                        'modPasien' => $modPasien,
                    ), true),
                    'active' => $modPemakaian->is_pasien,
                ),
            ),
        ));
        ?>
        ...
        <div class="panel panel-success">
            <div class="panel-heading">
                <div class="panel-title">
                    ...
                    echo '<i class="fas fa-money-bill"></i> Tarif Ambulans Berdasarkan Kota Tujuan ' . CHtml::htmlButton('<i class="entypo-search"></i>', array(
                        'class' => 'btn btn-default', 'onclick' => "refreshTarif();$('#dialogTarif').dialog('open');",
            ...
            <div class="panel-body table-responsive">
                <table id="tblTarifAmbulans" class="table table-bordered">
                    ...
                    <tbody>
                        <?php
                        for ($i = 0; $i < count((array)$tarif['tarifAmbulans']); $i++) : ?>
                            <?php if (!empty($tarif['tarifAmbulans'][$i])) { ?>
                                ...
                                <tr>
                                    ...
                                    <td hidden><label><?php echo $tarif['layanan'][$i]; ?></label><input type="hidden" value="<?php echo $tarif['layanan'][$i]; ?>" name="tarif[kelurahan][]" class="span2" /></td>
                                    <td><label><?php echo $tarif['tindakan'][$i]; ?></label><input type="hidden" value="<?php echo $tarif['tindakan'][$i]; ?>" name="tarif[kelurahan][]" class="span2" /></td>
                                    <td class="ubah"><input type="text" value="<?php echo $tarif['jmlKM'][$i]; ?>" onblur="hitungTarif(this);" name="tarif[jmlKM][]" class="span1 number" />
                                        <input type="hidden" value="<?php echo $tarif['daftartindakanId'][$i]; ?>" name="tarif[daftartindakanId][]" class="span1 number" />
                                    </td>
                                    <td class="ubah"><input type="text" value="<?php echo $tarif['tarifKM'][$i]; ?>" onblur="hitungTarif(this);" name="tarif[tarifKM][]" class="span1 integer" /></td>
                                    <td class="ubah"><input type="text" value="<?php echo empty($tarif['biayatol'][$i]) ? null : $tarif['biayatol'][$i]; ?>" onblur="hitungTarif(this);" name="tarif[biayatol][]" class="span1 integer" /></td>
                ...
                <?php $this->Widget('ext.bootstrap.widgets.BootAccordion', array(
                    ...
                    'content' => array(
                        'content-riwayatpasien' => array(
                            ...
                            'isi' => $this->renderPartial($this->path_view . '_daftarTarifAPI', array('modPemakaian' => $modPemakaian, 'form' => $form), true),
                            'active' => false,
                        ),
                    ),
        ...
        <div class="form-actions">
            ...
            if (!isset($_GET['sukses'])) {
                echo CHtml::htmlButton(
                    ...
                    array('title' => 'Simpan', 'class' => (isset($_GET['sukses'])) ? 'btn btn-danger' : 'btn btn-danger submit', 'type' => 'button', 'onclick' => 'cekTarif();', 'onKeypress' => 'return formSubmit(this,event)', 'disabled' => (isset($_GET['sukses'])) ? true : false)
                );
            } else {
                if (!isset($_GET['det'])) {
                    echo CHtml::htmlButton(
...
$this->renderPartial($this->path_view . '_daftarDokPendamping', array('modPemakaian' => $modPemakaian));
...
<script>
    function cekTarif() {
        ...
        $("#tblTarifAmbulansAPI > tbody > tr").each(function() {
            kosong++;
        });

        if (kosong == 0) {
            myAlert("Silakan pilih dahulu tarif Ambulans");
        } else {
            unformatNumberSemua();
