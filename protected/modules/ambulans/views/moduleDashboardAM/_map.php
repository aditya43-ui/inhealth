<div class="row tile-block">
	<div class="col-sm-3" style="padding:0">
		<div class="tile-header">
			<h3 style="color:#fff">Map</h3> <span>Penyebaran Pasien Bulan ini</span> 
		</div>
		<div class="scrollable" data-height="400" data-scroll-position="right" data-rail-color="#fff" data-rail-opacity=".9" data-rail-width="8" data-rail-radius="10" data-autohide="0">
			  <?php foreach ($dataMap as $i => $map) { ?>
				<div class="tile-entry list-map" style="cursor:pointer;background:rgba(0, 0, 0, 0.15); padding:10px 20px 10px 10px; margin:1px 0" onclick="panTo(<?php echo !empty($map['latitude'])?$map['latitude']:"'tidakada'"; ?>, <?php echo !empty($map['longitude'])?$map['longitude']:"'tidakada'"; ?>, '<?php echo $map['kelurahan_nama'] ?>')"> 
				   <span style="display:inline-block;color:#fff;font-size:11px;"><?php echo $map['kelurahan_nama'] ?></span> <span class="pull-right" style="display:inline-block;color:#fff;font-size:11px;"><?php echo $map['jumlah'] ?></span> 
			   </div> 
			<?php } ?>
		</div>
	</div>
