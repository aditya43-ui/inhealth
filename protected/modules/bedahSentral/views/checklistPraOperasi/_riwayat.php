    ...
    $this->widget('ext.bootstrap.widgets.BootGridView', array(
        ...
        'columns' => array(
            ...
            array(
                ...
                'type'=>'raw',
                'value'=>function($data) {
                    $html = MyFormatter::formatDateTimeForUser($data->tanggal_penginputan).' '.
                    CHtml::link(
                            '<icon class="icon-form-detail"></icon>', Yii::app()->createUrl("/bedahSentral/ChecklistPraOperasi/detail", array("prepostoperasi_id"=>$data->prepostoperasi_id,'typeruangan'=>'asal', 'type'=>$_GET['type'], 'frame'=>$_GET['frame'])),
                            array(
            ...
            array(
                ...
                'type'=>'raw',
                'value'=>function($data) {
                  $findCheck = PrepostoperasiT::model()->findByAttributes(array('pendaftaran_id' => $data->pendaftaran_id,'ruangantujuan_id' => Yii::app()->user->getState("ruangan_id"),'isterima'=>false));
                  $html = "";
                    if($data->isterima == false){
                      $html .= "Belum melakukan pengisian ".(!empty($findCheck) ?
                                CHtml::link('<i class="icon-form-check"></i>', $this->createUrl("ChecklistPraOperasi/index", array(
                                    'pendaftaran_id' => $data->pendaftaran_id,
                      ...
                      CHtml::link(
                              '<icon class="icon-form-detail"></icon>', Yii::app()->createUrl("/bedahSentral/ChecklistPraOperasi/detail", array("prepostoperasi_id"=>$data->prepostoperasi_id,'typeruangan'=>'tujuan', 'type'=>$_GET['type'], 'frame'=>$_GET['frame'])),
                              array(
