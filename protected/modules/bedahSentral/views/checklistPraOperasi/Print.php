              ...
              <?php
                  ...
                  if(count($modPrepostOperasi) > 0){
                    ...
                    foreach($modPrepostOperasi as $dataParentOp){
                      ...
                      $modPrepostOperasiChild = PrepostoperasideskM::model()->findAllByAttributes(array('status'=>true,'parent_id'=>$dataParentOp->prepostoperasidesk_id,'jenischecklist'=>'Pre Operasi'),array('order'=>'urutan ASC'));
                      if(count($modPrepostOperasiChild)>0){
                        foreach($modPrepostOperasiChild as $j => $dataChild){
                          $rowspan2 = 0;
                          $colspan2 = 2;
                          
                          $modPrepostOperasiChildStep3 = PrepostoperasideskM::model()->findAllByAttributes(array('status'=>true,'parent_id'=>$dataChild->prepostoperasidesk_id,'jenischecklist'=>'Pre Operasi'),array('order'=>'urutan ASC'));
                          ...
                          foreach($modPrepostOperasiChild as $j => $dataChild){
                            ...
                            if (count($modPrepostOperasiChildStep3) > 0){
                              ...
                              foreach($modPrepostOperasiChildStep3 as $k => $dataChildStep3){
                                if($dataChildStep3->parent_id == $dataChild->prepostoperasidesk_id){
                                  $parenChild = $dataChild->parent_id;
                                }
                              }
                            }
                            if(!empty($parenChild)){
                              if($parenChild == $dataParentOp->prepostoperasidesk_id){
                                $rowspanparent = (count($modPrepostOperasiChildStep3)-1);
                              }
                            }else{
                              $colspanparent[$dataChild->prepostoperasidesk_id] = 2;
                            }
                          }
                      }else{
                        $colspanparent[$dataParentOp->prepostoperasidesk_id] = 3;
                      }
                      ?>
                      <tr>
                        <td valign="top" rowspan="<?php echo (((count($modPrepostOperasiChild) + $rowspanparent)>0)?(count($modPrepostOperasiChild) + $rowspanparent):""); ?>"><?php echo $noUrut; ?></td>
                        <td colspan="<?php echo ((isset($colspanparent[$dataParentOp->prepostoperasidesk_id]) && !empty($colspanparent[$dataParentOp->prepostoperasidesk_id]))? $colspanparent[$dataParentOp->prepostoperasidesk_id] : "" ); ?>"  valign="top" rowspan="<?php echo (((count($modPrepostOperasiChild) + $rowspanparent)>0)?(count($modPrepostOperasiChild) + $rowspanparent):""); ?>"><?php echo $dataParentOp->nama_prepostoperasidesk; ?></td>
                        ...
                        <?php
                          if(count($modPrepostOperasiChild)>0){
                            ...
                            foreach($modPrepostOperasiChild as $j => $dataChild){
                              ...
                              ?>
                              <?php if($j == 0){ ?>
                                <td colspan="<?php echo ((isset($colspanparent[$dataChild->prepostoperasidesk_id]) && !empty($colspanparent[$dataChild->prepostoperasidesk_id]))? $colspanparent[$dataChild->prepostoperasidesk_id] : ""); ?>" rowspan="<?php echo ((count($modPrepostOperasiChildStep3)>0)?count($modPrepostOperasiChildStep3):""); ?>"><?php echo $dataChild->nama_prepostoperasidesk; ?></td>
                                <?php
                                if (count($modPrepostOperasiChildStep3) > 0){
                                  foreach($modPrepostOperasiChildStep3 as $k => $dataChildStep3){
