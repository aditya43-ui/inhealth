...
{
    window.open('<?php echo $this->createUrl('printSignIn',array('pasienkirim_id'=>$model->pasienkirimkeunitlain_id)); ?>','printwin','left=100,top=100,width=793,height=1122,scrollbars=yes');
}
...
function pilihSignInIni(obj){   
	...
	if ($(obj).prop("checked") == false){
		$("#tampung-signin > tbody > tr ").each(function(){
			if ($(this).find(".identifier").val() == form_id+'_'+check_id){
				var delete_row = $("#tampung-signin").find('input[name$="[identifier]"][value="'+form_id+'_'+check_id+'"]').parents('tr');
				delete_row.detach();
			}
		});
		renameInputRow($("#tampung-signin"),'signin');
		return false;
	}
	
	$.ajax({				
		type: "POST", 
		url: "<?php echo $this->createUrl('AddTambahDetailSignin')?>", 
			...
			if(data.sukses == 0){
				...
				$("#tampung-signin > tbody > tr ").each(function(){
					if ($(this).find(".identifier").val() == data.identifier){
						var delete_row = $("#tampung-signin").find('input[name$="[identifier]"][value="'+data.identifier+'"]').parents('tr');
						delete_row.detach();
					}
				});
				$("#tampung-signin > tbody ").append(data.tr);																
				renameInputRow($("#tampung-signin"),'signin');
			}
