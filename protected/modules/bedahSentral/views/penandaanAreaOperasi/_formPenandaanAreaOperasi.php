    ...
    <?php //} else { ?>
        <div class="col-sm-12">
            <div class="col-sm-6">
                ...
                <div class="control-group">
                <?php echo CHtml::label('Visus', '', array('class' => 'control-label')); ?>
                <div class="controls">
                    <?php 
                        echo $form->textField($modRencanaOperasi, 'visus1', array('class' => 'integer2 span2', 'onkeyup' => '',  'onkeypress' => "return $(this).focusNextInputField(event);"));
                        echo "&emsp;/&emsp;";
                        echo $form->textField($modRencanaOperasi, 'visus2', array('class' => 'integer2 span2', 'onkeyup' => '', 'onkeypress' => "return $(this).focusNextInputField(event);"));
                    ?>
                </div>
            </div>
            </div>
            <div class="control-group">
                <?php echo CHtml::label('Biometri', '', array('class' => 'control-label')); ?>
                <div class="controls">
                    <?php echo $form->textField($modRencanaOperasi, 'biometri', array('class' => 'span4 float2', 'onkeypress' => "return $(this).focusNextInputField(event);", 'maxlength' => 100)); ?>
                </div>
            </div>
            <div class="clear"></div>
            <div class="panel panel-success">
                ...
                <div class="panel-body" style="width:880px !important;">
                    <?php
                        ...
                        $i = 1;
                        $css = '';
                        $gbrTubuh = $modGambarTubuh->getAllGambarAreaBedah($modKunjungan->jeniskelamin);
                    ...
                    <div align="center" id="imgtag<?php echo $tbh->gambartubuh_id ?>">
                        <img img-no="<?php echo $tbh->gambartubuh_id ?>" alt="<?php echo $tbh->gambartubuh_id ?>"
                            id="myImgId<?php echo $tbh->gambartubuh_id ?>"
                            src="<?php echo Params::urlPhotoAnatomiTubuh() . $tbh->nama_file_gbr; ?>"
                            class="taggd<?php echo $tbh->gambartubuh_id ?>" style="width:820px;" />
...
<script>
    function batalTambahBagianTubuh(obj) {
        ...
        if (conf) {
            ...
            $(obj).parents('tbody').find('input[name$="[bagiantubuh_id]"][value="' + bagiantubuh_id + '"]').each(
                function() {
            ...
            $("#imgtag" + gambartubuh_id).find('#titik_' + bagiantubuh_id + '_' + kordinat_tubuh_x + '_' + kordinat_tubuh_y)
                .detach();
    ...
    function hapusBagianTubuh(obj) {
        ...
        var pasienmasukpenunjang_id =
            <?php echo !empty($modKunjungan->pasienmasukpenunjang_id) ? $modKunjungan->pasienmasukpenunjang_id : "''"; ?>;
        ...
        if (conf) {
            $.ajax({
                ...
                data: "bagiantubuh_id=" + bagiantubuh_id + "&areaoperasidet_id=" + areaoperasidet_id +
                    "&gambartubuh_id=" + gambartubuh_id + "&kordinat_tubuh_x=" + kordinat_tubuh_x +
                    "&kordinat_tubuh_y=" + kordinat_tubuh_y + "&areaoperasidet_ket=" + areaoperasidet_ket +
                ...
                success: function(data) {
                    if (data.ok == 0) {
                        ...
                        $(obj).parents('tbody').find('input[name$="[bagiantubuh_id]"][value="' +
                            bagiantubuh_id + '"]').each(function() {
                            if ($(this).data('delete') == gambartubuh_id + '_' + koor_tubuh_x + '_' +
                                koor_tubuh_y) {
                                $(this).parents('tr').detach();
                            }
                        });
                        $("#imgtag" + gambartubuh_id).find('#titikbiru_' + bagiantubuh_id + '_' + koor_tubuh_x +
                            '_' + koor_tubuh_y).detach();
                        renameInputRow($('#table-bagtubuh'));
    ...
    function titikSebelumSimpan(ptitikX, ptitikY, bagiantubuh_id, img) {
        ...
        $(img).append(
            $('<div id="titik_' + bagiantubuh_id + '_' + xtitik + '_' + ytitik +
                '"><span class="entypo-check" style="font-size:20px;color:' + color +
                ';position:absolute;top:-12px;left:-8px;"></div>')
    ...
    function titikSesudahSimpan(titikX, titikY, urutan, bagiantubuh_id, img) {
        ...
        $(img).append(
            $('<div id="titikbiru_' + bagiantubuh_id + '_' + x_titik + '_' + y_titik +
                '"><b class="entypo-check" style="position:absolute;top:0;left:0px;color:#333;font-size:15px;color:' +
                color + '">' + nomor + '</b></div>')
            .css('position', 'absolute')
            .css('top', titikY + 'px')
            .css('left', titikX + 'px')
    ...
    function loadTitikSesudahSimpan() {
        ...
        titikSesudahSimpan(<?= $v->kordinat_tubuh_x; ?>, <?= $v->kordinat_tubuh_y . ',' . $i . ',' . $v->bagiantubuh_id ?>,
            '#imgtag<?php echo $v->gambartubuh_id; ?>');
    ...
    $(document).ready(function() {
        ...
        $("[id^=imgtag] img").click(function(e) { // make sure the image is click				
            ...
            $("#imgtag" + no_img).append(
                $('<div id="titikklik' + no_img +
                    '"><span class="entypo-check" style="color:rgba(219, 50, 92, 0.5);position:absolute;top:-12px;left:-8px;font-size:20px;"></div></div>'
                )
                .css('position', 'absolute')
        ...
        $(document).on('click', '[id^=tagit] [id^=btnsave]', function() {
            ...
            $.ajax({
                ...
                data: "pic_id=" + id + "&bagiantubuh_id=" + bagiantubuh_id + "&keterangan=" +
                    keterangan + "&pic_x=" + mouseX + "&pic_y=" + mouseY + "&type=insert" +
