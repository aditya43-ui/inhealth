...
<div style="padding: 5px; border: solid 1px #000;">
    <table class="noborder tbl-form">
        ...
        ?>
                <tr>
                    <td><?php echo $i + 1 ?></td>
                    <td style="font-size: 100px;"><?php echo $timeout['form_nama'] ?></td>
                    <?php
                    ...
                    }elseif ($timeout['type'] == Params::INPUTTYPE_DATE){              
                        $modDet->timeoutdet_isian = $timeout['isian'];
                        $arr = array(                                                
                            'form_id'=>$timeout['form_id'], 
                            'check_id'=>$timeout['check_id'], 
                            'onclose' => 'pilihTimeOutTextIni(this);'                         
                        );                        
                        echo '<td colspan="2">';
                        CustomFunction::loadField($modDet,'[' . $timeout['form_id'] . $timeout['check_id'] . ']]timeoutdet_isian', $timeout['type'], $arr)."&nbsp;";                                            
                        echo '</td>';
                    }
                    ?>
                ...
                ?>
                    <tr>
                        ...
                        if ($check['type'] == Params::INPUTTYPE_CHECK) {
                        ...
                        }elseif ($check['type'] == Params::INPUTTYPE_DATE){                            
                            $arr = array(                                                
                                'form_id'=>$timeout['form_id'], 
                            ...
                            echo '<td colspan="2">';
                            CustomFunction::loadField($modDet,'[' . $timeout['form_id'] . $check['check_id'] . ']]timeoutdet_isian', $check['type'], $arr)."&nbsp;";                                                                        
                            echo '</td>';
                        }
                        ?>
                    </tr>
