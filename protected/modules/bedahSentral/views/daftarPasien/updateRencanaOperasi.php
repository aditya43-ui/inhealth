...
<script>
    ...
    const setNo = (obj) => {
        const idx = $(obj).parents(".trcld_namaoa").attr("idx");
        
        $("#idx").val(idx);
    }
    
    const resetObatAlkes = (obj) => {
        const formtr = $(obj).parents(".trparent");
        
        formtr.find(".detail-oa").val("");                
    }
    
    const setObatAlkes = (data, obj, jenis) => {        
        let no = $("#idx").val();        
        const formtr = $("#tblpemakaianbahan > tr.trparent");
                
        if (data.mendekatiminimalstok == 'ya'){
            myAlert("Stok mendekati minimal stok");
        }
                        
        formtr.find("tr.trcld[idx='"+no+"']").removeClass("set-stok-habis");
        if (data.mendekatiminimalstok == "ya"){
            formtr.find("tr.trcld[idx='"+no+"']").addClass("set-stok-habis");
        }                
        
        formtr.find("tr.trcld_namaoa[idx='"+no+"']").find(".obatalkes_id").val(data.obatalkes_id);
        formtr.find("tr.trcld_namaoa[idx='"+no+"']").find(".obatalkes_nama").val(data.obatalkes_nama);
        formtr.find("tr.trcld_namaoa[idx='"+no+"']").find(".isbukanbebanpasien").val(data.isbukanbebanpasien);
        formtr.find("tr.trcld_tglkadaluarsaoa[idx='"+no+"']").find(".tglkadaluarsa").val(data.tglkadaluarsa);
        formtr.find("tr.trcld_jnsoa[idx='"+no+"']").find(".jenisobatalkes_nama").val(data.jenisobatalkes_nama);                        
        formtr.find("tr.trcld_hargajualoa[idx='"+no+"']").find(".hargajual").val(data.hargajual);                        
    ...
    function cekInput() {
        $('.currency,.float2,.integer-decimal-global').each(function() {
