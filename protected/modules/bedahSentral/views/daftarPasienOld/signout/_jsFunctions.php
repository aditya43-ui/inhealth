...
{
    window.open('<?php echo $this->createUrl('printSignOut',array('pasienpenunjang_id'=>$model->pasienmasukpenunjang_id)); ?>','printwin','left=100,top=100,width=793,height=1122,scrollbars=yes');
}

/**
 * - digunakan untuk memilih data form dan checklist signout
 * @param {type} obj
 * @returns {generate data ke hidden field}
 */
function pilihSignOutIni(obj){   
	...
	if ($(obj).prop("checked") == false){
		$("#tampung-signout > tbody > tr ").each(function(){
			if ($(this).find(".identifier").val() == form_id+'_'+check_id){
				var delete_row = $("#tampung-signout").find('input[name$="[identifier]"][value="'+form_id+'_'+check_id+'"]').parents('tr');
				delete_row.detach();
			}
		});
		renameInputRow($("#tampung-signout"),'signout');
		return false;
	}
	
	$.ajax({				
		type: "POST", 
		url: "<?php echo $this->createUrl('AddTambahDetailSignout')?>", 
			...
			if(data.sukses == 0){
				...
				$("#tampung-signout > tbody > tr ").each(function(){
					if ($(this).find(".identifier").val() == data.identifier){
						var delete_row = $("#tampung-signout").find('input[name$="[identifier]"][value="'+data.identifier+'"]').parents('tr');
						delete_row.detach();
					}
				});
				$("#tampung-signout > tbody ").append(data.tr);																
				renameInputRow($("#tampung-signout"),'signout');
			}
 ...
 * @returns {generate data ke hidden field}
 */
function pilihSignOutTextIni(obj){   
