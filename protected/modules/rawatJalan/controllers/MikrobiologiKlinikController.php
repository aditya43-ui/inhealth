...
class MikrobiologiKlinikController extends MyAuthController
{
    ...
    public function actionIndex($pendaftaran_id, $idPasienKirimKeUnitLain = null)
    {
        ...
        if (isset($idPasienKirimKeUnitLain)) {
            ...
            if (!empty($modKirimKeUnitLain->klinis_penunjang_infeksi)) {
                $klinik_penunjang = CJSON::decode(($modKirimKeUnitLain->klinis_penunjang_infeksi));
                $modKirimKeUnitLain->klinis_penunjang_infeksi = $klinik_penunjang;
            }
            $modPasien = $modKirimKeUnitLain->pasien;
        }

        $modPenunjang2 = new RJPermintaanPenunjangT;
        $jenis_sample_notif = null;
        ...
        if (isset($_POST['RJPasienKirimKeUnitLainT'])) {
            ...
            try {
                if (isset($_POST['pemeriksaan'])) {
                    ...
                    foreach($_POST['pemeriksaan']['jenispemeriksaanlab_id'] as $j => $jenis) {

                        $jenis = JenispemeriksaanlabM::model()->findByPk($jenis);
                        $sample = SamplelabM::model()->findByPk($_POST['pemeriksaan']['samplelab_id'][$j]);

                        array_push($jenis_sample, $jenis->jenispemeriksaanlab_nama . " (" . $sample->samplelab_nama . ")");

                    }

                    $jenis_sample_unique = array_unique($jenis_sample);

                    $jenis_sample_notif = implode(", ", $jenis_sample_unique);

                    // echo '<pre>'; var_dump($jenis_sample, $jenis_sample_unique, $jenis_sample_notif); die;
                }
                /*
                    if(isset($_POST['permintaanPenunjangAnatomi'])){
                        ...
                        $modKirimKeUnitLainAnatomi = $this->savePasienKirimKeUnitLain($modPendaftaran, Params::RUANGAN_ID_LAB_ANATOMI, Params::INSTALASI_ID_LAB_PA);    // ruangan "Patologi Anatomi"                   
                    }
                    */
                ...
                if (isset($_POST['pemeriksaan'])) {
                    if (isset($_POST['pemeriksaan'])) {
                        ...
                        $this->savePermintaanPenunjang($_POST['pemeriksaan'], $modKirimKeUnitLain);
                    }
                    // if(isset($_POST['permintaanPenunjangAnatomi'])){
                ...
                $judul = 'Pasien Rujuk ke Mikrobiologi Klinik';

                $isi = $modPasien->no_rekam_medik . ' - ' . $modPasien->nama_pasien;

                if(!empty($jenis_sample_notif)) {
                    $isi .= " order " . $jenis_sample_notif;
                }
                /*
                    if(isset($_POST['KirimspesimenlabT'])){
                        if($_POST['pemeriksaan']) {
                            $count = count($_POST['pemeriksaan']['jenispemeriksaanlab_id']);
                            for ($i = 0; $i < $count; $i++) {
                               $modKirimKeUnitLainMikrobiologi = $this->savePemeriksaanMikriobiologi(
                                    $modPendaftaran, 
                                    isset($_POST['pemeriksaan']['jenispemeriksaanlab_id'][$i]) ? $_POST['pemeriksaan']['jenispemeriksaanlab_id'][$i] : null,
                                    isset($_POST['pemeriksaan']['samplelab_id'][$i]) ? $_POST['pemeriksaan']['samplelab_id'][$i] : null,
                                    isset($_POST['pemeriksaan']['caraambilsample_id'][$i]) ? $_POST['pemeriksaan']['caraambilsample_id'][$i] : null,
                                    isset($_POST['pemeriksaan']['pemeriksaanlab_id'][$i]) ? $_POST['pemeriksaan']['pemeriksaanlab_id'][$i] : null,
                                    isset($_POST['pemeriksaan']['catatan'][$i]) ? $_POST['pemeriksaan']['catatan'][$i] : null
                                );

                                // $mr = RuanganM::model()->findByPk($modKirimKeUnitLainMikrobiologi->ruangan_id);
                                // $link = $this->createUrl('/laboratorium/RujukanPenunjang/Index',array(
                                //         'LBPasienKirimKeUnitLainV[tgl_awal]'=>date('Y-m-d', strtotime($modKirimKeUnitLainMikrobiologi->tgl_kirimpasien)),
                                //         'LBPasienKirimKeUnitLainV[tgl_akhir]'=>date('Y-m-d', strtotime($modKirimKeUnitLainMikrobiologi->tgl_kirimpasien)),
                                //         'LBPasienKirimKeUnitLainV[no_pendaftaran]'=>substr($modKirimKeUnitLainMikrobiologi->pendaftaran->no_pendaftaran,2),			
                                //         'LBPasienKirimKeUnitLainV[prefix_pendaftaran]'=>substr($modKirimKeUnitLainMikrobiologi->pendaftaran->no_pendaftaran,0,2),			
                        ...
                        // $modKirimKeUnitLainMikrobiologi = $this->savePasienKirimKeUnitLain($modPendaftaran, 1131, 17, );
                    }
                    */
    ...
    protected function savePemeriksaanMikriobiologi($modPendaftaran, $jenispemeriksaanmikro_id, $samplelab_id, $caraambilsampel_id, $pemeriksaanmikro_id, $catatan)
    {
        $modKirimKeUnitLain = new RJPasienKirimKeUnitLainT;
        ...
        $modKirimKeUnitLain->jenispemeriksaanlab_id = $jenispemeriksaanmikro_id;
        $modKirimKeUnitLain->samplelab_id = $samplelab_id;
        $modKirimKeUnitLain->pemeriksaanlab_id = $pemeriksaanmikro_id;
        $modKirimKeUnitLain->caraambilsampel_id = $caraambilsampel_id;
        $modKirimKeUnitLain->catatandokterpengirim = $catatan;
    ...
    protected function savePasienKirimKeUnitLain($modPendaftaran, $ruangan_lab, $instalasi_lab)
    {
        ...
        $modKirimKeUnitLain->is_nonprogram = $_POST['RJPasienKirimKeUnitLainT']['is_nonprogram'] !== "0";
        $modKirimKeUnitLain->is_programtbc = $_POST['RJPasienKirimKeUnitLainT']['is_programtbc'] !== "0";
        $modKirimKeUnitLain->is_programhiv = $_POST['RJPasienKirimKeUnitLainT']['is_programhiv'] !== "0";

        $modKirimKeUnitLain->is_cito =  $_POST['RJPasienKirimKeUnitLainT']['is_cito'];

        $modKirimKeUnitLain->caraambilsampel_id = isset($_POST['RJPasienKirimKeUnitLainT']['caraambilsampel_id']) ? $_POST['RJPasienKirimKeUnitLainT']['caraambilsampel_id'] : null;


        if (!empty($modKirimKeUnitLain->klinis_penunjang_infeksi) && is_array($modKirimKeUnitLain->klinis_penunjang_infeksi)) {
            $klinis_penunjang = CJSON::encode($modKirimKeUnitLain->klinis_penunjang_infeksi);
            $modKirimKeUnitLain->klinis_penunjang_infeksi = $klinis_penunjang;
        }

        $modKirimKeUnitLain->tglpengirimanspesimen = isset($_POST['RJPasienKirimKeUnitLainT']['tglpengirimanspesimen']) ? MyFormatter::formatDateTimeForDB($_POST['RJPasienKirimKeUnitLainT']['tglpengirimanspesimen']) : null;

        // var_dump($modKirimKeUnitLain->attributes, $_POST['RJPasienKirimKeUnitLainT']);
        // die;
    ...
    protected function savePermintaanPenunjang($permintaan, $modKirimKeUnitLain)
    {
        ...
        foreach ($permintaan['kode_unik'] as $i => $value) {
            ...
            $modPermintaan->no_sediaan = isset($_POST['RJPermintaanPenunjangT']['no_sediaan']) ? $_POST['RJPermintaanPenunjangT']['no_sediaan'] : null;
            $modPermintaan->jenis_pasientbc = isset($_POST['RJPermintaanPenunjangT']['jenis_pasientbc']) ? $_POST['RJPermintaanPenunjangT']['jenis_pasientbc'] : null;
            $modPermintaan->is_visual_lendirnanah1 = isset($_POST['RJPermintaanPenunjangT']['is_visual_lendirnanah1']) ? $_POST['RJPermintaanPenunjangT']['is_visual_lendirnanah1'] : null;
            $modPermintaan->is_visual_lendirnanah2 = isset($_POST['RJPermintaanPenunjangT']['is_visual_lendirnanah2']) ? $_POST['RJPermintaanPenunjangT']['is_visual_lendirnanah2'] : null;
            $modPermintaan->is_visual_bercakdarah1 = isset($_POST['RJPermintaanPenunjangT']['is_visual_bercakdarah1']) ? $_POST['RJPermintaanPenunjangT']['is_visual_bercakdarah1'] : null;
            $modPermintaan->is_visual_bercakdarah2 = isset($_POST['RJPermintaanPenunjangT']['is_visual_bercakdarah2']) ? $_POST['RJPermintaanPenunjangT']['is_visual_bercakdarah2'] : null;
            $modPermintaan->is_visual_airliur1 = isset($_POST['RJPermintaanPenunjangT']['is_visual_airliur1']) ? $_POST['RJPermintaanPenunjangT']['is_visual_airliur1'] : null;
            $modPermintaan->is_visual_airliur2 = isset($_POST['RJPermintaanPenunjangT']['is_visual_airliur2']) ? $_POST['RJPermintaanPenunjangT']['is_visual_airliur2'] : null;
            $modPermintaan->lokasianatomi = isset($_POST['RJPermintaanPenunjangT']['lokasianatomi']) ? $_POST['RJPermintaanPenunjangT']['lokasianatomi'] : null;
            $modPermintaan->alasanpemeriksaan = isset($_POST['RJPermintaanPenunjangT']['alasanpemeriksaan']) ? $_POST['RJPermintaanPenunjangT']['alasanpemeriksaan'] : null;
            $modPermintaan->pemantauan_kemajuan = isset($_POST['RJPermintaanPenunjangT']['pemantauan_kemajuan']) ? $_POST['RJPermintaanPenunjangT']['pemantauan_kemajuan'] : null;
            $modPermintaan->pemeriksaan_ulang = isset($_POST['RJPermintaanPenunjangT']['pemeriksaan_ulang']) ? $_POST['RJPermintaanPenunjangT']['pemeriksaan_ulang'] : null;
            $modPermintaan->pemeriksaan_selesai = isset($_POST['RJPermintaanPenunjangT']['pemeriksaan_selesai']) ? $_POST['RJPermintaanPenunjangT']['pemeriksaan_selesai'] : null;
            $modPermintaan->no_reg_fasyankes = isset($_POST['RJPermintaanPenunjangT']['no_reg_fasyankes']) ? $_POST['RJPermintaanPenunjangT']['no_reg_fasyankes'] : null;
            $modPermintaan->no_reg_kabkota = isset($_POST['RJPermintaanPenunjangT']['no_reg_kabkota']) ? $_POST['RJPermintaanPenunjangT']['no_reg_kabkota'] : null;
            ...
            $modPermintaan->caraambilsampel_id = isset($_POST['RJPasienKirimKeUnitLainT']['caraambilsampel_id']) ? $_POST['RJPasienKirimKeUnitLainT']['caraambilsampel_id'] : null;


            // echo'<pre>';
            // // var_dump($modPermintaan->save(), $modPermintaan->getErrors(), $modPermintaan->attributes);
            // var_dump($_POST['RJPermintaanPenunjangT']);
            // var_dump("--------------------------------------------------------------------");
            // var_dump($modPermintaan->save(), $modPermintaan->getErrors(), $modPermintaan->attributes);
            //  die;
    ...
    public function actionAjaxBatalKirim()
    {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            $modRiwayatKirimKeUnitLain = RJPasienKirimKeUnitLainT::model()->findAllByAttributes(
                ...
                'pasienmasukpenunjang_id IS NULL AND ruangan_id IN(' . Params::RUANGAN_ID_LAB_MIKROBIOLOGI . ')'
            );
    ...
    //Get List Bahan Spesimen untuk Pemeriksaan Lab Mikro
    ...
    public function actionLoadTarifLab()
    {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            $subjenis_pemeriksaanlab_id = isset($_POST['subjenis_pemeriksaanlab_id']) ? $_POST['subjenis_pemeriksaanlab_id'] : null;
            ...
            $cri->compare('lower(pemeriksaanlab_nama)', strtolower($periksalab), true);

            $genLabTarif = OrderpemeriksaanlabV::model()->findAll($cri);
            ...
            if (!empty($genLabTarif)) {
                foreach ($genLabTarif as $lb) {
                    ...
                    $tarif_gen[$lb->jenispemeriksaanlab_id]['jenispemeriksaanlab_id'] = $lb->jenispemeriksaanlab_id;
                    $tarif_gen[$lb->jenispemeriksaanlab_id]['jenispemeriksaanlab_kelompok'] = $lb->jenispemeriksaanlab_kelompok;
                    $tarif_gen[$lb->jenispemeriksaanlab_id]['det'][$lb->pemeriksaanlab_id]['kode_unik'] = $lb->kode_unik;
                    $tarif_gen[$lb->jenispemeriksaanlab_id]['det'][$lb->pemeriksaanlab_id]['pemeriksaanlab_nama'] = $lb->pemeriksaanlab_nama;
                    $tarif_gen[$lb->jenispemeriksaanlab_id]['det'][$lb->pemeriksaanlab_id]['pemeriksaanlab_id'] = $lb->pemeriksaanlab_id;
                    //$tarif_gen[$lb->jenispemeriksaanlab_id]['det'][$lb->pemeriksaanlab_id]['ruangan_id'] = $lb->ruangan_id;
                }
    ...
    public function actionLoadBahanSpesimen()
    {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            $samplelab = $_POST['samplelab'] ?? null;
            $jenispermintaan = $_POST['jenispermintaan'] ?? null;
            ...
            if($jenispermintaan == 'hiv') {
                $cri->addCondition('is_programhiv is true');
            }else  if($jenispermintaan == 'tbc') {
                $cri->addCondition('is_programtbc is true');
            } else if($jenispermintaan == 'non') {
                if(!empty($samplelab)) {
                    $cri->addCondition('samplelab_id = ' . $samplelab);
                }
                $cri->addCondition('is_programhiv is not true');
                $cri->addCondition('is_programtbc is not true');
            } 

            if(!empty($pendaftaran_id)) {
                $pendaftaran = PendaftaranT::model()->findByPk($pendaftaran_id);
                $cri->addCondition('kelaspelayanan_id = ' . $pendaftaran->kelaspelayanan_id);
            }


            $pemeriksaan = OrderpemeriksaanlabmikroV::model()->findAll($cri);
            ...
            // echo '<pre>'; var_dump($jenispemerisaan_id, $pemeriksaan_id, $pemeriksaan, $cri); die;
            ...
            if (count($pemeriksaan_id) > 0) {
                ...
                if(!empty($samplelab)) {
                    $cri->addCondition('o.samplelab_id = ' . $samplelab);
                }

                if($jenispermintaan == 'hiv') {
                    $cri->addCondition('o.is_programhiv is true');
                }else if($jenispermintaan == 'tbc') {
                    $cri->addCondition('o.is_programtbc is true');
                }else if($jenispermintaan == 'non') {
                    $cri->addCondition('o.is_programhiv is not true');
                    $cri->addCondition('o.is_programtbc is not true');
                } 

                if(!empty($pendaftaran_id)) {
                    $pendaftaran = PendaftaranT::model()->findByPk($pendaftaran_id);
                    $cri->addCondition('o.kelaspelayanan_id = ' . $pendaftaran->kelaspelayanan_id);
                }
                $cri->join = 'JOIN orderpemeriksaanlabmikro_v o ON o.jenispemeriksaanlab_id = t.jenispemeriksaanlab_id ';
                $bahan = JenispemeriksaanlabM::model()->findAll($cri);



                // echo '<pre>'; vaR_dump(count($bahan)); die;
            } else {
                $cri = new CDbCriteria();
                $cri->addCondition('t.jenispemeriksaanlab_aktif = true');
                $cri->addCondition("t.jenispemeriksaanlab_kelompok ='" . Params::JENISPEMERIKSAANLAB_KELOMPOK_MIKROBIOLOGI . "'");
            ...
            $render = $jenispermintaan == 'non' ? '_listBahanSpesimen' : '_listBahanSpesimenProgram';

            

            $tr = $this->renderPartial($this->path_view . $render, array('bahan_gen' => $bahan_gen, 'periksabahan' => $periksabahan, 'pemeriksaan_data' => $pemeriksaan_data), true);
