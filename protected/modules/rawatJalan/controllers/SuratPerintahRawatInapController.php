...
class SuratPerintahRawatInapController extends MyAuthController
{
    ...
    public function actionIndex($pendaftaran_id, $suratperintahranap_id = null)
    {
        ...
        if (isset($_POST['SuratperintahranapT'])) {
            ...
            try {
                ...
                $modPasienPulang = PasienpulangT::model()->findByAttributes(array('pendaftaran_id' => $_POST['SuratperintahranapT']['pendaftaran_id']));
                $model->attributes = $_POST['SuratperintahranapT'];
                $model->indikasirawatinap = $_POST['SuratperintahranapT']['indikasirawatinap'];
                ...
                if (empty($modPasienPulang)) {
                    ...
                    Yii::app()->user->setFlash('error', "SPRI tidak dapat tersimpan, karena pasien belum ditindak lanjut ke rawat inap! ");
                } else {
    ...
    public function actionPrint($pendaftaran_id, $suratperintahranap_id)
    {
        ...
        $modResume = ResumemedisR::model()->findByAttributes(['pendaftaran_id' => $pendaftaran_id], ['order' => 'tglresume DESC']);
        
        $caraPrint = $_REQUEST['caraPrint'];
        if ($caraPrint == 'PRINT') {
            $this->layout = '//layouts/printWindows';
            $this->render($this->path_view . 'Print', array('model' => $model, 'modPendaftaran' => $modPendaftaran, 'caraPrint' => $caraPrint, 'modSep' => $modSep, 'modResume' => $modResume));
        }
    }
