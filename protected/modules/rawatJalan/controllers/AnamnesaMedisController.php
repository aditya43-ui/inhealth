...
class AnamnesaMedisController extends MyAuthController {
    ...
    public $path_view = 'rawatJalan.views.anamnesaMedis.';
    ...
    public function actionIndex($pendaftaran_id = null, $id = null, $tipe = null, $is_triage = null, $notriage_pasien_id = null)
    {
        ...
        if(isset($id)) {
            ...
            $modAnamnesa->paramedis_nama = NULL;
        }
        ...
        if(isset($is_triage)) {
          ...
          $tabelAnamnesaRD = RJAnamnesaT::model()->findAllByAttributes(array('notriage_pasien_id' => $notriage_pasien_id, 'is_medis' => true), array('order' => 'create_time DESC'));
        
         // var_dump ($tabelAnamnesaRD); die;
         $tabelAnamnesaPasienRD = RJAnamnesaT::model()->findAllByAttributes(array('is_medis' => true), array('order' => 'create_time DESC'));
            ...
            $tabelAnamnesa = RJAnamnesaT::model()->findAllByAttributes(array('pendaftaran_id' => $pendaftaran_id, 'is_medis' => true), array('order' => 'create_time DESC'));
            
            $cekAnamnesa = RJAnamnesaT::model()->findByAttributes(array('pendaftaran_id' => $pendaftaran_id), array('order' => 'create_time DESC'));

            if(isset($id)) {
                $cekAnamnesa = RJAnamnesaT::model()->findByPk($id);
            }
    
            $modPasien = RJPasienM::model()->findByPk($modPendaftaran->pasien_id);
            $modPpds = PpdsM::model()->findByPk($modPendaftaran->ppds_id);
            $tabelAnamnesaPasien = RJAnamnesaT::model()->findAllByAttributes(array('pasien_id' => $modPasien->pasien_id, 'is_medis' => true), array('order' => 'create_time DESC'));
            ...
            // echo '<pre>';var_dump(Yii::app()->user->getState('modul_id'));die;
            ...
            if (!empty($cekAnamnesa)) {  //Jika Pasien Sudah Melakukan Anamnesa Sebelumnya				
                ...
                $modAnamnesa->attributes = $cekAnamnesa->attributes;

                if($tipe == 'salin') {
                    $modAnamnesa->pegawai_id = "";
                    $modAnamnesa->ppds_id = null;
                    // die;
                }
                ...
                $modAnamnesa = new RJAnamnesaT;
                $modAnamnesa->pegawai_id = Yii::app()->user->getState('pegawai_id');
                $modAnamnesa->nama_pegawai = Yii::app()->user->getState('nama_pegawai');
                ...
                if( Yii::app()->user->getState('kelompokpegawai_id') != Params::KELOMPOKPEGAWAI_ID_DOKTER_TETAP) {
                    $modAnamnesa->paramedis_nama = empty($pegawai) ? null : $pegawai->nama_pegawai;
                    $modAnamnesa->paramedis_id = empty($pegawai) ? null : $pegawai->pegawai_id;
                }
              ...
              //  $modAnamnesa->ispasienwanitahamil = "Tidak";
               ...
               // $modAnamnesa->keb_olahraga = "Tidak";
                ...
                $anamnesakhir = RJAnamnesaT::model()->findByAttributes(array('pendaftaran_id' => $pendaftaran_id, 'create_ruangan' => Yii::app()->user->getState('ruangan_id')), array('order' => 'create_time DESC'));
                if (!empty($anamnesakhir)){
                  //  $modAnamnesa->keluhanutama = $anamnesakhir->keluhanutama;

                    $modAnamnesa->keluhanutama = strip_tags($anamnesakhir->keluhanutama);
                    $modAnamnesa->keluhanutama = str_replace('&nbsp;', '', $anamnesakhir->keluhanutama);
                    $modAnamnesa->keluhantambahan = strip_tags($anamnesakhir->keluhantambahan);
                    $modAnamnesa->riwayatperjalananpasien = $anamnesakhir->riwayatperjalananpasien;
                    //$modAnamnesa->keluhantambahan = str_replace('&nbsp;', '', $anamnesakhir->keluhantambahan);
        ...
        // var_dump($is_triage); die;
        if(isset($is_triage)) {
            
         //   var_dump($notriage_pasien_id); die;
        ...
        if(!empty($modAnamnesa->keluhanutama)) {
            $modAnamnesa->keluhanutama = strip_tags($modAnamnesa->keluhanutama);
            $modAnamnesa->keluhantambahan = strip_tags($modAnamnesa->keluhantambahan);
        }
        if(empty($modAnamnesa->pegawai_id)) {
            if($tipe != 'salin')
                $modAnamnesa->pegawai_id = Yii::app()->user->getState('pegawai_id');
        }
        if($tipe != 'sukses' && $tipe != 'salin') {
            $modAnamnesa->pegawai_id = Yii::app()->user->getState('pegawai_id');
        }
        // echo '<pre>'; var_dump($modAnamnesa->attributes); die;
        ...
        if (isset($_POST['RJAnamnesaT'])) {
            ...
            try {
                $detTriase = (isset($_POST['RJTriase']) ? $_POST['RJTriase'] : null);
                if(!empty($modAnamnesa->create_ruangan) && $modAnamnesa->create_ruangan != Yii::app()->user->getState('ruangan_id')) {
                    $modAnamnesa = new RJAnamnesaT();
                    if(isset($pendaftaran_id)) {
                        $modAnamnesa->pendaftaran_id = $pendaftaran_id;
                        $modAnamnesa->pasien_id = $modPendaftaran->pasien_id;
                ...
                // $modAnamnesa = new RJAnamnesaT();
                // if(isset($id) && isset($tipe) && $tipe == 'ubah') {
                //     $modAnamnesa = RJAnamnesaT::model()->findByPk($id);
                // }
               ...
               // $modAnamnesa->keluhantambahan = isset($_POST['RJAnamnesaT']['keluhantambahan']) ? $_POST['RJAnamnesaT']['keluhantambahan'] : null;
                ...
                $modAnamnesa->tglanamnesis = $format->formatDateTimeForDb(date('Y-m-d H:i:s'));
              ...
              //  $modAnamnesa->hpht_tanggal = isset($_POST['RJAnamnesaT']['hpht_tanggal']) ? $format->formatDateTimeForDb($_POST['RJAnamnesaT']['hpht_tanggal']) : null;
                ...
                // echo '<pre>';
                // var_dump($modAnamnesa->attributes, $modAnamnesa->save());
                // die;
 
                $modAnamnesa->is_medis = true;
                $modAnamnesa->pendaftaran_id = isset($_GET['pendaftaran_id']) ? $_GET['pendaftaran_id'] : null;
                $modAnamnesa->pasien_id = isset($_GET['pendaftaran_id']) ? $modAnamnesa->pendaftaran->pasien_id : null;
                ...
                if ($modAnamnesa->save()) {
                    ...
                    if(isset($pendaftaran_id)) {
                        $this->redirect(array('index', 'pendaftaran_id' => $pendaftaran_id, 'id' => $modAnamnesa->anamesa_id, 'tipe' => 'sukses'));
                    } else {
        ...
        // echo '<pre>';var_dump($modAnamnesa);die;
        ...
        } else if(Yii::app()->user->getState('modul_id') == Params::MODUL_ID_REKAMMEDIS || isset($_GET['lihat'])){ 
     ...
     */
    public function actionPrintAnamnesa($pendaftaran_id, $notriage_pasien_id = null) {
        $this->layout = '//layouts/printWindows';
        $format = new MyFormatter;
        if(!empty($pendaftaran_id)) {
            $modPendaftaran = RJPendaftaranT::model()->findByPk($pendaftaran_id);
            $modPasien = RJPasienM::model()->findByPk($modPendaftaran->pasien_id);
            $modAnamnesa = RJAnamnesaT::model()->findByAttributes(array('pendaftaran_id' => $pendaftaran_id), array('order' => 'anamesa_id DESC'));
            ...
            if(!empty($notriage_pasien_id)) {
                $modAnamnesa = RJAnamnesaT::model()->findByAttributes(array('notriage_pasien_id' => $notriage_pasien_id, 'is_medis' => true), array('order' => 'create_time DESC'));
            }
    ...
    public function actionPrintAnamnesaDarurat($notriage_pasien_id) {
        $this->layout = '//layouts/printWindows';
        $format = new MyFormatter;
        $modAnamnesa = RJAnamnesaT::model()->findByAttributes(array('notriage_pasien_id' => $notriage_pasien_id));

        $judul_print = 'ANAMNESIS';
        $this->render($this->path_view . 'printAnamnesaTriage', array(
            'format' => $format,
            'judul_print' => $judul_print,
            'modAnamnesa' => $modAnamnesa,
        ));
    ...
    public function actionAjaxDetailAnamnesa2() {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            $modAnamnesis =AnamnesaT::model()->findByPk($notriage_pasien_id);
             $modAnamnesa = AnamnesaT::model()->findByPk($idAnamnesis);

            //  echo '<pre>'; var_dump($_POST); die;


            $data['result'] = $this->renderPartial($this->path_view . '_viewDetailAnamnesaDarurat', array('modAnamnesa' => $modAnamnesa,'modAnamnesis'=>$modAnamnesis), true);
