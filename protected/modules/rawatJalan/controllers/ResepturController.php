...
class ResepturController extends MyAuthController
{
    ...
    public function actionIndex($pendaftaran_id, $pasienadmisi_id = null, $salin = null)
    {
        ...
        if ($kelompokpegawai_id == Params::KELOMPOKPEGAWAI_ID_DOKTER_TETAP) {
            ...
            if(isset($_GET['pasienadmisi_id'])) {
                $modReseptur->pegawai_id = $modPendaftaran->admisi->pegawai_id;
            }
        }
        ...
        if(isset($_GET['pasienmasukpenunjang_id'])) {
            ...
            if(!empty($modPenunjang)) {
                $modReseptur->pegawai_id = $modPenunjang->pegawai_id;
            }
        }
            ...
            try {
                ...
                if ($this->successSave) {
                    ...
                    if(isset($_GET['pasienadmisi_id'])) {
                        ...
                        $this->redirect(array('index', 'pendaftaran_id' => $pendaftaran_id, 'reseptur_id' => $this->reseptur_id, 'sukses' => 1, 'is_obatkronis' => $this->is_obatkronis));
    ...
    protected function broadcastNotifReseptur($modPendaftaran)
    {
        ...
        $tanggalResep = explode(' ', MyFormatter::formatDateTimeForDb((string)$reseptur->tglreseptur));
    ...
    public function actionAutocompleteObatApi()
    {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            $returnVal = [];
            $data = $models->searchObat();
            if(isset($_GET['stokdepo'])) {
                $ruangantujuan_id = isset($_GET['ruangantujuan_id']) ? $_GET['ruangantujuan_id'] : Yii::app()->user->getState('ruangan_id');
                $data = $models->searchObatRuangan($ruangantujuan_id);
            }
            foreach ($data->data as $i => $model) {
                if($model['StFornas'] == 0) {
                ...
                $returnVal[$i]['jmlStok'] = $model['jmlStok'];
    ...
    public function actionAutocompleteObatApiRuangan()
    {
        if (Yii::app()->request->isAjaxRequest) {
            $term = explode(';', $_GET['term']);
			// var_dump($term);die;
    ...
    public function actionSetFormObatAlkesPasien()
    {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            // echo '<pre>';var_dump($modResepturDetail->qty_reseptur);die;
            $instalasi = Yii::app()->user->getState('instalasi_id');
