...
class RJInfokunjunganrjV extends InfokunjunganrjV {
    ...
    public function searchDaftarPasien() {
        ...
        /*
        if(Yii::app()->user->getState('loginpemakai_id') == Params::LOGINPEMAKAI_ID_ADMIN) {
            // $criteria->addCondition('t.ruangan_id = '.$this->ruangan_id);
            Params::setRuangIdTindakan([$this->ruangan_id]);
            $criteria->addInCondition('t.ruangan_id', Params::$RUANGAN_ID_TINDAKAN);
        }else{
            */
            Params::setRuangIdTindakan([Yii::app()->user->getState('ruangan_id')]);
            $criteria->addInCondition('t.ruangan_id', Params::$RUANGAN_ID_TINDAKAN);
  ...
  //      $criteria->order = 't.jenis_kunjungan';
        ...
        return new CActiveDataProvider($this, array(
            'criteria' => $criteria,
            'sort'=>array(
                'defaultOrder'=>'t.tgl_pendaftaran desc',
            ),
            'pagination' => [
                'pageSize' => 50
            ]
...
//				$status_dokumen = '<button id="red" class="btn btn-primary" name="yt1" onclick="setStatusDokumen(this,'.$pengirimanrm_id.',\''.$status.'\','.$pendaftaran_id.')">'.$status.'</button>';
   ...
   public function getLinkPeriksaPasien() {
       ...
       if (!empty($konsul)) { //RSPMC-1645
        ...
        if ($this->carabayar_id == 2) { 

            if(empty($this->sep_id)) {
                // return  'pasien sep_id null';
                return CHtml::link("<i class='icon-form-periksa'></i><br>Periksa Pasien", '#', array("id"=>$this->no_pendaftaran,"rel"=>"tooltip","title"=>"Klik untuk Pemeriksaan Pasien", "onclick"=>"return false;", "style" => 'opacity: 0.6; cursor: not-allowed;'));
            } else {
            ...
            return 'ini pasien bpjs';
    
       } else {
        //    return 'ini bukan soal bpjs bre, carabayarnya nomor ' . $this->carabayar_id;
            return CHtml::link("<i class='icon-form-periksa'></i><br>Periksa Pasien", Yii::app()->controller->createUrl("/rawatJalan/pemeriksaanPasien",array("pendaftaran_id"=>$this->pendaftaran_id)),array("id"=>$this->no_pendaftaran,"rel"=>"tooltip","title"=>"Klik untuk Pemeriksaan Pasien"));
       }

        // return 'not empty konsul';
       }
       ...
       if ($this->penjamin_id == Params::PENJAMIN_ID_UMUM) {
            ...
            if (!empty($pendaftaran->karcis_id)) {
                ...
                if (empty($tindakan)) {
                    $tindakan = TindakanpelayananT::model()->findByAttributes(array(
                        ...
                        'condition'=>'karcis_id is not null and tindakansudahbayar_id is null'
                    ));
                }
            }
            ...
            if (!empty($tindakan)) {
                // return 'not empty tindakan';
                return CHtml::link("<i class='icon-form-periksa'></i><br>Periksa Pasien", '#', array("id"=>$this->no_pendaftaran,"rel"=>"tooltip","title"=>"Klik untuk Pemeriksaan Pasien", "onclick"=>"myAlert('Pasien belum membayar karcis.'); return false;"));
            }
       } 
   ...
   public function getAlergiObat() {
    ...
    public function getTindakanPasien() {
        $model = RuangTindakanT::model()->findByAttributes(array('pendaftaran_id' => $this->pendaftaran_id, 'ruangan_id' => Yii::app()->user->getState('ruangan_id')));

        if (!empty($model)) {
            return $model->ruangtindakan_id;
        } else {
            return null;
        }
    }
