...
<table class="table table-bordered table-striped table-condensed" id="tblListPemeriksaanRad">
    <thead>
        <tr>
            ...
            <th hidden>Tanggal Rencana Pemeriksaan</th>
...
$bisa_hapus = (($ruangan_login == $ruangan_create) && ($pegawai_login == $pegawai_create) && in_array($modul_login, $modul_pel)) ? 1 : 0;
        ...
        <tr>
            ...
            <td hidden><?php echo !empty($riwayat->tglrencanapemeriksaan) ? MyFormatter::formatDateTimeForUser($riwayat->tglrencanapemeriksaan) : "-"; ?></td>
            ...
            <td <?=$hide_edit?>>
                <?php echo CHtml::link("<i class='icon-form-ubah'></i>", $this->createUrl('update', array('pendaftaran_id'=>$riwayat->pendaftaran_id, 'idPasienKirimKeUnitLain'=>$riwayat->pasienkirimkeunitlain_id)) , array('rel'=>'tooltip','title'=>'Klik untuk ubah kirim pasien', 'data-placement'=>'left')); ?>
            </td>
            <td>
                <?php echo CHtml::link("<i class='icon-form-silang'></i>", '#', array('onclick'=>'batalKirim('.$riwayat->pasienkirimkeunitlain_id.','.$riwayat->pasien_id.', ' . $bisa_hapus .  ');return false;','rel'=>'tooltip','title'=>'Klik untuk membatalkan kirim pasien', 'data-placement'=>'left')); ?>
            </td>
        </tr>
        <?php } ?>
