...
<div class="col-md-12">
	...
	<div class="panel-heading">
		<div class="panel-title">Pengaturan Sub kelas</div>
	</div>
...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('subkelas-m-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
<!--<div class="block-tabel">-->
	...
	'columns'=>array(
                ...
                array(
                        'name'=>'kelas_id',
                        'value'=>'(isset($data->kelas) ? $data->kelas->kelas_nama : "")'
                        // function($data){
                ...
                'subkelas_kode',
                'subkelas_noterperinci',
                'subkelas_nama',
                array(
                    'header'=>'Status',
                    'value'=>'($data->subkelas_aktif == 1 ) ? "Aktif" : "Tidak Aktif"',
		...
		// ),
                 array(
                    'header'=>'Hapus',
                    'type'=>'raw',
                    'value'=>'($data->subkelas_aktif)?CHtml::link("<i class=\'entypo-cancel\'></i> ","javascript:removeTemporary($data->subkelas_id)",array("id"=>"$data->subkelas_id","rel"=>"tooltip","title"=>"Menonaktifkan Subkelas"))." ".CHtml::link("<i class=\'entypo-trash\'></i> ", "javascript:deleteRecord($data->subkelas_id)",array("id"=>"$data->subkelas_id","rel"=>"tooltip","title"=>"Hapus Subkelas")):CHtml::link("<i class=\'icon-form-check\'></i> ","javascript:addTemporary($data->subkelas_id, 1)",array("id"=>"$data->subkelas_id","rel"=>"tooltip","title"=>"Mengaktifkan Subkelas"))." ".CHtml::link("<i class=\'entypo-trash\'></i> ", "javascript:deleteRecord($data->subkelas_id)",array("id"=>"$data->subkelas_id","rel"=>"tooltip","title"=>"Hapus Subkelas"));',
...
<!--</div>-->
<?php 
echo CHtml::link(Yii::t('mds', '{icon} Tambah Sub Kelas', array('{icon}'=>'<i class="entypo-plus"></i>')), $this->createUrl('create',array('modul_id'=> Yii::app()->session['modul_id'])), array('class'=>'btn btn-success'))."&nbsp&nbsp";
...
{
    $("#subkelas-m-search :input[name='"+ obj.name +"']").val(obj.value);
}
function print(caraPrint)
{
    window.open("${urlPrint}/"+$('#subkelas-m-search').serialize()+"&caraPrint="+caraPrint,"",'location=_new, width=900px');
}
JSCRIPT;
