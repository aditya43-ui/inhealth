...
<table width="100%">
    ...
    <TR>
        <TD ALIGN=CENTER VALIGN=MIDDLE>
            <B>
                <FONT FACE="Liberation Serif" SIZE=4>NO. <?php echo CHtml::activeTextField($model, 'nomorsurat', array(
                                                                'readonly' => true,
...
<div class="control-group ">
    ...
    if ((Yii::app()->user->getState('modul_id') == Params::MODUL_ID_RD) || (Yii::app()->user->getState('modul_id') == Params::MODUL_ID_PENDAFTARAN)) {
        echo CHtml::label("Spesialis <span class='required'>*</span>", "", array('class' => 'control-label', 'style' => 'text-align: left !important; padding-left: 30px'));
    } else {
        echo $form->label($model, 'spesialissubspesialis_id', array('class' => 'control-label', 'style' => 'text-align: left !important; padding-left: 30px', 'label' => 'Spesialis / Subspesialis <span class="required">*</span>'));
    }
    ?>
    <div class="controls">
        <?php
        echo $form->dropDownList(
            ...
            array('empty' => '-- Pilih --', 'class' => 'form-control', 'onchange' => 'cekSpesialisVClaim();')
        );
        ?>
...
<div class="control-group">
    <label for="" class="control-label" style = 'text-align: left !important; padding-left: 30px'>DPJP <span class="required">*</span></label>
    <div class="controls">
        <?php 
            if (isset($_GET['sukses'])){
                $modSuratPRI = SuratperintahranapT::model()->findByPk($_GET['suratperintahranap_id']);
                echo "<center>". $form->textField($model->dpjp, 'namaLengkap', ['disabled' => true]). "</center>";
            }else{
                $model->dokterpembuatsurat_id = $modPendaftaran->pegawai_id;
                echo "<center>". $form->dropDownList($model, 'dpjp_id', $model->getItemPegawaiDokter(), array('empty' => '-- Pilih  --', 'class' => 'span3 required', 'onkeyup' => "return $(this).focusNextInputField(event);")). "</center>";
            }
        ?>
    </div>
</div>
...
<div class="control-group ">
    ...
    <div class="controls">
        <?php 
            ...
            $modResume = ResumemedisR::model()->findByAttributes(['pendaftaran_id' => $modPendaftaran->pendaftaran_id, 'pasienadmisi_id' => null], ['order' => 'create_time DESC']);

            if(!empty($modResume)) {
                $modMorbi = ResumemedisMorbiditasR::model()->findByAttributes(['resumemedis_id' => $modResume->resumemedis_id, 'kelompokdiagnosa_id' => Params::KELOMPOKDIAGNOSA_UTAMA]);

                if(!empty($modMorbi)) {
                    $diagnosa = $modMorbi->diagnosa->diagnosa_nama;
                }
            }
        ?>
...
<div class="control-group ">
    <?php echo CHtml::label('Indikasi Rawat Inap', 'indikasirawatinap', array('class' => 'control-label', 'style' => 'text-align: left !important; padding-left: 30px')) ?>
    <div class="controls">
        <?php echo $form->textArea($model, 'indikasirawatinap') ?>
    </div>
</div>
<div class="control-group ">
...
<table width="100%" hidden>
    <tr>
        ...
        <td style="width:30%; text-align: left;" colspan="2">
            <center><b>Mengetahui DPJP</b> </center>
            <br><br><br><br><br><br>
            <?php
            if (isset($_GET['sukses'])){
                $modSuratPRI = SuratperintahranapT::model()->findByPk($_GET['suratperintahranap_id']);
                echo "<center>". $form->textField($model->dpjp, 'namaLengkap'). "</center>";
            }else{
                $model->dokterpembuatsurat_id = $modPendaftaran->pegawai_id;
                 "<center>". $form->dropDownList($model, 'dpjp_id', $model->getItemPegawaiDokter(), array('empty' => '-- Pilih  --', 'class' => 'form-control', 'onkeyup' => "return $(this).focusNextInputField(event);")). "</center>";
            }
            // if((Yii::app()->user->getState('modul_id')==Params::MODUL_ID_RD)||(Yii::app()->user->getState('modul_id')==Params::MODUL_ID_PENDAFTARAN)){
...
<script>
    $(document).ready(function() {
        ...
        var modul = '<?php echo Yii::app()->controller->module->id ?>';

        if (modul !== 'pendaftaranPenjadwalan') {
