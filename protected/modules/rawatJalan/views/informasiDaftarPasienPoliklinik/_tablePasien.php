...
<?php $this->widget('ext.bootstrap.widgets.BootGridView',array(
    ...
    'columns'=>array(   
		...
		array(
			'header'=>'Penjamin'.'/<br>'. 'Jenis Penjamin' . '/'. 'SEP',
			'name'=>'Penjamin'.'/<br>'. 'Jenis Penjamin' . '/'. 'SEP',
			'type'=>'raw',
			'value' => function ($data) {
				$sep = '-';
				if(!empty($data->sep_id)) {
					$sept = SepT::model()->findByPk($data->sep_id);
				...
				return $data->penjamin_nama . '/<br>' . $data->carabayar_nama . '/' . $sep;
...
// 		),
		...
		array(
			...
			'value' => function ($data) {
                ...
                echo '</div>';
				echo '<div class="">';
				echo CHtml::link('<i class="icon-form-print"></i><br>Cetak Identitas', '#', array('onclick'=>'print('.$data->pasien_id.'); return false;'));
                echo '</div>';
            },
		...
		array(
			...
			'type'=>'raw',
			'value'=>function ($data) {
				if (!empty($data->pendaftaran->pasienpulang_id)) {
					return "Pasien di Rawat Inap".
				...
				if ($data->statusperiksa==Params::STATUSPERIKSA_BATAL_PERIKSA) {
					return "";
				}

				return CHtml::link('<i class="icon-form-ri"></i>', 
				'#',
				//Yii::app()->createUrl("/rawatJalan/DaftarPasien/tindakLanjutRI", array("instalasi_id"=>$data->instalasi_id,"pendaftaran_id"=>$data->pendaftaran_id,"ruangan_id"=>$data->ruangan_id)),
				array(
					"class"=>"",
					"rel"=>"tooltip",
			...
			/*'(!empty($data->pendaftaran->pasienpulang_id) ) ?  "Pasien di Rawat Inap".
				CHtml::link("<i class=\'icon-form-sampah\'></i>", Yii::app()->controller->createUrl("/rawatJalan/DaftarPasien/BatalRawatInap",array("pendaftaran_id"=>$data->pendaftaran_id,"ruangan_id"=>$data->ruangan_id)) , array("title"=>"Klik untuk Batal Proses Tindak Lanjut Pasien","target"=>"iframeBatalRawatInap", "onclick"=>"$(\"#dialogBatalRawatInap\").dialog(\"open\");", "rel"=>"tooltip"))  :  
...
<script type="text/javascript">
{
	...
	function print(id)
    {
        window.open("<?php echo $this->createUrl('printIdentitas'); ?>&pasien_id=" + id + "&caraPrint=PDF","",'location=blank, width=900px');
    }
