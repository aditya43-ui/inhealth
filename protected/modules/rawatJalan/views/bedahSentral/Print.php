...
<table width="100%" <?php echo $style; ?> >
    ...
    <!-- <tr>
        ...
        <td width="60%"><?php
            // echo CJSON::encode($modRiwayatKirimKeUnitLain);
...
foreach ($modRiwayatKirimKeUnitLain as $i => $riwayat) {
    ...
    <tr>
        ...
        <td>
            <?php
            foreach($modPermintaan as $j => $permintaan){
                $modKirim = PasienkirimkeunitlainT::model()->findByAttributes(array('pasienkirimkeunitlain_id' => $permintaan->pasienkirimkeunitlain_id, ));
                echo ($modKirim->is_cyto == 1) ? "Cyto" : "Biasa";
        ...
        <td>
            ...
            foreach($modPermintaan as $j => $permintaan){
                ...
                if($modKirim->is_cyto == 1){
                    ...
                    echo !empty($modTarif->harga_tariftindakan) ? MyFormatter::formatNumberForPrint($modTarif->harga_tariftindakan).'<br>' : 0;
                }
        ...
        <td>
            ...
            foreach($modPermintaan as $j => $permintaan){
                ...
                if($modKirim->is_cyto == 1){
                    ...
                    echo isset($modTarif->harga_tariftindakan) ? MyFormatter::formatNumberForPrint($modTarif->harga_tariftindakan) : null.'<br>';
                }
                // echo CJSON::encode($modKirim);
                // echo (!empty($modTarif->harga_tariftindakan))? number_format($modTarif->harga_tariftindakan).'<br>':'0 <br>';
...
foreach ($modRiwayatKirimKeUnitLain as $i => $riwayat) {
    ...
    <tr>
        ...
        <td>
            <?php
            foreach($modPermintaan as $j => $permintaan){
                echo $permintaan->qtypermintaan ? $permintaan->qtypermintaan : "0".'<br>';
            } ?>
        </td>
        <td>
            <?php
