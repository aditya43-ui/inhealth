<?php 
$pendaftaranT = PendaftaranT::model()->findByPk($modPendaftaran->pendaftaran_id);
if(!empty($pendaftaranT)) {
    if($pendaftaranT->validasiRekamMedis()) {
...
<div class="form-actions">
    ...
    if (!(!in_array(Yii::app()->user->getState('pegawai_id'), array(1, 1028)) && Yii::app()->user->getState('kelompokpegawai_id') == Params::KELOMPOKPEGAWAI_ID_TENAGA_KEPERAWATAN)) {
        echo CHtml::htmlButton(Yii::t('mds', '{icon} Create', array('{icon}' => '<i class="entypo-check"></i>')), array('class' => 'btn btn-danger btn-save', 'id' => 'btn_simpan', 'type' => 'button', 'onclick' => 'animation(this, event)')) . ' &nbsp; ';
        if (@$_GET['frame'] != 1) {
