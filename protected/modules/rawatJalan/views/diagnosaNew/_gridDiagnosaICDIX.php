<div class="panel panel-success">
    <div class="panel-heading">
        <div class="panel-title">
            <i class="entypo-credit-card"></i><b>Tabel Prosedur Tindakan (ICD 9) CM</b>
        </div>
    </div>
    <div class="panel-body table-responsive">
        <div style="margin:3px;">
            <?php
            $is_pasienICD9 = Pasienicd9cmR::model()->findByAttributes(array('is_verifikasidiagnosa' => true, 'pendaftaran_id' => $modPendaftaran->pendaftaran_id));
            $statusICD9 = empty($is_pasienICD9) ? false : true;
            if(!$statusICD9){
                ...
                if (count((array)$model) > 0) {
                    ...
                    foreach ($model as $val) {
                        if ($model[$i]['ruangan_id'] != Yii::app()->user->getState('ruangan_id')) {
                            $status = TRUE;
                            $disabled = TRUE;
                        } else if($model[$i]['create_ruangan_id'] != Yii::app()->user->getState('ruangan_id')){
                            $status = TRUE;
                            $disabled = TRUE;
                            $ruangan_id = $model[$i]['create_ruangan_id'];
                            $modelpemakai = LoginpemakaiK::model()->findByPk($model[$i]['create_loginpemakai_id']);
                            if(!empty($modelpemakai)) {
                                $model[$i]->pegawai_id = $modelpemakai->pegawai_id;
                            }
                        }else {
                            $status = FALSE;
                            $disabled = FALSE;
                        ...
                        if(!empty($model[$i]->create_ruangan_id)) {
                            $ruangan_id = $model[$i]->create_ruangan_id;
                        }
                ...
                ?>
                        <tr>
                            ...
                            <td>
                                ...
                                echo $form->hiddenField($model[$i], "[$i]create_ruangan_id");
                            ...
                            <td>
                                ...
                                echo $form->dropDownList(
                                    $model[$i],
                                    "[$i]pegawai_id",
                                    DokterV::model()->getDropDokterByRuangan($ruangan_id),
                                    array(
                                        'onkeypress' => "return $(this).focusNextInputField(event)", 'class' => 'span2', 'disabled' => $disabled
                                    )
                                );
                            ...
                            <td>
                                ...
                                echo $form->textArea($model[$i], "[$i]keterangan", array('class' => 'span4 custom-only required', 'maxlength' => 200, 'rows' => 3));
                                ?>
                            </td>
                            <td>
                                <?php
...
<script type="text/javascript">
    ...
    function tambahDiagnosaix() {
        $.get('<?= $this->createUrl('cekDiagnosaUtama') ?>', {
            ...
            var is_kelompokdiagnosautama = 0;
            ...
            if(data.sudahAdaDiagnosaUtama < 1 && is_kelompokdiagnosautama < 1) {
    ...
    function hapusDiagnosaix(mine) {
        ...
        if (pasienicd9cm_id != '') {
            jQuery.ajax({
                'url': '<?php echo Yii::app()->createUrl('/rawatJalan/diagnosaNew/hapusDiagnosaix') ?>',
                'data': {
    ...
    function inputDiagnosaix(mine, params, kode) {
        ...
        if (kode_sudah_ada < 1) {
            // jika kode diagnosa belum ada di tabel
            ...
            $("#PPPasienMorbiditasix_99_pegawai_id").val('<?= Yii::app()->user->getState('pegawai_id') ?>');
            var x = 0;
            ...
            setTimeout(function() {
                ...
                renameInputix('PPPasienMorbiditasix', 'create_ruangan_id');
                renameInputix('PPPasienMorbiditasix', 'create_loginpemakai_id');
...
$modDiagnosaix->diagnosaicdix_aktif = true;
