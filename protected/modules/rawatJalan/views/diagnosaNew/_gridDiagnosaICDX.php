                ...
                ?>
                        <tr>
                            ...
                            <td>
                                ...
                                echo $form->textArea($model[$i], "[$i]ket_diagnosa", array('class' => 'span4 custom-only required', 'maxlength' => 200, 'rows' => 3));
...
<script type="text/javascript">
    ...
    function hapusDiagnosa(mine) {
        ...
        console.log(pasienmorbiditas_id.length, 'pasienmorbi')
        if (pasienmorbiditas_id.length > 0) {
            jQuery.ajax({
                'url': '<?php echo Yii::app()->createUrl('/rawatJalan/diagnosaNew/hapusDiagnosax') ?>',
                'data': {
            ...
            // cek apakah ada icd 9 pada tabel
            var icd_9_ada = 0;
            $('#tbl_diagnosaix tbody > tr').each(function() {
                if($(this).attr('id') != 'is_kosong') {
                    icd_9_ada +=1;
                }
            });

            
            var icd_10 = $(mine).parents('tr').find('select[name$="[kelompokdiagnosa_id]"]').val();

            console.log(icd_9_ada, icd_10);

            if(icd_9_ada > 0 && icd_10 == '<?php echo Params::KELOMPOKDIAGNOSA_UTAMA ?>') {
                myAlert('Data tidak dapat dihapus karena masih terdapat prosedur tindakan yang berelasi');
                return false;
            } else {
    ...
    function inputDiagnosa(mine, params, kode) {
        ...
        // cek diagnosa utama ini untuk cek ketika pasienmorbiditas sesuai dengan pendaftaran_id ditemukan tidak perduli dokter dan ruangannya maka dokter lain tidak boleh menambahkan diagnosa utama lagi. berlaku untuk di rawatjalan dan rawatinap
        $.get('<?= $this->createUrl('cekDiagnosaUtama') ?>', {
            pendaftaran_id: '<?= $_GET['pendaftaran_id'] ?>',
            pasienadmisi_id: '<?= isset($_GET['pasienadmisi_id']) ? $_GET['pasienadmisi_id'] : null?>'
        }, function(data){
            var kelompokdiagnosaklik = $(mine).parents('tr').find('.kelompokdiagnosa').val();
            if(data.sudahAdaDiagnosaUtama == 1 && kelompokdiagnosaklik == '<?php echo Params::KELOMPOKDIAGNOSA_UTAMA ?>') {
                myAlert('Diagnosa utama sudah ditambahkan oleh Dokter : ' + data.namadokter);
                return false;
            } else {
                var jumlah = 1;
