...
<div class="panel panel-success">
    ...
    <div class="panel-body table-responsive">
        <?php $this->renderPartial($this->path_view . '_listRuangTindakanForPerawat', array('modRiwayatPasienKeunitLain' => $modRiwayatPasienKeunitLain)); ?>
    </div>
</div>
        ...
        <div class="col-sm-6">
            ...
            <div class="control-gruop">
                ...
                <div class="controls">
                    ...
                    $list_peg = 'kelompokpegawai_id = 1 and ruangan_id='.Yii::app()->user->getState('ruangan_id') . ' order by nama_pegawai';

                    $pegs = PegawairuanganV::model()->findAll($list_peg);
                    //ruangan emergency care
                    if($ruangan === 271) {
                        $list_peg = 'kelompokpegawai_id = 1 and pegawai_aktif is true order by nama_pegawai';
                        $pegs = PegawaiM::model()->findAll($list_peg);
                    }
                    
                    
                    // var_dump($ruangan); die;

                    echo $form->dropDownList(
                        $modKirimKeUnitLain,
                        'pegawai_id',
                        CHtml::listData($pegs, 'pegawai_id', 'NamaLengkap'),
                        array( 'class' => 'span3 dokter-konsul', 'onkeypress' => "return $(this).focusNextInputField(event);", 'empty' => '-- Pilih --')
                    ); ?>
                </div>
            </div>
            <br><br><br>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
       ...
       $(document).ready(function() {
                var pegawai2 = jQuery('#<?php echo CHtml::activeId($modKirimKeUnitLain, 'pegawai_id') ?>');
                jQuery(pegawai2).multiselect({
                    includeSelectAllOption: false,
                    buttonClass: "form-control",
                    maxHeight: 300,
                    buttonWidth: '240px',
...
console.log('tes dpjp');
       
       var dpjp_id = jQuery('#<?php echo CHtml::activeId($modKirimKeUnitLain, 'pegawai_id') ?>');	
       jQuery(dpjp_id).multiselect({
               includeSelectAllOption: true,
               buttonClass: "form-control",
               maxHeight: 300,
               buttonWidth: '265px',
...
<div class="form-actions">
    ...
    <?php }else{ ?>
        <?php 
            ...
            $disabled = $modPendaftaran->isPasienPulangAtauTindakLanjut($_GET['konsulpoli_id'] ?? null);
            if(isset($_GET['sukses'])) {
                $disabled = true;
            }
            echo CHtml::htmlButton(
                Yii::t('mds', '{icon} Save', array('{icon}' => '<i class="' . MyIcon::getIcons('simpan') . '"></i>')),
                array('class' => 'btn btn-danger', 'type' => 'submit', 'onKeypress' => 'return formSubmit(this,event)', 'id' => 'btn_simpan', 'disabled'=>$disabled, 'onclick' => 'setLoading()')
            ); 
        ?>
        <?php
        if (isset($_GET['idKonsulTindakan'])) {
            ...
            <div class="control-gruop">
                ...
                <div class="controls">
                    ...
                    echo $form->dropDownList(
                        $modKirimKeUnitLain,
                        'pegawai_id',
                        CHtml::listData(PegawairuanganV::model()->findAll('kelompokpegawai_id = 1 and ruangan_id='.Yii::app()->user->getState('ruangan_id')), 'pegawai_id', 'NamaLengkap'),
                        array( 'class' => 'span3 dokter-konsul', 'onkeypress' => "return $(this).focusNextInputField(event);")
                    ); ?>
                </div>
            </div>
            <br><br><br>
