...
<table style="width: 100%; border: none;">
    <thead>
        ...
        <tr <?= (Yii::app()->user->getState('modul_id') == Params::MODUL_ID_GIZI && !isset($_GET['nopelayanan'])) ? 'hidden' : '' ?>>
            <?php
                    ...
                    <table class="status" width="100%">
                        <tbody class="body-status">
                            ...
                            <tr class="tr-status">
                                ...
                                <td style="">:</td>
                                <td style="">
                                    <?php $tempatlayanan = RuanganM::model()->findByPk($modTindakans[0]->ruangan_id)?>
                                    <?php echo $tempatlayanan->ruangan_nama; ?>
                                </td>
                                <td style="">Kelas</td>
                                <td style="">:</td>
                                <td> <?php 
                                    // ketika kelas pelayanan itu kelaspelayanan_id 6 atau rawat jalan 
                                    if(Yii::app()->user->getState('modul_id') == Params::MODUL_ID_RJ || Yii::app()->user->getState('modul_id') == Params::MODUL_ID_RD) {
                                        echo '-';
                                    } else if(Yii::app()->user->getState('modul_id') == Params::MODUL_ID_GIZI) {
                                        if(!empty($modPendaftaran->pasienadmisi_id)) {
                                            $modPasienAdmisi = PasienadmisiT::model()->findByPk($modPendaftaran->pasienadmisi_id);
                                            if(!empty($modPasienAdmisi->kelaspelayanan)) {
                                                echo $modPasienAdmisi->kelaspelayanan->kelaspelayanan_nama;
                                            }
                                        }
                                    }else {
                                        echo $modTindakans[0]->kelaspelayanan->kelaspelayanan_nama ?? $modPendaftaran->kelaspelayanan->kelaspelayanan_nama; 
                                    }
                                ?> </td>
                            </tr>
                            <tr class="tr-status">
    ...
    <tr>
        <td style="width: 70%;">
            <b>No. Nota :
                ...
                ?>
                <?php echo $modTindakans[0]->noNota ?? "-"; /*
                    if(!empty($modPendaftaran->pasienadmisi_id)) {
                        $modPasienAdmisi = PasienadmisiT::model()->findByPk($modPendaftaran->pasienadmisi_id);
                        //if($modTindakans[0]->tgl_tindakan >= $modPasienAdmisi->tgladmisi) {
                        if (!empty($modTindakans[0]->pasienadmisi_id) || in_array($modTindakans[0]->instalasi_id, Params::grupInstalasiRIID())) {
