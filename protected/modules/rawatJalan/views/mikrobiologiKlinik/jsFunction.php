...
function cekAntibiotik() {
    ...
    if (inf.is(":checked")) {
        ...
        $("#RJPasienKirimKeUnitLainT_antibiotikygdiberi, #RJPasienKirimKeUnitLainT_antibiotik_hari").parents(
            ".control-group").find('.control-label').find('span.required').remove();
    } else {
        ...
        $("#RJPasienKirimKeUnitLainT_antibiotikygdiberi, #RJPasienKirimKeUnitLainT_antibiotik_hari").parents(
            ".control-group").find('.control-label').append('<span class="required">*</span>');
...
function inputperiksanew() {
    ...
    if (sample_id != 'empty') {
        ...
        jQuery.ajax({
            'url': '<?php echo Yii::app()->createUrl('rawatJalan/mikrobiologiKlinik/loadTabelSpesimen')?>',
            'data': {
                sample_id: sample_id,
                samplelab_id: samplelab_id,
                caraambilsample_id: caraambilsample_id,
                catatan: catatan
            },
            'type': 'post',
            'dataType': 'json',
            'success': function(data) {
                ...
                if (count == 0) {
                    ...
                    $("#tabelBahan > tbody > tr:last .integer").maskMoney({
                        "symbol": "",
                        "defaultZero": true,
                        "allowZero": true,
                        "decimal": ".",
...
function cekInput() {
    ...
    if (requiredCheck($("#rjpasien-laboratorium-t-form"))) {
        ...
        if (tr > 0) {
            ...
            if (jenis == 'tbc') {
                var sample_tbc = $('#sample_tbc').val();


                if (sample_tbc == "0") {
                    myAlert("Silahkan isi yang bertanda bintang <span class='required'>*</span>!");
                    return false;
                } else {
                ...
                var sample_hiv = $('#RJPermintaanPenunjangT_samplelab_id_hiv').val();
                ...
                var sample_non = $('#RJPasienKirimKeUnitLainT_samplelab_id').val();
        ...
        success: function(data) {
            if (data.sukses == 1) {
                ...
                $("#tblFormPemeriksaanLab > tbody > tr ").each(function() {
                    ...
                    if (count < 1) {
                        $("#generate-pemeriksaanlab").find('input[id$="pemeriksaanlabid"][value="' +
                            $(this).find('.idpemeriksaanlab').val() + '"]').prop("checked",
                            true);
                    }
                });
...
function setBahan(obj, pendaftaran_id) {
    ...
    console.log('pendaftarannya adalah: <?php $modPendaftaran->pendaftaran_id ?>');
        ...
        myConfirm("Apakah anda akan membatalkan pemeriksaan ini?", "Perhatian!", function(r) {
            if (r) {
                ...
                $(obj).closest('.boxtindakan').find('.cekPemeriksaan:checked').each(function() {
                    ...
                    var samplelab_id = $(this).closest('.base_input_ceklis').find('.samplelab_id')
                .val();
...
function inputBahan(obj, pemeriksaanlab_id, samplelab_id) {

    var sampleid = $('#RJPasienKirimKeUnitLainT_samplelab_id').val();
    var jenispermintaan = $('#jenispermintaan').val();
    ...
    if (sampleid == '' && jenispermintaan == 'non') {
        ...
        if ($(obj).is(':checked')) {
            ...
            var kelaspelayanan_id = '<?= $modPendaftaran->kelaspelayanan_id ?>';
            ...
            if (jenispermintaan == 'hiv') {
                samplelab_id = $('#RJPermintaanPenunjangT_samplelab_id_hiv').val();
            }

            console.log('samplelab_id' + samplelab_id);
            ...
            jQuery.ajax({
                ...
                'success': function(data) {
                    ...
                    var rowCount = document.getElementsByClassName("jumlahspesimen");
                    var count = rowCount.length;
                    var cito = $('#RJPasienKirimKeUnitLainT_is_cito').val();
                    ...
                    if (count == 0) {
                        ...
                        if (jenispermintaan == jenispermintaan_prev || typeof jenispermintaan_prev ==
                            "undefined") {
                            $('#tabelBahan > tbody').append(data.form);
                            $("#tabelBahan > tbody > tr:last .integer").maskMoney({
                                "symbol": "",
