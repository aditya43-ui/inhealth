...
function setInputPilihJenisForm() {

    jenis_id = $(".input_jenisform_id").val();

    $("#tblFormPemeriksaanLab tbody").empty();
    $("#periksaTotal").val("");
    
    if (jenis_id == 2 || jenis_id == 3) {
        $("#RJPasienKirimKeUnitLainT_is_cito").val(1);
    } else {
        $("#RJPasienKirimKeUnitLainT_is_cito").val(0);
    }
}

function cekAutoCeklisUntukJenisForm() {
    jenis_id = $(".input_jenisform_id").val();

    if (jenis_id == 4 || jenis_id == 5) {
        $(".input_ceklis").each(function() {
            if (!$(this).is(":checked")) {
                $(this).click();
            }
        });
 ...
 * @returns {undefined}
 */
function inputperiksa(obj, ruangan_id, sub=null) {
    ...
    if ($(obj).is(':checked')) {
        ...
        var is_cito = $('#RJPasienKirimKeUnitLainT_is_cito').val();

        ajaxinputperiksa(pemeriksaanlab_id, kelaspelayanan_id, pendaftaran_id, ruangan_id, is_cito, obj);
        ...
        if(sub !== null) {
            ...
            $('.sub-' + sub).find('input[type="checkbox"]').each(function() {
                ...
                if($(this).is(':checked') === false) {
                    ...
                    ajaxinputperiksa(pemeriksaanlab_id, kelaspelayanan_id, pendaftaran_id, ruangan_id, is_cito, $(this));
                }
...
function ajaxinputperiksa(pemeriksaanlab_id, kelaspelayanan_id, pendaftaran_id, ruangan_id, is_cito, obj = null) {
    ...
    jQuery.ajax({
            ...
            'success': function(data) {
                ...
                $('.cek-cito:last').prop('checked', is_cito == 1);
                $('.apa-cito:last').val(is_cito == 1 ? 'ya' : 'tidak');
...
function inputperiksa_onload(pemeriksaanlab_id) {

    var kelaspelayanan_id = $('#<?php echo CHtml::activeId($modKirimKeUnitLain, 'kelaspelayanan_id') ?>').val();
    var pendaftaran_id = '<?php echo $modPendaftaran->pendaftaran_id; ?>';

    ajaxinputperiksa(pemeriksaanlab_id, kelaspelayanan_id, pendaftaran_id, <?php echo  Params::RUANGAN_ID_LAB_ANATOMI ?>);
...
function batalKirim(pasienkirimkeunitlain_id, pendaftaran_id, is_bisa) {
    if(is_bisa) {
    window.parent.myConfirm("Apakah Anda akan membatalkan kirim pasien ke Laboratorium?", "Perhatian!", function(r) {
        if (r) {
            $.post('<?php echo $this->createUrl('ajaxBatalKirim') ?>', {
                ...
                if (data.sukses == 1) {
                    ...
                    document.location.href =
                        '<?php echo $this->createUrl('/rawatJalan/laboratorium/index&pendaftaran_id='); ?>' +
                        pendaftaran_id;
                } else {
                    $('#tblListPemeriksaanLab').html(data.result);
                    window.parent.myAlert(data.pesan);
...
<?php
    $urlkirim = isset($_GET['pasienkirimkeunitlain_id']) ? '&pasienkirimkeunitlain_id=' . $_GET['pasienkirimkeunitlain_id'] : '';
?>

    console.log('<?php echo $urlkirim ?>');

    $('.daftar-pemeriksaan').addClass("animation-loading");
    $.ajax({
        type: 'POST',
        url: '<?php echo $this->createUrl('/rawatJalan/laboratorium/SetChecklistFormLab' . $urlkirim); ?>',
        data: {
        ...
        success: function(data) {
            ...
            $.each(cekc, function(key, val) {
                ...
                $('#formPeriksaLab').find('input[type="checkbox"][value="' + val + '"]').prop(
                    "checked", "checked");
                // cekc.push(val); 
...
function updateChecklistPemeriksaanLab() {
    var form_index = $('#form_index').val();
    var jenisForm = $("#RJTarifpemeriksaanlabruanganV_jenisform_nama").val();
    console.log(jenisForm);
    var cek = [];
...
?>
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo $this->createUrl('/rawatJalan/laboratorium/SetChecklistPemeriksaanLab' . $urlkirim); ?>',
        data: {
            data: $("#form-caripemeriksaan :input").serialize(),
            jenisForm:jenisForm
        },
...
function setJenisPemeriksaanLab() {
    var jenisform_id = $('.input_jenisform_id').val();
    $.post('<?= $this->createUrl('setJenisPemeriksaanLab') ?>', {
        jenisform_id:jenisform_id
    }, function(data){
        $('#RJTarifpemeriksaanlabruanganV_jenispemeriksaanlab_nama').html(data.option);
        $('#RJTarifpemeriksaanlabruanganV_jenispemeriksaanlab_nama').multiselect('rebuild');
    }, 'json');
}

$(document).ready(function () {
    setJenisPemeriksaanLab();
    if($('.tarif_satuan').length > 0) {

        var tarif = 0;
        $('.tarif_satuan').each(function () {
            tarif += parseInt($(this).val());
        });;

        $('#periksaTotal').val(formatNumber(tarif));
    }
