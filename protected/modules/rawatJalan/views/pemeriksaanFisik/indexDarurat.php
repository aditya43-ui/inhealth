...
$this->Widget('ext.bootstrap.widgets.BootAccordion', array(
    ...
    'content' => array(
        'content-detailpemeriksaanfisik' => array(
            ...
            'isi' => $this->renderPartial($this->path_view . '_tabelRiwayatFIsik2', array(
...
<div class="col-sm-12">
        ...
        <div class="panel panel-success">
            ...
            <div class="panel-body">
                ...
                <div class="control-group">
                    <?php echo $form->labelEx($modPemeriksaanFisik, 'tglperiksafisik', array('style'=>'margin-left:10px;','class' => 'control-label')) ?>
                    <div class="controls">
                ...
                ?>
                <div class="control-group">
                    <?php echo $form->labelEx($modPemeriksaanFisik, 'keadaanumum', array('style'=>'margin-left:10px;','class' => 'control-label required')) ?>
                    <div class="controls">
                    <?php echo $form->textArea($modPemeriksaanFisik, 'keadaanumum', array('placeholder' => '', 'class' => 'span3 autogrow', 'onkeypress' => "return $(this).focusNextInputField(event);")); ?>
                        
                        <?php echo $form->error($modPemeriksaanFisik, 'keadaanumum'); ?>
                    </div>
                </div>
                <?php echo $form->dropDownListRow($modPemeriksaanFisik, 'pegawai_id', CHtml::listData($modPemeriksaanFisik->getDokterItems(), 'pegawai_id', 'NamaLengkap'), array('style'=>'margin-left:10px; margin-right: -10px;','onkeypress' => "return $(this).focusNextInputField(event);", 'class' => 'span3')); ?>
                <div class="control-group">
                    <?php echo $form->label($modPemeriksaanFisik, 'ppds_id', array('class' => 'control-label', 'style' => 'margin-left: 10px;', 'label' => 'PPDS')) ?>
                    <div class="controls" style="margin-left: 0px;">
                        <?php echo $form->dropDownList($modPemeriksaanFisik, 'ppds_id', CHtml::listData($modPemeriksaanFisik->getPPDS(), 'ppds_id', 'ppds_nama'), array('style'=>'margin-left:-100px;','empty' => '-- Pilih --', 'onkeypress' => "return $(this).focusNextInputField(event);", 'class' => 'span3')); ?>
                    </div>
                </div>
                <div class="control-group">
                    <?php
            ...
            <div class="panel-body">
                <?php
                    if (!empty($hide)) {
                        ...
                        echo $this->renderPartial($this->path_view . "pemeriksaan/_tandaVital2", array('modPemeriksaanFisik' => $modPemeriksaanFisik,'modRJMetodeGSCM'=>$modRJMetodeGSCM,'modPasien'=>$modPasien,'modBagianTubuh'=>$modBagianTubuh,'modPemeriksaanGambar'=>$modPemeriksaanGambar,'modPendaftaran'=>$modPendaftaran, 'form' => $form), true);
                    }
                ?>
    ...
    <div class="col-sm-6">
        ...
        <div class="panel panel-success panel_cgsews">
            <div class="panel-heading">
                <div class="panel-title">
                    <?php echo $form->checkBox($modPemeriksaanFisik, 'thorax', array('class' => 'cek_ews', 'uncheckValue' => null)); ?><?php echo $headThorax; ?>
                </div>
            ...
            <div class="panel-body">
                ...
                <?php
                if (!empty($hide)) {
                    echo $this->renderPartial($this->path_view . "pemeriksaan/_tandaVital2", array('modPemeriksaanFisik' => $modPemeriksaanFisik,'modRJMetodeGSCM'=>$modRJMetodeGSCM, 'form' => $form), true);
                } else {
        ...
        <?php echo $this->renderPartial($this->path_view . 'pemeriksaan._ews', array('model' => $modPemeriksaanFisik, 'form' => $form), true); ?>
        <br>
        <div class="panel panel-success">
            <div class="panel-heading">
                <div class="panel-title">
            ...
            <div class="panel-body" style="width:412px !important;">
                ...
                <!-- khusus untuk rawat darurat karena pasien triage belum diketahui jenis kelaminnya maka dibuatkan pilihan untuk memunculkan laki2 atau perempuan untuk gambarnya -->
                <?php 
                    $checkedGambarPerempuan = '';
                    $checkedGambarLaki = 'checked="checked"';
                    $hideGambarPerempuan = 'hidden';
                    $hideGambarLakiLaki = '';
                    $hide = '';
                    if(!empty($modPemeriksaanGambar[0]->gambartubuh_id)) {
                        $hide = 'hidden';
                        if($modPemeriksaanGambar[0]->gambartubuh_id == Params::GAMBAR_TUBUH_ID_PEREMPUAN) {
                            $checkedGambarPerempuan = 'checked="checked"';
                            $hideGambarPerempuan = '';
                        } else {
                            $checkedGambarLaki = 'checked="checked"';
                            $hideGambarLakiLaki = '';
                ...
                ?>
                <div class="pilihanGender" <?= $hide ?>>
                    <input type = "radio" name = "radio" value = "LAKI-LAKI" id = "radio1" onclick="setGambarTubuh(this)" <?= $checkedGambarLaki ?>> Laki-Laki </input> 
                    <input type = "radio" name = "radio" value = "PEREMPUAN" id = "radio2" onclick="setGambarTubuh(this)" <?= $checkedGambarPerempuan ?>> Perempuan </input><br>
                </div>
                <div class="gambarTubuh">
                    <?php $this->renderPartial($this->path_view .'pemeriksaan/rd/_formGambarPilihan', ['modGambarTubuh' => $modGambarTubuh, 'hideGambarPerempuan' => $hideGambarPerempuan, 'hideGambarLakiLaki' => $hideGambarLakiLaki]) ?>
                </div>
            </div>
        </div>
        <br>
                        ...
                        <tbody>
                            ...
                            if (($modPemeriksaanGambar)) {
                                foreach ($modPemeriksaanGambar as $ii => $vv) {
                                    ...
                                    echo $this->renderPartial($this->path_view . '_rowDetail2', array('modPemeriksaanGbr' => $vv, 'i' => $ii + 1), true);
                                }
                            } ?>
                        </tbody>
                    </table>
...
<div class="form-actions">
    <?php
        if (!isset($_GET['sukses'])) {
            echo CHtml::htmlButton(Yii::t('mds', '{icon} Create', array('{icon}' => '<i class="entypo-check"></i>')), array('class' => 'btn btn-danger', 'type' => 'submit', 'onKeypress' => 'return formSubmit(this,event)', 'onsubmit' => 'return requiredCheck(this);',  'id' => 'btn_simpan'));
        } else {
    ...
    <?php
        if ($modPemeriksaanFisik->isNewRecord) {
            ...
            echo CHtml::link(Yii::t('mds', '{icon} Print Pemeriksaan Fisik', array('{icon}' => '<i class="entypo-print"></i>')), 'javascript:void(0);', array('class' => 'btn btn-info', 'onclick' => "printPemeriksaanFisik2();return false", 'disabled' => FALSE));
        }
    ?>
    <?php
...
JS;
    ...
    $urlSetGambarTubuh = $this->createUrl('SetGambarTubuh');
    $js = <<< JS
//==================================================Validasi===============================================
...
function setGambarTubuh(obj) { 
    var jumlahtr = $('#table-bagtubuh tbody > tr').length;

    if($(obj).is(':checked')) {
   
        var jeniskelamin = $(obj).val();
