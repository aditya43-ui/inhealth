  ...
  <div class="panel-body">
    <?php
      $form = $this->beginWidget('ext.bootstrap.widgets.BootActiveForm', array(
          'id' => 'rekonsiliasiobatadmisi-t-form',
    ...
    <div class="row">
      <div class="col-sm-6">
        ...
        <div class="control-group ">
            ...
            <div class="controls">
              <?php
                  $listDataRuangan = CHtml::listData(RuanganM::model()->findAll('ruangan_aktif = true and instalasi_id = '.Params::INSTALASI_ID_RD.' ORDER BY ruangan_nama ASC'),'ruangan_nama','ruangan_nama');
                  if(Yii::app()->user->getState('ruangan_id') == 59){
                    $listDataRuangan = CHtml::listData(RuangantransferpasienV::model()->findAll(),'ruangan_nama','ruangan_nama');
                  }
               ?>
              <?php echo $form->dropDownList($model,'rujukansebelumnya', $listDataRuangan,array('empty'=>'-- Pilih --','onkeypress'=>"return $(this).focusNextInputField(event);", 'maxlength' => 100,'class'=>'span3')); ?>
            </div>
        </div>
        <div class="control-group ">
            <?php echo CHtml::activeLabelEx($model, 'rujukanke', array('class' => 'control-label')); ?>
            <div class="controls">
              <?php echo $form->dropDownList($model,'rujukanke', CHtml::listData(RuanganM::model()->findAll('ruangan_aktif = true and instalasi_id = '.Params::INSTALASI_ID_RI.' ORDER BY ruangan_nama ASC'),'ruangan_nama','ruangan_nama'),array('empty'=>'-- Pilih --','onkeypress'=>"return $(this).focusNextInputField(event);", 'maxlength' => 100,'class'=>'span3')); ?>
            </div>
        </div>
      </div>
      <div class="col-sm-6">
      ...
      <table class="table table-striped table-bordered table-condensed" style="width: 100%" id="tblObat">
        <thead>
          <tr>
              ...
              <th style="width:100px">Diteruskan dari Admisi</th>
              <th style="width:80px">Aksi</th>
          </tr>
        </thead>
        <tbody>
          <?php
            if(count($detailRekObatAdmisi)>0){
                $htmlRekAdmisi = "";
              foreach ($detailRekObatAdmisi as $i => $dataRekAdmisi) {
                $dataRekAdmisi->waktu_pemberian = (!empty($dataRekAdmisi->waktu_pemberian)? MyFormatter::formatDateTimeForUser($dataRekAdmisi->waktu_pemberian): "");
                $htmlRekAdmisi .= "<tr>";
                $htmlRekAdmisi .= "<td>";
                $htmlRekAdmisi .= "<input type='hidden' id='RekonobatdischargedetT_".$i."_nama_obat' name='RekonobatdischargedetT[".$i."][nama_obat]' class='nama_obat' value='".$dataRekAdmisi->nama_obat."' />";
                $htmlRekAdmisi .= "<input type='hidden' id='RekonobatdischargedetT_".$i."_dosis' name='RekonobatdischargedetT[".$i."][dosis]' class='dosis' value='".$dataRekAdmisi->dosis."' />";
                $htmlRekAdmisi .= "<input type='hidden' id='RekonobatdischargedetT_".$i."_frekuensi' name='RekonobatdischargedetT[".$i."][frekuensi]' class='frekuensi' value='".$dataRekAdmisi->frekuensi."' />";
                $htmlRekAdmisi .= "<input type='hidden' id='RekonobatdischargedetT_".$i."_cara_pemberian' name='RekonobatdischargedetT[".$i."][cara_pemberian]' class='cara_pemberian' value='".$dataRekAdmisi->cara_pemberian."' />";
                $htmlRekAdmisi .= "<input type='hidden' id='RekonobatdischargedetT_".$i."_waktu_pemberian' name='RekonobatdischargedetT[".$i."][waktu_pemberian]' class='waktu_pemberian' value='".$dataRekAdmisi->waktu_pemberian."' />";
                $htmlRekAdmisi .= "<input type='hidden' id='RekonobatdischargedetT_".$i."_jumlah_obat' name='RekonobatdischargedetT[".$i."][jumlah_obat]' class='jumlah_obat' value='".$dataRekAdmisi->jumlah_obat."' />";
                $htmlRekAdmisi .= "<input type='hidden' id='RekonobatdischargedetT_".$i."_tindaklanjut' name='RekonobatdischargedetT[".$i."][tindaklanjut]' class='tindaklanjut' value='".$dataRekAdmisi->tindaklanjut."' />";
                $htmlRekAdmisi .= "<input type='hidden' id='RekonobatdischargedetT_".$i."_keterangan' name='RekonobatdischargedetT[".$i."][keterangan]' class='keterangan' value='".$dataRekAdmisi->keterangan."' />";
                $htmlRekAdmisi .= "<span>".$dataRekAdmisi->nama_obat."</span>";
                $htmlRekAdmisi .= "</td>";
                $htmlRekAdmisi .= "<td><span>".$dataRekAdmisi->frekuensi."</span></td>";
                $htmlRekAdmisi .= "<td><span>".$dataRekAdmisi->dosis."</span></td>";
                $htmlRekAdmisi .= "<td><span>".$dataRekAdmisi->cara_pemberian."</span></td>";
                $htmlRekAdmisi .= "<td><span>".$dataRekAdmisi->waktu_pemberian."</span></td>";
                $htmlRekAdmisi .= "<td><span>".$dataRekAdmisi->jumlah_obat."</span></td>";
                $htmlRekAdmisi .= "<td><span>".$dataRekAdmisi->tindaklanjut."</span></td>";
                $htmlRekAdmisi .= "<td style='text-align: center;'><span>Ya</span></td>";
                $htmlRekAdmisi .= "<td style='text-align: center;'>";
                $htmlRekAdmisi .= "<a onclick='batalObat(this);return false;'' rel='tooltip' href='javascript:void(0);'' title='Klik untuk membatalkan obat'><i class='icon-remove'></i></a>";
                $htmlRekAdmisi .= "</td>";
                $htmlRekAdmisi .= "</tr>";
              }
              echo $htmlRekAdmisi;
