...
<div class="col-md-12">
	...
	<div class="panel-heading">
		<div class="panel-title">Pengaturan Kelas</div>
	</div>
...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('kelas-m-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
<!--<div class="block-tabel">-->
	...
	'columns'=>array(
                ...
                array(
                        'name'=>'domain_id',
                        'value'=>'(isset($data->domain) ? $data->domain->domain_nama : "")'
                ...
                array(
                    'header'=>'Status',
                    'value'=>'($data->kelas_aktif == 1 ) ? "Aktif" : "Tidak Aktif"',
            ...
            array(
                ...
                'buttons'=>array(
                    'update'=>array(
                        'options'=>array('rel'=>'tooltip','title'=>'Ubah Kelas'),
                    ),
                ),
            ),
		// array(
		...
		// ),
                 array(
                    'header'=>'Hapus',
                    'type'=>'raw',
                    'value'=>'($data->kelas_aktif)?CHtml::link("<i class=\'entypo-cancel\'></i> ","javascript:removeTemporary($data->kelas_id)",array("id"=>"$data->kelas_id","rel"=>"tooltip","title"=>"Menonaktifkan Kelas"))." ".CHtml::link("<i class=\'entypo-trash\'></i> ", "javascript:deleteRecord($data->kelas_id)",array("id"=>"$data->kelas_id","rel"=>"tooltip","title"=>"Hapus Kelas")):CHtml::link("<i class=\'icon-form-check\'></i> ","javascript:addTemporary($data->kelas_id, 1)",array("id"=>"$data->kelas_id","rel"=>"tooltip","title"=>"Mengaktifkan Kelas"))." ".CHtml::link("<i class=\'entypo-trash\'></i> ", "javascript:deleteRecord($data->kelas_id)",array("id"=>"$data->kelas_id","rel"=>"tooltip","title"=>"Hapus Kelas"));',
...
<!--</div>-->
<?php 
echo CHtml::link(Yii::t('mds', '{icon} Tambah Kelas', array('{icon}'=>'<i class="entypo-plus"></i>')), $this->createUrl('create',array('modul_id'=> Yii::app()->session['modul_id'])), array('class'=>'btn btn-success'))."&nbsp&nbsp";
...
{
    $("#kelas-m-search :input[name='"+ obj.name +"']").val(obj.value);
}
function print(caraPrint)
{
    window.open("${urlPrint}/"+$('#kelas-m-search').serialize()+"&caraPrint="+caraPrint,"",'location=_new, width=900px');
}
JSCRIPT;
