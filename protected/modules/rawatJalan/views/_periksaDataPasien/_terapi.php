...
$daftarIdSekarang = isset($_GET['id'])?$_GET['id']:null;
if (!empty($daftarIdSekarang)){
    $admisi = PasienadmisiT::model()->findByAttributes([
        'pendaftaran_id' => $daftarIdSekarang
    ]);
    $pasienadmisi_id = !empty($admisi)?$admisi->pasienadmisi_id:'';
}
?>
...
<table class="items table table-bordered table-striped table-condensed">
    ...
    <tbody>
        ...
        if ($item['tipe'] == 1) {
            ...
            $isbatal = ($dat[0]->reseptur->isbatal)?'Resep Batal':'Resep ditebus';
            foreach ($dat as $item) {
                array_push($oa, array(
					'r'=>$item->rke,
                    ...
                    'signa'=>$item->signa_reseptur,
                    'etiket'=>$item->etiket,
                    'keterangan'=>$item->resepturketerangan
                ));
            }
        } else if ($tipe == 2) {
			$criRep = new CDbCriteria();
...
<?php
// if (!empty($r->reseptur_id)){
//     echo CHtml::link("<span class='icon-form-copy'></span> Salin Resep", $this->createUrl('/rawatJalan/reseptur/index',['daftarIdSekarang'=>$daftarIdSekarang,'reseptur_id'=>$r->reseptur_id,'pendaftaran_id'=>$daftarIdSekarang,'pasienadmisi_id'=>((Yii::app()->user->getState('instalasi_id') == Params::INSTALASI_ID_RI || Yii::app()->user->getState('instalasi_id') == Params::INSTALASI_ID_PI)?$pasienadmisi_id:''),'salin'=>'salin']),['class'=>'btn btn-default btn-sm']).'</br></br>';
// } 
endforeach; ?>
