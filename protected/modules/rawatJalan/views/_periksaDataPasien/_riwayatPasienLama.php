...
<table class="items table table-striped table-bordered table-condensed">
    <thead>
        <tr>
            ...
            <th rowspan="2">Ruangan Tindakan</th>
        ...
        <?php foreach ($modKunjungan as $modKunjungan) { ?>
            <tr>
                <td style="width: 60px; text-align: center;">
                    ...
                    ), array("target" => "_blank", "rel" => "tooltip", "title" => "Klik untuk Detail CPPT", "onclick" => "var text = $(this).attr('dialog-text');", "dialog-text" => "Riwayat CPPT")) ?>
                </td>
                <td style="width: 60px; text-align: center;">
                <?php  if(Yii::app()->user->getState('isppds')) {
                        ...
                        foreach($ppds as $itemz){
                            echo '<br>';
                            echo 'PPDS <br> &nbsp;',$x++.'-'.$itemz->ppds->ppds_nama;
                            echo '<br>';
                           }
                ...
                <?php 
                    echo CHtml::link("<i class='icon-bayarklaim'></i> ",  Yii::app()->controller->createUrl(
                        ...
                        array("id" => $modKunjungan->pendaftaran_id)
                    ), array("id" => "$modKunjungan->no_pendaftaran", "target" => "detailDialogPenunjang", "rel" => "tooltip", "title" => "Klik untuk Detail Hasil Pemeriksaan Penunjang", "onclick" => "var text = $(this).attr('dialog-text'); window.parent.$('#ui-dialog-title-dialogDetailData').text(text);window.parent.$('#dialogDetailDataPenunjang').dialog('open');", "dialog-text" => "Riwayat Konsul Poliklinik"));
                    ?>
                </td>
                <td style="width: 60px; text-align: center;">
                ...
                <td style="text-align: center;">
                    <?php 
                        ...
                        echo CHtml::Link(
                            ...
                            Yii::app()->createUrl("rawatJalan/ruangTindakan/DetailRiwayatHasilPemeriksaan", array(
                                'pendaftaran_id' => $modKunjungan->pendaftaran_id,
                                'pasien_id' => $modKunjungan->pasien_id
                            )),
                            array(
                                "class" => "",
                                "target" => "frameRiwayat",
                                "onclick" => "window.parent.$('#dialogDetailRiwayat').dialog('open');",
                                "rel" => "tooltip",
...
<script>
    ...
    function getRiwayatPeriksaCPPT(id) {
        window.open("<?php echo $this->createUrl('rawatDarurat/cppt/InformasiRiwayatPasien') ?>&id="+id,"",'location=_new, width=600px, height=480px, left=340px, top=100px');
    }
</script>