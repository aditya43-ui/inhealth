...
?>
	...
	<tr>
		...
		<td>
			<span name="[ii][obatalkes_kode]"><?php echo (!empty($modResepturDetail->sumberdana_id) ? $modResepturDetail->obatalkes->kodeobat_inventory : "") ?></span> /<br>
		...
		<td class="waktu-ri" <?php echo Yii::app()->user->getState('modul_id') == 7 ? "" : "hidden" ?>>
		...
		<td>
			<?php echo CHtml::activeTextField($modResepturDetail, '[ii]qty_reseptur', array('class' => 'qty ' . ((empty($isRacikan) || $isRacikan == 0) ? "number-char" : ""), 'readonly' => false, 'style' => 'width:50px; text-align: right', 'onblur' => 'hitungTotal()')); //,'onblur'=>'hitungSubTotal(this)'
			?>
...
<?php } else { ?>
	<tr>
		...
		<td>
			<?php
			echo CHtml::activeTextField($modResepturDetail, '[ii]qty_reseptur', array('class' => 'qty ' . ((empty($isRacikan) || $isRacikan == 0) ? "number-char" : ""), 'readonly' => false, 'style' => 'width:50px; text-align: right;', 'onblur' => 'hitungTotal()')); //,'onblur'=>'hitungSubTotal(this)'
		...
		<td <?= (Yii::app()->user->getState('kelompokpegawai_id') == 1 || Yii::app()->user->getState('pegawai_id') == Params::PEGAWAI_ID_SYSADMIN || Yii::app()->user->getState('kelompokpegawai_id') == 3) ? '' : 'hidden' ?>>
			<center>
