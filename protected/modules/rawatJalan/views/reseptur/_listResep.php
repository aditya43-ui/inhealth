...
$hide_edit = !in_array($modul_login, $modul_hide);
// $readonly = $pg_login->kelompokpegawai_id == 1 && $modul_id != 7;
...
if(!$readonly) {
    ...
    [
        ...
        'type' => 'raw',
        'value' => function ($data) {
            $kelompokinput =  LoginpemakaiK::model()->findByPk($data->create_loginpemakai_id)->pegawai;
            $kelompokperawat =  LoginpemakaiK::model()->findByPk(Yii::app()->user->getState('loginpemakai_id'))->pegawai;
            ...
            $pegawai_create = $data->create_loginpemakai_id;
            ...
            if(empty($data->penjualanresep_id)){
                if(in_array($kelompokperawat->kelompokpegawai_id, [1, 2, 3])) {
                    if($kelompokinput->pegawai_id == $kelompokperawat->pegawai_id) {
                        return "<center>" . CHtml::link("<i class='icon-trash'></i>",'#', array('rel' => 'tooltip', 'title' => 'Klik untuk menghapus Reseptur', 'onclick' => 'hapusresep('.$data->reseptur_id.', this, ' . $bisa_hapus . ')' )) . "</center>";  
                    } else {
                        return "<center>" . CHtml::link("<i class='icon-trash'></i>",'#', array('rel' => 'tooltip', 'title' => 'Klik untuk menghapus Reseptur', 'onclick' => 'window.parent.myAlert("Hanya dokter yang meresepkan yang dapat menghapus")' )) . "</center>";  
                    }
                } else {
                    return "<center>" . CHtml::link("<i class='icon-trash'></i>",'#', array('rel' => 'tooltip', 'title' => 'Klik untuk menghapus Reseptur', 'onclick' => 'window.parent.myAlert("Hanya dokter yang meresepkan yang dapat menghapus")' )) . "</center>";  
                }
...
<script>
    function hapusresep(reseptur_id,obj, is_bisa)
    {
    ...
    function refreshForm2(pendaftaran_id){
        window.location.href = "<?php echo Yii::app()->createUrl($this->module->id . '/'.$this->id.'/index/&pendaftaran_id='.$_GET['pendaftaran_id']); ?>";
    }
