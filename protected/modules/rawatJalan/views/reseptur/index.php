...
<div class="row formInputTab <?= $hide ?>">
    <div class="col-sm-12">
        <div class="panel panel-success">
            <div class="panel-heading">
                    ...
                    <table class="items table table-bordered table-striped table-condensed" id="table-obatalkespasien">
                        <thead>
                            <tr>
                                ...
                                <th <?php echo Yii::app()->user->getState('modul_id') == 7 ? "" : "hidden" ?>>Waktu Pemberian</th>
                                <th>Sediaan</th>
...
<div class="form-actions" <?=$hidden?>>
    <?php
        $disableSave = false;
        $disableSave = ((!empty($_GET['sukses'])) ? true : false) || $modPendaftaran->isPasienPulangAtauTindakLanjut($_GET['konsulpoli_id'] ?? null);
    ?>
...
<?php
    // CHtml::htmlButton(Yii::t('mds','{icon} Print Detail',array('{icon}'=>'<i class="entypo-print"></i>')),array('class'=>'btn btn-info', 'disabled'=>$disablePrint,'type'=>'button','onclick'=>'printRecordTerakhir(\'PRINT\')')).'&nbsp';
    echo CHtml::htmlButton(Yii::t('mds','{icon} Print Resep',array('{icon}'=>'<i class="entypo-print"></i>')),array('class'=>'btn btn-info', 'disabled'=>$disablePrint,'type'=>'button','onclick'=>'printResep(\'PRINT\')'));
?>
<?php
    $content = $this->renderPartial($this->path_view.'tips/tipsReseptur',array(),true);
