        ...
        <div class="panel-body">
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    ...
                    if (empty($modStainingGambar->temp_file)) {
                        ...
                        $img = Params::urlPemeriskaanGambarStaining() . $modStainingGambar->gambar;
                    }
                    ?>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    ...
                    <?php
                    echo CHtml::activeHiddenField($modStainingGambar, '[detail][' . $i . ']temp_file', array('readonly' => true));
                    ...
                    echo CHtml::activeHiddenField($modStainingGambar, '[detail][' . $i . ']staining_gambar_id', array('class' => 'staining_gambar_id span3', 'readonly' => true));
            ...
            <?php
            $modStainingDets = MKStainingdetT::model()->findAllByAttributes(array('staining_gambar_id' => $modStainingGambar->staining_gambar_id));
            if (!empty($modStainingDets)) {
                foreach ($modStainingDets as $modStainingDet) {
                ...
                <?php if (!empty(Yii::app()->user->getState('ppds_id'))) : ?>
                    ...
                    <?php if (!empty($cekPPDS)) : ?>
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <?php
                                if (!empty(Yii::app()->user->getState('ppds_id'))) {
                                    if (empty($modStainingGambar->tgl_verifikasi_ppds)) {
                                        if ($modStainingGambar->ppds_id == Yii::app()->user->getState('ppds_id')) {
                                            echo CHtml::link(Yii::t('mds', '{icon} Verifikasi PPDS', array('{icon}' => '<i class="icon-check icon-white"></i>')), $this->createUrl('index'), array(
                                                'class' => 'btn btn-danger',
                                                'onclick' => 'myConfirm("Apakah anda menyetujui pemeriksaan ini?","Perhatian!",function(r){if(r) window.location = "' . $this->createUrl('index&spesimen_id=' . $modStaining->spesimen_id . '&staining_id=' . $modStaining->staining_id.'&staining_gambar_id='.$modStainingGambar->staining_gambar_id) . '&verifikasippds=true' . '";}); return false;'));
                                        } else {
                                            echo CHtml::link(Yii::t('mds', '{icon} Verifikasi PPDS', array('{icon}' => '<i class="icon-check icon-white"></i>')), $this->createUrl('index'), array(
                                                'class' => 'btn btn-danger',
                                                'onclick' => 'toastr.error("Anda tidak dapat melakukan verifikasi.", "Perhatian!"); return false;'));
                                        }
                                    } else {
                                        echo CHtml::link(Yii::t('mds', '{icon} Terverifikasi PPDS', array('{icon}' => '<i class="icon-check icon-white"></i>')), $this->createUrl($this->id . '/index', array('spesimen_id' => $modStaining->spesimen_id, 'staining_id' => $modStaining->staining_id, 'verifikasippds' => true)), array('class' => 'btn btn-success', 'disabled' => true));
                                    }
                                } else {
                                    if (empty($modStainingGambar->tgl_verifikasi_ppds)) {
                                        echo CHtml::link(Yii::t('mds', '{icon} Verifikasi PPDS', array('{icon}' => '<i class="icon-check icon-white"></i>')), $this->createUrl('index'), array(
                ...
                <?php if (!empty(Yii::app()->user->getState('pegawai_id'))) : ?>
                    ...
                    <?php if (!empty($cekDPJTM)) : ?>
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <?php
                                if (!empty($modStainingGambar->dpjtm_id) && !empty($modStainingGambar->tgl_verifikasi_ppds)) {
                                    if (empty($modStainingGambar->tgl_verifikasi_dpjtm)) {
                                        if ($modStainingGambar->dpjtm_id == Yii::app()->user->getState('pegawai_id')) {
                                            echo CHtml::link(Yii::t('mds', '{icon} Verifikasi DPJTM', array('{icon}' => '<i class="icon-check icon-white"></i>')), $this->createUrl('index'), array(
                                                'class' => 'btn btn-danger',
                                                'onclick' => 'myConfirm("Apakah anda menyetujui pemeriksaan ini?","Perhatian!",function(r){if(r) window.location = "' . $this->createUrl('index&spesimen_id=' . $modStaining->spesimen_id . '&staining_id=' . $modStaining->staining_id.'&staining_gambar_id='.$modStainingGambar->staining_gambar_id) . '&verifikasi=true' . '";}); return false;'));
                                        } else {
                                            echo CHtml::link(Yii::t('mds', '{icon} Verifikasi DPJTM', array('{icon}' => '<i class="icon-check icon-white"></i>')), $this->createUrl('index'), array(
                                                'class' => 'btn btn-danger',
                                                'onclick' => 'toastr.error("Anda tidak dapat melakukan verifikasi.", "Perhatian!"); return false;'));
                                        ...
                                        echo CHtml::link(Yii::t('mds', '{icon} Terverifikasi DPJTM', array('{icon}' => '<i class="icon-check icon-white"></i>')), $this->createUrl($this->id . '/index', array('spesimen_id' => $modStaining->spesimen_id, 'staining_id' => $modStaining->staining_id, 'verifikasi' => true)), array('class' => 'btn btn-success', 'disabled' => true));
                                    }
                                } else {
                                    if (empty($modStainingGambar->tgl_verifikasi_dpjtm)) {
                                        echo CHtml::link(Yii::t('mds', '{icon} Verifikasi DPJTM', array('{icon}' => '<i class="icon-check icon-white"></i>')), $this->createUrl('index'), array(
