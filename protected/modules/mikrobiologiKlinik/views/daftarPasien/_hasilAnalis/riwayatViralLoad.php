...
<table class="table table-striped table-bordered table-condensed" style="width: 100%;">
    ...
    <tbody>
        ...
        <?php foreach ($riwayat as $no => $riwayat) {
            ...
            // echo '<pre>'; var_dump($riwayat->pemeriksaanviralload_id); die;
        ...
        <tr>
            ...
            <td><?= isset($riwayat->tindakanpelayanan->daftartindakan) ? $riwayat->tindakanpelayanan->daftartindakan->daftartindakan_nama : $riwayat->tindakanpelayanan->daftartindakan_id ?></td>
            <td class="link_col"><?php echo CHtml::link('<i class="icon-form-print"></i>', '#', array('onclick'=>'printViralLoad1(' . $riwayat->pemeriksaanviralload_id . ');return false;')); ?></td>
...
<script>
    ...
    function printViralLoad1(pemeriksaanviralload_id) {
        ...
        window.open(
            '<?php echo $this->createUrl('printViralLoad', array()); ?>&pemeriksaanviralload_id='+pemeriksaanviralload_id,
    ...
    function hapusRiwayatViralLoad(id, obj) {
        myConfirm('Anda yakin untuk menghapus hasil pemeriksaan ini ?', 'Peringatan', function(r) {
            if (r) {
                $.post('<?php echo $this->createUrl('hapusViralLoad'); ?>', {id: id}, function(data) {
                    if (data.ok == 1) {
