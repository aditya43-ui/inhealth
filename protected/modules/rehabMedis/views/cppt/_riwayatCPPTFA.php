...
<div class="panel panel-success panel-shadow" style="width: 60%">
...
<div style="overflow: auto;">
    ...
    $this->widget('ext.bootstrap.widgets.BootGridView',array(
        ...
        'columns'=>array(
            ...
            // array(
            //     'header' => 'Verifikasi Supervisi',
            //     'type' => 'raw',
            //     'value' => function ($data){
            //         $peg_validasi = array();
            //         $peg_validasi_nama = array();

            //         if (!empty($data->supervisi)) {
            //             $peg_validasi[] = $data->supervisi_id;
            //             $peg_validasi[] = $data->supervisi->namaLengkap;
            //         }
            ...
            //         $dataDialog = 'myAlert("Hanya '.implode(", ", $peg_validasi).' yang bisa mengakses");';
                    

            //         // if(in_array(Yii::app()->user->getState('pegawai_id'), $peg_validasi)){
            //            $dataDialog = "$('#dialogVerifikasi').dialog('open');";
            //         //}
            //     if($data->isverifikasisupervisi==true){
            //         $html = "<div class='green'><b>Sudah diverifikasi oleh : <br />".(isset($data->supervisi_id)? $data->supervisi->namaLengkap : "-").'<br /> Tgl : <br />'.MyFormatter::formatDateTimeForUser($data->verifikasisupervisi_tanggal).'<br />'.CHtml::link("<icon class='icon-form-detailtagihan'></icon> ", Yii::app()->createUrl(Yii::app()->controller->module->id.'/'.Yii::app()->controller->id.'/reviewVerifikasiSupervisi', array("cpptpasien_id"=>$data->cpptpasien_id,'type'=>'review',"frame"=>true)), array("target"=>"frameHasilReview","rel"=>"tooltip", "title"=>"Klik untuk Lihat Hasil Review", "onclick"=>"$('#dialogHasilReview').dialog('open');"))."</b></div>";
            //     }else{
            //         $html = CHtml::link("<icon class='icon-form-verifikasi'></icon> ", Yii::app()->createUrl(Yii::app()->controller->module->id.'/'.Yii::app()->controller->id.'/reviewVerifikasiSupervisi', array("cpptpasien_id"=>$data->cpptpasien_id,'type'=>'verifikasi',"frame"=>true)), array("target"=>"frameVerifikasi","rel"=>"tooltip", "title"=>"Klik untuk Review & Verifikasi Supervisi", "onclick"=>$dataDialog));
            //     }
            ...
            array(
                ...
                'value'=>function($data) {
                    if($data->pegawaippa_id==Yii::app()->user->getState('pegawai_id')){
                        return CHtml::link('<i class="entypo-pencil" style="font-size:14pt"></i>', Yii::app()->controller->createUrl('indexFA', array(
