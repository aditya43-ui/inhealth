...
<table style="width: 100%; border: none;">
	...
	<tr>
		...
		<?php
			...
			if(!empty($modInfo->pasienkirimkeunitlain_id)) {
				$kirim = PasienkirimkeunitlainT::model()->findByPk($modInfo->pasienkirimkeunitlain_id);
				$ruangan = RuanganM::model()->findByPk($kirim->create_ruangan);
				$cr = $ruangan->ruangan_nama;
			}

			$rt = $modInfo->ruangan_nama;

			if(isset($_GET['is_mikrobiologi'])) {
				if($_GET['is_mikrobiologi'] == 1) {
					$rt = $cr;
				}
			}
		...
		?>
		<td class="col-isi"><?php echo $rt; ?></td>
    </tr>
	<tr>
...
<table style="width: 100%; border: none;">
    ...
    <tbody>
		...
		<tr class="border-tr-td">
			...
			<td style="text-align: right;"><?php echo $format->formatNumberForUser($totalTindakan); ?></td>
		</tr>
    </tbody>
    <tfoot>
        <tr class="border-tr-td">
            <td colspan=5' class="text-right-bold">Total</td>
            <td class="text-right-bold"><?php echo $format->formatNumberForPrint($totalTindakan); ?></td>
        </tr>  
    </tfoot>
</table>
<br>
...
?>
    ...
    function print(){
        window.open("<?php echo Yii::app()->createUrl("rehabMedis/daftarPasien/RincianTagihanPenunjang", array("pendaftaran_id"=>$_GET['pendaftaran_id'], "instalasi_id"=>$_GET['instalasi_id'], "pasienmasukpenunjang_id"=>$_GET['pasienmasukpenunjang_id'] , "pasienadmisi_id"=>(isset($_GET['pasienadmisi_id']) ? $_GET['pasienadmisi_id'] : null), 'is_mikrobiologi' => (isset($_GET['is_mikrobiologi']) ? 1 : 2))) ?>","",'location=_new, width=1024px');
    }
    </script>
<?php
}
