...
function pilihPemeriksaanTindakanIni(obj){
    var tindakanrm_id = $(obj).val();
    var jenistindakanrm_id = $(obj).closest('tr').find('.jenistindakanrm_id').val();
    var jenistindakanrm_nama = $(obj).closest('tr').find('.jenistindakanrm_nama').val();
    var tindakanrm_nama = $(obj).closest('tr').find('.tindakanrm_nama').val();
...
function setPemeriksaan(tindakanrm_id, jenistindakanrm_id, tindakanrm_nama, daftartindakan_id, jenistarif_id, harga_tariftindakan){
    var rowtindakan = [];
    rowtindakan = '<?php echo CJSON::encode($this->renderPartial($this->path_view_pendaftaran.'_rowTindakanPemeriksaan',array('i'=>0,'modTindakan'=>$modTindakan),true));?>';
...
function setCheckedPemeriksaanDariPermintaan(){
    ...
    $("#form-permintaankepenunjang").find('input[name$="[tindakanrm_id]"]').each(function(){
        ...
        checkbox_pemeriksaan = $("div.checklists").find('input[name$="[is_pilih]"][value='+tindakanrm_id+']');
        console.log(tindakanrm_id);
...
function printStatus()
{
    ...
    var pasienmasukpenunjang_id = '<?= isset($_GET['pasienmasukpenunjang_id']) ? $_GET['pasienmasukpenunjang_id'] : null ?>';
    if(pendaftaran_id != ""){
        window.open('<?php echo $this->createUrl('pendaftaranRehabilitasiMedis/printStatusRehabMedis'); ?>&pendaftaran_id='+pendaftaran_id+'&pasienmasukpenunjang_id='+pasienmasukpenunjang_id,'printwin','left=100,top=100,width=480,height=640');
    }else{
