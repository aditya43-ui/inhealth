...
function pilihPemeriksaanIni(obj){
    ...
    if($(obj).is(':checked')){
        ...
        var tindakanpelayanan_id =  $('#form-tindakanpemeriksaan .tindakanpelayanan_id[data-master="' + daftartindakan_id + '"]').val();
        console.log(tindakanpelayanan_id);
        
        if(tindakanpelayanan_id == "" || typeof(tindakanpelayanan_id) == "undefined"){
            ...
            myConfirm("Apakah Anda yakin akan menghapus pemeriksaan yang sudah di simpan sebelumnya?","Perhatian!",
            function(r){
                if(r){
...
function hitungTotal()
{
    $('#form-tindakanpemeriksaan').find('tbody > tr').each(function(){
        var tarif_satuan = parseInt(unformatNumber($(this).find('input[name$="[tarif_satuan]"]').val()));
        var qty = parseInt(unformatNumber($(this).find('input[name$="[qty_tindakan]"]').val()));
        
        var total = (tarif_satuan * qty);

        if(total > 0){
            total = parseFloat(total.toFixed(2));
        }
        $(this).find('input[name$="[tarif_tindakan]"]').val(formatInteger(total));
    });
}
