...
<div class="row">
    <div class="col-sm-12">
        <div class="block-tabel">
            <table class="items table table-bordered table-striped table-condensed" id="table-pemakaianbahan">
                <thead>
                    <tr>
...
$moObatAlkes = new RMObatAlkesM('search');
$moObatAlkes->unsetAttributes();
if (isset($_GET['RMObatAlkesM']))
    $moObatAlkes->attributes = $_GET['RMObatAlkesM'];
...
<script type="text/javascript">
    ...
    function inputPemakaianBahan(obj) {
        ...
        if (obatalkes_id != '') {
            ...
            alert("Silakan pilih pemakaian bahan terlebih dahulu!");
        }
    ...
    function inputAlatmedis(idAlat) {
        ...
        jQuery.ajax({
            'url': '<?php echo $this->createUrl(Yii::app()->controller->id . '/addFormPemakaianAlat') ?>',
            'data': {
                idAlat: idAlat,
            ...
            'success': function(data) {
                if (!sudahAdaAlat(idAlat)) {
                    if (!sudahAdaTindakanAlat(idDaftartindakan, data.daftarTinNama)) {
                ...
                $("#table-pemakaianbahan > tbody tr:last .integer").maskMoney({
                    "symbol": "",
                    "defaultZero": true,
                    "allowZero": true,
                    "decimal": ".",
        ...
        function sudahAdaTindakanAlat(daftartindakan_id, daftarTinNama) {
            var ada;
            $('#table-pemakaianbahan').find('input[name$="[daftartindakan_id]"]').each(function() {
                var cek = true;
