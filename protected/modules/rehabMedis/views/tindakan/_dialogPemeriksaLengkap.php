...
<div class="col-sm-6">
    <div class="control-group">
        <?php echo CHtml::activeLabel($modTindakan, 'okupasiterapi_id'); ?>
        <div class="controls">
            <?php $this->widget('MyJuiAutoComplete',array(
                        'name'=>'okupasiterapi_id',
                        'value'=>'',
                        'sourceUrl'=> Yii::app()->createUrl('rawatInap/tindakanTRI/getOkupasi'),
                        'options'=>array(
                           'showAnim'=>'fold',
                           'minLength' => 4,
                           'focus'=> 'js:function( event, ui ) {
                                $("#okupasiterapi_id").val( ui.item.label);
                                return false;
                            }',
                           'select'=>'js:function( event, ui ) {
                                setPerawat(ui.item);
                        ...
                        'tombolDialog'=>array("idDialog"=>'dialogOkupasiterapi', 'jsFunction'=>"setPilihOkupasi(1);"),
                        'htmlOptions'=>array(
                            'onblur' => 'if(this.value === ""){ $("#okupasiterapi_id").val(""); updatePerawat(this.value);}',
                            'onkeypress'=>"return $(this).focusNextInputField(event)"),
            )); ?>
        </div>
    </div>
    <div class="control-group">
        <?php echo CHtml::activeLabel($modTindakan, 'terapiwicara_id'); ?>
        <div class="controls">
            <?php $this->widget('MyJuiAutoComplete',array(
                        'name'=>'terapiwicara_id',
                        'value'=>'',
                        'sourceUrl'=> Yii::app()->createUrl('rawatInap/tindakanTRI/GetTerapi'),
                        'options'=>array(
                           'showAnim'=>'fold',
                           'minLength' => 4,
                           'focus'=> 'js:function( event, ui ) {
                                $("#terapiwicara_id").val( ui.item.label);
                                return false;
                            }',
                           'select'=>'js:function( event, ui ) {
                                setPerawat(ui.item);
                        ...
                        'tombolDialog'=>array("idDialog"=>'dialogTerapiwicara', 'jsFunction'=>"setPilihTerapi(1);"),
                        'htmlOptions'=>array(
                            'onblur' => 'if(this.value === ""){ $("#terapiwicara_id").val(""); updateSuster(this.value); }',
                            'onkeypress'=>"return $(this).focusNextInputField(event)"),
            )); ?>
        </div>
    </div>
    <div class="control-group">
        <?php echo CHtml::activeLabel($modTindakan, 'fisioterapi_id'); ?>
        <div class="controls">
            <?php $this->widget('MyJuiAutoComplete',array(
                        'name'=>'fisioterapi_id',
                        'value'=>'',
                        ...
                        'options'=>array(
                           ...
                           'minLength' => 4,
                           'focus'=> 'js:function( event, ui ) {
                                $("#fisioterapi_id").val( ui.item.label);
                                return false;
                            }',
                           'select'=>'js:function( event, ui ) {
                                setPerawat(ui.item);
                        ...
                        'tombolDialog'=>array("idDialog"=>'dialogPerawat', 'jsFunction'=>"setPilihPerawat(5);"),
                        'htmlOptions'=>array(
                            'onblur' => 'if(this.value === ""){ $("#fisioterapi_id").val(""); updateSuster(this.value); }',
                            'onkeypress'=>"return $(this).focusNextInputField(event)"),
            )); ?>
        </div>
    </div>
...
//========= Dialog buat cari dokter =========================
$this->beginWidget('zii.widgets.jui.CJuiDialog', array( // the dialog
    'id'=>'dialogOkupasiterapi',
    'options'=>array(
        'title'=>'Data Okupasi Terapi',
        'autoOpen'=>false,
        'modal'=>true,
        'width'=>500,
        'height'=>500,
...
$datPerawat->kelompokpegawai_id = Params::KELOMPOKPEGAWAI_ID_OKUPASITERAPI;
...
$this->widget('ext.bootstrap.widgets.BootGridView',array(
        ...
        'columns'=>array(
                array(
                    ...
                    'value'=>'CHtml::Link("<i class=\"icon-form-check\"></i>","javascript:void(0);",array("class"=>"btn-small", 
                                    "id" => "selectPerawat",
                                    "onClick" => "pilihOkupasi(".$data->pegawai_id."); return false;"))',
                    //'filter'=>CHtml::activeHiddenField($modTindakan,'tipepaket_id').CHtml::activeHiddenField($modTindakan,'kelaspelayanan_id').CHtml::activeHiddenField($modTindakan,'penjamin_id').CHtml::activeHiddenField($modTindakan,'jenistarif_id'),
                ),
                array(
                    'name'=>'nama_pegawai',
...
//========= Dialog buat cari dokter =========================
$this->beginWidget('zii.widgets.jui.CJuiDialog', array( // the dialog
    'id'=>'dialogTerapiwicara',
    'options'=>array(
        'title'=>'Data Terapi Wicara',
        'autoOpen'=>false,
        'modal'=>true,
        'width'=>500,
        'height'=>500,
...
$datPerawat = new PegawaiV();
$datPerawat->unsetAttributes();
$datPerawat->kelompokpegawai_id = Params::KELOMPOKPEGAWAI_ID_TERAPIWICARA;
if (isset($_GET['PegawaiV'])) {
    $datPerawat->attributes = $_GET['PegawaiV'];
}
$provider = $datPerawat->search();
...
$this->widget('ext.bootstrap.widgets.BootGridView',array(
        ...
        'columns'=>array(
                array(
                    ...
                    'value'=>'CHtml::Link("<i class=\"icon-form-check\"></i>","javascript:void(0);",array("class"=>"btn-small", 
                                    "id" => "selectPerawat",
                                    "onClick" => "pilihTerapi(".$data->pegawai_id."); return false;"))',
                    //'filter'=>CHtml::activeHiddenField($modTindakan,'tipepaket_id').CHtml::activeHiddenField($modTindakan,'kelaspelayanan_id').CHtml::activeHiddenField($modTindakan,'penjamin_id').CHtml::activeHiddenField($modTindakan,'jenistarif_id'),
                ),
                array(
                    'name'=>'nama_pegawai',
...
function pilihPerawat(id) {
    ...
    $.post("<?php echo Yii::app()->createUrl('rawatJalan/tindakan/getPerawat'); ?>", {
        ...
        switch(idPilihPerawat) {
            ...
            // case 3: setPerawat3(res); $("#okupasiterapi_id").val(res.label); break;
            // case 4: setPerawat4(res); $("#terapiwicara_id").val(res.label); break;
            case 5: setPerawat5(res); $("#fisioterapi_id").val(res.label); break;
        }
...
function cekPerawat(){
	...
	var kelompoktindakan_id = $('#RMTindakanPelayananT_'+no+'_keltindakanid').val();
	
	var perawat = $('#RMTindakanPelayananT_'+no+'_perawat_id').val();
	// var perawat2 = $('#RMTindakanPelayananT_'+no+'_perawat2_id').val();
...
function setPilihOkupasi(val) {
    idPilihPerawat = val;
    $("#dialogOkupasiterapi").dialog('open');
}


function pilihOkupasi(id) {
    $("#dialogOkupasiterapi").dialog('close');
    $.post("<?php echo Yii::app()->createUrl('rawatJalan/tindakan/getOkupasi'); ?>", {
        id: id
    }, function(data) {
        var res = data[0];
        ...
        $("#okupasiterapi_id").val(res.label);  
        
    }, 'json');
}

function setPilihTerapi(val) {
    idPilihPerawat = val;
    $("#dialogTerapiwicara").dialog('open');
}


function pilihTerapi(id) {
    $("#dialogTerapiwicara").dialog('close');
    $.post("<?php echo Yii::app()->createUrl('rawatJalan/tindakan/getTerapi'); ?>", {
        id: id
    }, function(data) {
        var res = data[0];
        ...
        console.log(idPilihPerawat);
        setTerapi(res); 
        $("#terapiwicara_id").val(res.label);
        // switch(idPilihPerawat) {
            // case 1: setOkupasi(res); $("#perawat_id").val(res.label); break;
        // }
