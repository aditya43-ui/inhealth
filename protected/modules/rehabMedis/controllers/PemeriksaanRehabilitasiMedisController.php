...
class PemeriksaanRehabilitasiMedisController extends PendaftaranRehabilitasiMedisController
{
  ...
  public function actionPemeriksaan($pendaftaran_id, $pasienmasukpenunjang_id)
  {
    ...
    if (isset($_POST['pasienmasukpenunjang_id'])) {
      ...
      try {
        ...
        if ($this->tindakanpelayanantersimpan && $this->komponentindakantersimpan && $this->hasilpemeriksaantersimpan) {
          ...
          $this->redirect(array('pemeriksaan', 'pendaftaran_id' => $pendaftaran_id, 'pasienmasukpenunjang_id' => $modKunjungan->pasienmasukpenunjang_id));
        } else {
  ...
  {
    $this->pageTitle = Yii::app()->name . " - Pemeriksaan Rehabilitasi Medis";
    $format = new MyFormatter();
    $modKunjungan = new RMPasienMasukPenunjangV;
    $modKunjungan->ruangan_id = Yii::app()->user->getState("ruangan_id");
    $modPemeriksaanRm = new RMTarifpemeriksaanrmruanganV;
    ...
    if (isset($_POST['pasienmasukpenunjang_id'])) {
      ...
      try {
        ...
        if ($this->tindakanpelayanantersimpan && $this->komponentindakantersimpan && $this->hasilpemeriksaantersimpan) {
          $transaction->commit();
          Yii::app()->user->setFlash('success', "Data pemeriksaan rehab medis " . $modPendaftaran->pasien->namadepan . " " . $modPendaftaran->pasien->nama_pasien . " berhasil disimpan !");
  ...
  public function actionHapusTindakanPelayanan()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      try {
        ...
        if ($modTindakan->tindakansudahbayar_id) {
          $hapusTindakan = false;
        } else {
