...
class TindakanController extends MyAuthController
{
  ...
  protected $path_view = 'rehabMedis.views.tindakan.';

  public function actionIndex($pendaftaran_id, $pasienmasukpenunjang_id)
  {
    $ruangan_id = isset($_GET['ruangan_id']) ? $_GET['ruangan_id'] : Yii::app()->user->getState('ruangan_id'); //RND-6244
    $modPendaftaran = RMPendaftaranT::model()->with('jeniskasuspenyakit')->findByPk($pendaftaran_id);
    ...
    if (isset($_POST['RMTindakanPelayananT']) || isset($_POST['TindakanpelayananT'])) {
      ...
      $modTindakans = $this->saveTindakan($modPasien, $modPendaftaran, $penunjang);



      
      if ($this->succesSave)
      $this->redirect(array($this->id . '/', 'pendaftaran_id' => $pendaftaran_id, 'pasienmasukpenunjang_id' => $pasienmasukpenunjang_id));
    }
    
    $modViewBmhp = RMObatalkesPasienT::model()->with('obatalkes')->findAllByAttributes(array('pendaftaran_id' => $pendaftaran_id));
  ...
  public function saveTindakan($modPasien, $modPendaftaran, $penunjang)
  {
    $post = (isset($_POST['TindakanpelayananT'])) ? $_POST['TindakanpelayananT'] : $_POST['RMTindakanPelayananT'];
    ...
    foreach ($post as $i => $item) {
      ...
      if (!empty($item) && (!empty($item['daftartindakan_id']))) {
        ...
        $modTindakans[$i]->pasienmasukpenunjang_id = $penunjang->pasienmasukpenunjang_id;
...
//         var_dump($modTindakans[$i]->attributes, $modTindakans[$i]->errors);
      }
    }
...
//     die;
      ...
      if ($valid && (count((array)$modTindakans) > 0)) {
        foreach ($modTindakans as $i => $tindakan) {
          ...
          if ($tindakan->save()) {
            ...
            // simpan hasil periksa RM
            $periksa = TindakanrmM::model()->findByAttributes(array(
              'daftartindakan_id'=>$tindakan->daftartindakan_id
            ));

            if (!empty($periksa)) {
              $hasil = new HasilpemeriksaanrmT;
              $hasil->attributes = $tindakan->attributes;
              $hasil->attributes = $periksa->attributes;
              $hasil->ruangan_id = $tindakan->ruangan_id;
              $hasil->pasien_id = $tindakan->pasien_id;
              $hasil->jenistindakanrm_id = $periksa->jenistindakanrm_id;
              $hasil->tindakanpelayanan_id = $tindakan->tindakanpelayanan_id;
              $hasil->pegawai_id = $tindakan->dokterpemeriksa1_id;
              $hasil->tglpemeriksaanrm = $tindakan->tgl_tindakan;
              $hasil->nohasilrm = MyGenerator::noHasilPemeriksaanRM();
              $hasil->kunjunganke = 1;
              ...
              // var_dump($hasil->errors, $hasil->attributes, $periksa->attributes, $tindakan->attributes);
            }
          ...
          if (isset($_POST['paketBmhp'])) {
            if (count((array)$_POST['paketBmhp']) > 0) {
              ...
              foreach ($_POST['paketBmhp'] as $i => $postDetail) {
                $modDetails[$i] = new RMObatalkesPasienT();
    ...
    /**
    * method untuk menyimpan data pasien ke unit lain RMPasienKirimkeUnitLainT
    * digunakan di :
    * 1. rawatJalan/laboratorium/index
    * @param object $modPendaftaran model PendaftaranT
    * @return \RMPasienKirimKeUnitLainT 
    */
    protected function simpanPasienKirimKeUnitLain($tindakan)
    {
        $modKirimKeUnitLain = new RMPasienKirimKeUnitLainT;
    ...
    protected function savePermintaanPenunjang($tindakan,$modKirimKeUnitLain)
    {
        ...
        $modPermintaan = new RMPermintaanPenunjangT;
  ...
  public function actionAjaxDeleteTindakanPelayanan()
  {
    ...
    if (Yii::app()->request->isAjaxRequest) {
      ...
      $transaction = Yii::app()->db->beginTransaction();
      try {
        $obatAlkesT = RMObatalkesPasienT::model()->findAllByAttributes(
          array(
        ...
        if (count((array)$obatAlkesT) > 0) {
          ...
          $deleteObatPasien = RMObatalkesPasienT::model()->deleteAllByAttributes(
            array(
              'tindakanpelayanan_id' => $idTindakanpelayanan
            )
          );
  ...
  {
    // var_dump($paketBmhp);
    $modObatAlkesPasien = new RMObatalkesPasienT();
...
//                                                                                                               'kelompokumur_id'=>$idKelompokUmur));
  ...
  public function actionPrintTindakan($id)
  {
    ...
    $format = new MyFormatter;
    $modPendaftaran = RMPendaftaranT::model()->with('jeniskasuspenyakit')->findByPk($id);
    $modPasien = RMPasienM::model()->findByPk($modPendaftaran->pasien_id);
    $modViewTindakans = RMTindakanPelayananT::model()
      ->with(
        'daftartindakan',
    ...
    $modViewBmhp = RMObatalkesPasienT::model()->with('obatalkes')->findAllByAttributes(array('pendaftaran_id' => $id));
  ...
  public function actionHapus()
  {
    ...
    try {
      $model = RMTindakanPelayananT::model()->findByPk($_POST['tindakanpelayanan_id']);


      RMTindakanPelayananT::model()->deleteByPk($model->tindakanpelayanan_id);
