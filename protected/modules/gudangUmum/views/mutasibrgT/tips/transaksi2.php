<span class="required"><i>Bagian dengan tanda * harus diisi.</i></span>
<p>
<table style="width:100%; padding: none; border: none;">
    <tr>
        <td style="width: 20px;">1.</td>
        <td>Icon <i class="entypo-calendar"></i><i class="icon-time"></i> untuk menentukan tanggal dan waktu pendaftaran.</td>
    </tr>
    <tr>
        <td>2</td>
        <td>Icon <i class="icon-form-silang"></i> untuk membatalkan.</td>
    </tr>
    <tr>
        <td>3</td>
        <td>Gunakan icon ini <span class="add-on">
                <a id="" href="javascript:void(0);" onclick="$(" #dialogObatAlkes").dialog("open");return false;">
                    <i class="icon-list"></i>
                    <i class="entypo-search"></i>
                </a>
            </span> berfungsi untuk mencari dan menampilkan list datanya.</td>
    </tr>
    <tr>
        <td>4</td>
        <td>Gunakan tombol ini <button id="btnAddKelurahan" class="btn btn-primary" type="button" name="yt3" rel="tooltip" onkeypress="" onclick="" data-original-title="Klik untuk menambah Kelurahan">
                <i class="icon-plus icon-white"></i>
            </button> berfungsi untuk menambahkan data</td>
    </tr>
    <tr>
        <td>5</td>
        <td>Gunakan tombol ini <button class="btn btn-danger" name="yt0" onkeypress="return formSubmit(this,event)" type="submit">
                <i class="entypo-check"></i>
                Simpan
            </button> berfungsi untuk menyimpan data.</td>
    </tr>

    <tr>
        <td>6.</td>
        <td>Gunakan tombol ini <a class="btn btn-default" href="/simrs/index.php?r=rawatInap/pasienRawatInap/formTindakLanjutDariPasienRI">
                <i class="entypo-arrows-ccw"></i>
                Ulang
            </a> untuk mengulang kembali inputan.</td>
    </tr>
    </tr>
    <tr>
        <td>7</td>
        <td>tombol ini <a class="btn btn-info disabled"><i class="entypo-print"></i> Print</a> berarti tidak dapat digunakan (tidak aktif), sedangkan
            <a class="btn btn-info"><i class="entypo-print"></i> Print</a> berarti tombol dapat digunakan (aktif).
        </td>
    </tr>
    <tr>
        <td>8</td>
        <td>Gunakan tombol ini <a class="btn btn-info">
                <i class="entypo-print"></i>
                Print
            </a> untuk mencetak data.</td>
    </tr>
</table>
</p>