...
if (isset($caraPrint)) {
    ...
    // echo '<pre>'; var_dump($data->criteria); die;
    echo "
        <style>
            .border th, .border td{
...
$this->widget($table, array(
    ...
    'columns' => array(
        ...
        array(
            'header' => 'No. Bed',
            'headerHtmlOptions' => array('style' => 'text-align: left;vertical-align:middle; width: 75px;'),
        ...
        array(
            ...
            'headerHtmlOptions' => array('style' => 'text-align: left;vertical-align:middle; width: 30px;'),
            'value' => function($data) use (&$tglpesanmenu, &$modPesan){
                if($tglpesanmenu != date('Y-m-d')) {
                    if(!empty($modPesan->pendaftaran)) {
        ...
        array(
            'header' => 'Kelas',
            'headerHtmlOptions' => array('style' => 'text-align: left;vertical-align:middle; width: 10px;'),
            'value' => function($data) use (&$tglpesanmenu, &$modPesan){
                if($tglpesanmenu != date('Y-m-d')) {
                    if(!empty($modPesan->pendaftaran->admisi)) {
                        // if(isset($caraPrint)) {
                            echo str_replace("Kelas ", "", $modPesan->pendaftaran->admisi->kelaspelayanan->kelaspelayanan_nama) ;
                        // } else {
                    ...
                    // if(isset($caraPrint)) {
                        echo str_replace("Kelas ", "", $data->kelaspelayanan_nama);
                    // } else {
        ...
        array(
            'header' => 'Sore',
            'headerHtmlOptions' => array('style' => 'text-align: left;vertical-align:middle; width: 100px;'),
            'value' => function($data) {
