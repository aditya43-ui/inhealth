...
class PemesananPeralatanSterilRuanganTController extends MyAuthController
{
  ...
  public function actionIndex($linkHalaman = null)
  {
    ...
    if (!empty($model->pegpemesan_id))
      $model->pegawaimemesan_nama = $modLogin->pegawai->nama_pegawai;
    ...
    if($linkHalaman == null) $linkHalaman = CustomFunction::getUrlByMenuID(3016);

    $this->render($this->path_view . 'index', array(
      'model' => $model,
      'format' => $format,
      'ruangan_id' => $ruangan_id,
      'ruangan_cssd' => $ruangan_cssd,
      'linkHalaman' => $linkHalaman
    ));
  }

  public function simpanNotifPemesananPeralatanSteril($model)
  {
    ...
    $judul = 'Pemesanan Peralatan Steril - ' . $model->pesanperlinensteril_no;

    $isi = "Pemesan : " . $asal->ruangan_nama . "<br/>Tgl. Pemesanan : " . MyFormatter::formatDateTimeForUser($model->pesanperlinensteril_tgl)
      . "<br>No. Pemesanan : ";
    $isi .= $model->pesanperlinensteril_no;
  ...
  public function actionLoadFormLine()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      $modDetail->namaPeralatan = empty($modBarang) ? null : $modBarang->barang_nama;
