Bagian dengan tanda bintang <span class="required">*</span> harus diisi.
<table>
    <tr>       
        <td> 
            1. Tombol  <div class="btn btn-danger" name="yt0" type="button"><i class="entypo-search"></i> Tampilkan</div>
            berfungsi untuk mencari data berdasarkan pencarian stok obat alkes opname dan menampilkannya pada tabel stok obat alkes opname.
        </td>
    </tr>
    <tr>       
        <td> 
            2. Tombol  <div class="btn btn-danger" name="yt0" type="button"><i class="entypo-search"></i></div>
            berfungsi untuk mencari data berdasarkan form pencarian pada tabel stok obat alkes opname.
        </td>
    </tr>
    <tr>       
        <td> 
            3. Tombol  <div class="btn btn-default" name="yt0" type="button"><i class="entypo-arrows-ccw"></i></div>
            berfungsi untuk membersihkan form pencarian pada tabel stok obat alkes opname.
        </td>
    </tr>
    <tr>
        <td>
            4. Icon <span class="add-on"><a id="" href="javascript:void(0);"><i class="icon-list"></i><i class="icon-search"></i></a></span> 
            berfungsi untuk mencari dan menampilkan daftar data sesuai yang diketikan serta menampilkan dialog box jika diklik.
        </td>
    </tr>
    <tr>
        <td>
             5. Tombol  <div class="btn btn-primary" name="yt0" type="button"><i class="icon-ok icon-white"></i> Simpan</div>
            berfungsi untuk menyimpan.
        </td>
    </tr>
     <tr>
        <td>
             6. Tombol  <div class="btn btn-danger" name="yt0" type="button"><i class="entypo-arrows-ccw"></i> Ulang</div>
            berfungsi untuk mengulang kembali inputan.
        </td>
    </tr>
     <tr>
        <td>
            7. Tombol  <div class="btn btn-info" name="yt0" type="button"><i class="entypo-print"></i></div> dapat digunakan (sedang aktif)
            sedangkan tombol <div class="btn btn-info" name="yt0" type="button" disabled="disabled"><i class="entypo-print"></i></div> tidak dapat digunakan (tidak aktif).
        </td>
    </tr>
     <tr>
        <td>
             8.  Tombol  <div class="btn btn-info" name="yt0" type="button"><i class="entypo-print"></i> Print </div>
            berfungsi untuk mencetak Stock Opname.
        </td>
    </tr>     
</table>
  