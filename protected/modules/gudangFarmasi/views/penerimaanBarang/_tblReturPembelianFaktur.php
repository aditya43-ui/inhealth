        ...
        $totalSubTotal = 0;
            foreach ($modPenerimaanDet as $i => $mod) {
                ...
                <tr>
                    ...
                    <td><?php echo $mod->obatalkes->obatalkes_kode; ?>
                    <td><?php echo $mod->obatalkes->obatalkes_nama; ?>
                    </td>
					<td><?php echo CHtml::textField('jmlterima', $mod->jmlterima, array('class'=>'span1 integer2 qtyterima', 'style'=>'text-align:right;', 'readonly'=>true));?>
                        <?php echo " ".$mod->obatalkes->satuankecil->satuankecil_nama;?></td>
                    <td><?php echo CHtml::textField('GFReturDetailT['.$i.'][jmlretur]', $mod->jmlterima, array('class'=>'span1 integer2 qty', 'style'=>'text-align:right;', 'onblur'=>'validasiQty(this); hitungTabel();', 'onKeypress'=>'return formSubmit(this,event)'));?>
                        <?php echo " ". $mod->obatalkes->satuankecil->satuankecil_nama;?></td>
                    <td>
                        <?php echo CHtml::textField('GFReturDetailT['.$i.'][harganettoretur]', MyFormatter::formatNumberForPrint($mod->harganettofaktur,2),array('class'=>'span2 nettoretur float2', 'onchange'=>'hitungTabel();', 'readonly'=>true));?>
                    </td>
                    <td>
                        <?php echo CHtml::textField('GFReturDetailT['.$i.'][jmldiscount]', $mod->jmldiscount,array('class'=>'diskonretur span2 float2', 'readonly'=>true));?>
                    </td>
                    <td>
                        <?php echo CHtml::hiddenField('GFReturDetailT['.$i.'][persenppn]', $mod->persenppnfaktur,array('class'=>'persenppn'));?>
                        <?php echo CHtml::textField('GFReturDetailT['.$i.'][hargappnretur]', 0,array('class'=>'span2 jmlppnretur float2', 'readonly'=>true));?>
                    </td>
                    <td>
                        <?php echo CHtml::hiddenField('GFReturDetailT['.$i.'][persenpph]', (!empty($mod->persenpphfaktur)?$mod->persenpphfaktur:0),array('class'=>'persenpph'));?>
                        <?php echo CHtml::textField('GFReturDetailT['.$i.'][hargapphretur]', 0,array('class'=>'span2 jmlpphretur float2', 'readonly'=>true));?>
                    </td>
                    <td>
                        <?php // echo CHtml::hiddenField('GFReturDetailT['.$i.'][harganettoretur]', MyFormatter::formatNumberForPrint($mod->harganettofaktur,2),array('class'=>'nettoretur float2'));?>
                        <?php // echo CHtml::hiddenField('GFReturDetailT['.$i.'][hargappnretur]', $mod->persenppnfaktur);?>
                        <?php // echo CHtml::hiddenField('GFReturDetailT['.$i.'][hargapphretur]', (!empty($mod->persenpphfaktur)?$mod->persenpphfaktur:0));?>
                        <?php // echo CHtml::hiddenField('GFReturDetailT['.$i.'][jmldiscount]', $mod->jmldiscount,array('class'=>'diskonretur'));?>
                        <?php echo CHtml::textField('GFReturDetailT['.$i.'][hargasatuanretur]', MyFormatter::formatNumberForPrint($mod->hargasatuan,2), array('class'=>'span2 harga float2', 'style'=>'text-align:right;', 'readonly'=>true,'onchange'=>'hitungTabel();'));?>
                    </td>
                    <td>
                        <?php
//                            $subTotal = $mod->jmlterima * $mod->hargasatuan;
//                            $totalSubTotal += $subTotal;
	...
	<tfoot>
		<tr>
            <td colspan="11" style="text-align: right;"><b>Total Retur</b></td>
            <td><?php 
    ...
    function hitungTabel(){
        ...
        $('#tblReturPembelian tbody').find('tr').each(
            function(){
                ...
                var persenppn = unformatNumber($(this).find('.persenppn').val());
                var persenpph = unformatNumber($(this).find('.persenpph').val());
                var netto = unformatNumber($(this).find('.nettoretur').val());
                
                var jmlTotal = (netto * qty);
                var jmlppn = (((jmlTotal - diskon) * persenppn)/100);
                var jmlpph = (((jmlTotal - diskon) * persenpph)/100);
                
                var subtotal = (jmlTotal - diskon + jmlppn + jmlpph);
                    
                total += subtotal;
					
                $(this).find('.subtotal').val(formatThousandDecimal(Math.round(subtotal)));                
                $(this).find('.jmlppnretur').val(formatThousandDecimal(Math.round(jmlppn)));                
                $(this).find('.jmlpphretur').val(formatThousandDecimal(Math.round(jmlpph)));                
            }
        );

		$("#GFReturPembelianT_totalretur").val(formatNumber(Math.round(total)));
//		$("#totTagihan").val(formatNumber(Math.round(total)));
