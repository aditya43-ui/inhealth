 ...
 */
class AsesmenResikoJatuhController extends MyAuthController
{
   ...
   */
  public function actionIndex($pendaftaran_id, $skoringresikojatuh_id = null)
  {
    ...
    $modResikoJatuh = SkoringresikojatuhT::model()->findAllByAttributes(array('pendaftaran_id' => $pendaftaran_id), ['order' => 'create_time desc']);
    ...
    if (!empty($skoringresikojatuh_id)) {
      $model = SkoringresikojatuhT::model()->findByPk($skoringresikojatuh_id);
    }
    ...
    if (isset($_POST['SkoringresikojatuhT'])) {
      ...
      try {
        ...
        if($model->jenisresikojatuh == 'anak'){
          $model->totalskor = $_POST['SkoringresikojatuhT']['totalskor_anak'];
          $model->totalskor_keterangan = $_POST['SkoringresikojatuhT']['totalskor_keterangan_anak'];
        }
  ...
  public function actionPrint($id)
  {
    ...
    $print_view = in_array(trim(strtolower($modPasien->namadepan)), array("an.", "by.")) ? "printAnak" : "print";

    $this->render($this->path_view . $print_view, array(
      'modPendaftaran' => $modPendaftaran,
      'modPasien' => $modPasien,
      'model' => $model,
    ));
   ...
   * @param type $pendaftaran_id
   */
  public function actionDetail($pendaftaran_id, $skor_id)
  {
    $this->layout = '//layouts/printWindows';
    $format = new MyFormatter();
    $modPendaftaran = PendaftaranT::model()->findByPk($pendaftaran_id);
    $modResikoJatuh = SkoringresikojatuhT::model()->findByAttributes(array('pendaftaran_id' => $pendaftaran_id, 'skoringresikojatuh_id' => $skor_id));
