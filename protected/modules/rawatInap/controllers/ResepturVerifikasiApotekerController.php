...
class ResepturVerifikasiApotekerController extends MyAuthController
{
    public $layout = '//layouts/iframe';
    public $path_view = 'rawatInap.views.resepturVerifikasiApoteker.';
    public $successSave = false;
    public $reseptur_id;
    public $modSMS = null;
    public $keterangan = '';
    public $is_obatkronis = '';
    public $ruangan_apotek_tujuan_id = '';
    ...
    public function actionIndex($pendaftaran_id, $pasienadmisi_id = null, $salin = null)
    {
        ...
        // echo '<pre>'; var_dump($modPendaftaran->penjamin->penjamin_nama); die; 
        ...
        if (!empty($kelompokpegawai_id)) {
            ...
            if ($pegawai !== null) {
                ...
                $kelompokpegawai_id = Yii::app()->user->getState('kelompokpegawai_id');
        }
        
        if ($kelompokpegawai_id == Params::KELOMPOKPEGAWAI_ID_DOKTER_TETAP || $kelompokpegawai_id == Params::KELOMPOKPEGAWAI_ID_TENAGA_NONKEPERAWATAN) {
        ...
        $modMasterRuanganTujuan = RuanganapotektujuanK::model()->findByAttributes(['ruanganpelayanan_id' => Yii::app()->user->getState('ruangan_id')]);

        if(!empty($modMasterRuanganTujuan)) {
            $modReseptur->ruangan_id = $modMasterRuanganTujuan->ruanganapotektujuan_id;

            // jika alih irna true atau dicentang maka akan berpindah rungan tujuannya ke ruangan yang sudah di seting di master
            if($modMasterRuanganTujuan->is_alih && !empty($modMasterRuanganTujuan->alihkan_jam) && !empty($modMasterRuanganTujuan->alihke_ruanganapotektujuan_id)) {
                if(date('H:i:s') >= $modMasterRuanganTujuan->alihkan_jam) {
                    $modReseptur->ruangan_id = $modMasterRuanganTujuan->alihke_ruanganapotektujuan_id;
                }
            }
        } else {
        ...
        if(Yii::app()->controller->module->id == 'rawatDarurat') {
            $modReseptur->ruangan_id = 84;
        }

        if($modPendaftaran->carabayar_id == 3 && Yii::app()->controller->module->id == 'rawatJalan') {
            $modReseptur->ruangan_id = 237;
        }

        $this->ruangan_apotek_tujuan_id = $modReseptur->ruangan_id;
        ...
        // $modReseptur->ruangan_id = $ruanganDepo->ruangandepofarmasi_id ?? Params::RUANGAN_ID_APOTEK_1;
    ...
    protected function broadcastNotifReseptur($modPendaftaran)
    {
        ...
        $tanggalResep = explode(' ', MyFormatter::formatDateTimeForDb($reseptur->tglreseptur));

        if(isset($tanggalResep[0])) {
            $tanggalResep = $tanggalResep[0];
        } else {
            $tanggalResep = date('Y-m-d', strtotime($reseptur->tglreseptur));
        }
        $link = $this->createUrl('/farmasiApotek/InformasiPasienResep/Index', array(
            'FAInformasiresepturV[tgl_awal]' => $tanggalResep,
            'FAInformasiresepturV[tgl_akhir]' => $tanggalResep,
    ...
    protected function saveReseptur($post, $modPendaftaran)
    {
        ...
        if(Yii::app()->user->getState('kelompokpegawai_id') == Params::KELOMPOKPEGAWAI_ID_PARAMEDIS_KEPERAWATAN && !empty($modPendaftaran->admisi)) {
            $reseptur->pegawai_id = $modPendaftaran->admisi->dokterpenerima_id;
        } else {
            $reseptur->pegawai_id = isset($post['RJResepturT']['pegawai_id']) ? $post['RJResepturT']['pegawai_id'] : Yii::app()->user->getState('pegawai_id');
        }
        ...
        $reseptur->pasienadmisi_id = !empty($modPendaftaran->pasienadmisi_id) ? $modPendaftaran->pasienadmisi_id : null;
        ...
        if ($reseptur->validate()) {
            if ($reseptur->save()) {
                if (!in_array($instalasi_id, array(Params::INSTALASI_ID_PI, Params::INSTALASI_ID_RI))) {
                    // proses update status periksa hanya di lakukan manual di menu daftar pasien
    ...
    protected function saveDetailReseptur($post, $reseptur)
    {
        ...
        // echo '<pre>'; var_dump($deleteDetailResep); die;
        ...
        foreach ($post['RJResepturDetailT'] as $i => $detailreseptur) {
            ...
            $detail->waktupemberian_ranap = $detailreseptur['waktupemberian_ranap'];
            $detail->is_verifkasiapoteker = 1;
            if($detailreseptur['racikan_id'] == Params::RACIKAN_ID_RACIKAN) {
                // racikan
                $detail->jumlahpermintaan_obatracikan = MyFormatter::formatNumberForDb($detailreseptur['jumlahpermintaan_obatracikan']);
                $detail->permintaan_dosis = $detailreseptur['permintaan_temp'];

            } else {
                // non racikan
                $detail->jumlahpermintaan_obatnonracikan = MyFormatter::formatNumberForDb($detailreseptur['jumlahpermintaan_obatracikan']);
            }
            ...
            // $detail->hargajual_reseptur = round((($detail->hargasatuan_reseptur * $detail->qty_reseptur)+$detail->jumlahppn) * 100) / 100;
            // $detail->harganetto_reseptur = is_numeric($detail->harganetto_reseptur) ? $detail->harganetto_reseptur : MyFormatter::formatRupiahForDb($detail->harganetto_reseptur);
            // $detail->persenppnjual = is_numeric($detail->persenppnjual) ? $detail->persenppnjual : MyFormatter::formatRupiahForDb($detail->persenppnjual);
            // $detail->jumlahppn = is_numeric($detail->jumlahppn) ? $detail->jumlahppn : MyFormatter::formatRupiahForDb($detail->jumlahppn);
            // $detail->hargasatuan_reseptur = is_numeric($detail->hargasatuan_reseptur) ? $detail->hargasatuan_reseptur : MyFormatter::formatRupiahForDb($detail->hargasatuan_reseptur);
            // $detail->kekuatan_reseptur = is_numeric($detail->kekuatan_reseptur) ? $detail->kekuatan_reseptur : MyFormatter::formatRupiahForDB($detail->kekuatan_reseptur);
    ...
    public function actionAutocompleteObatApi()
    {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            $returnVal = [];
            foreach ($models->searchObat()->data as $i => $model) {
                if($model['StFornas'] == 0) {
                ...
                $returnVal[$i]['value'] = $model['Kode'];
                $returnVal[$i]['kode'] = $model['Kode'];
                $returnVal[$i]['jenis'] = $model['jenis'];
                $returnVal[$i]['stFornas'] = $model['StFornas'];
                $returnVal[$i]['HJual'] = $model['HJual'];
                $returnVal[$i]['satuan'] = $model['satuan'];
                $returnVal[$i]['nama'] = $model['Nama'];
                $returnVal[$i]['HPP'] = $model['HPP'];
    ...
    public function actionGetObat()
    {
        ...
        if(isset($_POST['kode_obat']) && isset($_POST['sumberdana']) && isset($_POST['stfornas'])) {
            ...
            if(!empty($modSumberdana)) {
                ...
                $data['sukses'] = 1;
                $modObat = ObatalkesM::model()->findByAttributes(['kodeobat_inventory' => $_POST['kode_obat'], 'stfornas' => $_POST['stfornas'], 'sumberdana_id' => $modSumberdana->sumberdana_id]);
                if(empty($modObat)) {
                    $satuan = isset($_POST['satuan']) ? $_POST['satuan'] : null;
                    $modSatuanKecil = SatuankecilM::model()->findByAttributes(['satuankecil_namalain' => $satuan]);
                    if(!empty($modSatuanKecil)) {
                        $result = $this->insertObat($modSumberdana, $_POST, $modSatuanKecil);
    
                        if($result['save']) {
                           $modObat = $result['modObat']; 
                        } else {
                            $modObat = null;
                        ...
                        $modObat = null;
                        $data['pesan'] .= 'Satuan Kecil Tidak Ditemukan. dengan pencarian nama = ' . $satuan . '. ';
                    }

                    // echo '<pre>';var_dump($result['modObat']);die;
                }
            } else {
                $data['sukses'] = 0;
            ...
            if(!empty($modObat)) {
                ...
                $data['obatalkes']['kodesementara'] = $modObat->obatalkes_kode;
    ...
    protected function insertObat($modSumberdana, $post, $modSatuanKecil) {
        $data['pesan'] = '';
        $data['save'] = 0;
        try {
            // proses menambahkan obat
            $transaction = Yii::app()->db->beginTransaction();
            $sql = "SELECT MAX(obatalkes_id) as obatalkes_id
                    FROM obatalkes_m";
            $idMax = Yii::app()->db->createCommand($sql)->queryRow();

            $count = ObatalkesM::model()->count("obatalkes_kode ilike '%kodesementara-%'");

            $modObat = new ObatalkesM();
            $modObat->obatalkes_id = $idMax['obatalkes_id']+1;
            $modObat->sumberdana_id = $modSumberdana->sumberdana_id;
            $modObat->satuankecil_id = $modSatuanKecil->satuankecil_id;
            $modObat->satuanbesar_id = Params::SATUAN_BESAR_BELUM_DI_SET;
            $modObat->obatalkes_kode = 'kodesementara-' . $count+1;
            $modObat->obatalkes_nama = isset($post['nama']) ? $post['nama'] : '';
            $modObat->obatalkes_namalain = isset($post['nama']) ? $post['nama'] : '';
            $modObat->ppn_persen = 0;
            $modObat->harganetto = 0;
            $modObat->hargajual = isset($post['harga_jual']) ? $post['harga_jual'] : '';
            $modObat->hargamaksimum = 0;
            $modObat->hargaminimum = 0;
            $modObat->hargaaverage = 0;
            $modObat->margin = 0;
            $modObat->discount = 0;
            $modObat->minimalstok = 0;
            $modObat->discountinue = 'f';
            $modObat->mintransaksi = 'f';
            $modObat->obatalkes_aktif = 'f';
            $modObat->obatalkes_farmasi = 'f';
            $modObat->marginresep = 0;
            $modObat->jasadokter = 0;
            $modObat->hjanonresep = 0;
            $modObat->hpp = isset($post['HPP']) ? $post['HPP'] : '';
            $modObat->jnskelompok = '';
            $modObat->stfornas = isset($post['stfornas']) ? $post['stfornas'] : '';
            $modObat->kodeobat_inventory = isset($post['kode_obat']) ? $post['kode_obat'] : '';
            if($modObat->validate()) {
                if($modObat->save()) {
                    $sql = "SELECT MAX(stokopnamedet_id) as stokopnamedet_id
                            FROM stokopnamedet_t";
                    $idMax = Yii::app()->db->createCommand($sql)->queryRow();

                    $modStokOpname = new StokopnamedetT;
                    $modStokOpname->stokopnamedet_id = $idMax['stokopnamedet_id']+1;
                    $modStokOpname->satuankecil_id = $modObat->satuankecil_id;
                    $modStokOpname->sumberdana_id = $modObat->sumberdana_id;
                    $modStokOpname->stokopname_id = 1;
                    $modStokOpname->obatalkes_id = $modObat->obatalkes_id;
                    $modStokOpname->volume_fisik = '100000';
                    $modStokOpname->volume_sistem = 0;
                    $modStokOpname->hargasatuan = $modObat->hargajual;
                    $modStokOpname->jumlahharga = ($modObat->hargajual * $modStokOpname->volume_fisik);
                    $modStokOpname->harganetto = 0;
                    $modStokOpname->jumlahnetto = 0;
                    $modStokOpname->tglkadaluarsa = '2028-01-01';
                    $modStokOpname->kondisibarang = 'Baik';
                    $modStokOpname->tglperiksafisik = date('Y-m-d H:i:s');
                    $modStokOpname->jmlselisihstok = '10000';
                    if($modStokOpname->validate()) {
                        if($modStokOpname->save()) {

                            $ruanganDepo = [84, 90, 222, 223, 225, 226, 229, 231, 235, 237, 240, 241, 243, 244, 245, 246, 249, 251, 252, 255, 257, 258, 260, 263, 269];
                            
                            foreach($ruanganDepo as $val) {
                                $modStokObatAlkes = new StokobatalkesT();
                                $modStokObatAlkes->ruangan_id = $val;
                                $modStokObatAlkes->stokopnamedet_id = $modStokOpname->stokopnamedet_id;
                                $modStokObatAlkes->obatalkes_id = $modObat->obatalkes_id;
                                $modStokObatAlkes->tglkadaluarsa = '2028-01-01';
                                $modStokObatAlkes->tglstok_in = date('Y-m-d H:i:s');
                                $modStokObatAlkes->qtystok_in = '100000';
                                $modStokObatAlkes->qtystok_out = 0;
                                $modStokObatAlkes->harganetto = 0;
                                $modStokObatAlkes->persendiscount = 0;
                                $modStokObatAlkes->jmldiscount = 0;
                                $modStokObatAlkes->persenppn = 11;
                                $modStokObatAlkes->persenpph = 0;
                                $modStokObatAlkes->persenmargin = 0;
                                $modStokObatAlkes->jmlmargin = 0;
                                $modStokObatAlkes->stokoa_aktif = 't';
                                $modStokObatAlkes->create_time = date('Y-m-d H:i:s');
                                $modStokObatAlkes->update_time = date('Y-m-d H:i:s');
                                $modStokObatAlkes->create_loginpemakai_id = 1;
                                $modStokObatAlkes->update_loginpemakai_id = 1;
                                $modStokObatAlkes->create_ruangan = 80;
                                $modStokObatAlkes->satuankecil_id = 45;
                                $modStokObatAlkes->tglterima = date('Y-m-d H:i:s');
                                $modStokObatAlkes->sumberdana_id = 0;

                                if($modStokObatAlkes->validate()) {
                                    $is_validate = true;
                                    if($modStokObatAlkes->save()) {
                                        $is_save = true;
                                    } else {
                                        $is_save = false;
                                    }
                            ...
                            if($is_save && $is_validate) {
                                $transaction->commit();
                                $data['save'] = 1;
                            } else {
                                if(!$is_save) {
                                    $data['pesan'] = 'Gagal Saat Menambahkan Stok Obat Alkes [save]';
                                } else {
                                    $data['pesan'] = 'Gagal Saat Menambahkan Stok Obat Alkes [validasi]';
                                }
                            }
                        } else {
                            $data['pesan'] = 'Gagal Saat Menambahkan Stok Opname Obat [save]';
                        }
                    } else {
                        $data['pesan'] = 'Gagal Saat Menambahkan Stok Opname Obat [validasi]';
                    }
                } else {
                    $data['pesan'] = 'Gagal Saat Menambahkan Obat [save]';
                }
            } else {
                $data['pesan'] = 'Gagal Saat Menambahkan Obat [validasi]';
            }
        } catch (Exception $exc) {
            $data['pesan'] = $exc->getMessage();
            $transaction->rollback();
        }
        
        $data['modObat'] = $modObat;
    ...
    public function actionSetDetailPaketObat()
    {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            foreach ($detailPaket as $key => $value) {
                ...
                $modResepturDetail->permintaan_dosis = $value->permintaan_dosis;
    ...
    public function actionSetFormObatAlkesPasien()
    {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            $st_fornas = isset($_POST['st_fornas']) ? $_POST['st_fornas'] : null;
            $hargastuan_reseptur = isset($_POST['hargastuan_reseptur']) ? $_POST['hargastuan_reseptur'] : null;
            $namaObatNonRacikDariApi = isset($_POST['namaObatNonRacikDariApi']) ? $_POST['namaObatNonRacikDariApi'] : null;
            $sumberdana_id = isset($_POST['sumberdana_id']) ? $_POST['sumberdana_id'] : null;
            ...
            if(!empty($sumberdana_id)) {
                ...
                $modSumberdana = SumberdanaM::model()->findByPk($sumberdana_id);
                if(!empty($modSumberdana)) {
                    $modResepturDetail->sumberdana_nama = $modSumberdana->sumberdana_nama;
                }
            } else {
                $modResepturDetail->sumberdana_id = $modObatAlkes->sumberdana_id;
            }
            if(!empty($namaObatNonRacikDariApi)) {
                // dari api
                $modResepturDetail->obatalkes_nama = $namaObatNonRacikDariApi;
            } else {
            ...
            $modResepturDetail->waktupemberian_ranap = $waktu;
            ...
            if(!empty($hargastuan_reseptur)) {
                // dari api
                $modResepturDetail->hargasatuan_reseptur = $hargastuan_reseptur;
            } else {
                $modResepturDetail->hargasatuan_reseptur = number_format($modResepturDetail->hargasatuan_reseptur, 2, ",", ".");
            }

            // dari api
            $modResepturDetail->st_fornas = $st_fornas;
