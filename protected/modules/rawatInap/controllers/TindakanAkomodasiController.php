...
class TindakanAkomodasiController extends MyAuthController
{
  ...
  protected $path_view = 'rawatInap.views.tindakanAkomodasi.';
  ...
  public function actionIndex($pendaftaran_id)
  {
    ...
    $modViewTindakans = RJTindakanPelayananT::model()
      ...
      ->findAllByAttributes(array('pendaftaran_id' => $pendaftaran_id), array(
        'condition'=>'daftartindakan.daftartindakan_akomodasi = true and nopelayanan is not null'
      ));
    $modTindakans = array();
    $modTindakan = new RJTindakanPelayananT;
    $modTindakan->tarifcyto_tindakan = 0;
    $modTindakan->dokterpemeriksa1_id = null; // $modPendaftaran->pegawai_id;
    $modTindakan->dokterpemeriksa1Nama = null; // $modPendaftaran->pegawai->NamaLengkap;
    ...
    // load masuk kamar
    $masukKamar = MasukkamarT::model()->findAllByAttributes(array(
        'pasienadmisi_id'=>$modPendaftaran->pasienadmisi_id,
    ), array(
        'order'=>'tglmasukkamar asc'
    ));

    foreach ($masukKamar as $idx => $item) {
        ...
        // foreach ($list_masuk_res as $item_masuk) {
          // echo "Tgl tarif : ".$item_masuk['tgl_tindakan_awal']." x ".$item_masuk['tgl_tindakan_akhir']." - qty : ".($item_masuk['is_setengah'] ? "0.5" : "1")." - masukkamar_id : ".$item_masuk['masukkamar_id']."\n";
        
          $det = RJTindakanPelayananT::model()->findByAttributes(array(
          ...
          if (empty($det)) {
            ...
            $daftartindakan = TariftindakanperdaruanganV::model()->findByAttributes(array(
            ...
            $det = new RJTindakanPelayananT;
            
            /*
            $det->is_setengah = $item_masuk['is_setengah'];
            if ($det->is_setengah) {
              $daftartindakan->harga_tariftindakan = $daftartindakan->harga_tariftindakan * 0.5;
            }
            */

            if (!empty($daftartindakan)) {
              $det->pendaftaran_id = $daftartindakan->daftartindakan_id;
              $det->attributes = $daftartindakan->attributes;
              $det->tarif_satuan = $daftartindakan->harga_tariftindakan;
              $det->tarif_tindakan = $daftartindakan->harga_tariftindakan * $det->qty_tindakan;
            } else {
              $det->pendaftaran_id = null;
              $det->tarif_satuan = 0;
              $det->tarif_satuan = 0;
            }
            $det->pasien_id = $modPendaftaran->pasien_id;
            $det->pendaftaran_id = $modPendaftaran->pendaftaran_id;
            $det->pasienadmisi_id = $modPendaftaran->pasienadmisi_id;
            $det->masukkamar_id = $item->masukkamar_id;
            $det->attributes = $item->attributes;
            $det->tgl_tindakan = MyFormatter::formatDateTimeForUser($item_masuk['tgl_tindakan_awal']);
            $det->qty_tindakan = $item_masuk['qty'];
            $det->cyto_tindakan = false;
  
          } else {
            // $det->is_setengah = $item_masuk['is_setengah'];
            $det->qty_tindakan = $item_masuk['qty'];
            $det->tarif_tindakan = $det->tarif_satuan * $det->qty_tindakan;
          }

          $det->daftartindakanNama = $det->daftartindakan->daftartindakan_nama ?? "";
          $det->jumlahTarif = $det->tarif_tindakan;
          $det->kode_tindakan = $det->daftartindakan->daftartindakan_kode ?? "";
          $det->tarif_tindakan = MyFormatter::formatNumberForPrint($det->tarif_satuan * $det->qty_tindakan, 2);
          $det->tarif_satuan = MyFormatter::formatNumberForPrint($det->tarif_satuan, 2);
          $det->tglmasukkamar = $item->tglmasukkamar;
          $det->tglkeluarkamar = $item->tglkeluarkamar;
          $det->qty_tindakan = number_format($det->qty_tindakan, 2, ",", "");
          
          if (!empty($det->nopelayanan)) {
            continue;
          }
    ...
    if (isset($_POST['RJTindakanPelayananT']) || isset($_POST['TindakanpelayananT'])) {
      ...
      try {
        ...
        if ($this->succesSave) {
          $transaction->commit();
          Yii::app()->user->setFlash('success', "Data Akomodasi Pasien berhasil disimpan");
          $this->redirect(array($this->id . '/', 'pendaftaran_id' => $pendaftaran_id, 'sukses' => 1));
        } else {
            // $transaction->rollback();
            Yii::app()->user->setFlash('error', "Data Akomodasi Pasien gagal disimpan <br>");
        }

      } catch (Exception $exc) {
        echo '<pre>'; var_dump($exc); die; 
        Yii::app()->user->setFlash('error', "Data Akomodasi Pasien gagal disimpan " . MyExceptionMessage::getMessage($exc, true));
        $transaction->rollback();
  ...
  public function saveTindakan($modPasien, $modPendaftaran)
  {
    ...
    foreach ($post as $i => $item) {
      ...
      if (!empty($item) && (!empty($item['daftartindakan_id']))) {
        ...
        if (isset($item['tindakanpelayanan_id']) && !empty($item['tindakanpelayanan_id'])) {
            $modTindakans[$i] = RJTindakanPelayananT::model()->findByAttributes(array(
                'tindakanpelayanan_id' => $item['tindakanpelayanan_id']
            ));
        }

        if (empty($modTindakans[$i])) {
        ...
        $modTindakans[$i]->pasienadmisi_id = $pasienadmisi_id;
        ...
        if (empty($no_nota_tindakan) && !empty($modTindakans[$i]->nopelayanan)) {
          $no_nota_tindakan = $modTindakans[$i]->nopelayanan;
        }
    ...
    try {
      if ($valid && (count((array)$modTindakans) > 0)) {
        ...
        // $dataMasukKamar = array();
        $arr_tindakan = array();
        foreach ($modTindakans as $i => $tindakan) {

            if (!empty($no_nota_tindakan)) {
              $tindakan->nopelayanan = $no_nota_tindakan;
            } else {
              $noawal = $tindakan->nopelayanan;
              ...
              $no_nota_tindakan = $tindakan->nopelayanan;
            }
            ...
            if ($tindakan->save()) {
              ...
              $arr_tindakan[] = $tindakan->tindakanpelayanan_id;
                // $dataMasukKamar[] = $tindakan->masukkamar_id;
            }
        ...
        $cr = new CDbCriteria;
        $cr->addNotInCondition('tindakanpelayanan_id', $arr_tindakan);
        $cr->addCondition("masukkamar_id is not null and pendaftaran_id = ".$modPendaftaran->pendaftaran_id);

        $ada = TindakanpelayananT::model()->deleteAll($cr);
        // var_dump(count($ada)); die;
        ...
        // var_dump($this->statusSavePermintaanPenunjang , $this->statusSaveKirimkeUnitLain , $statusSaveKomponen);die;
        if ($this->statusSavePermintaanPenunjang && $this->statusSaveKirimkeUnitLain && $statusSaveKomponen) {
          $transaction->commit();
          $this->succesSave = true;
          Yii::app()->user->setFlash('success', "Data Akomodasi Pasien berhasil disimpan");

          $this->redirect(array('index', 'pendaftaran_id'=>$modPendaftaran->pendaftaran_id, 'pasienadmisi_id'=>$modPendaftaran->pasienadmisi_id));
...
//                                                                                                               'kelompokumur_id'=>$idKelompokUmur));
  ...
  public function actionPrintTindakan($id)
  {
    ...
    // $this->path_view = 'rawatJalan.views.tindakan.';
    $format = new MyFormatter;
    ...
    $modViewTindakans = RJTindakanPelayananT::model()
      ...
      ->findAll(array(
        'condition'=>"t.pendaftaran_id = $id and t.verifbataltindakan_id is null and masukkamar_id is not null",
      ...
      $crit2->addCondition(' pendaftaran_id = ' . $modPendaftaran->pendaftaran_id . ' and masukkamar_id is not null');
      ...
      $condition = " pendaftaran_id = $id and masukkamar_id is not null and verifbataltindakan_id is null ";
        ...
        $modTindakan = TindakanpelayananT::model()->findAll(array(
          'condition'=>$condition,
          'order'=>'tgl_tindakan, tindakanpelayanan_id',
        ));
      ...
      // $tinnull = TindakanpelayananT::model()->findAll('nopelayanan = null and pendaftaran_id = ' . $id . 'and ruangan_id = ' . $ruangan_id);
    ...
    $judul_print = 'Akomodasi Pasien ' . $modPasien->nama_pasien;
    $this->render(
  ...
  public function actionPrintTindakanPenunjang($pasienmasukpenunjang_id)
  {
    ...
    $modViewTindakans = RJTindakanPelayananT::model()
      ...
      ->findAll("t.pasienmasukpenunjang_id = $pasienmasukpenunjang_id and t.verifbataltindakan_id is null"); // RND-6244
      ...
      // $tinnull = TindakanpelayananT::model()->findAll('nopelayanan = null and masukkamar_id is null and pasienmasukpenunjang_id = ' . $pasienmasukpenunjang_id);
      ...
      */
      ...
      $modViewBmhp = RJObatalkesPasienT::model()->with('obatalkes')->findAllByAttributes(array('pasienmasukpenunjang_id' => $pasienmasukpenunjang_id));

    $judul_print = 'Akomodasi Pasien Penunjang ' . $modPasien->nama_pasien;
    $this->render(
