...
class AsesmenPraBedahController extends MyAuthController {
    ...
    public $path_view = 'rawatInap.views.asesmenPraBedah.';
    ...
    /**
     * 
     * @param type $id merupakan kolom asesmenprabedah_id
     */
    public function actionIndex($pendaftaran_id, $id = null) {   
        ...
        if (empty($id)){
            ...
            $anamnesa = AnamnesaT::model()->findByAttributes(array('pendaftaran_id' => $pendaftaran_id), array('order' =>  'anamesa_id DESC', 'limit' => 1));
            if(!empty($anamnesa)) {
            ...
            $model = AsesmenprabedahT::model()->findByPk($id);
            $model->loadInput();            
        }
        
        $this->setAjaxReload($model);
                
        if (isset($_POST['AsesmenprabedahT'])) {
            $ok = true;
            $trans = Yii::app()->db->beginTransaction();
            $pesan = '';            
            try {                
                $_POST['AsesmenprabedahT']['pendaftaran_id'] = $modPendaftaran->pendaftaran_id;
                $_POST['AsesmenprabedahT']['pasienadmisi_id'] = $modPendaftaran->pasienadmisi_id;
                $_POST['AsesmenprabedahT']['pasien_id'] = $modPendaftaran->pasien_id;
                $proses = AsesmenprabedahT::simpanData($model, $_POST['AsesmenprabedahT']);
                $model = $proses['model'];
                $ok &= $proses['sukses'];
                $pesan .= $proses['pesan'];                                                                                     
                ...
                if ($ok) {
                    ...
                    $trans->commit();
                    $this->redirect(array('index','pendaftaran_id'=>$pendaftaran_id, 'id' => $model->asesmenprabedah_id,'sukses'=>1));
                } else {
                    $trans->rollback();
        ...
        if (isset($_POST['AsesmenprabedahT'])) {
            ...
            try {                
                ...
                if ($ok) {
                    Yii::app()->user->setFlash('success', '<strong>Berhasil!</strong> Data berhasil disimpan.');
                    $trans->commit();
                    $this->redirect(array('index','pendaftaran_id'=>$model->pendaftaran_id, 'id' => $model->asesmenprabedah_id,'sukses'=>1));
                } else {
                    $trans->rollback();
                    Yii::app()->user->setFlash('error', "Data gagal disimpan".$pesan);
                }
        ...
        if (isset($_POST['AsesmenprabedahT'])) {
            ...
            try {                
                ...
                if ($ok) {
                    Yii::app()->user->setFlash('success', '<strong>Berhasil!</strong> Data berhasil disimpan.');
                    $trans->commit();
                    $this->redirect(array('index','pendaftaran_id'=>$model->pendaftaran_id, 'id' => $model->asesmenprabedah_id,'dukses'=>1));
                } else {
                    $trans->rollback();
                    Yii::app()->user->setFlash('error', "Data gagal disimpan".$pesan);
                }
