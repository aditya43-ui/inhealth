...
<script type="text/javascript">
    function loadFormTindakanGizi(idRuangan) {
        ...
        if (idRuangan != "") {
            ...
            $.post("<?php echo $this->createUrl('KonsulGiziTRI/loadFormTindakanGizi') ?>", {
                idRuangan: idRuangan,
    ...
    function cekInputan() {
        ...
        var pegawai_id = $("#RIPasienKirimKeUnitLainT_pegawai_id").val();
        var ruangan_id = $("#RIPasienKirimKeUnitLainT_ruangan_id").val();
        
        if ($(".ceklis:checked").length < 1) {
            myAlert("Pilih pelayanan konsultasi gizi terlebih dahulu");
            return false;
        } else if (pegawai_id == "" || ruangan_id == "") {
            myAlert("Pilih ruangan dan dokter perujuk terlebih dahulu");
            return false;
        } else {
    ...
    function ceklist(obj) {
        ...
        if ($(obj).is(':checked')) {
            $.post('<?php echo $this->createUrl('konsulGiziTRI/CekTindakanGiziKelas') ?>', {
