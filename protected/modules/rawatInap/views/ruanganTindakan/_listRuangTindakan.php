        ...
        <tr>
            ...
            <td style="text-align: center" <?= $visible ?>>
                <?php 
                    ...
                    $bisa_hapus = CustomFunction::hakAksesHapus(Yii::app()->user->getState('loginpemakai_id'), $permintaan->create_ruangan, $permintaan->update_loginpemakai_id);
                    
                    if($bisa_hapus) {
                        $onclick = "removeRecord('" . $permintaan->pasienkirimkeunitlain_id ."')";
                    }
...
<script>
    function removeRecord(pasienkirimkeunitlain_id) {
        window.parent.myConfirm("apakah yakin ingin membatalkan rujukan?", 'perhatian', function(r) {
            if(r) {
                $.post('<?php echo $this->createUrl('/rawatJalan/ruangTindakan/deleteRujukan') ?>', {
