...
function setDokter6sampai10(item, identitas, nomor)
{
...
function setTindakan(obj,item)
{
    ...
    var is_setengah = $(obj).parents('tr').find('input[name$="[is_setengah]"]').val();
    ...
    if (is_setengah == 1) {
        hargaTindakan = hargaTindakan * 0.5;
        subsidiAsuransi = subsidiAsuransi * 0.5;
        subsidiPemerintah = subsidiPemerintah * 0.5;
        subsidiRumahsakit = subsidiRumahsakit * 0.5;
        hargacyto_tarif = hargacyto_tarif * 0.5;
    }
...
function setDialog(obj){
    ...
    var ruangan_id = $(obj).parents("tr").find(".ruangantindakan_id").val();
...
//    });
    ...
    $.fn.yiiGridView.update('giladiagnosa-m-grid2', {
        data: {
            ...
            "RJTarifTindakanPerdaRuanganV[tipepaket_id]":tipepaket_id,
            "RJTarifTindakanPerdaRuanganV[jenistarif_id]":jenistarif_id,
            "RJTarifTindakanPerdaRuanganV[penjamin_id]":penjamin_id,
            "RJTarifTindakanPerdaRuanganV[ruangan_id]":ruangan_id,
        }
    });
...
function verifikasiTindkan(obj) {
    var is_verif = $(obj).parents(".row_verif").find(".is_verif").val();

    if (is_verif == 1) {
        $(obj).parents(".row_verif").find(".is_verif").val(0);
        $(obj).parents(".row_verif").removeClass("cls_verif");
    } else {
        $(obj).parents(".row_verif").find(".is_verif").val(1);
        $(obj).parents(".row_verif").addClass("cls_verif");
    }

    cekVerifAda();
}

function cekVerifAda() {
    var is_sudah = true;
    $("#tblInputTindakan .is_verif").each(function() {
        if ($(this).val() != 1) {
            is_sudah = false;
        }
    });

    $("#btn_simpan").prop("disabled", !is_sudah);
}
