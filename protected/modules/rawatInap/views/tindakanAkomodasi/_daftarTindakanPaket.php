<?php 
    ...
    $modTindakan = new RJTarifTindakanPerdaRuanganV('searchDialogTindakan');

    if($konfig->tindakankelas == true) {
    ...
    if(isset($_GET['RJTarifTindakanPerdaRuanganV'])) {
        var_dump($_GET);
        $modTindakan->attributes = $_GET['RJTarifTindakanPerdaRuanganV'];
        if($konfig->tindakankelas == true) {
            $modTindakan->kelas_konfig = $pendaftaran->kelaspelayanan_id;
        }
    }
    
    $this->widget('ext.bootstrap.widgets.BootGridView',array(
            'id'=>'giladiagnosa-m-grid2',
            'dataProvider'=>$modTindakan->searchDialogTindakanAkomodasi(),
            'filter'=>$modTindakan,
            'template'=>"{summary}\n{items}\n{pager}",
            ...
            'columns'=>array(
                    array(
                        ...
                        'value'=>'CHtml::Link("<i class=\"icon-form-check\"></i>","javascript:void(0);",array("class"=>"btn-small", 
                                        ...
                                        "))',
                        'filter'=>
                        CHtml::activeHiddenField($modTindakan,'ruangan_id')
                        .CHtml::activeHiddenField($modTindakan,'tipepaket_id')
                        .CHtml::activeHiddenField($modTindakan,'jenistarif_id')
                        .CHtml::activeHiddenField($modTindakan,'penjamin_id'),
                    ),
                    array(
                        'name'=>'kategoritindakan_nama',
                        'header'=>'Kategori Akomodasi',
