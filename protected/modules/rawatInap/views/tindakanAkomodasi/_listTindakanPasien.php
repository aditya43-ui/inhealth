...
<table class="table table-bordered table-condensed">
    <thead>
        <th>Tindakan</th>
        <th hidden>Free Of Charge (FOC)</th>
        <th>Retur Tagihan</th>
        <th>No Nota</th>
        <th>Alat Medis</th>
        <th>Total Tarif</th>
...
$readonly = $pg_login->kelompokpegawai_id == 1 && $modul_id != 7;
        ...
        $hide = $readonly ? " hide" : "";
        $modPasienAdmisi = null;
    ?>

<?php foreach ($modTindakans as $i => $modTindakan) { 
    $verif = VerifbataltindakanT::model()->findByPk($modTindakan->verifbataltindakan_id);
    if(!empty($verif) && $verif->isverif == true) {
        continue; //di skip agar tidak terbangun tr
    }
    ?>
    <tr>
        <td>
            ...
            <?php echo " (" . $modTindakan->daftartindakan->daftartindakan_kode . ") "; ?>,
            ...
            Pemeriksa : 
            <?php
                echo isset($modTindakan->dokter1) ? $modTindakan->dokter1->namaLengkap : '-';
            ?><br>
            Perawat : 
            <?php
                echo isset($modTindakan->perawat) ? $modTindakan->perawat->namaLengkap : '-';
            ?>
        </td>
        <td width="400px" hidden>
        ...
        <td width="400px">
            ...
            <div style="text-align: center;">
                <?php 
                    echo CHtml::checkbox('returtindakan_riwayat',false, array('onchange'=>'dialogReturTindakan(this,'.$modTindakan->tindakanpelayanan_id.','.$modTindakan->tindakansudahbayar_id.');', 'disabled' => $readonly));
                ?>
            </div>
        ...
        if (empty($modPasienAdmisi)) {
            $modPasienAdmisi = PasienadmisiT::model()->findByPk($modTindakan->pendaftaran->pasienadmisi_id);
        }

        $tgl_tindakan = strtotime($modTindakan->tgl_tindakan);
        $tgl_admisi = empty($modPasienAdmisi) ? null : strtotime($modPasienAdmisi->tgladmisi);

        $nota = !empty($modTindakan->nopelayanan) ? $modTindakan->pendaftaran->no_pendaftaran . $modTindakan->nopelayanan : $modTindakan->pendaftaran->no_pendaftaran . "001";
        ...
        if (
            ...
            $nota = str_replace(array('RJ', 'RD'), 'RI', $nota);
        }
