...
{
    var obatalkes_id = $(obj).parents('.row').find('#obatalkes_id').val();
    var obatalkes_kode = $(obj).parents('.row').find('#obatalkes_kode').val();
    var obatalkes_nama = $(obj).parents('.row').find('#obatalkes_nama').val();
    var jumlah = $(obj).parents('.row').find('#qty_input').val();
    if(obatalkes_id != '')
    {
        $.ajax({
            type:'POST',
...
function print(pasienadmisi_id, pendaftaran_id)
{
    window.open('<?php echo $this->createUrl('print'); ?>&pasienadmisi_id='+pasienadmisi_id+'&pendaftaran_id='+pendaftaran_id,'printwin','left=100,top=100,width=480,height=640');
}
