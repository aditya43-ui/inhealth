...
class RIGambartubuhM extends GambartubuhM
{
	...
	public function getFileNameGambar(){
		$model = RIGambartubuhM::model()->find('gambartubuh_aktif = true AND ispemeriksaanfisik IS TRUE ORDER BY gambartubuh_urutan ASC');
	...
	public function getAllDataGambarAnatomi() {
        $model = RIGambartubuhM::model()->findAll('gambartubuh_aktif = true AND ispemeriksaanfisik IS TRUE ORDER BY gambartubuh_urutan ASC');
		return $model;
    }
}