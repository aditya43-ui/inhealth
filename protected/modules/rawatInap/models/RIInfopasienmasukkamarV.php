...
class RIInfopasienmasukkamarV extends InfopasienmasukkamarV
{
    ...
    public function searchRI()
	{
		...
		// $criteria->select = "*, hasilpemeriksaanlab_t";
		// $criteria->join = "
		...
		if(Yii::app()->user->getState('pegawai_id') != Params::LOGINPEMAKAI_ID_ADMIN) {
			if($this->is_global == 1) {
				$modPegawai = PegawaiM::model()->findByPk($pegawai_id_login);
				if($modPegawai->kelompokpegawai_id == Params::KELOMPOKPEGAWAI_ID_DOKTER_TETAP) {
					$criteria->addCondition('dpjp1_id=' .$pegawai_id_login , 'OR');
					
				}
			} else {
				$criteria->addCondition("(
					t.ruangan_id = ".Yii::app()->user->getState('ruangan_id')." and t.is_konsul = false
				) or (t.is_konsul = true and t.dpjp1_id = ".$pegawai_id_login.")");
			}
        } else {
			if($this->is_nursestation == 1 && Yii::app()->user->getState('nursestation_id') != null){ //RSKG-864
				...
				$criteria->addInCondition('t.ruangan_id', $ruangan);
			}else{
				$criteria->addCondition("(
					t.ruangan_id = ".Yii::app()->user->getState('ruangan_id')." and t.is_konsul = false
				) or (t.is_konsul = true and t.dpjp1_id = ".$pegawai_id_login.")");
		...
		// if($this->is_global == 1 && !empty($this->pegawai_id) && !empty($kelPegawai->kelompokpegawai_id == Params::KELOMPOKPEGAWAI_ID_DOKTER_TETAP)){ //RSKG-864
		
		// 	$criteria->addCondition("t.kelompokpegawai_id = ".Yii::app()->user->getState(Params::KELOMPOKPEGAWAI_ID_DOKTER_TETAP)); 	
		// 	$criteria->addCondition(
        ...
        // }
		//}
                $criteria->order = "t.urutan_konsul desc, t.tgl_pendaftaran DESC";
...
//				$status_dokumen = '<button id="red" class="btn btn-primary" name="yt1" onclick="setStatusDokumen(this,'.$pengirimanrm_id.',\''.$status.'\','.$pendaftaran_id.')">'.$status.'</button>';
   ...
   public function searchPasienPembebasanTarif() {
       ...
       $criteria = new CDbCriteria;
        $criteria->join = "LEFT JOIN pembayaranpelayanan_t pp ON pp.pendaftaran_id = t.pendaftaran_id";
...
//        $criteria->addCondition(" t.statusperiksa = '".Params::STATUSPERIKSA_SUDAH_DIPERIKSA."' ");
	...
	public function getLinkPeriksaPasien() {
		...
		if (!empty($konsul)) { //RSPMC-1645
			return CHtml::link("<i class='icon-form-periksa'></i><br>Periksa Pasien", Yii::app()->controller->createUrl("/rawatJalan/pemeriksaanPasien",array("pendaftaran_id"=>$this->pendaftaran_id, 'lihat' => 1)),array("id"=>$this->no_pendaftaran,"rel"=>"tooltip","title"=>"Klik untuk Pemeriksaan Pasien"));
		}
