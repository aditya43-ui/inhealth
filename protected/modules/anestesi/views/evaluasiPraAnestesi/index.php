...
<div class="panel panel-gradient">
    <div class="panel-heading">    
        <div class="panel-title"> Evaluasi Pra Anestesi</div>
        <?php if (!empty($_GET['pendaftaran_id'])) { ?>
            <span style="float:right; padding: 10px" class="<?= $hide; ?>">
                <?php echo CHtml::link(Yii::t('mds', '{icon} Kembali', array('{icon}' => '<i class="entypo-back"></i>')), $this->createUrl('DaftarPasienAT/Index', array()), array('class' => 'btn btn-danger')); ?>
            </span>
        <?php } ?>
    </div>
    <div class="panel-body">
        ...
        <div class="panel panel-success <?= $hide; ?>">
            ...
            <div class="panel-body">
                <?php
                if (!empty($_GET['pasienanestesi_id'])) {
                    $this->renderPartial($this->path_view . '_dataPasienAnestesi', array('modKunjungan' => $modKunjungan));
                } else {
                    $this->renderPartial($this->path_view . '_dataPasien', array('modKunjungan' => $modKunjungan));
                }
                ?>
            </div>
        </div>
        ...
        <?php
        $this->renderPartial($this->path_view . '_tabMenu', array('modKunjungan' => $modKunjungan, 'hide' => $hide));
