<div class="control-group">
    <?php echo CHtml::label('Ruangan/Poli','',array('class'=>'control-label')); ?>
    <div class="controls">
        <?php 
            echo $form->hiddenField($modEvaluasi, 'ruanganasal_id', array('class' => 'span3', 'readonly' => true, 'onkeypress' => "return $(this).focusNextInputField(event)")); 
            echo $form->textField($modEvaluasi, 'ruangan_nama', array('class' => 'span3', 'readonly' => true, 'onkeypress' => "return $(this).focusNextInputField(event)")); ?>
    </div>
</div>
...
<div class="panel panel-darkk">
    ...
    <p align="justify">
        1. &nbsp;&nbsp;&nbsp;&nbsp;Untuk dilakukan sedasi dan anestesia serta operasi berencana pasien harus berpuasa. Puasa ini penting ditaati oleh
    ...
    <table width="100%" align="center">
        ...
        <td width="30%">
            <?php 
			$modEvaluasi->tglpemberiinformasi = isset($modEvaluasi->tglpemberiinformasi) ? MyFormatter::formatDateTimeForUser($modEvaluasi->tglpemberiinformasi) : "";
			$this->widget('MyDateTimePicker', array(
				'model'=>$modEvaluasi,
				'attribute'=>'tglpemberiinformasi',
				'value' => 'tglpemberiinformasi', 
				'mode' => 'datetime',
				'options' => array(
					'dateFormat' => Params::DATE_FORMAT,
				),
				'htmlOptions' => array('class'=>'span1','readonly' => true, 'onkeypress'=>"return $(this).focusNextInputField(event);",'class'=>'tglpemantauan required'),
	    ));
            ?>
            <label>Nama dan Tanda Tangan pemberi informasi</label>
        </td>
        <td width="30%">
            <?php 
                $modEvaluasi->tglterimainformasi_walipasien = isset($modEvaluasi->tglterimainformasi_walipasien) ? MyFormatter::formatDateTimeForUser($modEvaluasi->tglterimainformasi_walipasien) : "";
                $this->widget('MyDateTimePicker', array(
                        'model'=>$modEvaluasi,
                        'attribute'=>'tglterimainformasi_walipasien',
                        'value' => 'tglterimainformasi_walipasien', 
                        'mode' => 'datetime',
                        'options' => array(
                                'dateFormat' => Params::DATE_FORMAT,
                        ),
                        'htmlOptions' => array('class'=>'span1','readonly' => true, 'onkeypress'=>"return $(this).focusNextInputField(event);",'class'=>''),
	    ));?>
            <label>Nama dan Tanda Tangan Pasien/Wali/dan keluarga</label>
        </td>
        <td width="20%"></td>
    </tr>
    <tr>
    ...
    <tr>
        <td width="20%"></td>
        <td width="30%">
        <?php echo $form->dropDownList($modEvaluasi,'pegawai_pemberiinformasi_id', CHtml::listData($modEvaluasi->getPegawaiItems(Yii::app()->user->getState('ruangan_id')), 'pegawai_id', 'namaLengkap') ,array('empty'=>'-- Pilih --','onkeypress'=>"return $(this).focusNextInputField(event)",'class'=>'span3 kruanestesi_id')); ?>
    
        </td>
        <td width="30%">
        <?php echo CHtml::activeTextField($modEvaluasi,'namawali_pasien',array('readonly'=>false,'placeholder'=>'Ketikkan nama pasien/wali/keluarga')); ?>
