...
class IntraAnestesiController extends MyAuthController
{
	/**
	...
	public $path_view='anestesi.views.intraAnestesi.';
	public $path_tips='anestesi.views.tips.';
	
	public $pasienanestesitersimpan = false;
	public $intraanastesitersimpan = false;
	...
	public function actionIndex()
	{
		...
		$modIntraAnestesi = new ATIntraanestesiT();
		...
		$modIntraAnestesi->nointraanestesi = "-Otomatis-";
		$modIntraAnestesi->tglintraanestesi = date('Y-m-d H:i:s');
		...
		if(!empty($pasienanastesi_id)){
			...
			$modIntraAnestesi = ATIntraanestesiT::model()->findByAttributes(array('pasienanastesi_id'=>$model->pasienanastesi_id),array('order'=>'intraanestesi_id DESC'));
			if(!empty($modIntraAnestesi)){
				$modDetails = ATKondisipasienanestesiT::model()->findAllByAttributes(array('intraanestesi_id'=>$modIntraAnestesi->intraanestesi_id));
			}else{
				$modDetails = array();
				$modIntraAnestesi = new ATIntraanestesiT();
			}
		...
		if(isset($_POST['ATPasienanastesiT']))
		{
			...
			try {
				$pasienanastesi_id = isset($_POST['ATPraanestesiT']['pasienanastesi_id']) ? $_POST['ATPraanestesiT']['pasienanastesi_id'] : null;
				$model = ATPasienanastesiT::model()->findByPk($pasienanastesi_id);
				$model = $this->simpanPasienAnestesi($model, $_POST['ATPasienanastesiT']);
				
				$modPraAnestesi = ATPraanestesiT::model()->findByPk($_POST['ATIntraanestesiT']['praanestesi_id']);
				
				$modIntraAnestesi = $this->simpanIntraAnestesi($model, $modIntraAnestesi, $modPraAnestesi, $_POST['ATIntraanestesiT']);
				if(isset($_POST['ATKondisipasienanestesiT'])){
					if(count($_POST['ATKondisipasienanestesiT']) > 0){
						foreach($_POST['ATKondisipasienanestesiT'] as $i=>$pemantauan){
							$modDetailsPemantauan[$i] = $this->SimpanKondisiPemantauan($modIntraAnestesi,$pemantauan);																					
						}
					}
				}
				if(isset($_POST['ATTindakananestesiT'])){
					if(count($_POST['ATTindakananestesiT']) > 0){
						foreach($_POST['ATTindakananestesiT'] as $i=>$tindakan){
							$modDetailsTindakan[$i] = $this->simpanTindakanPelayanan($model,$tindakan);																					
							$modDetailsAnestesi = $this->simpanTindakanAnestesi($model,$modPraAnestesi,$modDetailsTindakan[$i],$tindakan);																					
				...
				if($this->pasienanestesitersimpan && $this->intraanastesitersimpan && $this->kondisipasienanastesitersimpan && $this->tindakananestesitersimpan && $this->tindakanpelayanantersimpan && $this->obatalkesanestesitersimpan && $this->successSaveBmhp && $this->successSavePemakaianBahan){
					$transaction->commit();
					$modIntraAnestesi->isNewRecord = FALSE;
					$this->redirect(array('index','id'=>$modIntraAnestesi->intraanestesi_id,'sukses'=>1));
				}else{
	...
	*/
	public function simpanPasienAnestesi($model, $post){
		...
		if(empty($model->pasienanastesi_id)){
			...
			$model->statusanestesi = 'Intra Anestesia';
		}else{
			$model->statusanestesi = 'Intra Anestesia';
	...
	/**
	* proses simpan / ubah data intra anastesi
	* @param type $model
	* @param type $post
	* @return type
	*/
	public function simpanIntraAnestesi($model, $modIntraAnestesi, $modPraAnestesi, $post){
		
		$format = new MyFormatter();
		$modPendaftaran = array();
		if(isset($modIntraAnestesi->intraanestesi_id) && (!empty($post['intraanestesi_id']))){
			$load = new $modIntraAnestesi;
			$modIntraAnestesi = ATIntraanestesiT::model()->findByPk($modIntraAnestesi->intranaestesi_id);
			$modPendaftaran = PendaftaranT::model()->findByPk($model->pendaftaran_id);
		}
		$modIntraAnestesi->attributes = $post;
		$modIntraAnestesi->tglintraanestesi = $format->formatDateTimeForDb($post['tglintraanestesi']);
		$modIntraAnestesi->tglpuasa = $format->formatDateTimeForDb($modPraAnestesi->tglpuasa);
		$modIntraAnestesi->monitoring = $modPraAnestesi->monitoring;
		$modIntraAnestesi->komplikasi =  $post['komplikasi'];
		if(empty($modIntraAnestesi->pasienanastesi_id)){
			$modIntraAnestesi->pasienanastesi_id = $model->pasienanastesi_id;
			$modIntraAnestesi->praanestesi_id = $modPraAnestesi->praanestesi_id;
			$modIntraAnestesi->nointraanestesi = MyGenerator::noIntraAnestesi();
			$modIntraAnestesi->ruangan_id = $modPraAnestesi->ruangan_id;
			$modIntraAnestesi->kamarruangan_id = $modPraAnestesi->kamarruangan_id;
			$modIntraAnestesi->dokter_id = $modPraAnestesi->dokter_id;
			$modIntraAnestesi->perawat1_id = $modPraAnestesi->perawat1_id;
			$modIntraAnestesi->perawat2_id = $modPraAnestesi->perawat2_id;
			$modIntraAnestesi->tekniksedasi = $modPraAnestesi->tekniksedasi;
			$modIntraAnestesi->create_ruangan = Yii::app()->user->getState('ruangan_id');
			$modIntraAnestesi->create_loginpemakai_id = Yii::app()->user->id;
			$modIntraAnestesi->create_time = date('Y-m-d H:i:s');		   
		}else{
			$modIntraAnestesi->update_loginpemakai_id = Yii::app()->user->id;
			$modIntraAnestesi->update_time = date('Y-m-d H:i:s');
		}
		
		if($modIntraAnestesi->save()){
			$this->intraanastesitersimpan = true;
		}

		return $modIntraAnestesi;
	}
	
	/**
	* proses simpan KondisipasienanestesiT
	*/
	public function simpanKondisiPemantauan($modIntraAnestesi, $post){
		$format = new MyFormatter();
		$modKondisiPasienAnestesi = new ATKondisipasienanestesiT();
		$modKondisiPasienAnestesi->attributes = $post;
		$modKondisiPasienAnestesi->intraanestesi_id = $modIntraAnestesi->intraanestesi_id;
	...
	*/
	public function simpanTindakanPelayanan($model, $post){
	...
	*/
	public function simpanTindakanAnestesi($model, $modPraAnestesi, $tindakan, $post){
		if(!empty($post['tindakananestesi_id'])){
			$modTindakanAnestesi = ATTindakananestesiT::model()->findByPk($post['tindakananestesi_id']);
		}else{
			$modTindakanAnestesi = new ATTindakananestesiT();
		}	   
		$modPendaftaran = PendaftaranT::model()->findByPk($model->pendaftaran_id);
	...
	{
		if(!empty($stokOa['obatalkespasien_id'])){
			$modObatAlkesPasien = ATObatalkespasienT::model()->findByPk($stokOa['obatalkespasien_id']);
		}else{
	...
	{
		if(!empty($obatalkes['obatalkesanestesi_id'])){
			$modObatAnestesi = ATObatalkesanestesiT::model()->findByPk($obatalkes['obatalkesanestesi_id']);
		}else{
			$modObatAnestesi = new ATObatalkesanestesiT();
		}
...
//		   $criteria->addCondition('instalasi_id = '.Yii::app()->user->getState('instalasi_id'));
     ...
     */
    public function actionPrintHasil($intraanestesi_id,$caraprint = null) 
    {
        ...
        $modPasienAnestesi = ATPasienanastesiT::model()->findByPk($modIntraAnestesi->pasienanastesi_id);     
        $modPraAnestesi = ATPraanestesiT::model()->findByAttributes(array('pasienanastesi_id'=>$modPasienAnestesi->pasienanastesi_id),array('order'=>'praanestesi_id DESC'));     
    ...
    */
	...
	/**
     * Mengurai data tindakan anestesi berdasarkan:
	 * - praanestesi_id
     * @throws CHttpException
     */
    public function actionGetDataTindakanAnestesi()
    {
        if(Yii::app()->request->isAjaxRequest) {
            $format = new MyFormatter();
            $returnVal = array();
            ...
            $model = ATTindakananestesiT::model()->findAll($criteria);
            $attributes = $model->attributeNames();
            foreach($attributes as $j=>$attribute) {
                $returnVal["$attribute"] = $model->$attribute;
            }
	...
	/**
     * Mengurai data pemakaian bahan berdasarkan:
	 * - praanestesi_id
     * @throws CHttpException
     */
    public function actionGetDataPemakaiaBahan()
    {
        if(Yii::app()->request->isAjaxRequest) {
            $format = new MyFormatter();
            $returnVal = array();
            ...
            $model = ATObatalkesanestesiT::model()->findAll($criteria);
            $attributes = $model->attributeNames();
            foreach($attributes as $j=>$attribute) {
                $returnVal["$attribute"] = $model->$attribute;
            }
	...
	/**
     * Mengurai data pemakaian bmhp berdasarkan:
	 * - praanestesi_id
     * @throws CHttpException
     */
    public function actionGetDataPemakaiaBmhp()
    {
        if(Yii::app()->request->isAjaxRequest) {
            $format = new MyFormatter();
            $returnVal = array();
     ...
     * @throws CHttpException
     */
    public function actionGetDataAlatMedis()
    {
        if(Yii::app()->request->isAjaxRequest) {
            $format = new MyFormatter();
            $returnVal = array();
