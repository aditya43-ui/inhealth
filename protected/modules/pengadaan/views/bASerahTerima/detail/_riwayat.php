    ...
    if (!empty($mod)) {
        if (!empty($modDet)) {
            foreach ($modDet as $det) {
                $modTermin = SuratperjanjiankerjaterminT::model()->findByAttributes(array('suratperjanjiankerja_id' => $_GET['suratperjanjiankerja_id'], 'terminke' => $det->terminke));
                ?>
                <tbody>
                    <tr>
                        <td> <?php echo $no++; ?></td>
                        <td> <?php echo $det->baserahterima_nomor; ?></td>
                        <td> <?php echo  CHtml::link($det->nomor_beritaacara,
                            ...
                            ?>
                                <?php echo "<div align=center>" .
                                    CHtml::link("<i class ='glyphicon glyphicon-print' style='font-size:12px;'> </i>", 
                                        Yii::app()->createUrl('pengadaan/bASerahTerima/print&id=' . $det->baserahterima_id), 
                                            array(
                                                    "rel" => "tooltip", 
                                                    "title" => "Klik untuk Mencetak BA Serah Terima", 
                                                    "target" => "frame1", "onclick" => "window.parent.$(\"#dialog1\").dialog(\"open\");")) . 
                                    "</div>";
                            }else{
                                ?>
                                <?php echo "<div align=center>" .
                                    CHtml::link("<i class ='glyphicon glyphicon-print' style='font-size:12px;'> </i>", 
                                        Yii::app()->createUrl('pengadaan/bASerahTerima/printTermin&id=' . $det->baserahterima_id), 
                                            array(
                                                    "rel" => "tooltip", 
