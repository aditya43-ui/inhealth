<div class="panel panel-success">
    <div class="panel-heading">
        ...
        <span style="float:right; padding: 10px" >
            <?php echo CHtml::link("<i class='fa fa-download' style='font-size: 16px;'></i>", Yii::app()->createUrl('pengadaan/suratPerjanjianKerja/unduhExcel&id=' . $_GET['id']), array('class' => 'btn btn-info', 'data-placement' => 'left', "rel" => "tooltip", "title" => "Klik untuk Download Rincian Pekerjaan")); ?>
            <?php echo CHtml::link("<i class='fa fa-upload' style='font-size: 16px;'></i>", Yii::app()->createUrl('pengadaan/suratPerjanjianKerja/loadformImport&id=' . $_GET['id']), array('class' => 'btn btn-info', 'data-placement' => 'left', "rel" => "tooltip", "title" => "Klik untuk Upload Rincian Pekerjaan", "target" => "iframe3", "onclick" => "$('#dialogUpload').dialog('open');")); ?>
        </span>
    </div>
    ...
    if ($total_harga < 10000000 && $konfig->is_banegosiasiaktif == true) {        
        $readonlyHPS = false;        
    } else {
        if ($konfig->is_banegosiasiaktif == false){
            $readonlyHPS = false;
        }else{
            $readonlyHPS = true;
        }
        ...
        <div class="overflow-x">
            <table class="table table-condensed table-bordered table-striped" id="tabel-hps">
                ...
                <tbody>
                    <?php
                    $total_pagu = 0; 
                    $cekPerjanjiankerja = SuratperjanjiankerjaT::model()->findByAttributes(array('persiapanpengadaan_id' => $_GET['id'], 'isbatal' => false, 'isaddendum' => true));
                    if (!empty($cekPerjanjiankerja)) {
                        if (!empty($modDet)) {
                            foreach ($modDet as $i => $d) {
                                ...
                                $d->sisa_pagu = $modDPA->sisapagu_pengadaan + $d->barang_total;
                                ...
                                echo $this->renderPartial($this->path_view . 'form/_rowHPS2', array('model' => $d, 'i' => $i, 'readonlyHPS' => $readonlyHPS), true);
                            }
                        }
                    } else {
                        $cekNegosiasi = BanegosiasiT::model()->findByAttributes(array('persiapanpengadaan_id' => $_GET['id'], 'isbatal' => false, 'isaddendum' => true));
