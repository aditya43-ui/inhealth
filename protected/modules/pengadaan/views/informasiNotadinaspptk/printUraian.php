...
if (!empty($model->suratperjanjiankerja_id)) {
    ...
    $modInfo = DaftarnomorNotadinaspptkV::model()->findByAttributes(array('nomor_id' => $model->rencanaumumpengadaan_id, 'kategori_pengadaan' => 'Swakelola'));
            ...
            $cekpengadaansumberdana = PengadaansumberdanaT::model()->findAllByAttributes(array('rencanaumumpengadaan_id' => $modInfo->nomor_id));
$modSubKegiatan = SubkegiatanprogramM::model()->findByPk($modInfo->subkegiatanprogram_id);
$modMapping = MappingrekeninganggaranM::model()->findByPk($modInfo->mappingrekeninganggaran_id); 
if (!empty($modMapping)) {
    //Kode Rekening
    $model->koderekening = '';
    ...
    <div class="row-fluid" >
        <table width="100%">
            <tr>
                <td width="30%"></td>
                <td width="40%"></td>
                <td width="30%" height="50px" style="vertical-align:top; text-align: left;">Surabaya, <?php echo date('d ', strtotime($model->notadinaspptk_tanggal)) . MyFormatter::getMonthId(date('m', strtotime($model->notadinaspptk_tanggal))) . date(' Y', strtotime($model->notadinaspptk_tanggal)); ?></td>
            </tr>
            <tr>
                <td width="30%" style="vertical-align:top; text-align: center;">Kepala <?php echo $model->pegpjk->unitkerja->namaunitkerja; ?></td>
                <td width="40%"></td>
