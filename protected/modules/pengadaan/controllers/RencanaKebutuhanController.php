...
class RencanaKebutuhanController extends MyAuthController
{
	...
	public function actionIndex($rencanakebfarmasi_id = null)
	{
		...
		if (isset($modApprovalotorisasiM)) {
			$modRencanaKebFarmasi->pegawaimenyetujui_id = $modApprovalotorisasiM->kepalafarmasi_id;
		}

		if (!empty($modRencanaKebFarmasi->pegawai_id)) $modRencanaKebFarmasi->nama_pegawai = $modRencanaKebFarmasi->pegawai->namaLengkap;
		if (!empty($modRencanaKebFarmasi->pegawaimenyetujui_id)) $modRencanaKebFarmasi->pegawaimenyetujui_nama = $modRencanaKebFarmasi->pegawaimenyetujui->namaLengkap;
		...
		if (!empty($rencanakebfarmasi_id)) {
			...
			$modRencanaKebFarmasi->nama_pegawai = !empty($modRencanaKebFarmasi->pegawai_id) ? $modRencanaKebFarmasi->pegawai->NamaLengkap : "";
		...
		if (isset($_GET['ADObatalkesV'])) {
			$modObatAlkesVv->attributes = $_GET['ADObatalkesV'];
			// var_dump($modObatAlkesVv->supplier_nama);die;
		}
		...
		if (isset($_GET['ubah'])) {
			...
			$modPegawaiKpf = PegawaiM::model()->findbyPk($modRencanaKebFarmasi->pegawaimenyetujui_id);

			if (isset($modPegawaiKpf)) $modRencanaKebFarmasi->pegawaimenyetujui_nama = $modPegawaiKpf->namaLengkap;
		}
		...
		if (isset($_POST['ADRencanaKebFarmasiT'])) {
			...
			try {
				...
				if ($modRencanaKebFarmasi->save()) {
					...
					if (count((array)$_POST['ADRencDetailkebT']) > 0) {
	...
	public function simpanRencanaKebutuhan($modRencanaKebFarmasi, $post)
	{
		...
		if ($post['satuanobat'] == PARAMS::SATUANOBAT_KECIL) {
			...
			$modRencanaDetailKeb->hargatotalrenc = $post['subtotal'];
		...
		$modRencanaDetailKeb->hpp = MyFormatter::formatNumberForDb($modRencanaDetailKeb->hpp);
		$modRencanaDetailKeb->harganettorenc = MyFormatter::formatNumberForDb($modRencanaDetailKeb->harganettorenc);
		$modRencanaDetailKeb->hargatotalrenc = MyFormatter::formatNumberForDb($modRencanaDetailKeb->hargatotalrenc);
		$modRencanaDetailKeb->ppn = MyFormatter::formatNumberForDb($modRencanaDetailKeb->ppn);
		$modRencanaDetailKeb->supplier_id = $post['supplier_id'];
		$modRencanaDetailKeb->jmlharusorder = $post['jmlharusorder'];
		// echo "<pre>";
		// var_dump($post);die;
	...
	public function actionLoadFormRencanaKebutuhan()
	{
		if (Yii::app()->request->isAjaxRequest) {
			...
			$modObatAlkesV = ADObatalkesV::model()->findByAttributes(array('obatalkes_id' => $obatalkes_id));
			...
			$modRencanaDetailKeb->harganettorenc = MyFormatter::formatNumberForPrint($modObatAlkes->harganetto, 2);
			...
			$modRencanaDetailKeb->persenpph = 0;
			$modRencanaDetailKeb->persenppn = (!empty($konfigFarmasi) ? $konfigFarmasi->persenppn : 0);
			...
			$modRencanaDetailKeb->satuankecil_nama = $modObatAlkes->satuankecil_id;
			$modRencanaDetailKeb->satuanbesar_nama = $modObatAlkes->satuanbesar_id;
			$modRencanaDetailKeb->jmlpermintaanlama = $jumlah;
			$modRencanaDetailKeb->satuanobat = Params::SATUANOBAT_KECIL;
			$modRencanaDetailKeb->satuanobat_nama = $modRencanaDetailKeb->satuanobat;
			$modRencanaDetailKeb->supplier_id = $modObatAlkesV->supplier_id;
			$rata_rata = $modRencanaDetailKeb->stokakhir / 14;
			$totalMinimalOrder = (17 * $rata_rata) - $modRencanaDetailKeb->stokakhir;

			$modRencanaDetailKeb->jmlharusorder = floor($totalMinimalOrder);
			...
			echo CJSON::encode(
				array(
					...
					'form' => $this->renderPartial(
						$this->path_view . '_rowObatRencanaKebutuhan',
						array(
							'modRencanaKebFarmasi' => $modRencanaKebFarmasi,
				...
				foreach ($modObatAlkes as $i => $obat) {
					...
					if (count((array)$mod_penerimaan) > 0) {
						foreach ($mod_penerimaan as $d => $terima) {
							...
							$tglpermintaan = $terima->permintaanpembelian->tglpermintaanpembelian;
