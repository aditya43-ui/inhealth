...
$modDetail = UraiankeluarumumT::model()->findAllByAttributes(array(
    ...
    'join' => 'join pesangonpeg_t pp on pp.pesangonpeg_id = t.pesangonpeg_id',
    'order' => 'pp.nopesangon asc',
));
...
foreach ($modDetail as $item) {
    $peg = PesangonpegT::model()->findByPk($item->pesangonpeg_id);

    if (empty($periode)) $periode = empty($peg->periodegaji) ? $peg->tglpesangon : $peg->periodegaji;
}
...
<table class="tab_detail">
    ...
    <tbody>
        ...
        foreach ($modDetail as $det) :
            ...
            $data = PesangonpegT::model()->findByPk($det->pesangonpeg_id);
        ...
        ?>
            <tr>
                <td><?php echo $cnt++; ?></td>
                <td><?php echo $data->nopesangon; ?></td>
