...
$main_data = $model->searchLaporanSelisihGaji();

$prov = clone $main_data['prov'];
...
$col = array(
    array(
        ...
        array(
            'header'=>'Periode Gaji',
            'name'=>'periodegaji',
            'value'=>'MyFormatter::formatMonthForUser($data->periodegaji)',
            'footer'=>false,
            'footerHtmlOptions'=>array('hidden'=>true),
        ),
);
...
foreach ($main_data['komponen'] as $item) {
    array_push($col, array(
        'header'=>$item->komponengaji_nama,
        'type'=>'raw',
        'value'=>function($data) use ($item) {
            return MyFormatter::formatNumberForPrint($data->data_komponen[$item->komponengaji_id]['nilai']);
        },
        'htmlOptions'=>array(
