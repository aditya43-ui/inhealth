...
class GJInformasipembayarangajiV extends InformasipembayarangajiV
{
    ...
    public function searchInformasi() {
        ...
        $criteria->group = "totalharga, (case when periodegaji is null then (extract(year from tglpenggajian) || '-' || extract(month from tglpenggajian) || '-01')::date else periodegaji end)::date, pengeluaranumum_id, nopengeluaran, nokaskeluar";
        $criteria->select = "totalharga, (case when periodegaji is null then (extract(year from tglpenggajian) || '-' || extract(month from tglpenggajian) || '-01')::date else periodegaji end)::date as periodegaji, pengeluaranumum_id, nopengeluaran, nokaskeluar, sum(totalterima) as totalterima, sum(totalpotongan) as totalpotongan";
        
        $criteria->addCondition("(case when periodegaji is null then (extract(year from tglpenggajian) || '-' || extract(month from tglpenggajian) || '-01')::date else periodegaji end)::date between '".date('Y-m-01', strtotime($this->periodegaji.'-01'))."'::date and '".date('Y-m-t', strtotime($this->periodegaji.'-01'))."'::date");
