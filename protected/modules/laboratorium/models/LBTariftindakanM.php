...
class LBTariftindakanM extends TariftindakanM {
	...
	public $tipepaket_id, $tipepaket_nama, $pemeriksaanlab_id, $pemeriksaanlab_nama, $jenispemeriksaanlab_nama;
    ...
    public function search2()
	{
		...
		$criteria->select = "t.*, k.*, pr.pemeriksaanlab_id, pr.pemeriksaanlab_nama, jm.jenispemeriksaanlab_nama";
		...
		$criteria->join = 'join daftartindakan_m d on d.daftartindakan_id = t.daftartindakan_id 
							...
							left join jenispemeriksaanlab_m jm on pr.jenispemeriksaanlab_id = jm.jenispemeriksaanlab_id';
		...
		if(Yii::app()->user->getState('instalasi_id') == Params::INSTALASI_ID_LAB) {
			$input = strtolower($this->daftartindakan_kode);
			// filter khusus load awal untuk lab PK
			if(empty($this->daftartindakan_kode)) {
				// saat pertama kalo load
				$criteria->addCondition("d.daftartindakan_kode LIKE '%LPK3%' OR d.daftartindakan_kode LIKE '%NLPK%' or d.daftartindakan_kode LIKE '%ULPK%'");
			} else if(stripos($input, 'lpk3') !== false || stripos($input, 'nlpk') !== false || stripos($input, 'ulpk') !== false){
