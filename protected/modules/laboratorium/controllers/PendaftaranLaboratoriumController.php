...
class PendaftaranLaboratoriumController extends MyAuthController
{
  ...
  public function actionIndex($id = null, $idAntrian = null)
  {
    ...
    if (isset($_POST['LBPendaftaranT'])) {
      ...
      try {
        ...
        // echo '<pre>';var_dump($modPasien);die;
            ...
            if ($postPenunjang['is_pilihpenunjang']) {
              ...
              if (isset($_POST['LBTindakanPelayananT'][$i])) {
                if (count((array)$_POST['LBTindakanPelayananT'][$i]) > 0) {
                  ...
                  //   var_dump($modHasilPemeriksaanPA = $this->simpanHasilPemeriksaanPA($modPasienMasukPenunjangs[$i], $dataTindakans[$i][$ii], $tindakan)); die;
                    // var_dump($modHasilPemeriksaanPA); die;
                     $modPasienMasukPenunjangs[$i]->ispatologianatomi = true;
                      $modPasienMasukPenunjangs[$i]->save(false, array('ispatologianatomi'));
                    } else {
        ...
        if (!empty($modHasilPemeriksaan) && !$modHasilPemeriksaan->isNewRecord) {
          $sysmex = new Sysmex;
          $sysmex->kirim_tambah($modHasilPemeriksaan->hasilpemeriksaanlab_id, null, $_POST['LBPasienmasukpenunjangT'][0]['dokterperujuk'] ?? "");
        }


      //    var_dump($this->pasienpenunjangtersimpan , $this->hasilpemeriksaantersimpan); die;
  ...
  protected function broadcastNotifDaftarLab($model, $modPasien)
  {
    ...
    $ok = CustomFunction::broadcastNotif($judul, $isi, array(
      array('instalasi_id' => $ruangan->instalasi_id, 'ruangan_id' => $ruangan->ruangan_id, 'modul_id' => Params::MODUL_ID_LAB,  'link_proses' => $linkDaftarPasien), //, 'link_proses'=>$link_rj
    ));
  }
  ...
  public function actionVerifikasi()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      if (isset($_POST['LBPendaftaranT'])) {
        ...
        if (count((array)$_POST['LBPasienmasukpenunjangT']) > 0) {
          foreach ($_POST['LBPasienmasukpenunjangT'] as $i => $postPenunjang) {
            if (isset($postPenunjang['is_pilihpenunjang'])) {
              if ($postPenunjang['is_pilihpenunjang']) {
              ...
              if (isset($postPenunjang['is_adakarcis'])) {
                if ($postPenunjang['is_adakarcis']) {
                if (isset($_POST['LBKarcisV'][$i])) {
                  if (count((array)$_POST['LBKarcisV'][$i]) > 0) {
                    foreach ($_POST['LBKarcisV'][$i] as $ii => $karcis) {
              ...
              if (isset($postPenunjang['is_adasample'])) {
                if ($postPenunjang['is_adasample']) {
        ...
        // vaR_dump(count($modPasienMasukPenunjangs), $_POST); die;
      }
   ...
   */
  public function simpanPasienMasukPenunjang($modPasienMasukPenunjang, $modPendaftaran, $post, $is_mikro = false, $tindakan = null)
  {
    ...
    $modPasienMasukPenunjang->no_masukpenunjang = MyGenerator::noMasukPenunjang2($kode_instalasi);
    ...
    $modPasienMasukPenunjang->tglmasukpenunjang = isset($post['tglmasukpenunjang']) ? MyFormatter::formatDateTimeForDb($post['tglmasukpenunjang']) : date("Y-m-d");
    
    // echo '<pre>'; var_dump($modPasienMasukPenunjang->attributes); die;
    ...
    if (!empty($modPasienMasukPenunjang->pasienkirimkeunitlain_id)) {
      ...
      $modPasienMasukPenunjang->dokter_perujuk = $post['dokter_perujuk'] ?? null;
    }

    $modPasienMasukPenunjang->is_mikro = $is_mikro;

    if(!empty($tindakan)) {
      $modPasienMasukPenunjang->samplelab_id = $tindakan[0]['samplelab_id'];
      $modPasienMasukPenunjang->jenispemeriksaanlab_id = $tindakan[0]['jenispemeriksaanlab_id'];
    }

    // var_dump( $modPasienMasukPenunjang->attributes, $post['dokter_perujuk']);die;
    // var_dump($modPasienMasukPenunjang->attributes); die;
    // var_dump($modPasienMasukPenunjang->save()); die;
  ...
  public function simpanTindakanPelayanan($modPendaftaran, $modPasienMasukPenunjang, $post, $no_nota = null)
  {
    ...
    $modTindakan->samplelab_id = isset($post['samplelab_id']) ? $post['samplelab_id'] : null;
    ...
    $modTindakan->dokterpemeriksa1_id = isset($modPasienMasukPenunjang->pegawai_id) ? $modPasienMasukPenunjang->pegawai_id : null;
    ...
    $total_cyto = isset($modTarif->totaltarifakhir_cyto) ? $modTarif->totaltarifakhir_cyto : 0;

    $modTindakan->tarifcyto_tindakan = !$modTindakan->cyto_tindakan ? 0 : $total_cyto;
  ...
  public function simpanDetailHasilPemeriksaanLab($modHasilPemeriksaan, $modTindakan, $post)
  {
    ...
    if (count((array)$modPemeriksaanLadDet) > 0) {
      foreach ($modPemeriksaanLadDet as $i => $pemeriksaanDet) {
       ...
       // $modDetailHasilPemeriksaans[$i]->kelaspelayanan_id = $modHasilPemeriksaan->kelaspelayanan_id;
        
        if ($modDetailHasilPemeriksaans[$i]->validate()) {

        //   var_dump($modDetailHasilPemeriksaans[$i]->attributes); 
        
    
        //   var_dump($modDetailHasilPemeriksaans[$i]->getErrors()); die;
            
      
        //  var_dump($modDetailHasilPemeriksaans[$i]->save()); die;
          $modDetailHasilPemeriksaans[$i]->save();

    //  var_dump($modDetailHasilPemeriksaans[$i]->attributes); die;
        } else {
          $this->hasilpemeriksaantersimpan &= false;
        }
      }
  ...
  public function simpanHasilPemeriksaanPA($modPasienMasukPenunjang, $modTindakan, $post)
  {
    ...
    $modHasilPemeriksaanPA->tglperiksapa = date("Y-m-d");
    ...
    $modHasilPemeriksaanPA->statushasilperiksapa = "BELUM";
  //  $modHasilPemeriksaanPA->kelaspelayanan_id = !empty($modPasienMasukPenunjang->kelaspelayanan_id) ? $modPasienMasukPenunjang->kelaspelayanan_id : Params::KELASPELAYANAN_ID_TANPA_KELAS;
  //   echo '<pre>';
  //   var_dump($modHasilPemeriksaanPA->attributes); die;
    
  // var_dump($modHasilPemeriksaanPA->getErrors()); die;
    
  // var_dump($modHasilPemeriksaanPA->save()); die;
  ...
  public function simpanPengambilanSample($modPasienMasukPenunjang, $post)
  {
    ...
    //  var_dump($modPengambilanSample->attributes); die;
      $this->pengambilansampletersimpan &= true;
    } else {
  ...
  public function actionPrintStatusLab($pendaftaran_id)
  {
    ...
    $criteria2->addCondition('ispatologianatomi = true');
    $loadPasienMasukPenunjangs[1] = LBPasienmasukpenunjangT::model()->find($criteria2);
    if (isset($loadPasienMasukPenunjangs[1])) {
      $modPasienMasukPenunjangs[1] = $loadPasienMasukPenunjangs[1];
  ...
  public function actionPrintRincianBelumBayar($pendaftaran_id,$pasienadmisi_id=null){
    $this->layout='//layouts/printWindows';
    if (isset($_GET['frame'])){
        $this->layout='//layouts/iframe';
    }
    ...
    $modBayarUangMuka = BayaruangmukaT::model()->findAllByAttributes(array('pendaftaran_id'=>$pendaftaran_id), array('condition'=>'pembatalanuangmuka_id is null'));
