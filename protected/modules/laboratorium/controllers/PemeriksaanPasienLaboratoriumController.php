...
class PemeriksaanPasienLaboratoriumController extends PendaftaranLaboratoriumController
{
  ...
  public function actionIndex($id = null, $idAntrian = null)
  {
    ...
    // $pasienmasukpenunjang_id = (isset($_GET['pasienmasukpenunjang_id']) ? $_GET['pasienmasukpenunjang_id'] : $pasienmasukpenunjang_id);
    if (!empty($pasienmasukpenunjang_id)) {
      ...
      if (isset($loadModKunjungan)) {
        ...
        $modPasienMasukPenunjang->pegawai_nama = !empty($modPasienMasukPenunjang->pegawai->namaLengkap) ? $modPasienMasukPenunjang->pegawai->namaLengkap : "-" ;
    ...
    if (isset($_POST['pasienmasukpenunjang_id'])) {
      ...
      try {
        ...
        if (isset($_POST['LBTindakanPelayananT'][0])) {
          if (count((array)$_POST['LBTindakanPelayananT'][0]) > 0) {
        ...
        if (!empty($modHasilPemeriksaan)) {
          $sysmex = new Sysmex;
          $sysmex->kirim_tambah($modHasilPemeriksaan->hasilpemeriksaanlab_id, Sysmex::ORDER_CONTROL_UPDATE_TRANSAKTI);
        } 
  ...
  public function actionSetTindakanRegistrasi()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      $modTindakans = LBTindakanPelayananT::model()->findAllByAttributes(array('pasienmasukpenunjang_id' => $_POST['pasienmasukpenunjang_id']), 'karcis_id IS NOT NULL');
      if (count((array)$modTindakans) > 0) {
        foreach ($modTindakans as $i => $modTindakan) {
  ...
  public function actionAddTindakanPilihan()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      foreach ($pemeriksaan as $item) {
        ...
        $tindakan->tarif_tindakan = MyFormatter::formatNumberForUser($item->harga_tariftindakan * $tindakan->qty_tindakan);
    ...
    if (Yii::app()->request->isAjaxRequest) {
      ...
      if(empty($modTindakanPembebasan)){
        ...
        if(!empty($komponens)){
          ...
          if(!empty($returnVal)){
            $form = $this->renderPartial($this->path_view . '_rowRefundBiaya', array('data' => $returnVal), true);
          }
        }
