...
if (isset($caraPrint)) {
    ...
    if ($caraPrint != "EXCEL") {
        echo $this->renderPartial('application.views.headerReport.headerDefaultNew', array('judulLaporan' => $data['judulLaporan']));
    } else {
        echo $this->renderPartial('application.views.headerReport.headerDefaultNewExcel', array('judulLaporan' => $data['judulLaporan'], 'colspan' => 7));
    }
}
?>
<?php
...
<table class='border table'>
    ...
    foreach ($modRincian as $i => $row) {
        ...
        // '.$row->kategoritindakan_nama.' ('.$p->namaLengkap.')<br>'.$row->daftartindakan_nama.'
        $gelDep = $row->gelardepan;
        $nam = $row->nama_pegawai;
        $gelBel = $row->gelarbelakang_nama;
        echo '<tr>
                    ' . $ruanganTd . '
                    <td> ' . $row->kategoritindakan_nama . ' (' . $gelDep . ' ' . $nam . ' ' . $gelBel . ') <br>' . $row->daftartindakan_nama . '
                    </td>
