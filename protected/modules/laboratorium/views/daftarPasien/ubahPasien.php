...
<div class="white-container">
    ...
    <?php echo CHtml::htmlButton(Yii::t('mds','{icon} Save',array('{icon}'=>'<i class="entypo-check"></i>')),
                                                    array('class' => 'btn btn-danger', 'type'=>'button', 
                                                        'onKeypress'=>'return formSubmit(this,event)',
                                                        'id'=>'btn_simpan', 'onclick'=>'$(\'#update-pasien-form\').submit();  ',
                                                       )); 
    ?>
    <?php echo CHtml::link(Yii::t('mds', '{icon} Cancel', array('{icon}'=>'<i class="entypo-arrows-ccw"></i>')), $this->createUrl('/'.Yii::app()->controller->module->id.'/'.Yii::app()->controller->id,array('modul_id'=>Yii::app()->session['modul_id'])), array('class'=>'btn btn-danger')); ?>
    <?php $this->endWidget(); ?>
</div>
<script>     
function cekForm(){
    ...
    if(reqPasienKosong > 0){
        myAlert('Silakan isi data yang bertanda * !');
        return false;
    }else{
        return true;
    }
