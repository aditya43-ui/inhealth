<?php $riwayatPasien = Yii::app()->createUrl('rawatJalan/daftarPasien/getRiwayatPasienLama&id=' . $modKunjungan->pasien_id); ?>
...
function setPermintaanKePenunjang(){
    ...
    $.ajax({
        ...
        dataType: "json",
        success:function(data){
            console.log(data.rows_pemeriksaan)
            is_permintaan = true;
            $('#form-permintaankepenunjang table > tbody').html(data.rows);
            $('#form-tindakanpemeriksaan table > tbody').html(data.rowTablePemeriksaan);
            ...
            $.each(data.rows_pemeriksaan, function(idx, val) {

                pilihPemeriksaanIniPenunjang(val.pemeriksaanlab_nama, val.pemeriksaanlab_id, val.daftartindakan_id, val.jenistarif_id,
                                            val.harga_tariftindakan, val.daftartindakan_kode, val.kelaspelayanan_id, val.jenispemeriksaanlab_nama)
...
function setTindakanPelayanan(){
    ...
    $.ajax({
        type:'POST',
        url:'<?php echo $this->createUrl('SetTindakanPelayananAfterSave'); ?>',
...
// keperluan script untuk laboratorium untuk mengecek tgl pemeriksan
function cekTglPemeriksaan(obj, event) {
    var tglrencanapemeriksaan = $('.tglrencanapemeriksaan').val();
    var tglmasukpenunjang = $('.tglmasukpenunjang').val();

    if(tglmasukpenunjang === undefined) {
        myAlert('Pemeriksaan telah direncanakan, tanggal pemeriksaan tidak sesuai');
        return false;
    } else {
        var required = requiredCheck($('#pemeriksaanlaboratorium-form'));
        if(required) {
            $(('#pemeriksaanlaboratorium-form')).submit();
        }
        // formSubmit(obj, event);
    }
}
