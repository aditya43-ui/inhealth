...
// "DATE(pendaftaran_t.tgl_pendaftaran) BETWEEN '". $format->formatDateTimeForDb($tgl_awal) ."' AND '". $format->formatDateTimeForDb($tgl_akhir) ."'",
    if(!empty($pendaftaran_id) && !empty($pasienmasukpenunjang_id)){
        $cond = array(
            ...
            "daftartindakan_m.daftartindakan_id in (".
            implode(", ", array(104, 105, 106, 107, 108, 109))
            .")"
        );
    }
    //else{
   ...
   // }
    ...
    {   
        ...
        {
            echo ($datas['daftartindakan_id'] == Params::DAFTARTINDAKAN_ID_IGG) ?  "<p style=\"margin: 0; text-align: center;\"><i class=icon-ok icon-black></i></p>":  "<p style=\"margin: 0; text-align: center;\"><i class=icon-minus icon-black></i></p>";
        }
    }
    else
    {
            echo "<p style=\"margin: 0; text-align: center;\"><i class=icon-minus icon-black></i></p>";
    }    
?>