...
<div style="padding-top:0; line-height:1.5; padding-right:10px;" width="100%">
    ...
    <table>
        <tr>
            <td>
                <?php
                echo '<td>Reg</td><td> : </td><td>'.$modPendaftaran->no_pendaftaran.' '.date('d/m/Y H:i', strtotime($modPendaftaran->tgl_pendaftaran)).'</td>';
                ?>
            <td>
        </tr>
        <tr>
        ...
        <?php if(Yii::app()->user->getState('ruangan_id') == Params::RUANGAN_ID_LAB_ANATOMI) {  
                $ruangan_nama = '';
                if(!empty($modPendaftaran->pasienmasukpenunjang)) {
                    $ruangan_nama = $modPendaftaran->pasienmasukpenunjang->ruangan->ruangan_nama;
                }
            ?>
            <tr>
                <td>
                    <?php echo '<td>Poli</td><td> : </td><td>'.$ruangan_nama.'</td>'; ?>
                </td>
            </tr>
        <?php } else { ?>
            <tr>
                <td>
                    <?php echo '<td>Poli</td><td> : </td><td>'.$modPendaftaran->ruangan->ruangan_nama.' kskakakak</td>'; ?>
                </td>
            </tr>
        <?php } ?>
        <tr>
