...
function checkOto() {
    ...
    if (otoval == 1) {
        ...
        $("#no_rekam_medik_baru, #LBPasienM_nomorindukpegawai").val("");

        $("#pendaftaranFP").hide();
        $(".rm_nip_baru").show().find(":input").prop("disabled", false);
        $(".normpilihan").removeClass('hide');
        // $(".labelPasienBaru").html('Data Pasien Baru');
        ...
        // $(".labelPasienBaru").html('Data Pasien Lama');
...
function setPegawai(pegawai_id, nip) {
    $.post('<?php echo $this->createUrl('getDataPegawaiUntukPasienBaru'); ?>', {
        ...
        if (data.ok == 0) {
            myAlert(data.msg);
            $("#LBPasienM_nomorindukpegawai").val("").focus();
        } else {
            $("#<?php echo CHtml::activeId($modPasien, "pegawai_id"); ?>").val(data.res.pegawai_id);
            $("#<?php echo CHtml::activeId($modPasien, "jenisidentitas"); ?>").val(data.res.jenisidentitas
            .trim());
            $("#<?php echo CHtml::activeId($modPasien, "no_identitas_pasien"); ?>").val(data.res.noidentitas);
            $("#<?php echo CHtml::activeId($modPasien, "nama_pasien"); ?>").val(data.res.nama_pegawai
                .toUpperCase());
            ...
            setDaerahPasien(data.res.propinsi_id, data.res.kabupaten_id, data.res.kecamatan_id, data.res
                .kelurahan_id);
...
function setPasienLama(pasien_id, no_rekam_medik) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.adaInap) {
                myAlert("Pasien " + data.listDaftar.pasien.namadepan + data.listDaftar.pasien.nama_pasien +
                    " (" + data.listDaftar.no_pendaftaran + ")\n\
            ...
            if (data.tindakLanjut) {
                myAlert("Pasien " + data.listDaftar.pasien.namadepan + data.listDaftar.pasien.nama_pasien +
                    " (" + data.listDaftar.no_pendaftaran + ")\n\
                    Hari ini menunggu tindak lanjut ke rawat inap di " + data.listDaftar.instalasi.instalasi_nama +
                    " -> " + data.listDaftar.ruangan.ruangan_nama + ".");
                $("#form-pasien > div").removeClass("animation-loading");
                $("#<?php echo CHtml::activeId($modPasien, 'nama_pasien'); ?>").val("");
                setPasienBaru();
                isSetLama = false;
            ...
            if (data.adaDaftar) {
                myAlert("Pasien " + data.listDaftar.pasien.namadepan + data.listDaftar.pasien.nama_pasien +
                    " (" + data.listDaftar.no_pendaftaran + ")\n\
                Hari ini sedang di instalasi " + data.listDaftar.instalasi.instalasi_nama + " -> " + data.listDaftar
                    .ruangan.ruangan_nama + " dengan status pemeriksan '" +
            ...
            if (data.statusrekammedis.trim() == "<?php echo Params::STATUSREKAMMEDIS_AKTIF ?>") {
                ...
                $("#cari_no_rekam_medik, #no_rekam_medik_baru").val(data.no_rekam_medik);
                ...
                $("#<?php echo CHtml::activeId($modPasien, "no_identitas_pasien"); ?>").val(data
                    .no_identitas_pasien);
                ...
                $("#<?php echo CHtml::activeId($modPasien, "kelompokumur_id"); ?>").val(data
                    .kelompokumur_id);
                $("#<?php echo CHtml::activeId($modPasien, "statusperkawinan"); ?>").val(data
                    .statusperkawinan);
                ...
                $("#<?php echo CHtml::activeId($modPasien, "no_telepon_pasien"); ?>").val(data
                    .no_telepon_pasien);
                $("#<?php echo CHtml::activeId($modPasien, "no_mobile_pasien"); ?>").val(data
                    .no_mobile_pasien);
                ...
                $("#<?php echo CHtml::activeId($modPasien, "jumlah_bersaudara"); ?>").val(data
                    .jumlah_bersaudara);
                ...
                myConfirm('Apakah Anda akan menggunakan No. Rekam Medik Non-Aktif?', 'Perhatian!', function(
                    r) {
                    if (r) {
                        $("#cari_no_rekam_medik").val(data.no_rekam_medik);
                        $("#<?php echo CHtml::activeId($modPasien, 'pasien_id'); ?>").val(data
                            .pasien_id);
                        $(".labelPasienBaru").html('Data Pasien No. Rekam Medik Lama ');
                        $("#form-pasien > legend > .tombol").attr('style', 'display:true;');
                        $("#form-pasien > .box").addClass("well").removeClass("box");
...
function setNamaDepan() {
    ...
    if (umur <= 5) {
        var namadepan = $('#LBPasienM_namadepan').val('By. ');
        ...
        var namadepan = $('#LBPasienM_namadepan').val('An. ');
        if (statusperkawinan.length > 0 && statusperkawinan != "DIBAWAH UMUR") {
            $('#LBPasienM_statusperkawinan').val('');
            alert('Maaf status perkawinan belum cukup usia');
        }
        ...
        if ($('#LBPasienM_jeniskelamin_0').is(':checked')) {
            if (statusperkawinan !== 'JANDA') {
                var namadepan = $('#LBPasienM_namadepan').val('Tn. ');
            } else {
        ...
        if ($('#LBPasienM_jeniskelamin_1').is(':checked')) {
            $('#LBPasienM_namadepan').val('Nn. ');
            if (statusperkawinan !== 'DUDA') {
                var namadepan = $('#LBPasienM_namadepan').val('Nn. ');
                if (statusperkawinan === 'KAWIN' || statusperkawinan == 'JANDA' || statusperkawinan == 'NIKAH SIRIH' ||
                    statusperkawinan == 'POLIGAMI') {
                    var namadepan = $('#LBPasienM_namadepan').val('Ny. ');
                } else {
                    var namadepan = $('#LBPasienM_namadepan').val('Nn. ');
                }
            } else {
...
function setTglLahirPjp(obj) {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function(data) {
            $("#<?php echo CHtml::activeId($modPenanggungJawab, "tgllahir_pj"); ?>").val(data
            .tanggal_lahir);
        },
        error: function(jqXHR, textStatus, errorThrown) {
            console.log(errorThrown);
...
function setClearDropdownKecamatan() {
    $("#<?php echo CHtml::activeId($modPasien, "kecamatan_id"); ?>").find('option').remove().end().append(
        '<option value="">-- Pilih --</option>').val('').multiselect('rebuild');
}
/** bersihkan dropdown kelurahan */
function setClearDropdownKelurahan() {
    $("#<?php echo CHtml::activeId($modPasien, "kelurahan_id"); ?>").find('option').remove().end().append(
        '<option value="">-- Pilih --</option>').val('').multiselect('rebuild');
}
/**
...
function setDropdownJeniskasuspenyakit(ruangan_id) {
    $.ajax({
        ...
        dataType: "json",
        success: function(data) {
            $("#<?php echo CHtml::activeId($model, "jeniskasuspenyakit_id"); ?>").html(data
                .listKasuspenyakit);
        },
        error: function(jqXHR, textStatus, errorThrown) {
            console.log(errorThrown);
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.penjamin_nama != '') {
                myConfirm("Apakah pasien ini akan menggunakan penjamin " + data.penjamin_nama + "?",
                    "Konfirmasi!",
                    function(r) {
                        if (r) {
                            setFormAsuransi(data.carabayar_id);
                            $("#<?php echo CHtml::activeId($model, "carabayar_id"); ?>").val(data
                                .carabayar_id);
                            $("#<?php echo CHtml::activeId($model, "penjamin_id"); ?>").html(data
                                .listPenjamin);
                            $("#<?php echo CHtml::activeId($model, "penjamin_id"); ?>").val(data
                                .penjamin_id);
                            if (data.carabayar_id == <?php echo Params::CARABAYAR_ID_BPJS ?>) {
                                getAsuransiNoKartu(data.nopeserta);
                            } else if ((data.carabayar_id ==
                                <?php echo Params::CARABAYAR_ID_BADAK; ?>) || (data.carabayar_id ==
                                    <?php echo Params::CARABAYAR_ID_DEP_BADAK; ?>) || (data
                                    .carabayar_id == <?php echo Params::CARABAYAR_ID_PEKERJA; ?>)) {
                                setAsuransiBadak(data);
                            } else {
                                $("#<?php echo CHtml::activeId($modAsuransiPasien, 'nopeserta') ?>")
                                    .val(data.nopeserta);
                                $("#<?php echo CHtml::activeId($modAsuransiPasien, 'asuransipasien_id') ?>")
                                    .val(data.asuransipasien_id);
                                $("#<?php echo CHtml::activeId($modAsuransiPasien, 'nokartuasuransi') ?>")
                                    .val(data.nokartuasuransi);
                                $("#<?php echo CHtml::activeId($modAsuransiPasien, 'namapemilikasuransi') ?>")
                                    .val(data.namapemilikasuransi);
                                $("#<?php echo CHtml::activeId($modAsuransiPasien, 'nomorpokokperusahaan') ?>")
                                    .val(data.nomorpokokperusahaan);
                                $("#<?php echo CHtml::activeId($modAsuransiPasien, 'kelastanggunganasuransi_id') ?>")
                                    .val(data.kelastanggunganasuransi_id);
                                $("#<?php echo CHtml::activeId($modAsuransiPasien, 'namaperusahaan') ?>")
                                    .val(data.namaperusahaan);
                                $("#<?php echo CHtml::activeId($modAsuransiPasien, 'status_konfirmasi') ?>")
                                    .val(data.status_konfirmasi);
                                $("#<?php echo CHtml::activeId($modAsuransiPasien, 'tgl_konfirmasi') ?>")
                                    .val(data.tgl_konfirmasi);
                            }
                        }
                    });
            }
...
$('#form-pemeriksaan-0 > div > .accordion-heading').click(function() {
    var is_pilihpenunjang = $(
        "#<?php echo CHtml::activeId($modPasienMasukPenunjangs[0], "[0]is_pilihpenunjang"); ?>");
...
$('#form-pemeriksaan-1 > div > .accordion-heading').click(function() {
    var is_pilihpenunjang = $(
        "#<?php echo CHtml::activeId($modPasienMasukPenunjangs[1], "[1]is_pilihpenunjang"); ?>");
    if (is_pilihpenunjang.val() > 0) { //hide
        is_pilihpenunjang.val(0);
    } else { //show
        is_pilihpenunjang.val(1);
...
function clearRujukan() {
    $('#<?php echo CHtml::activeId($modRujukan, 'rujukandari_id') ?>').find('option').remove().end().append(
        '<option value="">-- Pilih --</option>').val('');
}
/**
...
function setNamaPerujuk() {
    ...
    var nama_perujuk = $("#<?php echo CHtml::activeId($modRujukan, 'rujukandari_id') ?>").find('option[value="' +
        rujukandari_id + '"]').text();
...
function sembunyiFormAsuransi() {
    ...
    $('#form-asuransi > .accordion-group > .accordion-heading').find(".icon-minus").addClass("icon-plus").removeClass(
        "icon-minus");
...
function tampilFormAsuransi() {
    ...
    $('#form-asuransi > .accordion-group > .accordion-heading').find(".icon-plus").addClass("icon-minus").removeClass(
        "icon-plus");
...
function sembunyiFormAsuBadak() {
    ...
    $('#form-asubadak > .accordion-group > .accordion-heading').find(".icon-minus").addClass("icon-plus").removeClass(
        "icon-minus");
...
function tampilFormAsuBadak() {
    ...
    $('#form-asubadak > .accordion-group > .accordion-heading').find(".icon-plus").addClass("icon-minus").removeClass(
        "icon-plus");
...
function sembunyiFormAsuDepartemen() {
    ...
    $('#form-asudepartemen > .accordion-group > .accordion-heading').find(".icon-minus").addClass("icon-plus")
...
function tampilFormAsuDepartemen() {
    ...
    $('#form-asudepartemen > .accordion-group > .accordion-heading').find(".icon-plus").addClass("icon-minus")
...
function sembunyiFormAsuPekerja() {
    ...
    $('#form-asupekerja > .accordion-group > .accordion-heading').find(".icon-minus").addClass("icon-plus").removeClass(
        "icon-minus");
...
function tampilFormAsuPekerja() {
    ...
    $('#form-asupekerja > .accordion-group > .accordion-heading').find(".icon-plus").addClass("icon-minus").removeClass(
        "icon-plus");
...
function cekCaraBayarBadak(carabayar_id) {
    ...
    if ((carabayar_id == <?= Params::CARABAYAR_ID_BADAK; ?>) || (carabayar_id ==
        <?= Params::CARABAYAR_ID_DEP_BADAK; ?>) || (carabayar_id == <?= Params::CARABAYAR_ID_PEKERJA; ?>)) {
        if (pegawai_id == '') {
...
function setAsuransiBadak() {
    ...
    $.ajax({
        ...
        success: function(data) {
            setAsuransiBadakReset();
            if (data != null) {
                $("#<?php echo CHtml::activeId($modAsuransiPasienBadak, 'nopeserta') ?>").val(data
                    .nopeserta);
                $("#<?php echo CHtml::activeId($modAsuransiPasienBadak, 'asuransipasien_id') ?>").val(data
                    .asuransipasien_id);
                $("#<?php echo CHtml::activeId($modAsuransiPasienBadak, 'namapemilikasuransi') ?>").val(data
                    .namapemilikasuransi);
                $("#<?php echo CHtml::activeId($modAsuransiPasienBadak, 'kelastanggunganasuransi_id') ?>")
                    .val(data.kelastanggunganasuransi_id);
                $("#<?php echo CHtml::activeId($modAsuransiPasienBadak, 'namaperusahaan') ?>").val(data
                    .namaperusahaan);
                $("#<?php echo CHtml::activeId($modAsuransiPasienBadak, 'hubkeluarga') ?>").val(data
                    .hubkeluarga);

                $("#<?php echo CHtml::activeId($modAsuransiPasienDepartemen, 'asuransipasien_id') ?>").val(
                    data.asuransipasien_id);
                $("#<?php echo CHtml::activeId($modAsuransiPasienDepartemen, 'namaperusahaan') ?>").val(data
                    .namaperusahaan);
                $("#<?php echo CHtml::activeId($modAsuransiPasienDepartemen, 'nomorpokokperusahaan') ?>")
                    .val(data.nomorpokokperusahaan);
                $("#<?php echo CHtml::activeId($modAsuransiPasienDepartemen, 'nopeserta') ?>").val(data
                    .nopeserta);
                $("#<?php echo CHtml::activeId($modAsuransiPasienDepartemen, 'namapemilikasuransi') ?>")
                    .val(data.namapemilikasuransi);
                $("#<?php echo CHtml::activeId($modAsuransiPasienDepartemen, 'kelastanggunganasuransi_id') ?>")
                    .val(data.kelastanggunganasuransi_id);

                $("#<?php echo CHtml::activeId($modAsuransiPasienPekerja, 'asuransipasien_id') ?>").val(data
                    .asuransipasien_id);
                $("#<?php echo CHtml::activeId($modAsuransiPasienPekerja, 'nopeserta') ?>").val(data
                    .nopeserta);
                $("#<?php echo CHtml::activeId($modAsuransiPasienPekerja, 'namapemilikasuransi') ?>").val(
                ...
                $("#<?php echo CHtml::activeId($modAsuransiPasienPekerja, 'kelastanggunganasuransi_id') ?>")
                    .val(data.kelastanggunganasuransi_id);
            }
...
function cekValiditasPenjamin(penjamin_id) {
    ...
    if (carabayar_id == <?= Params::CARABAYAR_ID_BADAK; ?>) {
        ...
        if ((penjamin_id == <?= Params::PENJAMIN_ID_PISA; ?>) || (penjamin_id ==
            <?= Params::PENJAMIN_ID_PROKESPEN; ?>)) {
            var pasien_id = $("#<?php echo CHtml::activeId($modPasien, "pasien_id"); ?>").val();
            $.ajax({
                type: 'POST',
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function(data) {
                $("#<?php echo CHtml::activeId($modAsuransiPasienDepartemen, "namaperusahaan"); ?>").val(
                    data.data.penjamin_nama);
...
function setDropdownStatushubungankeluarga(penjamin_id) {
    $.ajax({
        ...
        dataType: "json",
        success: function(data) {
            $("#<?php echo CHtml::activeId($modAsuransiPasienBadak, "hubkeluarga"); ?>").html(data
...
function setVerifikasi() {
    var is_pilihpenunjangs = ($("#<?php echo CHtml::activeId($modPasienMasukPenunjangs[0], "[0]is_pilihpenunjang"); ?>")
        .val()) + ($("#<?php echo CHtml::activeId($modPasienMasukPenunjangs[1], "[1]is_pilihpenunjang"); ?>").val());
    var pilihperiksaklinik = $("#form-tindakanpemeriksaan-0 > table > tbody >tr").length;
    var pilihperiksaanatomi = $("#form-tindakanpemeriksaan-1 > table > tbody >tr").length;
    var totalperiksa = parseInt(pilihperiksaklinik) + parseInt(pilihperiksaanatomi);
    ...
    if (requiredCheck($(".form_pendaftaran"))) {
        if (totalperiksa == 0) {
            myAlert("Silakan pilih pemeriksaan laboratorium!");
            return false;
        }
        if (is_pilihpenunjangs > 0) {
            ...
            myAlert("Silakan pilih tujuan pemeriksaan laboratorium!");
        }
    }
    return false;
}
...
function setVerifikasiMikro() {
    ...
    var jmlperiksa = $('.tr-pemeriksaan').length;
    ...
    if (requiredCheck($(".form_pendaftaran"))) {
        ...
        // console.log('is pilih penunjang: ' + is_pilihpenunjangs);
...
function updateChecklistPemeriksaanLab() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            setCheckedPemeriksaan($("#form-tindakanpemeriksaan-" + form_index), $(
                '#dialog-pilihpemeriksaan .dialog-content'));
        },
        error: function(jqXHR, textStatus, errorThrown) {
            console.log(errorThrown);
        }
...
function setChecklistPemeriksaanLab(obj, form_index) {
    ...
    var pegawai_id = $(obj).find("select[name$='[pegawai_id]']").val();
    var perawat_id = $(obj).find("select[name$='[perawat_id]']").val();

    if (pegawai_id == '') {
        pegawai_id = perawat_id;
    }
    ...
    // } else if (kelaspelayanan_id == "") {
        ...
        $("#form-caripemeriksaan").find("input[name$='[pegawai_id]']").val(pegawai_id);
        updateChecklistPemeriksaanLab();
        $('#dialogLab2').dialog('open');
    }
}
/**
...
function pilihPemeriksaanIni(obj) {
    ...
    var rowtindakan = [];
    rowtindakan[0] =
        '<?php echo CJSON::encode($this->renderPartial($this->path_view . '_rowTindakanPemeriksaan', array('i' => 0, 'modTindakan' => $modTindakan), true)); ?>';
    rowtindakan[1] =
        '<?php echo CJSON::encode($this->renderPartial($this->path_view . '_rowTindakanPemeriksaan', array('i' => 1, 'modTindakan' => $modTindakan), true)); ?>';
    if ($(obj).is(':checked')) {
        ...
        $("#form-tindakanpemeriksaan-" + form_index).find('input[name$="[ii][pemeriksaanlab_id]"]').val(
            pemeriksaanlab_id);
        $("#form-tindakanpemeriksaan-" + form_index).find('input[name$="[ii][daftartindakan_id]"]').val(
            daftartindakan_id);
        $("#form-tindakanpemeriksaan-" + form_index).find('input[name$="[ii][jenistarif_id]"]').val(jenistarif_id);
        $("#form-tindakanpemeriksaan-" + form_index).find('span[name$="[ii][pemeriksaanlab_nama]"]').html(
            pemeriksaanlab_nama);
        $("#form-tindakanpemeriksaan-" + form_index).find('input[name$="[ii][satuantindakan]"]').val(
            "<?php echo Params::SATUAN_TINDAKAN_LABORATORIUM; ?>");
        $("#form-tindakanpemeriksaan-" + form_index).find('input[name$="[ii][tarif_satuan]"]').val(formatInteger(
            harga_tariftindakan));
        $("#form-tindakanpemeriksaan-" + form_index).find('input[name$="[ii][tarif_tindakan]"]').val(formatInteger(
            harga_tariftindakan));
    } else {
        var delete_row = $("#form-tindakanpemeriksaan-" + form_index).find(
            'input[name$="[pemeriksaanlab_id]"][value="' + pemeriksaanlab_id + '"]').parents('tr');
        delete_row.detach();
    }
...
function pilihPemeriksaanIniNew(obj) {

    console.log('pilih new nih');
    var form_index = $('#form_index').val();
    var pemeriksaanlab_id = $(obj).val();
    
    var pemeriksaanlab_id = $(obj).closest('tr').find('.pemeriksaanlab_id_dialog').val();
    var pemeriksaanlab_nama = $(obj).closest('tr').find('.pemeriksaanlab_nama').val();
    ...
    var kelaspelayanan_id = $(obj).closest('tr').find('.kelaspelayanan_id_dialog').val();

    var rowtindakan = [];

    var is_ada = $('.pemeriksaanlab_id_dialog[value="' + pemeriksaanlab_id + '"]').val();

    console.log("pemeriksaan lab: " + pemeriksaanlab_nama);
    ...
    $('.kelaspelayanan_0').val(kelaspelayanan_id).change();
    // } else {
        // var delete_row = $("#form-tindakanpemeriksaan-" + form_index).find(
            // 'input[name$="[pemeriksaanlab_id]"][value="' + pemeriksaanlab_id + '"]').parents('tr');
        // delete_row.detach();
...
function printKartuPasien(pasien_id) {
    window.open('<?php echo $this->createUrl('PendaftaranRawatJalan/printKartuPasien'); ?>&pasien_id=' + pasien_id,
        'printwin', 'left=100,top=100,width=480,height=640');
}

function printStatus(pendaftaran_id) {
    if(pendaftaran_id != ""){
        window.open('<?php echo Yii::app()->createUrl('/rawatJalan/tindakan/printTindakan'); ?>&id='+pendaftaran_id+'&is_rad=1','printwin','left=100,top=100,width=720,height=640');
    }else{
...
function printStatusLabel(pendaftaran_id) {
    window.open('<?php echo $this->createUrl('printLabel'); ?>&pendaftaran_id=' + pendaftaran_id, 'printwin',
        'left=100,top=100,width=480,height=640');
}
...
function autoPrint() {
    ...
    window.open(
        '<?php echo $this->createUrl('PendaftaranRawatJalan/printKartuPasien', array('pasien_id' => $model->pasien_id)); ?>',
        '', 'left=100,top=100,width=480,height=640');
    <?php  } ?>
    <?php if (Yii::app()->user->getState('printkunjunganlsng') == TRUE) { ?>
    window.open('<?php echo Yii::app()->createUrl('/rawatJalan/tindakan/printTindakan', array('id' => $model->pendaftaran_id)); ?>',
        'printwin', 'left=100,top=100,width=720,height=640');
    <?php  } ?>
...
function setDataPJP() {
    ...
    var pasien_id = $('#LBPasienM_pasien_id').val();
    if (pasien_id) {
        $.ajax({
            type: 'POST',
            ...
            success: function(data) {
                console.log(data);
                if (data) {
                    if (data.tgllahir_pj != null) {
                        var str = data.tgllahir_pj;
                        var myArray = str.split("-");
                        var tgllahir_pj = myArray[2] + '/' + myArray[1] + '/' + myArray[0]

                    } else {
                        var tlglahir_pj = '';
                    }
                    console.log(tgllahir_pj);
                    $("#<?php echo CHtml::activeId($modPenanggungJawab, 'pengantar') ?>").val(data
                        .pengantar);
                    $("#<?php echo CHtml::activeId($modPenanggungJawab, 'nama_pj') ?>").val(data.nama_pj);
                    // $("#<?php echo CHtml::activeId($modPenanggungJawab, 'jeniskelamin') ?>").prop('value', data.jeniskelamin);
                    $('input:radio[value=' + data.jeniskelamin + ']').prop('checked', true);
                    $("#<?php echo CHtml::activeId($modPenanggungJawab, 'jenisidentitas') ?>").val(data
                        .jenisidentitas);
                    $("#<?php echo CHtml::activeId($modPenanggungJawab, 'no_identitas') ?>").val(data
                        .no_identitas);
                    $("#<?php echo CHtml::activeId($modPenanggungJawab, 'no_teleponpj') ?>").val(data
                        .no_teleponpj);
                    $("#<?php echo CHtml::activeId($modPenanggungJawab, 'no_mobilepj') ?>").val(data
                        .no_mobilepj);
                    $("#<?php echo CHtml::activeId($modPenanggungJawab, 'hubungankeluarga') ?>").val(data
                        .hubungankeluarga);
                    $("#<?php echo CHtml::activeId($modPenanggungJawab, 'tempatlahir_pj') ?>").val(data
                        .tempatlahir_pj);
                    $("#<?php echo CHtml::activeId($modPenanggungJawab, 'tgllahir_pj') ?>").val(
                    tgllahir_pj);
                    $("#<?php echo CHtml::activeId($modPenanggungJawab, 'alamat_pj') ?>").val(data
                        .alamat_pj);
                    setUmurPjp(data.tgllahir_pj);
                } else {
                    myAlert('Pasien belum memiliki penanggungjawab sebelumnya')
                }
...
$(document).ready(function() {
    ...
    jQuery(cara).multiselect({
        ...
        onChange: function(element, checked) {
            var cara = jQuery('#<?php echo CHtml::activeId($model, 'carabayar_id') ?>');
            var cara_all = jQuery(
                '#<?php echo CHtml::activeId($model, 'carabayar_id') ?>   option:selected');
            var penj = jQuery('#<?php echo CHtml::activeId($model, 'penjamin_id') ?>');
            var v = $(element).val();
