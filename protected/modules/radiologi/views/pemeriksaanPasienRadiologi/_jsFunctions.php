...
function setKunjungan(pasienmasukpenunjang_id){
    $("#rujukanKeluar").find('#PemeriksaankeluarT_daftartindakan_id option').remove();
    ...
    $.ajax({
        ...
        success:function(data){
			if ( (data.statusperiksa == '<?php echo Params::STATUSPERIKSA_NUNGGU_DAFTAR_SO ?>') || (data.statusperiksa == '<?php echo Params::STATUSPERIKSA_SUDAH_PULANG ?>') ){
				alert("Maaf, Pasien "+data.statusperiksa);
			...
			//generate pemeriksaan rujukan keluar 
			if (data.rsrujukkeluar == 1){
				$("#table-rujukankeluar").html(data.rujuk);
			}
 ...
 *  
 * @param {type} data
 * @returns {mengenerate form rujukan keluar, jika datanya sudah ada akan ditampilkan} */
function setRujukanKeluarRs(data){
    if (data.ada != ''){
        
        $("#<?php echo CHtml::activeId($modRujukKeluar, 'labklinikrujukan_id') ?>").val(data.labklinikrujukan_id);
        $("#<?php echo CHtml::activeId($modRujukKeluar, 'pemeriksaankeluar_tgl') ?>").val(data.pemeriksaankeluar_tgl);
        $("#<?php echo CHtml::activeId($modRujukKeluar, 'pemeriksaankeluar_alasan') ?>").val(data.pemeriksaankeluar_alasan);
        $("#<?php echo CHtml::activeId($modRujukKeluar, 'pemeriksaankeluar_ket') ?>").val(data.pemeriksaankeluar_ket);
        setTimeout(function(){ 
            $("#<?php echo CHtml::activeId($modRujukKeluar, 'dokterpengirim_id') ?>").val(data.dokterpengirim_id);
        });
        $.each(data.daftartindakan_id,function(key,val){
            setTimeout(function(){ 
                $('#<?php echo CHtml::activeId($modRujukKeluar, 'daftartindakan_id') ?>').multiselect('select', val, true);            
            }, 1000); 
        });
...
function setTindakanPelayanan(){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            $("#rujukanKeluar").find('#PemeriksaankeluarT_daftartindakan_id').append(data.drop);
            //$('#PemeriksaankeluarT_daftartindakan_id').append(data.drop);
            //jQuery(jQuery('#<?php echo CHtml::activeId($modRujukKeluar, 'daftartindakan_id') ?>')).multiselect('rebuild');	
            //renameInput(data.rujuk.daftartindakan_id);
            if (data.rujuk.labklinikrujukan_id != ''){
                //setRujukanKeluarRs(data.rujuk);
 ...
 * @returns {memanggil window dialog box untuk menampilkan prinout} */
function printHasil()
{                
    var pasienmasukpenunjang_id = $("#pasienmasukpenunjang_id").val();
...
function renameInput() {
	...
	$("#form-tindakanpemeriksaan > table >tbody > tr").each(function(){
		option += '<option value="'+$(this).find("input[name$='[daftartindakan_id]']").val()+'-'+$(this).find("input[name$='[tindakanpelayanan_id]']").val()+'">'+$(this).find("span[name$='[pemeriksaanrad_nama]']").html()+'</option>';
	});
	...
	$(obj_table).find("tbody > tr").each(function () {		
		$(this).find('input,select,textarea').each(function () { //element <input>		
			...
			if (old_name_arr[2] == 'daftartindakan_id'){				
				...
				if ($(this).parents('tr').find(".pemeriksaankeluar_id").val() != ''){
...
//penambahan animasi untuk load text dokter luar
$("#judul").hide(300);
$("#ceklis").change(function(){
    var checkBox = document.getElementById("ceklis");
    var text = document.getElementById("dokter");
...
<?php if(!empty($modKunjungan->pasienmasukpenunjang_id)){ ?>
	...
	//$(".add-on").hide();
      //  $("#PemeriksaankeluarT_pemeriksaankeluar_tgl_date").show();
    setTindakanPelayanan();
    setTindakanRegistrasi();
    setChecklistPemeriksaanRad();
<?php } ?>
       ...
       //setTimeout(function(){ 
         //    jQuery(jQuery('#<?php //echo CHtml::activeId($modRujukKeluar, 'daftartindakan_id') ?>')).multiselect({
