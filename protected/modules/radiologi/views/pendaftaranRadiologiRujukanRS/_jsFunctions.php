...
function setKunjungan(pasienkirimkeunitlain_id){
    $("#form-datakunjungan > div > div.panel-body").addClass("animation-loading");
    $.ajax({
        type:'POST',
...
function pilihPemeriksaanIni(obj){
    ...
    var rowtindakan = [];
    rowtindakan = '<?php echo CJSON::encode($this->renderPartial('radiologi.views.pendaftaranRadiologiRujukanRS._rowTindakanPemeriksaan',array('i'=>0,'modTindakan'=>$modTindakan),true));?>';
    if($(obj).is(':checked')){
...
function setPemeriksaanPaket(obj){
    ...
    var pemeriksaanrad_id = $(obj).val();
    var pemeriksaanrad_nama = parents.find('span[name$="[pemeriksaanrad_nama]"]').html();
...
function setPermintaanKePenunjang(){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            $('#form-tindakanpemeriksaan table > tbody').html(data.rows_periksa);
            renameInputRow($("#form-tindakanpemeriksaan"));
...
function cekHargaKelasPelayanan(obj) {
    ...
    var pemeriksaanrad_id = $(obj).parents("tr").find(".permintaan_pemeriksaanrad_id").val();
...
function tambahPeriksaPermintaan(obj) {
    ...
    $.post('<?php echo $this->createUrl('setFormTindakanDariPermintaan0'); ?>', {
        ...
        var last = $("#form-tindakanpemeriksaan table > tbody tr").last();

        setTimeout(() => {
                
                jQuery(last).find('.tgl_tindakan_last').datetimepicker(
                jQuery.extend({showMonthAfterYear: false},
                    jQuery.datepicker.regional['id'], {
                        'dateFormat': 'dd M yy', 
                        'timeText': 'Waktu', 
