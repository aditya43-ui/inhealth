<script>
    function tambah()
    {
        ...
        var peluang_id = $('#peluang_id').val();
        var konsekuensi_id = $('#konsekuensi_id').val();
        var tingkatrisiko_riskregister_id = $('#tingkatrisiko_riskregister_id').val();
        var detectability_id = $('#detectability_id').val();
        var warnarisiko = $('#warnarisiko').val();
        ...
        $.post('<?php echo $this->createUrl('getTabel'); ?>', { 
            ...
            tingkatrisiko_riskregister_id:tingkatrisiko_riskregister_id, 
            detectability_id:detectability_id, 
            warnarisiko:warnarisiko, 
            aktif:aktif},
        function(data){
            if(data.message == 'sukses'){
                var cekAda = 0;
                $('#table-master').find("tbody > tr").each(function(){
                   var peluang_id = $(this).find('.peluang_id').val();
                   var konsekuensi_id = $(this).find('.konsekuensi_id').val();
                   var tingkatrisiko_riskregister_id = $(this).find('.tingkatrisiko_riskregister_id').val();
                   var detectability_id = $(this).find('.detectability_id').val();
                   if((peluang_id == data.peluang_id) && (konsekuensi_id == data.konsekuensi_id) && (tingkatrisiko_riskregister_id == data.tingkatrisiko_riskregister_id) && (detectability_id == data.detectability_id)){
                       cekAda = 1;
                   }
                });
                if(cekAda == 0){
    ...
    function setWarnaRisiko(){
        var tingkatrisiko_riskregister_id = $('#tingkatrisiko_riskregister_id').val();
        $.post('<?php echo $this->createUrl('getWarnaRisiko'); ?>', { 
            tingkatrisiko_riskregister_id:tingkatrisiko_riskregister_id},
        function(data){
            $('#warnarisiko').val(data.return);
        }, "json");
    }
    
    function setWarnaRisikoUpdate(){
        var tingkatrisiko_riskregister_id = $('#GradingrisikoRiskregisterM_tingkatrisiko_riskregister_id').val();
        $.post('<?php echo $this->createUrl('getWarnaRisiko'); ?>', { 
            tingkatrisiko_riskregister_id:tingkatrisiko_riskregister_id},
        function(data){
            $('#GradingrisikoRiskregisterM_warnarisiko').val(data.return);
        }, "json");
    }
    ...
    function resetForm(){
        $('#peluang_id').val("");
        $('#konsekuensi_id').val("");
        $('#tingkatrisiko_riskregister_id').val("");
        $('#detectability_id').val("");
        $('#warnarisiko').val("");
