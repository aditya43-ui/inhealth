...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('tipeinsiden-m-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
<div class="panel panel-gradient">
    <div class="panel-heading">
        <div class="panel-title">Pengaturan <b>Tipe Insiden</b></div>
    </div>
	...
	<div class="cari-lanjut2 search-form" style="display:none">
        ...
        <div class="panel panel-success">
            <div class="panel-heading">
                <div class="panel-title">Tabel <b>Tipe Insiden</b></div>
            </div>
            <div class="panel-body">
                <?php $this->widget('ext.bootstrap.widgets.BootGridView',array(
                    ...
                    'columns'=>array(
                            ...
                            array(
                                ...
                                'filter' => Chtml::activeTextField($model, 'tipeinsiden_nama', array('class' => 'hurufs-alatmedis'))
                            ),
                            array(
                                'header' => 'Nama Lain Tipe Insiden',
                                'name' => 'tipeinsiden_namalainnya',
                                'filter' => Chtml::activeTextField($model, 'tipeinsiden_namalainnya', array('class' => 'hurufs-only'))
                            ),
                            array(
                                'header'=>'<center>Status</center>',
                                'value'=>'($data->tipeinsiden_aktif == 1 ) ? "Aktif" : "Tidak Aktif"',
                            ...
                            array(
                                ...
                                'buttons'=>array(
                                    'remove' => array (
                                        ...
                                        'url'=>'Yii::app()->createUrl("'.Yii::app()->controller->module->id.'/'.Yii::app()->controller->id.'/nonActive",array("id"=>$data->tipeinsiden_id))',
                                        'click'=>'function(){nonActive(this);return false;}',
                                        'visible'=>'(($data->tipeinsiden_aktif) ? TRUE : FALSE)',
                                    ),
                                    'add' => array (
                                        'label'=>"<i class='glyphicon glyphicon-ok'></i> ",
                                        'options'=>array('title'=>Yii::t('mds','Add Temporary')),
                                        'url'=>'Yii::app()->createUrl("'.Yii::app()->controller->module->id.'/'.Yii::app()->controller->id.'/active",array("id"=>$data->tipeinsiden_id))',
                                        'click'=>'function(){active(this);return false;}',
                                        'visible'=>'(($data->tipeinsiden_aktif) ? FALSE : TRUE)',
                                     ),
                                    'delete'=> array(
                                        'label'=>"<i class='glyphicon glyphicon-ok'></i> ",
                                        'options'=>array('title'=>Yii::t('mds','Hapus')),
                                        'url'=>'Yii::app()->createUrl("'.Yii::app()->controller->module->id.'/'.Yii::app()->controller->id.'/deleteRecord",array("id"=>$data->tipeinsiden_id))',
...
<?php 
    echo CHtml::link(Yii::t('mds','{icon} Tambah Tipe Insiden',array('{icon}'=>'<i class="entypo-plus"></i>')),$this->createUrl('create',array('modul_id'=> Yii::app()->session['modul_id'])), array('class'=>'btn btn-success'))."&nbsp&nbsp"; 
...
{
    window.open("${urlPrint}/"+$('#tipeinsiden-m-search').serialize()+"&caraPrint="+caraPrint,"",'location=_new, width=900px');
}
JSCRIPT;
...
<script type="text/javascript">	
	...
	{
            $("#tipeinsiden-m-search :input[name='"+obj.name+"']").val(obj.value);
	}
                ...
                function(r){
                    if(r){ 
                        $.ajax({
                            ...
                            error: function (jqXHR, textStatus, errorThrown) { myAlert('Data gagal dihapus karena data digunakan oleh <br>Master Kelompok Subtipe Insiden atau Master Subtipe Insiden.'); console.log(errorThrown);}
                        });
                    }
                }
            );
