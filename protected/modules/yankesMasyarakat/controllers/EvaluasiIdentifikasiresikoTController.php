 ...
 * RSST-5696
 */
class EvaluasiIdentifikasiresikoTController extends MyAuthController {

    /**
     * Default menu transaksi identifikasi resiko
     * @param integer $identifikasiresiko_id
     */
    public function actionIndex($identifikasiresiko_id = null, $evaluasiidentifikasirisiko_id = null) {
        ...
        if (isset($_POST['YKMEvaluasiidentifikasirisikoT'])) {
            $trans = Yii::app()->db->beginTransaction();
            try {
                $modEvaluasi->attributes = $_POST['YKMEvaluasiidentifikasirisikoT'];
                $modEvaluasi->identifikasirisiko_id = $identifikasiresiko_id;
                ...
                $modIdentifikasi = IdentifikasiresikoT::model()->findByPk($identifikasiresiko_id);
                $modIdentifikasi->attributes = $_POST['YKMIdentifikasiresikoT'];
                if ($modEvaluasi->save() && $modIdentifikasi->save()) {
                    $trans->commit();
                    $this->redirect(array('index', 'sukses' => 1, 'identifikasiresiko_id' => $identifikasiresiko_id, 'evaluasiidentifikasirisiko_id' => $modEvaluasi->evaluasiidentifikasirisiko_id));
                } else {
    ...
    /**
     * Autocomplete evaluator pegawai 
     */
