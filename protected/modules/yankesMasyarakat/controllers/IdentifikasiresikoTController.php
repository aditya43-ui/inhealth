 ...
 * RSST-5636
 */
class IdentifikasiresikoTController extends MyAuthController{
    public $path_view = 'yankesMasyarakat.views.identifikasiresikoT.';
    /**
     * Default menu transaksi identifikasi resiko
     * @param integer $identifikasiresiko_id
     */
    public function actionIndex($identifikasiresiko_id = null){
        ...
        if(isset($_POST['YKMIdentifikasiresikoT']) || isset($_POST['YKMEvaluasiidentifikasirisikoT']) ){
            $trans = Yii::app()->db->beginTransaction();
            try{
                $model->attributes = $_POST['YKMIdentifikasiresikoT'];
                
                if (empty($model->identifikasiresiko_id)){
                    $modEvaluasi->create_time = date('Y-m-d H:i:s');
                    $modEvaluasi->create_loginpemakai_id = Yii::app()->user->getState('loginpemakai_id');
                    $modEvaluasi->create_ruangan = Yii::app()->user->getState('ruangan_id');
                ...
                if($model->save()){
                        $modEvaluasi->attributes = $_POST['YKMEvaluasiidentifikasirisikoT'];
                        $modEvaluasi->identifikasirisiko_id=$model->identifikasiresiko_id;
                        $modEvaluasi->tgl_mulai= $format->formatDateTimeForDb($_POST['YKMEvaluasiidentifikasirisikoT']['tgl_mulai']);
                        $modEvaluasi->tgl_tinjauan = $format->formatDateTimeForDb($_POST['YKMEvaluasiidentifikasirisikoT']['tgl_tinjauan']);

                    if (empty($modEvaluasi->identifikasiresiko_id)){
                        $modEvaluasi->create_time = date('Y-m-d H:i:s');
                        $modEvaluasi->create_loginpemakai_id = Yii::app()->user->getState('loginpemakai_id');
                        $modEvaluasi->create_ruangan = Yii::app()->user->getState('ruangan_id');
                        $model->create_time = date('Y-m-d H:i:s');
    ...
    public function actionGetTingkatRisiko() {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            if (!empty($modSum)){
                $data['tingkatrisiko_id'] = $modSum->tingkatrisiko_riskregister_id;
    ...
    public function actionLihatPetunjuk(){
        $modPetunjuk = PetunjuktransaksiM::model()->findByAttributes(array('petunjuktransaksi_type' => 'Transaksi Risk Register',));
        $this->render($this->path_view.'petunjuk', array('modPetunjuk' => $modPetunjuk));
    }
}