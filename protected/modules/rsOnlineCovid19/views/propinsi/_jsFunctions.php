<script type="text/javascript">
    function searchData() {
        $.ajax({
            ...
            dataType: "json",
             beforeSend: function(){
                $("#table-propinsi").addClass("animation-loading");
            },
            success: function (data) {
                $("#table-propinsi").removeClass("animation-loading");
                if(data.status!= '200'){
                  $("#table-propinsi > tbody > tr").remove();
                    $('#table-propinsi > tbody').html(data.form);
                    renameInputRow($("#table-propinsi"));  
                }else{
                    myAlert(data.pesan);
                }
            },
