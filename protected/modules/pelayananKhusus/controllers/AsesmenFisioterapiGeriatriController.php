...
/**
 *
 * controller transaksi asesmen fisioterapi geriatri
 *
 * @package      application.modules.rehabMedis
 * @subpackage   controllers
 * @author      M Iqbal Laksana <iqbal.laksana@piindonesia.co.id>
 */
class AsesmenFisioterapiGeriatriController extends MyAuthController
{
    public $defaultAction = 'index';
    public $path_view = 'rehabMedis.views.asesmenFisioterapiGeriatri.';
    public $init = '';
    ...
    public function actionIndex($pendaftaran_id,$pasienadmisi_id=null,$pasienmasukpenunjang_id)
    {
      ...
      $model = AsesmenFisioterapiGeriatriT::model()->findByAttributes(array('pendaftaran_id'=>$modPendaftaran->pendaftaran_id,'pasienmasukpenunjang_id' => $pasienmasukpenunjang_id));
      ...
      if(isset($model) && !empty($model->asesmen_fisioterapi_geriatri_id)){
        $modAsesmenmmtT = AsesmenmmtT::model()->findAllByAttributes(array('asesmen_fisioterapi_geriatri_id'=>$model->asesmen_fisioterapi_geriatri_id));
        $oriPeriksaExtra = PeriksagerakdasardextraasesmenT::model()->findAllByAttributes(array('asesmen_fisioterapi_geriatri_id'=>$model->asesmen_fisioterapi_geriatri_id), array(
            'select' =>'distinct on (periksafungsigerakdasar_id) *', 'order'=>'periksafungsigerakdasar_id, periksagerakdasardextra_id'
        ));
        $oriPeriksaExtraSinistra = PeriksagerakdasarsinistraasesmenT::model()->findAllByAttributes(array('asesmen_fisioterapi_geriatri_id'=>$model->asesmen_fisioterapi_geriatri_id), array(
        ...
        $oriPeriksaDextra = PeriksagerakdasardextraasesmenT::model()->findAllByAttributes(array('asesmen_fisioterapi_geriatri_id'=>$model->asesmen_fisioterapi_geriatri_id));
        $oriPeriksaSinistra = PeriksagerakdasarsinistraasesmenT::model()->findAllByAttributes(array('asesmen_fisioterapi_geriatri_id'=>$model->asesmen_fisioterapi_geriatri_id));

        $loadDet = RMAsesmenFisioterapiGeriatridetT::model()->findAllByAttributes(array('asesmen_fisioterapi_geriatri_id' => $model->asesmen_fisioterapi_geriatri_id));
        if(count($loadDet) > 0){
          foreach($loadDet as $det){
              $getDet[$det->activitydailyliving_id]['asesmen_fisioterapi_geriatridet_id'] = $det->asesmen_fisioterapi_geriatridet_id;
              $getDet[$det->activitydailyliving_id]['asesmen_fisioterapi_geriatri_id'] = $det->asesmen_fisioterapi_geriatri_id;
              $getDet[$det->activitydailyliving_id]['skor'] = $det->skor;
              $getDet[$det->activitydailyliving_id]['keterangan_det'] = $det->keterangan;
          }
        }
        ...
        $model = new AsesmenFisioterapiGeriatriT();
      ...
      $adp = RMActivitydailylivingM::model()->findAll(" activitydailyliving_aktif = TRUE ORDER BY activitydailyliving_urutan ASC ");
      foreach($adp as $a){
          if (isset($getDet[$a->activitydailyliving_id])){
              $loadMaster[$a->activitydailyliving_id] = $getDet[$a->activitydailyliving_id];
          }
          $loadMaster[$a->activitydailyliving_id]['activitydailyliving_id'] = $a->activitydailyliving_id;
          $loadMaster[$a->activitydailyliving_id]['fungsi'] = $a->activitydailyliving_fungsi;
          $loadMaster[$a->activitydailyliving_id]['skormax'] = $a->activitydailyliving_skormax;
          $loadMaster[$a->activitydailyliving_id]['skormin'] = $a->activitydailyliving_skormin;
          $loadMaster[$a->activitydailyliving_id]['keterangan'] = $a->activitydailyliving_keterangan;
      }
      ...
      if(isset($pemeriksaanFisik) && empty($model->asesmen_fisioterapi_geriatri_id)){
      ...
      if(isset($_POST['AsesmenFisioterapiGeriatriT'])) {
          $transaction = Yii::app()->db->beginTransaction();

          try {
              $model->attributes = $_POST['AsesmenFisioterapiGeriatriT'];
              $model->tanggal_catat = MyFormatter::formatDateTimeForDb($_POST['AsesmenFisioterapiGeriatriT']['tanggal_catat']);
              $model->jam_pengisian = (!empty($_POST['AsesmenFisioterapiGeriatriT']['jam_pengisian'])? $_POST['AsesmenFisioterapiGeriatriT']['jam_pengisian'] : null);

              if(!empty($model->asesmen_fisioterapi_geriatri_id)){
              ...
              $tersimpanNilaiDet = true;


              if(isset($_POST['AsesmenFisioterapiGeriatriT']['statik']) && count($_POST['AsesmenFisioterapiGeriatriT']['statik']) > 0){


                $statik_data = array();
                foreach($_POST['AsesmenFisioterapiGeriatriT']['statik'] as $statik){
            ...
            if(isset($_POST['AsesmenFisioterapiGeriatriT']['dinamis']) && count($_POST['AsesmenFisioterapiGeriatriT']['dinamis']) > 0){

                $dinamis_data = array();
                foreach($_POST['AsesmenFisioterapiGeriatriT']['dinamis'] as $dinamis){
            ...
            if(isset($_POST['AsesmenFisioterapiGeriatriT']['palpasi']) && count($_POST['AsesmenFisioterapiGeriatriT']['palpasi']) > 0){

                $palpasi_data = array();
                foreach($_POST['AsesmenFisioterapiGeriatriT']['palpasi'] as $palpasi){
              ...
              if($model->save()){
                    ...
                    PeriksagerakdasardextraasesmenT::model()->deleteAllByAttributes(array('asesmen_fisioterapi_geriatri_id'=>$model->asesmen_fisioterapi_geriatri_id));
                    PeriksagerakdasarsinistraasesmenT::model()->deleteAllByAttributes(array('asesmen_fisioterapi_geriatri_id'=>$model->asesmen_fisioterapi_geriatri_id));
                    ...
                    if (isset($_POST['PeriksagerakdasardextraT']) && is_array($_POST['PeriksagerakdasardextraT'])) {
                        ...
                        foreach ($_POST['PeriksagerakdasardextraT'] as $idx => $item) {
                            ...
                            foreach ($item as $idx2 => $detail) {
                                ...
                                $modPeriksaExt->asesmen_fisioterapi_geriatri_id = $model->asesmen_fisioterapi_geriatri_id;
...
//                                var_dump($modPeriksaExt->attributes, $detail);
                            ...
                            if (isset($_POST['PeriksagerakdasarsinistraT'][$idx]) && is_array($_POST['PeriksagerakdasarsinistraT'][$idx])) {
                                ...
                                foreach ($_POST['PeriksagerakdasarsinistraT'][$idx] as $idx2 => $detail) {
                                    ...
                                    $modDetSinistra->asesmen_fisioterapi_geriatri_id = $model->asesmen_fisioterapi_geriatri_id;
                  ...
                  if (isset($_POST['PemeriksaanFisikMMT']) && count($_POST['PemeriksaanFisikMMT']) > 0) {
                    AsesmenmmtT::model()->deleteAllByAttributes(array(
                        'asesmen_fisioterapi_geriatri_id' => $model->asesmen_fisioterapi_geriatri_id
                    ));
                    foreach ($_POST['PemeriksaanFisikMMT'] as $fisikMMt) {
                      if(!empty($fisikMMt['kiri']) || !empty($fisikMMt['kanan'])){
                        ...
                        $modDetFisik->asesmen_fisioterapi_geriatri_id = $model->asesmen_fisioterapi_geriatri_id;
                  ...
                  if (isset($_POST['RMAsesmenFisioterapiGeriatridetT']) && count($_POST['RMAsesmenFisioterapiGeriatridetT']) > 0) {

                    foreach($_POST['RMAsesmenFisioterapiGeriatridetT'] as $det){
                          $modDet = new RMAsesmenFisioterapiGeriatridetT;

                          $cekDet = RMAsesmenFisioterapiGeriatridetT::model()->findByPK($det['asesmen_fisioterapi_geriatridet_id']);

                          if (!empty($cekDet)){
                              $modDet = $cekDet;
                          }
                          $modDet->attributes = $det;
                          $modDet->asesmen_fisioterapi_geriatri_id = $model->asesmen_fisioterapi_geriatri_id;
                          if(!$modDet->save()){
                            $tersimpanNilaiDet = false;
                          }
                      }
                  }
              ...
              if($this->tersimpan == true && $tersimpanSinistra == true && $tersimpanExtra == true && $tersimpanMMT == true && $tersimpanNilaiDet == true){
                  $transaction->commit();
    ...
    public function actionPrint($pendaftaran_id, $pasienmasukpenunjang_id) {
      ...
      if(isset($model) && !empty($model->asesmen_fisioterapi_geriatri_id)){
        ...
        if (count($oriPeriksaExtraSinistra) > count($oriPeriksaExtra)) {
            $oriPeriksaExtra = $oriPeriksaExtraSinistra;
        }
