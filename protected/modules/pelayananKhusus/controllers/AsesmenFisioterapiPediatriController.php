 ...
 */
class AsesmenFisioterapiPediatriController extends MyAuthController
{
    public $defaultAction = 'index';
    public $path_view = 'rehabMedis.views.asesmenFisioterapiPediatri.';
    public $init = '';
     ...
     */
     ...
     public function actionIndex($pendaftaran_id,$pasienadmisi_id=null,$pasienmasukpenunjang_id)
     {
         ...
         $model = AsesmenFisioterapiPediatriT::model()->findByAttributes(array('pendaftaran_id'=>$modPendaftaran->pendaftaran_id,'pasienmasukpenunjang_id' => $pasienmasukpenunjang_id));
         $modAsesmenmmtT = array();
         $oriPeriksaExtra = array();
         $oriPeriksaSinistra = array();
         $oriPeriksaDextra = array();

         if(isset($model) && !empty($model->asesmen_fisioterapi_pediatri_id)){
            ...
            $oriPeriksaExtra = PeriksagerakdasardextraasesmenT::model()->findAllByAttributes(array('asesmen_fisioterapi_pediatri_id'=>$model->asesmen_fisioterapi_pediatri_id), array(
                'select' =>'distinct on (periksafungsigerakdasar_id) *', 'order'=>'periksafungsigerakdasar_id, periksagerakdasardextra_id'
            ));
            $oriPeriksaExtraSinistra = PeriksagerakdasarsinistraasesmenT::model()->findAllByAttributes(array('asesmen_fisioterapi_pediatri_id'=>$model->asesmen_fisioterapi_pediatri_id), array(
            ...
            $oriPeriksaDextra = PeriksagerakdasardextraasesmenT::model()->findAllByAttributes(array('asesmen_fisioterapi_pediatri_id'=>$model->asesmen_fisioterapi_pediatri_id));
            ...
            if(!empty($hasilpemeriksaan)){
                ...
                if (empty($model->evaluasidant)){
                    $model->evaluasidant = $hasilpemeriksaan->evaluasi;
                }

            }

         }else{
           $model = new AsesmenFisioterapiPediatriT();
         ...
         if(isset($pemeriksaanFisik) && empty($model->asesmen_fisioterapi_pediatri_id)){
        ...
        if (!empty($model->skala_wongbaker_nrs)) {
         ...
         if(isset($_POST['AsesmenFisioterapiPediatriT'])) {
             $transaction = Yii::app()->db->beginTransaction();

             try {
                 $model->attributes = $_POST['AsesmenFisioterapiPediatriT'];
                 $model->tanggal_catat = MyFormatter::formatDateTimeForDb($_POST['AsesmenFisioterapiPediatriT']['tanggal_catat']);
                 $model->jam_pengisian = (!empty($_POST['AsesmenFisioterapiPediatriT']['jam_pengisian'])? $_POST['AsesmenFisioterapiPediatriT']['jam_pengisian'] : null);

                 if(!empty($model->asesmen_fisioterapi_pediatri_id)){
                 ...
                 if(isset($_POST['AsesmenFisioterapiPediatriT']['inspeksi']) && count($_POST['AsesmenFisioterapiPediatriT']['inspeksi']) > 0){


                    $inspeksi_data = array();
                    foreach($_POST['AsesmenFisioterapiPediatriT']['inspeksi'] as $inspeksi){

                        $inspeksi_data[] = $inspeksi;
                    }

                    if(count($inspeksi_data) > 0){
                        $model->inspeksi = json_encode($inspeksi_data);
                    }
                }

                if(isset($_POST['AsesmenFisioterapiPediatriT']['dinamis']) && count($_POST['AsesmenFisioterapiPediatriT']['dinamis']) > 0){

                    $dinamis_data = array();
                    foreach($_POST['AsesmenFisioterapiPediatriT']['dinamis'] as $dinamis){
                ...
                if(isset($_POST['AsesmenFisioterapiPediatriT']['palpasi']) && count($_POST['AsesmenFisioterapiPediatriT']['palpasi']) > 0){

                    $palpasi_data = array();
                    foreach($_POST['AsesmenFisioterapiPediatriT']['palpasi'] as $palpasi){
                 ...
                 if($model->save()){
                    ...
                    PeriksagerakdasardextraasesmenT::model()->deleteAllByAttributes(array('asesmen_fisioterapi_pediatri_id'=>$model->asesmen_fisioterapi_pediatri_id));
                    PeriksagerakdasarsinistraasesmenT::model()->deleteAllByAttributes(array('asesmen_fisioterapi_pediatri_id'=>$model->asesmen_fisioterapi_pediatri_id));
                    ...
                    if (isset($_POST['PeriksagerakdasardextraT']) && is_array($_POST['PeriksagerakdasardextraT'])) {
                        ...
                        foreach ($_POST['PeriksagerakdasardextraT'] as $idx => $item) {
                            ...
                            foreach ($item as $idx2 => $detail) {
                                ...
                                $modPeriksaExt->asesmen_fisioterapi_pediatri_id = $model->asesmen_fisioterapi_pediatri_id;
...
//                                var_dump($modPeriksaExt->attributes, $detail);
                            ...
                            if (isset($_POST['PeriksagerakdasarsinistraT'][$idx]) && is_array($_POST['PeriksagerakdasarsinistraT'][$idx])) {
                                ...
                                foreach ($_POST['PeriksagerakdasarsinistraT'][$idx] as $idx2 => $detail) {
                                    ...
                                    $modDetSinistra->asesmen_fisioterapi_pediatri_id = $model->asesmen_fisioterapi_pediatri_id;
                     ...
                     if (isset($_POST['PemeriksaanFisikMMT']) && count($_POST['PemeriksaanFisikMMT']) > 0) {
                       AsesmenmmtT::model()->deleteAllByAttributes(array(
                           'asesmen_fisioterapi_pediatri_id' => $model->asesmen_fisioterapi_pediatri_id
                       ));
                       foreach ($_POST['PemeriksaanFisikMMT'] as $fisikMMt) {
                         if(!empty($fisikMMt['kiri']) || !empty($fisikMMt['kanan'])){
                           ...
                           $modDetFisik->asesmen_fisioterapi_pediatri_id = $model->asesmen_fisioterapi_pediatri_id;
    ...
    public function actionTambahDextra()
    {
     ...
     public function actionPrint($pendaftaran_id, $pasienmasukpenunjang_id=null) {
       ...
       $model = AsesmenFisioterapiPediatriT::model()->findByAttributes(array('pendaftaran_id'=>$modPendaftaran->pendaftaran_id,'pasienadmisi_id'=>$modPendaftaran->pasienadmisi_id,'pasienmasukpenunjang_id' => $pasienmasukpenunjang_id));
       $modAsesmenmmtT = array();
       $oriPeriksaExtra = array();
       $oriPeriksaSinistra = array();
