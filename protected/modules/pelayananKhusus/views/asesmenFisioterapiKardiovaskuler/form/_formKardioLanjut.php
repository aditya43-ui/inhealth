...
<div class="col-sm-12 satu-check">
    ...
    <div class="control-group">
        <label class="control-label">- Suara Jantung</label>
        <div class="controls">
            <table width="100%">
                <?php
                    $i = 1;
                    foreach($look[Params::LOOKUPTYPE_KARDIOPULMONAL_KHUSUS_AUSKULTASI_JANTUNG]['type'] as $det){
                        if ($i ==0){
                            echo "<tr>";
                        }
                            echo "<td>".$form->checkBox($model,'[det_khusus_auskultasi_jantung][]khusus_auskultasi_suarajantung',array('checked'=>($model->khusus_auskultasi_suarajantung == $det['value'])?true:false,'uncheckValue'=>null,'value'=>$det['value']))." <label>".$det['name']."&nbsp;&nbsp;&nbsp;</label></td>";
                        if ($i != 0){
                            if ($i == count($look[Params::LOOKUPTYPE_KARDIOPULMONAL_KHUSUS_AUSKULTASI_JANTUNG]['type'])+1){
                                echo "</tr>";
                            }else{
                                if ($i % 2 == 0){
                                    echo "</tr><tr>";
