       ...
       <table width="100%" class="tablepadding">
         ...
         <tr>
           <td style="padding-left: 20px" colspan="5">
             ...
             $reflek = json_decode($model->inspeksi_statik_bentukdada);
             $cek = false;
             if(count($lookupStatikDada) >0 ){
               ...
               foreach($lookupStatikDada as $i => $look_risiko){
                ...
                $htmlRisiko .= "<span ".$paddingstyle." class='".((!empty($model->inspeksi_statik_bentukdada) && ($cek == true))?'fa fa-dot-circle-o':'fa fa-circle-o')."'></span> ".$look_risiko->lookup_value;
               }
               echo $htmlRisiko;
             }
              ?>
         ...
         <tr>
           <td style="padding-left: 20px" colspan="5">
             <div class="borderbottomclass">
               <?php echo $model->inspeksi_detail; ?>
             </div>
           </td>
         </tr>
         <tr>
         ...
         <tr>
           <td style="padding-left: 30px" colspan="5">
             <table width="100%">
               <tr>
                 <td width="120px">Ekspansi Thorax :</td>
                 <td>
                   <?php
                   ...
                   $reflek = json_decode($model->palpasi_ekspansi_thorax);
                   $cek = false;
                   if(count($lookupEkspansi) >0 ){
                     ...
                     foreach($lookupEkspansi as $i => $look_risiko){
                      ...
                      $htmlRisiko .= "<span ".$paddingstyle." class='".((!empty($model->palpasi_ekspansi_thorax) && ($cek == true))?'fa fa-dot-circle-o':'fa fa-circle-o')."'></span> ".$look_risiko->lookup_value;
                     }
                     echo $htmlRisiko;
                   }
                    ?>
               ...
               <tr>
                 <td width="100px">Spasme Otot :</td>
                 <td>
                   <?php
                   $lookupEkspansi = LookupM::model()->findAll("lookup_type = '".Params::LOOKUPTYPE_KARDIOPULMONAL_PALPASI_SPASME."' order by lookup_urutan ASC");
                       ...
                       if(!empty($model->palpasi_spasme_otot)){
                         ...
                         if(count($arrOriSpasme) > 0){
                           foreach ($arrOriSpasme as $dataSpasme) {
                             if($look_risiko->lookup_value == $dataSpasme){
                               $ischeck = true;
                             }
                           }
                         }
         ...
         <tr>
           <td style="padding-left: 20px" valign="top">a. Perkusi :</td>
           <td  colspan="3">
             <table width="100%">
               <?php
               ...
               if(count($lookupPerkusi) >0 ){
                 ...
                 foreach($lookupPerkusi as $i => $look_risiko){
                   ...
                   $htmlRisiko .= "<td width='50%'><span class='".((!empty($model->khusus_perkusi) && ($model->khusus_perkusi ==$look_risiko->lookup_value))?'fa fa-dot-circle-o':'fa fa-circle-o')."'></span> ".$look_risiko->lookup_value."</td>";
                   if($indxRefleks == 2){
                     $htmlRisiko .= "</tr>";
                     $indxRefleks = 0;
                   }
         ...
         <tr>
           <td style="padding-left: 20px" colspan="5">
             <table width="100%">
               ...
               <tr>
                 <td width="100px">Suara Jantung</td>
                 <td width="5px">:</td>
                 <td colspan="2">
                   <?php
                   $lookupJantung = LookupM::model()->findAll("lookup_type = '".Params::LOOKUPTYPE_KARDIOPULMONAL_KHUSUS_AUSKULTASI_JANTUNG."' order by lookup_urutan ASC");

                   if(count($lookupJantung) >0 ){
                     $htmlRisiko = "";
                     foreach($lookupJantung as $i => $look_risiko){
                       $paddingstyle = "";
                      ...
                      $htmlRisiko .= "<span ".$paddingstyle." class='".((!empty($model->khusus_auskultasi_suarajantung) && ($model->khusus_auskultasi_suarajantung ==$look_risiko->lookup_value))?'fa fa-dot-circle-o':'fa fa-circle-o')."'></span> ".$look_risiko->lookup_value;
                     }
                     echo $htmlRisiko;
                   }
                    ?>
               ...
               <tr>
                 <td>Lokasi Sputum</td>
                 <td>:</td>
                 <td width="50%" class="borderbottomclass"><?php echo $model->khusus_auskultasi_lokasisputum; ?></td>
                  <td></td>
               </tr>
             </table>
           </td>
