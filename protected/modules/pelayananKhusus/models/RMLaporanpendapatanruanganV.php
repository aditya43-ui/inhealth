...
class RMLaporanpendapatanruanganV extends LaporanpendapatanruanganV {
    ...
    public function functionCriteria(){
		...
		if (!empty($this->dokterpemeriksa1_id)){
			$criteria->addCondition('dokterpemeriksa1_id ='.$this->dokterpemeriksa1_id);
		}
        $criteria->compare('LOWER(nama_pegawai)', strtolower($this->nama_pegawai), true);
		if (!empty($this->dokterpemeriksa2_id)){
			$criteria->addCondition('dokterpemeriksa2_id ='.$this->dokterpemeriksa2_id);
		}
		if (!empty($this->dokterpendamping_id)){
			$criteria->addCondition('dokterpendamping_id ='.$this->dokterpendamping_id);
		}
		if (!empty($this->dokteranastesi_id)){
			$criteria->addCondition('dokteranastesi_id ='.$this->dokteranastesi_id);
		}
		if (!empty($this->dokterdelegasi_id)){
			$criteria->addCondition('dokterdelegasi_id ='.$this->dokterdelegasi_id);
		}
		if (!empty($this->bidan_id)){
			$criteria->addCondition('bidan_id ='.$this->bidan_id);
		}
		if (!empty($this->suster_id)){
			$criteria->addCondition('suster_id ='.$this->suster_id);
		}
