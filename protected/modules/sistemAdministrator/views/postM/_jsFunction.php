<script>
    ...
    function hapusLookup(obj){
		var postgambar_id = $(obj).parents("tr").find("input[name$='[postgambar_id]']").val();
		if(postgambar_id !== ""){
			...
			function(r){
				if(r){
					$.ajax({
						type:'POST',
						url:'<?php echo $this->createUrl('DeleteGambar'); ?>&id='+postgambar_id,
						data: {id : postgambar_id},//
						dataType: "json",
						success:function(data){
							...
							myAlert(data.pesan);
							var rowCount = $("#tablePetunjuk").find('tbody tr').length;
							if(rowCount==0){
								tambahLookup();
							}
						},
    ...
    function tambahLookup() {
        row = '<?php echo CJSON::encode($this->renderPartial('_rowGambar', array('model' => $modDetail, 'form' => $form), true)); ?>'
