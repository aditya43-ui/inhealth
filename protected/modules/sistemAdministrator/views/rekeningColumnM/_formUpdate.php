...
<div class="row">
    <div class="col-sm-6">
        ...
        <div class='control-group'>
            <?php echo CHtml::label('Nama Tabel <span class="required">*</span>', '', array('class' => 'control-label required')) ?>
            <div class="controls">
                <?php echo $form->textField($model, 'table_name', array('class' => 'span3', 'maxlength' => 100, 'readonly' => true)); ?>
            </div>
        </div>
        <div class='control-group'>
            <?php echo CHtml::label('Nama Kolom <span class="required">*</span>', 'column_name', array('class' => 'control-label required')) ?>
            <div class="controls">
                <?php echo $form->textField($model, 'column_name', array('class' => 'span3', 'maxlength' => 100, 'readonly' => true)); ?>
            </div>
        </div>
    ...
    <div class="col-sm-6">
        <div class='control-group'>
            <?php if ($model->debitkredit == 'D') { ?>
                <?php echo CHtml::label('Rekening Debit', 'rekening debit', array('class' => 'control-label')) ?>
                <div class="controls">
                    <?php echo CHtml::hiddenField('SARekeningcolumnM[rekening][1][debitkredit]', 'D', array('readonly' => true)); ?>
                    <?php echo CHtml::hiddenField('SARekeningcolumnM[rekening][1][rekening5_id]', '', array('readonly' => true, 'class' => '')); ?>
                    <?php
                    $this->widget('MyJuiAutoComplete', array(
                        ...
                        'options' => array(
                            ...
                            'select' => 'js:function( event, ui ) {
                                                $(this).val(ui.item.nmrekening5);
                                                $("#SARekeningcolumnM_rekening_1_rekening5_id").val(ui.item.rekening5_id);
                                            return false;
                                          }'
                        ),
                        'htmlOptions' => array(
            ...
            <?php } else if ($model->debitkredit == 'K') { ?>
                <?php echo CHtml::label('Rekening Kredit', 'rekening kredit', array('class' => 'control-label')) ?>
                <div class="controls">
                    <?php echo CHtml::hiddenField('SARekeningcolumnM[rekening][2][debitkredit]', 'K', array('readonly' => true)); ?>
                    <?php echo CHtml::hiddenField('SARekeningcolumnM[rekening][2][rekening5_id]', '', array('readonly' => true, 'class' => '')); ?>
                    <?php
                    $this->widget('MyJuiAutoComplete', array(
                        'model' => $model,
                        'attribute' => 'rekKredit',
                        ...
                        'options' => array(
                            ...
                            'select' => 'js:function( event, ui ) {
                                                                        $(this).val(ui.item.nmrincianobyek);
                                                                         $("#SARekeningcolumnM_rekening_2_rekening5_id").val(ui.item.rekening5_id);
                                                                                return false;
                                                                  }'
                        ),
                        'htmlOptions' => array(
    ...
    <div class="panel panel-success">
        ...
        <div class="panel-body table-responsive">
            <table class="table table-bordered datatable" id="tab_rekening_debit">
                ...
                <tbody>
                    <?php
                    $r = Rekening5M::model()->findByPk($model->rekening5_id);
                    echo $this->renderPartial($this->path_view . '_rowRekening', array('r' => $r, 'dk' => 'D'), true);
                    ?>
                </tbody>
            </table>
        </div>
        ...
        <div class="panel-body table-responsive">
            <table class="table table-bordered datatable" id="tab_rekening_kredit">
                ...
                <tbody>
                    <?php
                    $r = Rekening5M::model()->findByPk($model->rekening5_id);
                    echo $this->renderPartial($this->path_view . '_rowRekening', array('r' => $r, 'dk' => 'K'), true);
                    ?>
                </tbody>
            </table>
        </div>
...
<div class="form-actions">
    ...
    <?php echo CHtml::link(
        ...
        Yii::app()->createUrl($this->module->id . '/rekeningcolumn/admin'),
        array(
            'title' => 'Ulang',
    ...
    ?>
    <?php echo CHtml::link(Yii::t('mds', '{icon} Pengaturan Jurnal Rekening Kolom', array('{icon}' => '<i class="' . MyIcon::getIcons('pengaturan') . '"></i>')), $this->createUrl('admin', array('modul_id' => Yii::app()->session['modul_id'])), array('class' => 'btn btn-success')); ?>
    <?php
    $tips = array(
    ...
    'columns' => array(
        array(
            ...
            'value' => 'CHtml::Link("<i class=\"icon-form-check\"></i>","#",array("class"=>"btn-small",
				"id" => "selectRekDebit",
				"onClick" =>"
                $(\"#SARekeningcolumnM_rekening_1_rekening5_id\").val(\"$data->rekeninglast_id\");
                $(\"#' . CHtml::activeId($model, 'rekDebit') . '\").val(\"$data->namarekeninglast\");
    ...
    'columns' => array(
        array(
            ...
            'value' => 'CHtml::Link("<i class=\"icon-form-check\"></i>","#",array("class"=>"btn-small",
				"id" => "selectRekDebit",
				"onClick" =>"
                $(\"#SARekeningcolumnM_rekening_2_rekening5_id\").val(\"$data->rekeninglast_id\");
                $(\"#SARekeningcolumnM_rekKredit\").val(\"$data->namarekeninglast\");
...
<script type="text/javascript">
    ...
    function tambahRekeningDebit() {
        id = $("#SARekeningcolumnM_rekening_1_rekening5_id").val();
        if (id.trim() == "") {
        ...
        $.post('<?php echo $this->createUrl('formRekening'); ?>', {
            ...
            $("#SARekeningcolumnM_rekening_1_rekening5_id").val("");
            $("#SARekeningcolumnM_rekDebit").val("");
            $("#tab_rekening_debit tbody").append(data.dat);
        }, 'json');
    }

    function tambahRekeningKredit() {
        id = $("#SARekeningcolumnM_rekening_2_rekening5_id").val();
        nama = $("#SARekeningcolumnM_rekKredit").val();
        if (id.trim() == "") {
        ...
        $.post('<?php echo $this->createUrl('formRekening'); ?>', {
            ...
            $("#SARekeningcolumnM_rekening_2_rekening5_id").val("");
            $("#SARekeningcolumnM_rekKredit").val("");
