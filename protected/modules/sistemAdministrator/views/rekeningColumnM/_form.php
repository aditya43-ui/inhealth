...
<div class="row">
    <div class="col-sm-6">
        <?php echo $form->textAreaRow($model, 'keterangan', array('placeholder' => 'Keterangan', 'class' => 'span3 ', 'onkeyup' => "return $(this).focusNextInputField(event)")) ?>
        <div class="control-group">
            <?php echo CHtml::label("Nama Tabel", '', array('class' => 'control-label')); ?>
            <div class="controls">
                <?php
                echo $form->hiddenField($model, 'table_name', array('readonly' => true, 'class' => 'required'));
                $this->widget('MyJuiAutoComplete', array(
                    'model' => $model,
                    'attribute' => 'table_name_nama',
                    'source' => 'js: function(request, response) {
                                                                    $.ajax({
                                                                    url: "' . Yii::app()->createUrl('actionAutoComplete/getListTable') . '",
                                                                    dataType: "json",
                                                                    data: {
                                                                            term: request.term,
                                                                    },
                    ...
                    'options' => array(
                        ...
                        'select' => 'js:function( event, ui ) {
                                                                    $("#' . CHtml::ActiveId($model, 'table_name') . '").val(ui.item.value);
                                                                    return false;
                                                             }',
                    ),
                    'htmlOptions' => array(
        ...
        <div class="control-group">
            <?php echo CHtml::label("Nama Kolom", '', array('class' => 'control-label')); ?>
            <div class="controls">
                <?php
                $this->widget('MyJuiAutoComplete', array(
                    'model' => $model,
                    ...
                    'source' => 'js: function(request, response) {
                                                        $.ajax({
                                                                url: "' . Yii::app()->createUrl('/sistemAdministrator/rekeningColumnM/getListColumn') . '",
                                                                dataType: "json",
                                                                data: {
                                                                        term: request.term,
                                                                        tablename:$("#' . CHtml::ActiveId($model, 'table_name') . '").val()
                                                                },
                                                                success: function (data) {
                                                                                response(data);
                                                                }
...
<div class="form-actions">
    ...
    ?>
    <?php echo CHtml::link(Yii::t('mds', '{icon} Pengaturan Jurnal Rekening Kolom', array('{icon}' => '<i class="' . MyIcon::getIcons('pengaturan') . '"></i>')), $this->createUrl('admin', array('modul_id' => Yii::app()->session['modul_id'], 'tab' => isset($_GET['tab']) ? $_GET['tab'] : '')), array('class' => 'btn btn-success')); ?>
    <?php
    $tips = array(
