...
WHERE
penjaminrek_m.penjaminrek_id = $penjaminrek_id AND penjaminrek_m.debitkredit = 'D'");
if (COUNT($modRek) > 0) {
	foreach ($modRek as $i => $data) {
//		echo $data->nmrekening5;exit;
		if (isset($_GET['caraPrint'])) {
			...
			echo "<pre>";
			echo $data->nmrekening5 . 
					CHtml::Link("<i class=\"icon-pencil\"></i>", Yii::app()->controller->createUrl(Yii::app()->controller->id ."/ubahRekeningDebit", array("id" => $data->penjaminrek_id)), array("class" => "",
