<div class="panel panel-gradient">
    <div class="panel-heading">
        <div class="panel-title">
		 ...
		 <i class="fas fa-layer-group"></i> Pengaturan <b>Slot Bed</b>
            <?php //else : ?>
    ...
    <div class="panel-body">
        ...
        $this->breadcrumbs = array(
            'Slot Bed' => array('admin'),
            'Pengaturan',
        );

        $arrMenu = array();
        (Yii::app()->user->checkAccess(Params::DEFAULT_ADMIN)) ? array_push($arrMenu, array('label' => Yii::t('mds', 'Manage') . ' Slot Bed ', 'header' => true, 'itemOptions' => array('class' => 'heading-master'))) :  '';
...
$('.search-button').click(function(){
	$('.search-form').toggle();
    $('#SASlotBedM_kelaspelayanan_id').focus();
	return false;
});
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('saslot-bed-m-grid', {
		data: $(this).serialize()
	});
	return false;
});
");
        ...
        <div class="form-actions">
            ...
            // if($this->module->id == 'sistemAdministrator'){
                echo CHtml::link(Yii::t('mds', '{icon} Tambah Slot Bed', array('{icon}'=>'<i class="entypo-plus"></i>')), $this->createUrl(Yii::app()->controller->id.'/create',array('modul_id'=> Yii::app()->session['modul_id'])), array('class'=>'btn btn-danger'))."&nbsp&nbsp";
...
{
    $("#saslot-bed-m-grid :input[name='"+ obj.name +"']").val(obj.value);
}
function print(caraPrint)
{
    window.open("${urlPrint}/"+$('#saslot-bed-m-search').serialize()+"&caraPrint="+caraPrint,"",'location=_new, width=900px');
}
JSCRIPT;
