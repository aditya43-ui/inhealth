...
<div class="col-sm-6">
    ...
    ?>
    <?php
    echo $form->dropDownListRow($model, 'ruangan_id', CHtml::listData($model->RuanganSlotItems, 'ruangan_id', 'ruangan_nama'), array('class' => 'inputRequire', 'onkeypress' => "return $(this).focusNextInputField(event)", 'empty' => '-- Pilih Ruangan --', 'disabled' => true));
...
<div class="col-sm-6">
    <?php 
        if ($this->module->id != 'hemodialisa'){
            echo $form->textFieldRow($model, 'slotbed_noslot', array('onkeypress' => "return $(this).focusNextInputField(event);")); 
        }
    ?>
    <?php echo $form->textFieldRow($model, 'slotbed_jmlbed', array('class' => 'span1', 'onkeypress' => "return $(this).focusNextInputField(event);", 'maxlength' => 2, 'readonly' => true)); ?>
</div>
...
<div class="panel panel-success">
    ...
    <div class="panel-body table-responsive">
        <table id="tabel-slotbed" class="table  table-bordered ">
            <thead>
                <tr>
                    <th>No. Bed</th>
                    <th>Status</th>
            ...
            <tbody>
                <?php
                if (count((array)$loadSlot) > 0) {
                    $i = 0;
                    foreach ($loadSlot as $det) {
                        $det->slotTerpakai = ($det->slotbed_status == true) ? false : true;
                        //$det->pernah_dipakai = !empty(MasukkamarT::model()->find(" slotbed_id = ".$det->slotbed_id));
...
<div class="form-actions">
    ...
    <?php
    echo CHtml::link(Yii::t('mds', '{icon} Pengaturan Slot Bed', array('{icon}' => '<i class="entypo-folder"></i>')), $this->createUrl(Yii::app()->controller->id . '/admin', array('modul_id' => Yii::app()->session['modul_id'])), array('class' => 'btn btn-info',)) . "&nbsp";
...
<script>
    function tambahBaris() {
        ...
        var jmlbed = $("#<?php echo CHtml::activeId($model, 'slotbed_jmlbed') ?>").val();
        $('#tabel-slotbed > tbody ').append(row);

        renameInputRow($("#tabel-slotbed"), 'banyakbed');
        if (jmlbed != '') {
            $("#<?php echo CHtml::activeId($model, 'slotbed_jmlbed') ?>").val(parseInt(jmlbed) + 1);
        }
        //alert('adasda');
    }
