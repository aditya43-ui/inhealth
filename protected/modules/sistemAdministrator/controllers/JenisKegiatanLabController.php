...
class JenisKegiatanLabController extends MyAuthController
{
  /**
  ...
  public $path_view = 'sistemAdministrator.views.jenisKegiatanLab.';
  ...
  {
    $model = new JeniskegiatanlabM();
    // new SAJenisKegiatanM()
    // Uncomment the following line if AJAX validation is needed


    if (isset($_POST['JeniskegiatanlabM'])) {
      $model->attributes = $_POST['JeniskegiatanlabM'];
      if ($model->save()) {
        Yii::app()->user->setFlash('success', "Data " . $model->jeniskegiatanlab3 . " berhasil disimpan");
        $this->redirect(array('view', 'id' => $model->jeniskegiatanlab_id));
      } else {
  ...
  {
    $model = new JeniskegiatanlabM('search');
    $model->unsetAttributes();  // clear any default values
    // $model->pemeriksaanlab_aktif = true;
    if (isset($_GET['JeniskegiatanlabM'])) {
      $model->attributes = $_GET['JeniskegiatanlabM'];
  ...
  {
    $model = JeniskegiatanlabM::model()->findByPk($id);
    if ($model === null)
  ...
  {
    $model = new JeniskegiatanlabM;
    $model->attributes = $_REQUEST['JeniskegiatanlabM'];
