...
class MyFormatter extends CFormatter
{
     ...
     */
    public static function formatMonthForUser($inputmonth,$onlymonth='')
    {
        if(empty($inputmonth)) 
            return null;
        $return = null;
        ...
        if (count($month) > 1){
            if(strlen($month[0]) == 4){
				if (!empty($onlymonth)){
					$return = self::getMonthUser($month[1]);
				}else{
    ...
    public static function formatMonthForUserGaji($inputmonth,$onlymonth='')
    {
        if(empty($inputmonth)) 
            return null;
        $return = null;
        ...
        if (count($month) > 1){
            if(strlen($month[0]) == 4){
				if (!empty($onlymonth)){
					$return = self::getMonthUserGaji($month[1]);
				}else{
					$return = self::getMonthUserGaji($month[1])." ".$month[0];
				}
            }else{
                $return = self::getMonthUserGaji($month[0])." ".$month[1];
            }
        }
        return $return;
    }
    ...
    public static function getMonthUserGaji($inputmonth)
    {
        if(empty($inputmonth)) 
            return null;
        $return = null;
     ...
     * @return Rp 12.000
     */
    public static function formatNumberForPrint($input_number, $decimals = 0, $neg = false, $excel_formula = false){
        $is_excel = false;
        if (isset($_GET['caraPrint']) && strtolower($_GET['caraPrint']) == 'excel') {
            $is_excel = true;
        } else if (isset($_GET['caraprint']) && strtolower($_GET['caraprint']) == 'excel') {
            $is_excel = true;
        }
        
        if ($is_excel) {
            if ($excel_formula) {
                $return = '="'.number_format(str_replace(",","",abs((float)trim($input_number))), (int)$decimals, ',', '.').'"';
            } else {
                $return = number_format(str_replace(",","",abs((float)trim($input_number))), (int)$decimals, '.', '');
            }
        } else {
        ...
        if ($neg) {
            if ($input_number < 0) return "(".$return.")";
            return $return;
        }
        // $return = number_format(str_replace(",","",trim($input_number)), (int)$decimals, ',', '.');
        return $return;
    ...
    public static function formatUang($input_number, $mata_uang = "Rp", $decimals = 0){
        $return = number_format(str_replace(",","",(float)trim($input_number)), (int)$decimals, ',', '.');
    ...
    public static function kataTerbilang($input_number){
        $input_number = abs((float)$input_number);
	...
	public static function formatRupiahForDB($input_number) {
