...
class LaporankunjunganbydokterV extends CActiveRecord
{
	...
	public function search()
	{
		...
		/**
		 * - digunakan untuk mendapatkan data dokter beserta gelar depan dan belakang
		 * @return type
		 */
		public function getNamaLengkap(){
			$dok = PegawaiM::model()->findByPk($this->dokter_id);
			
			return $dok->namaLengkap;
		}
		
		public function getNamaRuangan(){
			$r = RuanganM::model()->findByPk($this->ruangan_id);
			
			return $r->ruangan_nama;
		}
		
		public function getJumlahPasien($ruangan_id, $tgl,$jam_awal,$jam_akhir, $dokter_id){
			$cri = new CDbCriteria();
			$cri->addCondition("ruangan_id = ".$ruangan_id." ");
			$cri->addBetweenCondition(" DATE(tgl_pendaftaran) ", $tgl, $tgl);
			$cri->addCondition(" dokter_id = ".$dokter_id." ");
			$cri->addCondition(" buatjanjipoli_id IS NOT NULL ");
			
			$jmlPesan = LaporankunjunganbydokterV::model()->findAll($cri);
			
			return count((array)$jmlPesan);
		}
}