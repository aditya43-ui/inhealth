...
class InfopasienmasukkamarV extends CActiveRecord
{
	...
	public function search()
	{
		...
		public function getRenPulang(){
			$adm = PasienadmisiT::model()->findByPk($this->pasienadmisi_id);
			
			return $adm->rencanapulang;
		}
		...
		public function getKonsulPasien() {
			$model = KonsulpoliT::model()->findByAttributes(array('konsulpoli_id' => $this->konsulpoli_id));
		...
		{
			$modelA = KonsulpoliT::model()->findByAttributes(array('konsulpoli_id' => $this->konsulpoli_id)); //, 'ruangan_id' => Yii::app()->user->getState('ruangan_id')
			if (!empty($modelA)) {
