 ...
 * 
 * This is the model class for table "rinciantagihanobatalkes_v".
 *
 * The followings are the available columns in table 'rinciantagihanobatalkes_v':
 ...
 */
class RinciantagihanobatalkesV extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return RinciantagihanobatalkesV the static model class
	 */
	...
	{
		return 'rinciantagihanobatalkes_v';
	}
	...
	public function rules()
	{
		...
		return array(
			array('obatalkespasien_id, sumberdana_id, racikan_id, returresepdet_id, tipepaket_id, ruangan_id, carabayar_id, pegawai_id, daftartindakan_id, tindakanpelayanan_id, satuankecil_id, shift_id, pendaftaran_id, obatalkes_id, pasien_id, penjamin_id, kelaspelayanan_id, pasienanastesi_id, pasienmasukpenunjang_id, pasienadmisi_id, oasudahbayar_id, penjualanresep_id, rke, permintaan_oa, jmlkemasan_oa, kekuatan_oa, verifikasitagihan_id, jurnalrekening_id, permohonanoadetail_id, persenppnjual, resepturdetail_id, jeniskelas_id', 'numerical', 'integerOnly'=>true),
			...
			array('etiket, jenispenjualan', 'length', 'max'=>100),
			array('no_rekam_medik', 'length', 'max'=>10),
			array('nama_pasien, penjamin_nama, ruangan_nama, instalasi_nama, kelaspelayanan_nama, noresep', 'length', 'max'=>50),
			array('tempat_lahir', 'length', 'max'=>25),
			array('daftartindakan_nama, tindakanmedis_nama, obatalkes_kode, obatalkes_nama', 'length', 'max'=>200),
			array('tglpelayanan, create_time, update_time, create_loginpemakai_id, update_loginpemakai_id, create_ruangan, tanggal_lahir, alamat_pasien, tgl_pendaftaran, tglresep, tglpenjualan', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('obatalkespasien_id, sumberdana_id, racikan_id, returresepdet_id, tipepaket_id, ruangan_id, carabayar_id, pegawai_id, daftartindakan_id, tindakanpelayanan_id, satuankecil_id, shift_id, pendaftaran_id, obatalkes_id, pasien_id, penjamin_id, kelaspelayanan_id, pasienanastesi_id, pasienmasukpenunjang_id, pasienadmisi_id, oasudahbayar_id, penjualanresep_id, tglpelayanan, r, rke, permintaan_oa, jmlkemasan_oa, kekuatan_oa, satuankekuatan_oa, qty_oa, hargasatuan_oa, signa_oa, harganetto_oa, hargajual_oa, etiket, jmlexposerad, kontrasrad, biayaservice, biayakonseling, jasadokterresep, biayakemasan, biayaadministrasi, tarifcyto, discount, subsidiasuransi, subsidipemerintah, subsidirs, iurbiaya, oa, create_time, update_time, create_loginpemakai_id, update_loginpemakai_id, create_ruangan, verifikasitagihan_id, jurnalrekening_id, permohonanoadetail_id, persenppnjual, resepturdetail_id, no_rekam_medik, namadepan, nama_pasien, jeniskelamin, tempat_lahir, tanggal_lahir, alamat_pasien, umur, no_pendaftaran, tgl_pendaftaran, penjamin_nama, ruangan_nama, instalasi_nama, kelaspelayanan_nama, jeniskelas_id, noresep, tglresep, tglpenjualan, jenispenjualan, daftartindakan_nama, tindakanmedis_nama, daftartindakan_kode, obatalkes_kode, obatalkes_nama', 'safe', 'on'=>'search'),
		);
	}
    ...
    public function TotalOAPerInstalasi($pendaftaran_id,$instalasi_nama){
        $criteria = new CDbCriteria();
        $criteria->select = "sum( tarifcyto + biayaadministrasi + biayakemasan + biayakonseling + biayaservice + (qty_oa*hargasatuan_oa) ) AS harganetto_oa";
        $criteria->addCondition('pendaftaran_id = '.$pendaftaran_id);
        $criteria->addCondition("instalasi_nama = '".$instalasi_nama."'");
        $modRincianOA = self::model()->find($criteria);
        return $modRincianOA->harganetto_oa;
    }
}