	...
	$.extend(neonChat, {	
		...
		init: function()
		{
			...
			$textarea.on('input', function(){
				var id = $('#chat').attr('data-current-user');
				var partner = $conversation_header.find('.display-name').html();
				var conv = $("#"+partner).attr('conv-id');
				if($(this).val()==''){
		...
		submitMessage: function() // Submit whats on textarea
		{
			...
			if(this.isOpen && this.$current_user)
			{
				...
				var convId = this.$current_user.uniqueId().attr('conv-id');
				var obj = this;
				$.post("index.php?r=chat&action=sendchat", {to: id, message: msg} , function(datas){
					obj.pushMessage(id, msg.replace( /<.*?>/g, '' ), $chat.data('current-user'), new Date());
					obj.renderMessages(id);
					socket.emit('send',{message:msg.replace( /<.*?>/g, '' ), userID:$chat.data('current-user'), strCurDate:'', conversationID:convId,partnerID:id, type:'msg'});
				});
		...
		showChat: function(animated)
		{
			...
			{			
				...
				$chat.show();
				TweenMax.set($chat, {css: {transform: "translateX("+chat_width+"px)"}});
		...
		hideChat: function(animated)
		{
			...
			{			
				...
				TweenMax.to($chat, .4, {css: {autoAlpha: 1, transform: "translateX("+ chat_width +"px)"}, ease: Sine.easeIn});
