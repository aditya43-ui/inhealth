...
;(function($, window, undefined){
	...
	$(document).ready(function()
	{
				...
				$this.daterangepicker(opts, function(start, end)
				{
					...
					if($this.hasClass('daterange-inline'))
					{
						$this.find('span').html(start.format(drp.format) + drp.separator + end.format(drp.format));
                                        ...
                                        if($this.hasClass('input-inline'))
					{                                                                                           
						$this.find('input.start').val(changeFormat(start.format(drp.format),'/'));
                                                $this.find('input.end').val(changeFormat(end.format(drp.format),'/'));
					}
                                        
                                        if($this.hasClass('setIndikator')){
                                            setIndikator();
                                        }
				});
			});
		...
		{
			$(".form-wizard_neon").each(function(i, el)
			{
				var $this = $(el),				
...
function changeFormat(date, separator){
    var date    = new Date(date),
    yr      = date.getFullYear(),
    month   = date.getMonth() < 9 ? '0' + (date.getMonth()+1) : (date.getMonth()+1),
    day     = date.getDate()  < 10 ? '0' + date.getDate()  : date.getDate(),
    newDate = day + separator + month + separator + yr;
    
    return newDate;
}

/**
 * - digunakan untuk memeriksa aturan pembuatan passsword
 * @param {type} obj
 * @param {type} int
 * @returns {Boolean}
 */
function checkPass(obj,int){
    ...
    if (data.length < int){
            $("#reset_password-error2").show();
            $("#reset_password-error2").html("<br/>kata kunci minimal terdiri dari 8 karakter");            
            return false;
    }else{
        if (regex.test(data)){        
            $("#reset_password-error2").hide();
            $("#reset_password-error2").html("");
            return true;
        }else{
            $("#reset_password-error2").show();
            $("#reset_password-error2").html("<br/>password harus terdiri dari a-z, A-Z, 0-9 dan - ");            
            return false;
        }       
    }
